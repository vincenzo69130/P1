...
{
    $translations = array('Invalid key.' => Tools::displayError('Invalid key.'), 
    'This product does not exist in our store.' => Tools::displayError(
    'This product does not exist in our store.'), 
    'This product has been deleted.' => Tools::displayError(
    'This product has been deleted.'), 
    'This file no longer exists.' => Tools::displayError(
    'This file no longer exists.'), 
    'The product deadline is in the past.' => Tools::displayError(
    'The product deadline is in the past.'), 
    'Expiration date exceeded' => Tools::displayError(
    'Expiration date exceeded'), 
    'You have reached the maximum number of allowed downloads.' => Tools::displayError(
    'You have reached the maximum number of allowed downloads.'));
    ?>
<script type="text/javascript">
//<![CDATA[
alert("<?php
    echo html_entity_decode($translations[$msg], ENT_QUOTES, 'utf-8');
    ?>");
...
if ($cookie->isLoggedBack() and Tools::getValue('file')) {
    ...
    $filename = ProductDownload::getFilenameFromFilename(
    Tools::getValue('file'));
    if (empty($filename)) {
    ...
    if (! $cookie->isLogged() and ! Tools::getValue('secure_key') and
     ! Tools::getValue('id_order'))
        Tools::redirect('authentication.php?back=get-file.php&key=' . $key);
    elseif (! $cookie->isLogged() and Tools::getValue('secure_key') and
     Tools::getValue('id_order')) {
    ...
    if (! isset($info['id_product_download']) or
     empty($info['id_product_download']))
    ...
    if ($now > $product_deadline and
     $info['download_deadline'] != '0000-00-00 00:00:00')
        displayError('The product deadline is in the past.');
    $customer_deadline = strtotime($info['date_expiration']);
    if ($now > $customer_deadline and
     $info['date_expiration'] != '0000-00-00 00:00:00')
        displayError('Expiration date exceeded');
    if ($info['download_nb'] >= $info['nb_downloadable'] and
     $info['nb_downloadable'])
        displayError(
        'You have reached the maximum number of allowed downloads.');
...
if (empty($mimeType)) {
    ...
    'doc' => 'application/msword', 'oda' => 'application/oda', 
    ...
    'midi' => 'audio/midi', 'kar' => 'audio/midi', 'mpga' => 'audio/mpeg', 
    'mp2' => 'audio/mpeg', 'mp3' => 'audio/mpeg', 'aif' => 'audio/x-aiff', 
    ...
    'pdb' => 'chemical/x-pdb', 'xyz' => 'chemical/x-xyz', 'bmp' => 'image/bmp', 
    'gif' => 'image/gif', 'ief' => 'image/ief', 'jpeg' => 'image/jpeg', 
    'jpg' => 'image/jpeg', 'jpe' => 'image/jpeg', 'png' => 'image/png', 
    'tiff' => 'image/tiff', 'tif' => 'image/tif', 'djvu' => 'image/vnd.djvu', 
    ...
    'xwd' => 'image/x-windowdump', 'igs' => 'model/iges', 'iges' => 'model/iges', 
    ...
    'tsv' => 'text/tab-seperated-values', 'wml' => 'text/vnd.wap.wml', 
    'wmls' => 'text/vnd.wap.wmlscript', 'etx' => 'text/x-setext', 
    'xml' => 'text/xml', 'xsl' => 'text/xml', 'mpeg' => 'video/mpeg', 
