...
class AdminPDF extends AdminPreferences
{
    public function __construct ()
    {
        ...
        foreach ($fontFiles as $file)
            if (substr($file, - 4) == '.php' and $file != 'index.php' and
             substr($file, - 6) != 'bi.php' and substr($file, - 5) != 'b.php' and
             substr($file, - 5) != 'i.php') {
        ...
        'PS_PDF_ENCODING' => array('title' => $this->l('Encoding:'), 
        'desc' => $this->l('Encoding for PDF invoice'), 'type' => 'selectLang', 
        'cast' => 'strval', 'identifier' => 'mode', 'list' => $encodingList), 
        'PS_PDF_FONT' => array('title' => $this->l('Font:'), 
        'desc' => $this->l('Font for PDF invoice'), 'type' => 'selectLang', 
        'cast' => 'strval', 'identifier' => 'mode', 'list' => $fontList));
        parent::__construct();
    }
        ...
        if (isset($_POST['submitPDF' . $this->table])) {
            ...
            foreach ($this->_fieldsPDF as $field => $fieldvalue)
                foreach ($languages as $lang)
                    if (Tools::getValue(
                    $field . '_' . strtoupper($lang['iso_code'])))
                        $fieldLangPDF[$field . '_' .
                         strtoupper($lang['iso_code'])] = array(
                        'type' => 'select', 'cast' => 'strval', 
                        'identifier' => 'mode', 'list' => $fieldvalue['list']);
    ...
    public function display ()
    {
        ...
        $this->_displayForm('PDF', $this->_fieldsPDF, 
        $this->l('PDF settings for the current language:') . ' ' .
         $language->name, 'width2', 'pdf');
    }
}
