...
class AdminUpgrade extends AdminPreferences
{
    ...
    private function _setFields ()
    {
        ...
        'title' => $this->l('Don\'t save images'), 'cast' => 'intval', 
        ...
        'validation' => 'isBool', 'type' => 'bool', 
        'desc' => $this->l(
        'If set too yes, you will keep all your translations'));
    ...
    public function init ()
    {
        ...
        $this->autoupgradePath = $this->adminDir . DIRECTORY_SEPARATOR .
         $this->autoupgradeDir;
    ...
    public function ajaxProcessUpgradeFiles ()
    {
        ...
        for ($i = 0; $i < self::$loopUpgradeFiles; $i ++) {
            ...
            if (! $this->upgradeThisFile($file)) {
                ...
                $totalFiles = array_unshift(
                $this->nextParams['filesToUpgrade'], $file);
                $this->next = 'error';
                ...
                sizeof($this->nextParams['filesToUpgrade']));
            }
        }
    }
    /**
    ...
    public function _modelDoUpgrade ()
    {
        ...
        $logger->setFilename(
        $this->latestRootDir . '/' . date('Ymd') . '_autoupgrade.log');
        // init env.
        @set_time_limit(0);
        @ini_set('max_execution_time', '0');
        ...
        $this->prodRootDir . '/config/settings.inc.php');
    ...
    public function ajaxProcessUpgradeDb ()
    {
        ...
        if (isset(
        $this->currentParams['customModule']))
            ...
            if (is_dir($file)) {
                if (! file_exists($dest)) {
                    if (@mkdir($dest)) {
                        $this->nextQuickInfo[] = sprintf(
                        $this->l(
                        'created dir %2$s. %3$s files left to upgrade.'), $file, 
                        $dest, sizeof($this->nextParams['filesToUpgrade']));
                        return true;
                    } else {
                        $this->next = 'error';
                        $this->nextQuickInfo[] = sprintf(
                ...
                if (copy($file, $dest)) {
                    $this->nextQuickInfo[] = sprintf(
                    $this->l('copied %1$s in %2$s. %3$s files left to upgrade.'), 
                    $file, $dest, sizeof($this->nextParams['filesToUpgrade']));
                    return true;
                } else {
                    $this->next = 'error';
                    $this->nextQuickInfo[] = sprintf(
        ...
        if (! empty($this->backupFilesFilename)) {
            ...
            if (count($list) > 0) {
                ...
                if (self::ZipExtract($filepath, $destExtract)) {
                    ...
                    if (! empty($this->backupDbFilename)) {
                        $this->nextDesc = $this->l(
                        'Files restored. No database backup found. Restoration done.');
                        $this->next = '';
                    } else {
    ...
    public function ajaxProcessBackupFiles ()
    {
        ...
        if (! isset($this->nextParams['filesForBackup'])) {
            ...
            $this->backupFilesFilename = $this->autoupgradePath .
             DIRECTORY_SEPARATOR . 'backupfile-' . date('Y-m-d') . '-' . $time .
             '.zip';
            Configuration::updateValue('UPGRADER_BACKUPFILES_FILENAME', 
            $this->backupFilesFilename);
            $this->nextQuickInfo[] = sprintf(
        ...
        sizeof($this->nextParams['filesForBackup']));
        if (is_array($this->nextParams['filesForBackup'])) {
            ...
            if ($zip->open($this->backupFilesFilename, ZIPARCHIVE::CREATE)) {
                ...
                for ($i = 0; $i < self::$loopBackupFiles; $i ++) {
                    ...
                    $file = array_shift(
                    $this->nextParams['filesForBackup']);
                    $archiveFilename = str_replace($this->prodRootDir, '', 
                    $file);
                    // @TODO : maybe put several files at the same times ?
                    if ($zip->addFile($file, 
                    $archiveFilename))
    ...
    public function displayConf ()
    {
        if (version_compare(_PS_VERSION_, '1.4.4.0', '<') and false) {
            $this->_errors[] = Tools::displayError(
            'This class depends of several files modified in 1.4.4.0 version and should not be used in an older version');
        }
        parent::displayConf();
    }
    public function ajaxPreProcess ()
    {
        ...
        if (isset(self::$skipAction[strtolower($action)])) {
            ...
            if (! method_exists(get_class($this), 'ajaxProcess' . $action)) {
                $this->nextDesc = sprintf($this->l('action "%1$s" not found'), 
                $action);
                $this->next = 'error';
                $this->error = '1';
            }
...
txtError[37] = "' .
    ...
    private function _displayUpgraderForm ()
    {
        ...
        echo '<b>' . $this->l('Root directory') . ' : </b>' . $this->prodRootDir .
         '<br/><br/>';
        if ($this->rootWritable)
            $srcRootWritable = '../img/admin/enabled.gif';
        else
            $srcRootWritable = '../img/admin/disabled.gif';
        echo '<b>' . $this->l('Root directory status') . ' : </b>' . '<img src="' .
         $srcRootWritable . '" /> ' . ($this->rootWritable ? $this->l(
        ...
        if ($this->upgrader->needUpgrade) {
            ...
            echo '<b>' . $this->l('Autoupgrade allowed') . ' : </b>' .
             '<img src="' . $srcAutoupgrade . '" /> ' . ($this->upgrader->autoupgrade ? $this->l(
            'This release allow autoupgrade.') : $this->l(
        ...
        echo '<b>' . $this->l('Shop status') . ' : </b>' . '<img src="' .
         $srcShopStatus . '" />' . $label . '<br/><br/>';
        $max_exec_time = ini_get('max_execution_time');
        if ($max_exec_time == 0)
            $srcExecTime = '../img/admin/enabled.gif';
        else
            $srcExecTime = '../img/admin/warning.gif';
        echo '<b>' . $this->l('PHP time limit') . ' : </b>' . '<img src="' .
        ...
        if ($this->rootWritable)
            ...
            $srcRootWritable = '../img/admin/disabled.gif';
        echo '<b>' . $this->l('Root directory') . ' : </b>' . '<img src="' .
         $srcRootWritable . '" /> ' . ($this->rootWritable ? $this->l(
        'writable recursively') : $this->l('not writable recursively')) .
         '. <br/><br/>';
        echo '<a class="button" id="scrollToOptions" href="#options">' .
         $this->l('Modify your options') . '</a>';
        echo '</fieldset>';
        echo '<br/>';
        ...
        if ($this->isUpgradeAllowed()) {
            ...
            echo '<br/>';
            if ($this->upgrader->needUpgrade) {
                echo '<a href="" id="upgradeNow" class="button-autoupgrade upgradestep">' .
                 $this->l('Upgrade PrestaShop now !') . '</a>';
            } else {
            ...
            echo '<br class="clear"/>';
            if (defined('_PS_MODE_DEV_') and _PS_MODE_DEV_) {
                echo '<fieldset class="autoupgradeSteps"><legend>' .
                 $this->l('Step') . '</legend>';
    ...
    public function display ()
    {
        ...
        echo '<fieldset class="error"><legend><img src="' .
         __PS_BASE_URI__ . 'modules/autoupgrade/logo.gif" />' .
         $this->l('New upgrade module') . '</legend>';
        echo '<p>' .
         $this->l(
        'The tab AdminUpgrade present in 1.4.4.0 and 1.4.4.1 version has been removed and that functions is now available as module.') .
         '</p>';
        $tokenModule = Tools::getAdminToken(
        'AdminModules' . (int) (Tab::getIdFromClassName('AdminModules')) .
         (int) $cookie->id_employee);
        $tokenAdminTabs = Tools::getAdminToken(
        'AdminTabs' . (int) (Tab::getIdFromClassName('AdminTabs')) .
         (int) $cookie->id_employee);
        $tokenAdminTools = Tools::getAdminToken(
        'AdminTools' . (int) (Tab::getIdFromClassName('AdminTools')) .
         (int) $cookie->id_employee);
        echo '<p id="ContainerActivateButton">
			<a class="button" id="activateAutoupgradeModule" href="index.php?tab=AdminModules&token=' .
         $tokenModule . '&amp;install=autoupgrade&amp;module_name=autoupgrade">' . $this->l(
        'Click to remove that tab and activate the autoupgrade module') .
         '</a></p>';
        echo '<script type="text/javascript">
			...
			{
				console.log(res);
				$.get("index.php?token=' . $tokenAdminTabs . '&tab=AdminTabs&id_tab=' .
         Tab::getIdFromClassName('AdminUpgrade') . '&deletetab",
				function(e){
					alert("' .
         $this->l(
        'Previous UpgradeTab has been removed. Please go to the new Upgrade tab, or install the module first.') . '");
					window.location = "' . 'index.php?token=' . $tokenAdminTools .
         '&tab=AdminTools&id_tab=' . Tab::getIdFromClassName('AdminTools') . '";
			...
			error: function(res,textStatus,jqXHR)
			{
				alert("' .
         $this->l(
        "Unable to install the module automatically. Please manually remove the Upgrade AdminTab and install the autoupgrade module.") . '");
			}
		});
	});
        ...
        if ($this->isUpgradeAllowed()) {
            ...
            '<a href="" name="options" id="options">' . $this->l('Options') .
             '</a>', '', 'prefs');
            echo '<script type="text/javascript">' . $this->_getJsInit() .
             '</script>';
        } else {
            echo '<fieldset>
			<legend>' . $this->l('Update') .
             '</legend>';
            echo '<p>' .
             $this->l('You currently don\'t need to use this feature.') . '</p>';
            echo '</fieldset>';
        }
    }
...
function parseXMLResult(xmlRet)
{
	...
	if (ret.getAttribute("result") == "ok")
	{
		$("#dbResultCheck")
			...
			.html("<p>' .
         $this->l(
        'upgrade complete. Please check your front-office (try to make an order, check theme)') . '</p>")
			.show("slow");
		$("#dbCreateResultCheck")
			.hide("slow");
...
';
    ...
    private function _cleanUp ($path)
    {
        ...
        'config.inc.php', 'Tools.php', 'AdminUpgrade.php', 'ajax-tab.php');
        if (is_dir($path)) {
            $fp = opendir($path);
    ...
    private static function ZipExtract ($fromFile, $toDir)
    {
        ...
        if (class_exists('ZipArchive', false)) {
            $zip = new ZipArchive();
            if ($zip->open($fromFile) === true and $zip->extractTo($toDir) and
             $zip->close())
                return true;
            return false;
        } else {
    ...
    private function _listArchivedFiles ()
    {
        if (! empty($this->nextParams['backupFilesFilepath'])) {
            if (class_exists('ZipArchive', false)) {
                ...
                require_once ($this->prodRootDir . '/tools/pclzip/pclzip.lib.php');
                if ($zip = new PclZip(
                $this->currentParams['backupFilesFilepath']))
