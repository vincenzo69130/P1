...
class AdminStores extends AdminTab
{
    ...
    public function __construct ()
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'country_lang` cl ON (cl.`id_country` = a.`id_country` AND cl.`id_lang` = ' .
         (int) $cookie->id_lang . ')
        ...
        'id_store' => array('title' => $this->l('ID'), 'align' => 'center', 
        'width' => 25), 
        ...
        'desc' => $this->l(
        'Display a link to the store locator in the sitemap page'), 
        'cast' => 'intval', 'type' => 'bool'), 
        ...
        'desc' => $this->l('Used for the default map position'), 
        'cast' => 'floatval', 'type' => 'float', 'size' => '10'), 
    ...
    protected function postImage ($id)
    {
        ...
        if (($id_store = (int) (Tools::getValue('id_store'))) and isset($_FILES) and
         sizeof($_FILES) and file_exists(
    ...
    public function displayOptionsList ()
    {
        ...
        echo '<br /><p><img src="../img/admin/asterisk.gif" class="middle" /> ' .
         $this->l(
        'You can also replace the icon representing your store in Google Maps. Go to the Preferences tab, and then the Appearance subtab.') .
         '</p>';
    }
    ...
    public function displayForm ($isMainTab = true)
    {
			...
			<fieldset>
				...
				<div style="padding-right: 40px; border-right: 1px solid #E0D0B1; float: left;">
					...
					<div class="margin-form">
						<input type="text" size="33" name="name" value="' .
         htmlentities(Tools::getValue('name', $obj->name), ENT_COMPAT, 'UTF-8') . '" />
						<span class="hint" name="help_box">' .
         $this->l('Allowed characters: letters, spaces and') . ' (-)<span class="hint-pointer">&nbsp;</span></span>
						<p class="clear">' . $this->l(
        'Store name, e.g. Citycentre Mall Store') . '</p>
					</div>
        ...
        $this->getFieldValue($obj, 'address2'), ENT_COMPAT, 'UTF-8') . '" />
					...
					<div class="margin-form">
						<input type="text" size="6" name="postcode" value="' . htmlentities(
        $this->getFieldValue($obj, 'postcode'), ENT_COMPAT, 'UTF-8') . '" />
					</div>
					<label>' . $this->l('City:') . '</label>
					<div class="margin-form">
						<input type="text" size="33" name="city" value="' .
         htmlentities($this->getFieldValue($obj, 'city'), ENT_COMPAT, 'UTF-8') . '" style="text-transform: uppercase;" /> <sup>*</sup>
					</div>
        ...
        foreach ($this->countriesArray as $id_country => $name)
            echo '		<option value="' . $id_country . '"' . ((! $selectedCountry and
        ...
        echo '		</select> <sup>*</sup>
					...
					<div class="margin-form">
						<input type="text" size="11" maxlength="12" name="latitude" value="' . htmlentities(
        $this->getFieldValue($obj, 'latitude'), ENT_COMPAT, 'UTF-8') .
         '" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');" /> / <input type="text" size="11" maxlength="12" name="longitude" value="' . htmlentities(
        $this->getFieldValue($obj, 'longitude'), ENT_COMPAT, 'UTF-8') . '" onKeyUp="javascript:this.value = this.value.replace(/,/g, \'.\');" />
						<sup>*</sup>
						<p class="clear">' .
         $this->l('Store coords, eg. 45.265469 / -47.226478') . '</p>
					</div>
         ...
         htmlentities($this->getFieldValue($obj, 'phone'), ENT_COMPAT, 'UTF-8') . '" />
					...
					<div class="margin-form">
						<input type="text" size="33" name="fax" value="' .
         htmlentities($this->getFieldValue($obj, 'fax'), ENT_COMPAT, 'UTF-8') . '" />
					</div>
					<label>' . $this->l('E-mail address:') . ' </label>
					<div class="margin-form">
						<input type="text" size="33" name="email" value="' .
         htmlentities($this->getFieldValue($obj, 'email'), ENT_COMPAT, 'UTF-8') . '" />
					</div>
					<label>' . $this->l('Note:') . ' </label>
					<div class="margin-form">
						<textarea name="note" style="width: 250px; height: 75px;">' .
         htmlentities($this->getFieldValue($obj, 'note'), ENT_COMPAT, 'UTF-8') . '</textarea>
					</div>
         ...
         $this->l('Disabled') . '" title="' . $this->l('Disabled') . '" /></label>
				...
				<div style="padding-left: 40px; float: left;">
					<label style="text-align: left; width: inherit;">' .
         $this->l('Picture:') . ' </label>
        ...
        _PS_STORE_IMG_DIR_ . '/' . $obj->id . '.jpg', 350, NULL, 
        Tools::getAdminToken(
        'AdminStores' . (int) (Tab::getIdFromClassName('AdminStores')) .
        ...
        echo '</div>
         ...
         $this->l('Sample: 10:00AM - 9:30PM') . '</td>
						</tr>';
        $days = array();
        ...
        for ($i = 1; $i < 8; $i ++)
            echo '
							<tr style="color: #7F7F7F; font-size: 0.85em;">
								<td>' . $days[(int) ($i)] . '</td>
								<td><input type="text" size="25" name="hours_' . (int) ($i) .
             '" value="' .
             (isset($hoursUnserialized) ? htmlentities(
            $hoursUnserialized[$i - 1], ENT_COMPAT, 'UTF-8') : '') . '" /><br /></td>
							</tr>';
        echo '
					</table>
				</div>
