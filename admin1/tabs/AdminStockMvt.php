...
class AdminStockMvt extends AdminTab
{
    public function __construct ()
    {
        ...
        $this->_join = 'LEFT JOIN `' . _DB_PREFIX_ .
         'product_lang` pl ON (a.id_product = pl.id_product AND pl.id_lang = ' .
         (int) $cookie->id_lang . ')
							LEFT JOIN `' . _DB_PREFIX_ .
         'stock_mvt_reason_lang` mrl ON (a.id_stock_mvt_reason = mrl.id_stock_mvt_reason AND mrl.id_lang = ' .
         (int) $cookie->id_lang . ')
    ...
    public function displayForm ($isMainTab = true)
    {
        ...
        echo '<form action="' . $currentIndex . '&submitAdd' . $this->table .
         '=1&token=' . $this->token . '&addstock_mvt_reason" method="post">
		' . ($obj->id ? '<input type="hidden" name="id_' . $this->table .
         '" value="' . $obj->id . '" />' : '') . '
			<fieldset><legend><img src="../img/admin/search.gif" />' .
         $this->l('Stock Movement') . '</legend>
				<label>' . $this->l('Name:') . '</label>
				<div class="margin-form">';
        foreach ($this->_languages as $language)
            echo '<div id="name_' . $language['id_lang'] . '" style="display: ' .
        ...
        $this->displayFlags($this->_languages, $this->_defaultFormLanguage, $dl, 
        'name');
        echo '</div>
    ...
    public function viewstock_mvt ()
    {
        ...
        $product = new Product((int) $stockMvt->id_product, true, 
        (int) $cookie->id_lang);
    ...
    public function display ()
    {
        global $currentIndex, $cookie;
        $old_post = false;
        if (! isset($_GET['addstock_mvt_reason']) or (Tools::isSubmit(
        'submitAddstock_mvt_reason') and Tools::getValue('id_stock_mvt_reason'))) {
            if (isset($_POST)) {
                $old_post = $_POST;
            }
            ...
            if (! isset($_GET['view' . $this->table]))
				...
				<fieldset>
					<form method="post" action="' . $currentIndex . '&token=' . $this->token . '&rebuildMvt=1">
						<label for="stock_rebuild">' .
                 $this->l('Calculate the movement of inventory missing') . '</label>
						<div class="margin-form">
							<input class="button" type="submit" name="rebuildStock" value="' .
                 $this->l('Submit') . '" />
						</div>
					</form>
				</fieldset><br />';
        ...
        $this->fieldsDisplay = array(
        'id_stock_mvt_reason' => array('title' => $this->l('ID'), 'width' => 40), 
        'sign' => array('title' => $this->l('Sign'), 'width' => 15, 
        'align' => 'center', 'type' => 'select', 
        'icon' => array(- 1 => 'arrow_down.png', 1 => 'arrow_up.png'), 
        'orderby' => false), 
        ...
        'cast' => 'intval', 'type' => 'select', 'list' => $reasons, 
        'identifier' => 'id_stock_mvt_reason'));
        unset($this->_select, $this->_join, $this->_group, $this->_filterHaving, 
        $this->_filter);
