...
class AdminCatalog extends AdminTab
{
    ...
    {
        global $currentIndex;
        if (((Tools::isSubmit('submitAddcategory') or
         Tools::isSubmit('submitAddcategoryAndStay')) and
         sizeof($this->adminCategories->_errors)) or
         isset($_GET['updatecategory']) or isset($_GET['addcategory'])) {
         ...
         Tools::isSubmit('submitAddproductAndPreview') or
         Tools::isSubmit('submitAddproductAndStay') or
         Tools::isSubmit('submitSpecificPricePriorities') or
         Tools::isSubmit('submitPriceAddition') or
         Tools::isSubmit('submitPricesModification')) and
         sizeof($this->adminProducts->_errors)) or
         Tools::isSubmit('updateproduct') or Tools::isSubmit('addproduct')) {
            $this->adminProducts->displayForm($this->token);
            if (Tools::getValue('id_category') > 1)
                echo '<br /><br /><a href="index.php?tab=' .
                 Tools::getValue('tab') . '&token=' . $this->token .
                 '"><img src="../img/admin/arrow2.gif" /> ' .
                 $this->l('Back to home') . '</a><br />';
            else
                echo '<br /><br /><a href="index.php?tab=' .
                 Tools::getValue('tab') . '&token=' . $this->token .
                 '"><img src="../img/admin/arrow2.gif" /> ' .
                 $this->l('Back to catalog') . '</a><br />';
        } elseif (isset($_GET['attributegenerator'])) {
            if (! isset($this->attributeGenerator)) {
                include_once (PS_ADMIN_DIR . '/tabs/AdminAttributeGenerator.php');
                $this->attributeGenerator = new AdminAttributeGenerator();
            ...
            echo '<div class="cat_bar"><span style="color: #3C8534;">' .
             ...
             getPath($catBarIndex, $id_category) . '</div>';
