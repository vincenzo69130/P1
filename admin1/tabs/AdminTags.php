...
class AdminTags extends AdminTab
{
    public function __construct ()
    {
        ...
        'id_tag' => array('title' => $this->l('ID'), 'align' => 'center', 
        'width' => 25, 'filter_key' => 'a!id_seller_message'), 
        'lang' => array('title' => $this->l('Language'), 
        'filter_key' => 'l!name'), 
    ...
    {
        if ($this->tabAccess['edit'] === '1' and
         Tools::getValue('submitAdd' . $this->table))
            if ($id = (int) (Tools::getValue($this->identifier)) and
             $obj = new $this->className($id) and Validate::isLoadedObject($obj))
    ...
    public function displayForm ($isMainTab = true)
    {
        ...
        echo '
		<form id="formTag" action="' . $currentIndex . '&submitAdd' . $this->table .
         '=1&token=' . $this->token . '" method="post" class="width5">
        ...
        foreach ($this->_languages as $language)
            echo '		<option value="' . $language['id_lang'] . '" ' .
             ($language['id_lang'] == $this->getFieldValue($obj, 'id_lang') ? 'selected="selected"' : '') .
