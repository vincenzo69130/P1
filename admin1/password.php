...
if (isset($_POST['Submit'])) {
    ...
    elseif (! Validate::isEmail($_POST['email']))
        ...
        if (! $employee->getByemail($_POST['email']) or ! $employee)
            ...
            if ((strtotime(
            $employee->last_passwd_gen . '+' .
             Configuration::get('PS_PASSWD_TIME_BACK') . ' minutes') - time()) > 0)
                ...
                if (! $result)
                    $errors[] = Tools::displayError(
                    'An error occurred during your password change.');
                else {
                    if (Mail::Send((int) $id_lang, 'password', 
                    Mail::l('Your new admin password'), 
                    array('{email}' => $employee->email, 
                    '{lastname}' => $employee->lastname, 
                    '{firstname}' => $employee->firstname, '{passwd}' => $pwd), 
                    $employee->email, 
...
echo '
	<div id="login">
		...
		<form action="' . Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']) . '" method="post">
			<div class="page-title center">' . translate('Forgot your password?') .
 '</div><br />';
if (isset($confirmation))
    echo '	<br />
     ...
     '.</div>
			<div style="margin: 2em 0 0 0; text-align: right;"><a href="login.php?email=' .
     Tools::safeOutput(Tools::getValue('email')) . '">> ' .
     translate('back to login home') . '</a></div>';
else
    echo '	<span style="font-weight: bold;">' .
     translate('Please, enter your e-mail address') . ' </span>
			' .
     translate(
    '(the one you wrote during your registration) in order to receive your access codes by e-mail') . '.<br />
     ...
     translate('Send') . '" class="button" /></div>
				<div id="lost">&nbsp;</div>
			</div>
		</form>
	</div>
