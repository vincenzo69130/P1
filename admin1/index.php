...
if ($id_tab = checkingTab($tab)) {
    ...
    foreach ($tabs as $key => $item)
        $bread .= ' <img src="../img/admin/separator_breadcrum.png" style="margin-right:5px" alt="&gt;" />
			' . ((sizeof($tabs) - 1 >
         $key) ? '<a href="?tab=' . $item['class_name'] . '&token=' . Tools::getAdminToken(
        $item['class_name'] . intval($item['id_tab']) .
    ...
    echo '<script type="text/javascript">
		...
		$(function() {
			$.ajax({
				type: \'POST\',
				url: \'ajax.php\',
				data: \'helpAccess=1&item=' . $item['class_name'] . '&isoUser=' .
     $isoUser . '&country=' .
     Country::getIsoById(Configuration::get('PS_COUNTRY_DEFAULT')) . '&version=' .
     _PS_VERSION_ . '\',
				async : true,
    ...
    echo '<div class="path_bar">
		<div id="help-button" class="floatr" style="display: none; font-family: Verdana; font-size: 10px; margin-right: 4px; margin-top: 4px;">
			...
			<a href="?token=' .
     Tools::getAdminToken(
    $tab . intval(Tab::getIdFromClassName($tab)) . intval($cookie->id_employee)) .
     '">' . translate('Back Office') . '</a>
			' . $bread . '
		</div>';
        ...
        if ($adminObj->checkToken()) {
                ...
                foreach ($_POST as $key => $value)
                    if (is_array($adminObj->table)) {
                        foreach ($adminObj->table as $table)
                            if (strncmp($key, $table . 'Filter_', 7) === 0 or
                             strncmp($key, 'submitFilter', 12) === 0)
                                $cookie->$key = ! is_array($value) ? $value : serialize(
                                $value);
                    } elseif (strncmp($key, $adminObj->table . 'Filter_', 7) ===
                     0 or strncmp($key, 'submitFilter', 12) === 0)
                        $cookie->$key = ! is_array($value) ? $value : serialize(
                        $value);
            if (isset($_GET) and ! empty($_GET) and isset($adminObj->table))
                foreach ($_GET as $key => $value)
                    if (is_array($adminObj->table)) {
                        foreach ($adminObj->table as $table)
                            if (strncmp($key, $table . 'OrderBy', 7) === 0 or
                             strncmp($key, $table . 'Orderway', 8) === 0)
                                $cookie->$key = $value;
                    } elseif (strncmp($key, $adminObj->table . 'OrderBy', 7) ===
                     0 or strncmp($key, $adminObj->table . 'Orderway', 12) === 0)
                        $cookie->$key = $value;
            ...
            $url = preg_replace('/([&?]token=)[^&]*(&.*)?$/', 
            '${1}' . $adminObj->token . '$2', $_SERVER['REQUEST_URI']);
            if (false === strpos($url, '?token=') and
             false === strpos($url, '&token='))
            ...
            translate(
            'I understand the risks and I really want to display this page')) . '" style="height:30px;margin-top:5px" />
					</a>
					<a href="index.php" method="get" style="float:left;margin:10px">
						<input type="button" value="' .
             Tools::htmlentitiesUTF8(translate('Take me out of here!')) . '" style="height:40px" />
					</a>
				</body></html>';
            die();
        }
