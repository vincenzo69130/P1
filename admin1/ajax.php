...
if (isset($_GET['installBoughtModule'])) {
    ...
    $sourceFile = 'http://addons.prestashop.com/iframe/getboughtfile.php?id_order_detail=' .
     Tools::getValue('id_order_detail') . '&token=' . Tools::getValue('token');
    if (! copy($sourceFile, $file)) {
        if (! ($content = file_get_contents($sourceFile)))
            die(
            displayJavascriptAlert(
            'Access denied: Please download your module directly from PrestaShop Addons website'));
        elseif (! file_put_contents($file, $content))
            die(
            displayJavascriptAlert(
            'Local error: your module directory is not writable'));
    }
...
{
    echo '<script type="text/javascript">alert(\'' . addslashes($s) .
     '\');</script>';
}
if (isset($_GET['ajaxProductManufacturers'])) {
    ...
    if ($manufacturers) {
        ...
        foreach ($manufacturers as $manufacturer)
            $jsonArray[] = '{"optionValue": "' . $manufacturer['id_manufacturer'] .
             '", "optionDisplay": "' .
...
if (isset($_GET['ajaxProductSuppliers'])) {
    ...
    if ($suppliers) {
        ...
        foreach ($suppliers as $supplier)
            $jsonArray[] = '{"optionValue": "' . $supplier['id_supplier'] .
             '", "optionDisplay": "' . htmlspecialchars(trim($supplier['name'])) .
             '"}';
        die('[' . implode(',', $jsonArray) . ']');
    }
}
if (isset($_GET['ajaxProductAccessories'])) {
    ...
    foreach ($products as $accessory)
        $jsonArray[] = '{"value: "' . (int) ($accessory['id_product']) . '-' .
         addslashes($accessory['name']) . '", "text":"' .
         (int) ($accessory['id_product']) . ' - ' .
         addslashes($accessory['name']) . '"}';
    die('[' . implode(',', $jsonArray) . ']');
}
if (isset($_GET['ajaxDiscountCustomers'])) {
	...
	AND ' . (Validate::isUnsignedInt($filter) ? '`id_customer` = ' .
     (int) ($filter) : '(`email` LIKE "%' . pSQL($filter) . '%"
	' . ((Validate::isBool_Id($filter) and $filterArray[0] == 0) ? 'OR `id_customer` = ' .
     (int) ($filterArray[1]) : '') . '
	' .
	...
	LEFT JOIN `' . _DB_PREFIX_ .
     'group_lang` AS gl ON (g.`id_group` = gl.`id_group` AND gl.`id_lang` = ' .
     (int) ($cookie->id_lang) . ')
	WHERE ' . (Validate::isUnsignedInt($filter) ? 'g.`id_group` = ' .
     (int) ($filter) : 'gl.`name` LIKE "%' . pSQL($filter) . '%"
	' . ((Validate::isBool_Id($filter) and $filterArray[0] == 1) ? 'OR g.`id_group` = ' .
     (int) ($filterArray[1]) : '')) . '
	ORDER BY gl.`name` ASC
	LIMIT 50');
    $json = '{"customers" : ';
    foreach ($customers as $customer)
        $jsonArray[] = '{"value":"0_' . (int) ($customer['id_customer']) .
         '", "text":"' . addslashes($customer['name']) . ' (' .
         addslashes($customer['email']) . ')"}';
    $json .= '[' . implode(',', $jsonArray) . '],
		"groups" : ';
    $jsonArray = array();
    foreach ($groups as $group)
        $jsonArray[] = '{"value":"1_' . (int) ($group['id_group']) .
         '", "text":"' . addslashes($group['name']) . '"}';
    $json .= '[' . implode(',', $jsonArray) . ']}';
    die($json);
}
if (Tools::getValue('page') == 'prestastore' and
 @fsockopen('addons.prestashop.com', 80, $errno, $errst, 3))
    readfile(
    'http://addons.prestashop.com/adminmodules.php?lang=' .
     Language::getIsoById($cookie->id_lang));
if (Tools::getValue('page') == 'themes' and
 @fsockopen('addons.prestashop.com', 80, $errno, $errst, 3))
    readfile(
    'http://addons.prestashop.com/adminthemes.php?lang=' .
     Language::getIsoById($cookie->id_lang));
if ($step = (int) (Tools::getValue('ajaxProductTab'))) {
    ...
    $switchArray = array(3 => 'displayFormPrices', 4 => 'displayFormAttributes', 
    5 => 'displayFormFeatures', 6 => 'displayFormCustomization', 
    7 => 'displayFormAttachments');
    $currentIndex = 'index.php?tab=AdminCatalog';
...
if (array_key_exists('ajaxModulesPositions', $_POST)) {
    ...
    $position = (is_array($positions)) ? array_search(
    $id_hook . '_' . $id_module, $positions) : null;
    ...
    if (Validate::isLoadedObject($module))
        if ($module->updatePosition($id_hook, $way, $position))
            ...
            die(
            '{"hasError" : true, "errors" : "Can not update module position"}');
    else
...
if (array_key_exists('ajaxCategoriesPositions', $_POST)) {
    ...
    if (is_array($positions))
        foreach ($positions as $key => $value) {
            $pos = explode('_', $value);
            if ((isset($pos[1]) and isset($pos[2])) and
             ($pos[1] == $id_category_parent and $pos[2] == $id_category_to_move)) {
                $position = $key;
                break;
            }
        }
    ...
    if (Validate::isLoadedObject($category)) {
        if (isset($position) && $category->updatePosition($way, $position)) {
            ...
            die(
            '{"hasError" : true, errors : "Can not update categories position"}');
    } else
...
if (array_key_exists('ajaxCMSCategoriesPositions', $_POST)) {
    ...
    if (is_array($positions))
        foreach ($positions as $key => $value) {
            $pos = explode('_', $value);
            if ((isset($pos[1]) and isset($pos[2])) and ($pos[1] ==
             $id_cms_category_parent and $pos[2] == $id_cms_category_to_move)) {
                $position = $key;
                break;
            }
        }
    ...
    if (Validate::isLoadedObject($cms_category)) {
        if (isset($position) && $cms_category->updatePosition($way, $position))
            ...
            die(
            '{"hasError" : true, "errors" : "Can not update cms categories position"}');
    } else
        die(
        '{"hasError" : true, "errors" : "This cms category can not be loaded"}');
}
if (array_key_exists('ajaxCMSPositions', $_POST)) {
    ...
    if (is_array($positions))
        foreach ($positions as $key => $value) {
            $pos = explode('_', $value);
            if ((isset($pos[1]) and isset($pos[2])) and
             ($pos[1] == $id_category and $pos[2] == $id_cms)) {
                $position = $key;
                break;
            }
        }
...
if (array_key_exists('ajaxProductsPositions', $_POST)) {
    ...
    if (is_array($positions))
        foreach ($positions as $position => $value) {
            ...
            $pos = explode('_', $value);
            if ((isset($pos[1]) and isset($pos[2])) and
             ($pos[1] == $id_category and (int) $pos[2] === $id_product)) {
                if ($product = new Product((int) $pos[2]))
                    if (isset($position) &&
                     $product->updatePosition($way, $position))
                        echo "ok position $position for product $pos[2]\r\n";
                    else
                        echo '{"hasError" : true, "errors" : "Can not update product ' .
                         $id_product . ' to position ' . $position . ' "}';
                else
                    echo '{"hasError" : true, "errors" : "This product (' .
                     $id_product . ') can t be loaded"}';
                break;
            }
        }
}
if (isset($_GET['ajaxProductPackItems'])) {
	...
	NATURAL LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl
	WHERE pl.`id_lang` = ' . (int) (Tools::getValue('id_lang')) . '
	AND p.`id_product` NOT IN (SELECT DISTINCT id_product_pack FROM `' .
     _DB_PREFIX_ . 'pack`)
	AND p.`id_product` != ' . (int) (Tools::getValue('id_product')));
    foreach ($products as $packItem)
        $jsonArray[] = '{"value": "' . (int) ($packItem['id_product']) . '-' .
         addslashes($packItem['name']) . '", "text":"' .
         (int) ($packItem['id_product']) . ' - ' . addslashes($packItem['name']) .
         '"}';
    die('[' . implode(',', $jsonArray) . ']');
}
if (isset($_GET['ajaxStates']) and isset($_GET['id_country'])) {
    ...
    if (is_array($states) and ! empty($states)) {
        ...
        foreach ($states as $state)
            $list .= '<option value="' . (int) ($state['id_state']) . '"' .
             ((isset($_GET['id_state']) and
             $_GET['id_state'] == $state['id_state']) ? ' selected="selected"' : '') .
             '>' . $state['name'] . '</option>' . "\n";
    } else
        $list = 'false';
    die($list);
}
if (Tools::isSubmit('submitCustomerNote') and
 $id_customer = (int) Tools::getValue('id_customer')) {
    $note = html_entity_decode(Tools::getValue('note'));
    if (! empty($note) and ! Validate::isCleanHtml($note))
        die('error:validation');
    if (! Db::getInstance()->Execute(
    'UPDATE ' . _DB_PREFIX_ . 'customer SET `note` = "' . pSQL($note, true) .
     '" WHERE id_customer = ' . (int) $id_customer . ' LIMIT 1'))
        die('error:update');
    die('ok');
}
if (Tools::getValue('form_language_id')) {
    if (! ($cookie->employee_form_lang = (int) (Tools::getValue(
    'form_language_id'))))
...
if (Tools::getValue('submitPublishProduct')) {
    ...
    if (Tools::getIsset('id_product')) {
        ...
        'AdminCatalog' . (int) ($id_tab_catalog) . (int) ($cookie->id_employee));
        $bo_product_url = dirname($_SERVER['PHP_SELF']) .
         '/index.php?tab=AdminCatalog&id_product=' . $id_product .
         '&updateproduct&token=' . $token;
        if (Tools::getValue('redirect'))
            die($bo_product_url);
        $profileAccess = Profile::getProfileAccess((int) $cookie->profile, 
        $id_tab_catalog);
...
if (Tools::getValue('submitPublishCMS')) {
    ...
    if (Tools::getIsset('id_cms')) {
        ...
        $token = Tools::getAdminToken(
        'AdminCMSContent' . (int) ($id_tab_cms) . (int) ($cookie->id_employee));
        $bo_cms_url = dirname($_SERVER['PHP_SELF']) .
         '/index.php?tab=AdminCMSContent&id_cms=' . (int) $id_cms .
         '&updatecms&token=' . $token;
        if (Tools::getValue('redirect'))
            die($bo_cms_url);
        $profileAccess = Profile::getProfileAccess((int) $cookie->profile, 
        $id_tab_cms);
...
if (Tools::isSubmit('saveImportMatchs')) {
    ...
    'INSERT INTO  `' . _DB_PREFIX_ . 'import_match` (
								`id_import_match` ,
								`name` ,
								`match`,
								`skip`
...
if (Tools::isSubmit('loadImportMatchs')) {
    ...
    'SELECT * FROM `' . _DB_PREFIX_ . 'import_match` WHERE `id_import_match` = ' .
     (int) Tools::getValue('idImportMatchs'));
    die(
    '{"id" : "' . $return[0]['id_import_match'] . '", "matchs" : "' .
     $return[0]['match'] . '", "skip" : "' . $return[0]['skip'] . '"}');
}
...
if (Tools::isSubmit('ajaxAddZipCode') or Tools::isSubmit('ajaxRemoveZipCode')) {
    ...
    if (Tools::isSubmit('ajaxAddZipCode')) {
        if ($county->isZipCodeRangePresent($zipcodes))
            die(
            'error:' . Tools::displayError('This Zip Code is already in use.'));
        if ($county->addZipCodes($zipcodes))
            die(AdminCounty::renderZipCodeList($county->getZipCodes()));
    } elseif (Tools::isSubmit('ajaxRemoveZipCode') and
     $county->removeZipCodes($zipcodes))
        die(AdminCounty::renderZipCodeList($county->getZipCodes()));
    die('error');
}
...
if (Tools::isSubmit('getHookableList')) {
    if (! strlen(Tools::getValue('hooks_list')))
        die(
        '{"hasError" : true, "errors" : ["Live Edit : no module on this page"]}');
...
if (Tools::isSubmit('getHookableModuleList')) {
    ...
    'SELECT id_module, name FROM `' . _DB_PREFIX_ . 'module` ');
    foreach ($modules as $module) {
        if (file_exists(
        _PS_MODULE_DIR_ . $module['name'] . '/' . $module['name'] . '.php')) {
            include_once (_PS_MODULE_DIR_ . $module['name'] . '/' .
             $module['name'] . '.php');
            $mod = new $module['name']();
            if ($mod->isHookableOn($hook_name))
                $hookableModulesList[] = array('id' => (int) $mod->id, 
                'name' => $mod->displayName, 
                'display' => Module::hookExec($hook_name, array(), (int) $mod->id));
        }
    }
...
if (Tools::isSubmit('saveHook')) {
    ...
    foreach ($hooks_list as $hook) {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'hook_module` WHERE `id_hook` = (SELECT id_hook FROM `' . _DB_PREFIX_ .
         'hook` WHERE `name` = \'' . pSQL($hook) . '\' LIMIT 0, 1)');
        $hookedModules = explode(',', Tools::getValue($hook));
        $i = 1;
        $value = '';
        foreach ($hookedModules as $module) {
            $ids = explode('_', $module);
            $value .= '(' . (int) $ids[1] . ', (SELECT id_hook FROM `' .
             _DB_PREFIX_ . 'hook` WHERE `name` = \'' . pSQL($hook) .
             '\' LIMIT 0, 1), ' . (int) $i . '),';
            $i ++;
        }
        $value = rtrim($value, ',');
        ...
        'INSERT INTO  `' . _DB_PREFIX_ .
         'hook_module` (`id_module`, `id_hook`, `position`) VALUES ' . $value);
    }
...
if (Tools::isSubmit('getAdminHomeElement')) {
    ...
    $protocol .
     '://www.prestashop.com/partner/preactivation/preactivation-block.php?version=1.0&shop=' .
     urlencode(Configuration::get('PS_SHOP_NAME')) . '&protocol=' . $protocol .
     '&url=' . urlencode($_SERVER['HTTP_HOST']) . '&iso_country=' . $isoCountry .
     '&iso_lang=' . Tools::strtolower($isoUser) . '&id_lang=' .
     (int) $cookie->id_lang . '&email=' .
     urlencode(Configuration::get('PS_SHOP_EMAIL')) . '&date_creation=' .
     _PS_CREATION_DATE_ . '&v=' . _PS_VERSION_ . '&security=' .
     md5(Configuration::get('PS_SHOP_EMAIL') . _COOKIE_IV_), false, 
    $stream_context);
    if (! $content)
        $result['partner_preactivation'] = 'NOK';
    else {
        $content = explode('|', $content);
        if ($content[0] == 'OK' && Validate::isCleanHtml($content[2]) &&
         Validate::isCleanHtml($content[1])) {
            ...
            foreach ($content[1] as $partnerPopUp)
                if ($partnerPopUp) {
                    ...
                    'PS_PREACTIVATION_' . strtoupper($partnerPopUp[0]))) {
    ...
    $content = @file_get_contents(
    'https://www.prestashop.com/partner/preactivation/preactivation-warnings.php?version=1.0&partner=paypal&iso_country=' .
     Tools::strtolower(
    Country::getIsoById(Configuration::get('PS_COUNTRY_DEFAULT'))) . '&iso_lang=' .
     Tools::strtolower(Language::getIsoById(intval($cookie->id_lang))) .
     '&id_lang=' . (int) $cookie->id_lang . '&email=' .
     urlencode(Configuration::get('PS_SHOP_EMAIL')) . '&security=' .
     md5(Configuration::get('PS_SHOP_EMAIL') . _COOKIE_IV_), false, 
    $stream_context);
    $content = explode('|', $content);
    ...
    $content = @file_get_contents(
    $protocol .
     '://www.prestashop.com/partner/prestashop/prestashop-link.php?iso_country=' .
     $isoCountry . '&iso_lang=' . Tools::strtolower($isoUser) . '&id_lang=' .
     (int) $cookie->id_lang, false, $stream_context);
    if (! $content)
        ...
        if ($content[0] == 'OK' && Validate::isCleanHtml($content[1]))
            ...
            $result['discover_prestashop'] = 'NOK';
        if (@fsockopen('www.prestashop.com', 80, $errno, $errst, 3))
            $result['discover_prestashop'] .= '<iframe frameborder="no" style="margin: 0px; padding: 0px; width: 315px; height: 290px;" src="' .
             $protocol . '://www.prestashop.com/rss/news2.php?v=' . _PS_VERSION_ .
             '&lang=' . $isoUser . '"></iframe>';
        $content = @file_get_contents(
        $protocol .
         '://www.prestashop.com/partner/paypal/paypal-tips.php?protocol=' .
         $protocol . '&iso_country=' . $isoCountry . '&iso_lang=' .
         Tools::strtolower($isoUser) . '&id_lang=' . (int) $cookie->id_lang, 
...
if (Tools::isSubmit('getChildrenCategories') &&
 Tools::getValue('id_category_parent')) {
    ...
    Tools::getValue('id_category_parent'), Tools::getValue('selectedCat'), 
    $cookie->id_lang);
