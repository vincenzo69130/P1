...
class Search
{
    ...
    var $searchkeywords = array('mtime_from' => '', 'mtime_to' => '', 
    'name' => '', 'size_from' => '', 'size_to' => '', 'recursive' => '0');
    ...
    function __construct ($rootFolder)
    {
        ...
        $tem['size'] = (is_dir($this->rootFolder) ? '' : transformFileSize(
        @filesize($this->rootFolder)));
        //$tem['ctime'] = date(DATE_TIME_FORMAT, $tem['ctime']);
        //$tem['mtime'] = date(DATE_TIME_FORMAT, $tem['mtime']);
        $tem['flag'] = (array_search($tem['path'], 
        $selectedDocuments) !== false ? ($this->sessionAction->getAction() ==
    ...
    function doSearch ($baseFolderPath = null)
    {
        ...
        backslashToSlash(
        (is_null($baseFolderPath) ? $this->rootFolder : $baseFolderPath)));
            ...
            while (false !== ($file = readdir($dirHandler))) {
                if ($file != '.' && $file != '..') {
                    ...
                    if (is_file($path)) {
                        ...
                        if ($this->searchkeywords['name'] !== '' &&
                         @eregi($this->searchkeywords['name'], $file) === false) {
                            $isValid = false;
                        }
                        if ($this->searchkeywords['mtime_from'] != '' && $fileTime <
                         @strtotime($this->searchkeywords['mtime_from'])) {
                            $isValid = false;
                        }
                        if ($this->searchkeywords['mtime_to'] != '' && $fileTime >
                         @strtotime($this->searchkeywords['mtime_to'])) {
                            $isValid = false;
                        }
                        if ($this->searchkeywords['size_from'] != '' && $fileSize <
                         @strtotime($this->searchkeywords['size_from'])) {
                            $isValid = false;
                        }
                        ...
                        if ($isValid && isListingDocument($path)) {
                            ...
                            $tem['ctime'] = date(DATE_TIME_FORMAT, 
                            $tem['ctime']);
                            $tem['mtime'] = date(DATE_TIME_FORMAT, 
                            $tem['mtime']);
                            $tem['flag'] = (array_search($tem['path'], 
                            $selectedDocuments) !== false ? ($this->sessionAction->getAction() ==
                             "copy" ? 'copyFlag' : 'cutFlag') : 'noFlag');
