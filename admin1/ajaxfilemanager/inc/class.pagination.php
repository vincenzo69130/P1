...
class pagination
{
    ...
    {
        $this->currentPage = ((isset($_GET[$this->pageIndex]) &&
         (int) ($_GET[$this->pageIndex]) > 0) ? (int) ($_GET[$this->pageIndex]) : 1);
    }
    /**
    ...
    function __setBaseUrl ()
    {
        ...
        if ($this->friendlyUrl) {
            ...
            if (sizeof($appendingQueryStrings)) {
                $this->baseUrl = $this->__appendQueryString($this->url, 
                implode("&", $appendingQueryStrings));
            } else {
                $this->baseUrl = $this->getUrl();
            }
        }
    ...
    function getLastUrl ()
    {
        ...
        if ($this->currentPage < $totalPages) {
            ...
            if ($this->friendlyUrl) {
                ...
                $url = $this->__appendQueryString($url, 
                $this->pageIndex . "=" . $totalPages);
            }
            $url = sprintf(
            '<a href="%s" class="pagination_last"><span>%s</span></a>', $url, 
            $this->lastText);
        }
        return $url;
    }
    ...
    function getFirstUrl ()
    {
        ...
        if ($this->currentPage > 1) {
            ...
            $url = sprintf(
            '<a href="%s" class="pagination_first"><span>%s</span></a>', $url, 
            $this->firstText);
        }
        return $url;
    }
    ...
    function getPreviousUrl ()
    {
        ...
        if ($this->currentPage > 1 && $this->totalItems > 0) {
            ...
            $url = sprintf(
            '<a href="%s" class="pagination_previous"><span>%s</span></a>', $url, 
            $this->previousText);
        }
        return $url;
    }
    ...
    function getNextUrl ()
    {
        ...
        if ($this->totalItems > ($this->currentPage * $this->itemsPerPage)) {
            ...
            $url = sprintf(
            '<a href="%s" class="pagination_next"><span>%s</span></a>', $url, 
            $this->nextText);
        }
        return $url;
    }
    ...
    function getGroupUrls ()
    {
        ...
        if ($this->totalItems > 0) {
            ...
            $itemEnd = $itemStart;
            while ($itemEnd < ($itemStart + $this->groupLimit - 1) &&
             $itemEnd < $totalPages) {
                $itemEnd ++;
            }
            if ($totalPages > ($itemEnd - $itemStart)) {
                for ($i = $itemStart; $i > 1 &&
                 ($itemEnd - $itemStart + 1) < $this->groupLimit; $i --) {
                    $itemStart --;
                }
            }
            for ($item = $itemStart; $item <= $itemEnd; $item ++) {
                if ($item != $this->currentPage) { //it is not the active link
                    if ($this->friendlyUrl) {
                        ...
                        $temUrl = $this->__appendQueryString($url, 
                        $this->pageIndex . "=" . $item);
                    }
                    $output .= sprintf(
                    ' <a class="pagination_group" href="%s"><span>%d</span></a> ', 
                    $temUrl, $item);
                } else { //active link
                    $output .= sprintf(
                    ' <a class="pagination_group pagination_active" href="#"><span>%d</span></a> ', 
                    $item);
                }
            }
        }
    ...
    function getPaginationHTML ($type = 1, $cssClass = "pagination")
    {
        ...
        switch ($type) {
            case "2":
                $output .= "<span class=\"pagination_summany\">" .
                 $this->getFirstItem() . " to " . $this->getLastItem() . " of " .
                 $this->getTotal() . " results.</span> ";
            ...
            default:
                $output .= "<span class=\"pagination_summany\">" .
                 $this->getFirstItem() . "/" . $this->getLastItem() . " (" .
                 $this->getTotal() . ")</span> ";
                ...
                $itemPerPage = '';
                $itemPerPage .= "<select name=\"" . $this->limitIndex .
                 "\" id=\"limit\" class=\"input inputLimit\" onchange=\"changePaginationLimit();\">\n";
                foreach ($this->limits as $v) {
                    $itemPerPage .= "<option value=\"" . $v . "\" " .
                     ($v == $this->itemsPerPage ? 'selected="selected"' : '') .
                     ">" . $v . "</option>\n";
                }
                $itemPerPage .= "</select>\n";
                ...
                $output .= "</span>";
                $output .= "<span class=\"pagination_parent\"><a href=\"#\" onclick=\"goParentFolder();\" id=\"pagination_parent_link\" title=\"" .
                 PAGINATION_GO_PARENT . "\">&nbsp;</a></span>";
        }
        $output .= "</p></div>";
        return $output;
    }
