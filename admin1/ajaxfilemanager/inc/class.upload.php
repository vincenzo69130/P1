...
class Upload
{
    ...
    var $errorCodes = array(0 => 'the file uploaded with success', 
    1 => 'The uploaded file exceeds the upload_max_filesize directive in php.ini', 
    2 => 'The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form', 
    3 => 'The uploaded file was only partially uploaded', 
    4 => 'No file was uploaded.', 6 => 'Missing a temporary folder', 
    7 => 'Failed to write file to disk', 8 => 'File upload stopped by extension', 
    ...
    function isFileUploaded ($indexInPost = "file")
    {
        ...
        if ((isset($_FILES[$indexInPost]['error']) && $_FILES[$indexInPost] == 0) || (! empty(
        $_FILES[$indexInPost]['tmp_name']) &&
         $_FILES[$indexInPost]['tmp_name'] != 'none')) {
    ...
    function isPermittedFileExt ($validFileExt = array())
    {
        ...
        if (sizeof($validFileExt) && sizeof($this->invalidFileExt)) {
            foreach ($validFileExt as $k => $ext) {
                if (array_search(strtolower($ext), $this->invalidFileExt) !==
                 false) {
        ...
        if (sizeof($validFileExt)) {
            if (array_search(strtolower($this->getFileExt()), $validFileExt) !==
             false) {
                return true;
            }
        } elseif (array_search(strtolower($this->getFileExt()), 
        $this->invalidFileExt) === false) {
            return true;
        }
    ...
    function moveUploadedFile ($dest, $fileBaseName = '', $overwrite = false)
    {
        ...
        if (! $overwrite && file_exists($filePath) && is_file($filePath)) { //rename
            ...
            while (file_exists($dest . $fileName) && is_file($dest . $fileName)) {
                $counter ++;
                $fileName = $this->fileBaseName . '_' . $counter .
                 $this->fileExtension;
            }
    ...
    function isImage ($invalidImageExts = array(), $delete = true)
    {
        ...
        if (array_search(strtolower($this->getFileExt()), $ValidImageExts) !==
         false) {
    ...
    {
        if (empty($filePath)) {
            $filePath = $this->dirPath . $this->fileBaseName . $thumb_suffix .
             $this->fileExtension;
        }
    ...
    function _resize ($fileName, $new_x, $new_y)
    {
        ...
        if (function_exists($functionName)) {
            ...
            $functionName . " function is unavailable");
            return false;
        }
        ...
        if (function_exists('ImageCopyResampled')) {
            ImageCopyResampled($new_img, $this->imgHandler, 0, 0, 0, 0, $new_x, 
            $new_y, $this->img_x, $this->img_y);
        } else {
            ImageCopyResized($new_img, $this->imgHandler, 0, 0, 0, 0, $new_x, 
            $new_y, $this->img_x, $this->img_y);
        }
        if ($this->_imageSave($new_img, $fileName, 80)) {
            return array("width" => $new_x, "height" => $new_y, 
     ...
     */
    function getThumbInfo ($originalImageName, $originaleImageWidth, 
    $originalImageHeight, $thumbnailSuffix, $thumbnailWidth, $thumbnailHeight)
    {
        $outputs = array("name" => "", "width" => 0, "height" => 0);
        $thumbnailWidth = (int) ($thumbnailWidth);
        $thumbnailHeight = (int) ($thumbnailHeight);
        if (! empty($originalImageName) && ! empty($originaleImageWidth) &&
         ! empty($originalImageHeight)) {
     ...
     */
    function deleteFileAndThumbs ($dirPath, $originalImageName, 
    $arrayThumbnailSuffix)
    {
        //ensure the directory path ending with /
        if ($dirPath != '' && substr($dirPath, - 1) != '/') {
            $dirPath .= '/';
        }
        if (! empty($originalImageName) &&
         file_exists($dirPath . $originalImageName) &&
         is_file($dirPath . $originalImageName)) {
