...
class file
{
    ...
    function copyTo ($source, $dest)
    {
        ...
        if (getAbsPath($source) == getAbsPath($dest)) {
            $this->_debug(
            'Unable to copy itself. source: ' . getAbsPath($source) . "; dest: " .
             getAbsPath($dest));
            return false;
        }
        ...
        if (is_file($source)) {
            ...
            if (file_exists(
            addTrailingSlash($dest) . basename($source))) {
                return false;
            } else {
                if (! file_exists(addTrailingSlash($dest) . basename($source)) ||
                 ! is_dir(addTrailingSlash($dest) . basename($source))) {
                    ...
                    addTrailingSlash($dest) . basename($source))) {
                        $this->_debug(
                        'Unable to create folder (' . addTrailingSlash($dest) .
                         basename($source) . ")");
                        return false;
                    }
                }
    ...
    function getNextAvailableFileName ($fileToMove, $destFolder)
    {
        ...
        if (file_exists($fileToMove)) {
            if (is_file()) {
                ...
                $count = 1;
                while (file_exists(
                $destFolder . $fileBaseName . $count . "." . $fileExt)) {
                    $count ++;
                }
            ...
            while (FALSE !== ($item = @readdir($handle))) {
                ...
                if ($item != '.' && $item != '..') {
                    ...
                    if (is_dir($path)) {
                        ...
                        $this->__recursive_remove_directory(
                        $path);
