...
class Image
{
    ...
    var $chmod = 0755;
    var $_imgInfoOrig = array('name' => '', 'ext' => '', 'size' => '', 
    'width' => '', 'height' => '', 'type' => '', 'path' => '');
    var $_imgInfoFinal = array('name' => '', 'ext' => '', 'size' => '', 
    'width' => '', 'height' => '', 'type' => '', 'path' => '');
    var $_imgQuality = 90;
    /**
     * constructor
    ...
    function loadImage ($filename)
    {
        ...
        if ($ext == "gif") {
            ...
            if ($colorFlag) {
                ...
                $this->transparentColorRed = ord(
                substr($result, $background * 3, 1));
                $this->transparentColorGreen = ord(
                substr($result, $background * 3 + 1, 1));
                $this->transparentColorBlue = ord(
                substr($result, $background * 3 + 2, 1));
            }
            fclose($fp);
        ...
        if ($this->_imgOrig == null) {
            $this->_debug(
            "The image could not be created from the '$filename' file using the '$func' function.");
            return false;
        } else {
            $this->imageFile = $filename;
            $this->_imgInfoOrig = array('name' => basename($filename), 
            'ext' => $ext, 'size' => filesize($filename), 'path' => $filename);
    ...
    function loadImageFromString ($string)
    {
        ...
        if (! $this->_imgOrig) {
            $this->_debug(
            'The image (supplied as a string) could not be created.');
            return false;
        }
        return true;
    }
    ...
    function saveImage ($filename, $quality = 90, $forcetype = '')
    {
        ...
        $ext = ($forcetype == '') ? $this->_getExtension($filename) : strtolower(
        $forcetype);
        ...
        switch ($ext) {
            case 'gif':
                if ($this->gdInfo['Truecolor Support'] &&
                 imageistruecolor($this->_imgFinal)) {
        ...
        if ($saved === false) {
            $this->_debug(
            "The image could not be saved to the '$filename' file as the file type '$ext' using the '$func' function.");
            return false;
        } else {
    ...
    function crop ($dst_x, $dst_y, $dst_w, $dst_h)
    {
        ...
        if (($dst_w <= 0) || ($dst_h <= 0)) {
            $this->_debug(
            'The image could not be cropped because the size given is not valid.');
            return false;
        }
        if (($dst_w > imagesx($this->_imgOrig)) ||
         ($dst_h > imagesy($this->_imgOrig))) {
            $this->_debug(
        ...
        if ($this->gdInfo['Truecolor Support']) {
            if (! @imagecopyresampled($this->_imgFinal, $this->_imgOrig, 0, 0, 
            $dst_x, $dst_y, $dst_w, $dst_h, $dst_w, $dst_h)) {
                $this->_debug('Unable crop the image.');
                return false;
            }
        } else {
            if (! @imagecopyresized($this->_imgFinal, $this->_imgOrig, 0, 0, 
            $dst_x, $dst_y, $dst_w, $dst_h, $dst_w, $dst_h)) {
                $this->_debug('Unable crop the image.');
                return false;
            }
        ...
        if ($new_x <= 0 || $new_y <= 0) {
            ...
            if ($constraint) {
                if ($new_x < 1 && $new_y < 1) {
                    ...
                    $new_x = floor(
                    $new_y / $this->_imgInfoOrig['height'] *
                     $this->_imgInfoOrig['width']);
                } elseif ($new_y < 1) {
                    $new_y = floor(
                    $new_x / $this->_imgInfoOrig['width'] *
                     $this->_imgInfoOrig['height']);
                } else {
                    $scale = min($new_x / $this->_imgInfoOrig['width'], 
                    $new_y / $this->_imgInfoOrig['height']);
            ...
            if ($unchangeIfsmaller) {
                if ($this->_imgInfoOrig['width'] < $new_x &&
                 $this->_imgInfoOrig['height'] < $new_y) {
                    $new_x = $this->_imgInfoOrig['width'];
                    $new_y = $this->_imgInfoOrig['height'];
                }
            }
    ...
    function _resize ($new_x, $new_y)
    {
        ...
        if ($this->_imgInfoOrig['type'] == 'png') {
            ...
            if (function_exists('ImageCopyResampled')) {
                @ImageCopyResampled($this->_imgFinal, $this->_imgOrig, 0, 0, 0, 
                0, $new_x, $new_y, $this->_imgInfoOrig['width'], 
                $this->_imgInfoOrig['height']);
            } else {
                @ImageCopyResized($this->_imgFinal, $this->_imgOrig, 0, 0, 0, 0, 
                $new_x, $new_y, $this->_imgInfoOrig['width'], 
                $this->_imgInfoOrig['height']);
            }
    ...
    function _isSupported ($filename, $extension, $function, $write = false)
    {
        ...
        $support = strtoupper($extension) .
         ($extension == 'gif' ? $giftype : ' Support');
        if (! isset($this->gdInfo[$support]) || $this->gdInfo[$support] == false) {
            $request = ($write) ? 'saving' : 'reading';
            $this->_debug(
            "Support for $request the file type '$extension' cannot be found.");
            return false;
        }
        if (! function_exists($function)) {
            $request = ($write) ? 'save' : 'read';
            $this->_debug(
            "The '$function' function required to $request the '$filename' file cannot be found.");
            return false;
        }
        return true;
    }
    ...
    {
        $this->_createFinalImageHandler($this->_imgInfoOrig['width'], 
        $this->_imgInfoOrig['height']);
        ...
        if (function_exists('ImageCopyResampled')) {
            ImageCopyResampled($this->_imgFinal, $this->_imgOrig, $dst_x, 
            $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h);
        } else {
            ImageCopyResized($this->_imgFinal, $this->_imgOrig, $dst_x, $dst_y, 
    ...
    function getGDInfo ($versionOnly = false)
    {
        ...
        if (function_exists('gd_info')) {
            ...
            $gd = array('GD Version' => '', 'GIF Read Support' => false, 
            'GIF Create Support' => false, 'JPG Support' => false, 
            'PNG Support' => false, 'FreeType Support' => false, 
            'FreeType Linkage' => '', 'T1Lib Support' => false, 
            'WBMP Support' => false, 'XBM Support' => false);
            ob_start();
            phpinfo();
            ...
            foreach (explode("\n", $buffer) as $line) {
                $line = array_map('trim', 
                (explode('|', strip_tags(str_replace('</td>', '|', $line)))));
                if (isset($gd[$line[0]])) {
    ...
    function _createFinalImageHandler ($dst_w, $dst_h)
    {
        ...
        if (! is_null($this->transparentColorRed) &&
         ! is_null($this->transparentColorGreen) &&
         ! is_null($this->transparentColorBlue)) {
            $transparent = @imagecolorallocate($targetImageIdentifier, 
            $this->transparentColorRed, $this->transparentColorGreen, 
            $this->transparentColorBlue);
            @imagefilledrectangle($this->_imgFinal, 0, 0, $dst_w, $dst_h, 
            $transparent);
