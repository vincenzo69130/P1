    ...
    if (! empty($_POST['mode'])) {
        ...
        if (sizeof($lastestSessionImageInfo) && CONFIG_SYS_DEMO_ENABLE) {
            ...
            if (sizeof($originalSessionImageInfo)) {
                $originalImage = $sessionDir .
                 $originalSessionImageInfo['info']['name'];
            }
        }
        ...
        if ($image->loadImage($originalImage)) {
            switch ($_POST['mode']) {
                case "resize":
                    if (! $image->resize($_POST['width'], $_POST['height'], 
                    (! empty($_POST['constraint']) ? true : false))) {
                        $error = IMG_SAVE_RESIZE_FAILED;
                    }
                    break;
                case "crop":
                    if (! $image->crop($_POST['x'], $_POST['y'], 
                ...
                if (! @copy($originalImage, $sessionNewPath)) { //keep a copy under the session folder
                    ...
                    if (CONFIG_SYS_DEMO_ENABLE) { //demo only
                        ...
                        if ($isSaveAsRequest) { //save as request
                            ...
                            backslashToSlash($_POST['save_to'])) .
                             $_POST['new_name'] . "." .
                             getFileExt($_POST['path']);
                            if (! file_exists($_POST['save_to']) ||
                             ! is_dir($_POST['save_to'])) {
                    ...
                    if ($image->saveImage($imagePath)) {
                        if (CONFIG_SYS_DEMO_ENABLE) {
                            if (! isset($originalSessionImageInfo) ||
                             ! sizeof($originalSessionImageInfo)) { //keep this original image information on session for future reference if demo only	
                                $originalSessionImageInfo = array(
                                'name' => basename($imagePath), 
                                'restorable' => 0, 'is_original' => 1);
                    ...
                    if (isset($imageInfo)) {
                        ...
                        $info .= ",size:'" .
                         transformFileSize($imageInfo['size']) . "'";
                        if ($isSaveAsRequest) {
                            $info .= ",save_as:'1'";
                        } else {
                            $info .= ",save_as:'0'";
...
echo $info;
echo ",history:" .
 (isset($history) && is_object($history) ? ($history->getNumRestorable()) : 0) .
 "";
echo "}";
?>