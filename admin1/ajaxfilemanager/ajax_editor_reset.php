...
if (! isset($_POST['path'])) {
    $_POST['path'] = "uploaded/Winter.jpg" . "?" . makeQueryString(
    array('path'));
    //for crop
    $_POST['mode'] = "crop";
    $_POST['x'] = 100;
    ...
    if (! empty($_POST['mode'])) {
        ...
        switch ($_POST['mode']) {
            case "resize":
                ...
                break;
            case "crop":
                if (! $image->cropToDimensions($_POST['x'], $_POST['y'], 
                (int) ($_POST['x']) + (int) ($_POST['width']), 
                (int) ($_POST['y']) + (int) ($_POST['height']))) {
        ...
        if (empty($error)) {
            $sessionNewPath = $session->getSessionDir() . uniqid(md5(time())) .
