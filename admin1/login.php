...
array_merge(array('127.0.0.1'), 
explode(',', Configuration::get('PS_MAINTENANCE_IP'))));
$errors = array();
if ((empty($_SERVER['HTTPS']) or strtolower($_SERVER['HTTPS']) == 'off') and
 Configuration::get('PS_SSL_ENABLED')) {
    ...
    if ($clientIsMaintenanceOrLocal)
        ...
        $warningSslMessage = translate(
        'SSL is activated. Please connect using the following url to log in in secure mode (https).') .
         '<br/><br/><a href="https://' . Tools::getServerName() .
         Tools::safeOutput($_SERVER['REQUEST_URI']) . '">https://' .
         Tools::getServerName() . Tools::safeOutput($_SERVER['REQUEST_URI']) .
         '</a>';
}
    ...
    elseif (! Validate::isPasswd($passwd))
        ...
        if (! $employee) {
			...
			<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="' . $iso . '" lang="' .
             ...
             str_replace('&amp;', '&', Tools::safeOutput($url, true)) . '">
				<head>
					<script language="javascript" type="text/javascript">
						window.location.replace("' .
             str_replace('&amp;', '&', Tools::safeOutput($url, true)) . '");
					</script>
					<div style="text-align:center; margin-top:250px;"><a href="' .
             str_replace('&amp;', '&', Tools::safeOutput($url, true)) . '">' .
             translate('Click here to launch Administration panel') . '</a></div>
				</head>
			</html>';
            exit();
        }
...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="' . $iso . '" lang="' .
 $iso . '">
	<head>
		...
		<title>PrestaShop&trade; - ' . translate('Administration panel') .
 '</title>';
echo '
	</head>
	<body>
...
if ($nbErrors = sizeof($errors)) {
    echo '
	<div id="error">
		<h3>' . ($nbErrors > 1 ? translate('There are') : translate('There is')) .
     ' ' . $nbErrors . ' ' .
     ($nbErrors > 1 ? translate('errors') : translate('error')) . '</h3>
...
if (file_exists(PS_ADMIN_DIR . '/../install') or
 file_exists(PS_ADMIN_DIR . '/../admin')) {
    echo '				<span>' .
     translate(
    'For security reasons, you cannot connect to the Back Office until after you have:') . '<br /><br />
    ...
    if ((empty($_SERVER['HTTPS']) or strtolower($_SERVER['HTTPS']) == 'off') and
     (Configuration::get('PS_SSL_ENABLED') and ! $clientIsMaintenanceOrLocal))
        echo '<div class="error">' . $warningSslMessage . '</div>';
    else
        echo '<label for="email">' . translate('E-mail address:') . '</label><br />
					<input type="text" id="email" name="email" value="' .
         Tools::safeOutput(Tools::getValue('email')) . '" class="input"/>
					...
					<div>
						<div id="submit"><input type="submit" name="Submit" value="' .
         translate('Log in') . '" class="button" /></div>
