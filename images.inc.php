        ...
        if (! file_exists(_PS_TMP_IMG_DIR_ . $cacheImage)) {
            ...
            if (function_exists('memory_get_usage') &&
             (int) $memory_limit != - 1) {
                ...
                if (($infos[0] * $infos[1] * $infos['bits'] *
                 (isset($infos['channels']) ? ($infos['channels'] / 8) : 1) +
                 pow(2, 16)) * 1.8 + $current_memory >
            ...
            if ($y < $size && $x <= $max_x)
                ...
                imageResize($image, _PS_TMP_IMG_DIR_ . $cacheImage, $ratioX, 
                $size, 'jpg');
            }
        }
        return '<img src="' . _PS_TMP_IMG_ . $cacheImage .
         ($disableCache ? '?time=' . time() : '') . '" alt="" class="imgm" />';
    }
    return '';
}
/**
...
function checkImage ($file, $maxFileSize)
{
    if ($file['size'] > $maxFileSize)
        return Tools::displayError('Image is too large') . ' (' .
         ($file['size'] / 1000) . Tools::displayError('KB') . '). ' .
         Tools::displayError('Maximum allowed:') . ' ' . ($maxFileSize / 1000) .
    ...
    if ($file['error'])
        ...
        'Error while uploading image; please change your server\'s settings.') .
         '(' . Tools::displayError('Error code: ') . $file['error'] . ')';
    return false;
}
...
function isPicture ($file, $types = NULL)
{
    ...
    $mimeType = false;
    if (! $types)
        $types = array('image/gif', 'image/jpg', 'image/jpeg', 'image/pjpeg', 
    ...
    elseif (function_exists('exec')) {
        $mimeType = trim(
        exec('file -b --mime-type ' . escapeshellarg($file['tmp_name'])));
        if (! $mimeType)
            $mimeType = trim(
            exec('file --mime ' . escapeshellarg($file['tmp_name'])));
        if (! $mimeType)
            $mimeType = trim(
            exec('file -bi ' . escapeshellarg($file['tmp_name'])));
    }
    if (empty($mimeType) or $mimeType == 'regular file' or
     $mimeType == 'text/plain')
        $mimeType = $file['type'];
 ...
 */
function imageResize ($sourceFile, $destFile, $destWidth = NULL, $destHeight = NULL, 
$fileType = 'jpg')
{
    ...
    if ($widthDiff > 1 and $heightDiff > 1) {
        ...
        if (Configuration::get('PS_IMAGE_GENERATION_METHOD') == 2 or (! Configuration::get(
        'PS_IMAGE_GENERATION_METHOD') and $widthDiff > $heightDiff)) {
            $nextHeight = $destHeight;
            $nextWidth = round(($sourceWidth * $nextHeight) / $sourceHeight);
            $destWidth = (int) (! Configuration::get(
            'PS_IMAGE_GENERATION_METHOD') ? $destWidth : $nextWidth);
        } else {
            $nextWidth = $destWidth;
            $nextHeight = round($sourceHeight * $destWidth / $sourceWidth);
            $destHeight = (int) (! Configuration::get(
            'PS_IMAGE_GENERATION_METHOD') ? $destHeight : $nextHeight);
        }
    }
    ...
    if ($fileType == 'png' && $type == IMAGETYPE_PNG) {
        ...
        imagefilledrectangle($destImage, 0, 0, $destWidth, $destHeight, 
        $transparent);
    } else {
        $white = imagecolorallocate($destImage, 255, 255, 255);
        imagefilledrectangle($destImage, 0, 0, $destWidth, $destHeight, $white);
    }
    imagecopyresampled($destImage, $sourceImage, 
    (int) (($destWidth - $nextWidth) / 2), 
    (int) (($destHeight - $nextHeight) / 2), 0, 0, $nextWidth, $nextHeight, 
 ...
 * @return boolean Operation result
 */
function imageCut ($srcFile, $destFile, $destWidth = NULL, $destHeight = NULL, 
$fileType = 'jpg', $destX = 0, $destY = 0)
{
    ...
    imagecopyresampled($dest['ressource'], $src['ressource'], 0, 0, $dest['x'], 
    $dest['y'], $dest['width'], $dest['height'], $dest['width'], $dest['height']);
...
function returnDestImage ($type, $ressource, $filename)
{
    ...
    switch ($type) {
        case 'gif':
            ...
            break;
        case 'png':
            $quality = (Configuration::get('PS_PNG_QUALITY') === false ? 7 : Configuration::get(
            'PS_PNG_QUALITY'));
            $flag = imagepng($ressource, $filename, (int) $quality);
            break;
        case 'jpeg':
        default:
            $quality = (Configuration::get('PS_JPEG_QUALITY') === false ? 90 : Configuration::get(
            'PS_JPEG_QUALITY'));
...
function deleteImage ($id_item, $id_image = NULL)
{
    ...
    if (! $id_image) {
        ...
        foreach ($imagesTypes as $k => $imagesType)
            if (file_exists(
            $path . $id_item . '-' . $imagesType['name'] . '.jpg'))
    ...
    } else // Product
{
        ...
        foreach ($imagesTypes as $k => $imagesType)
            if (file_exists(
            $path . $image->getExistingImgPath() . '-' . $imagesType['name'] .
             '.jpg'))
                unlink(
                $path . $image->getExistingImgPath() . '-' . $imagesType['name'] .
                 '.jpg');
