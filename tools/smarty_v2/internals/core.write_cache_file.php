...
function smarty_core_write_cache_file ($params, &$smarty)
{
    ...
    if (preg_match_all('!\{(/?)nocache\:[0-9a-f]{32}#\d+\}!', 
    $params['results'], $match, PREG_PATTERN_ORDER)) {
        ...
        $results = preg_split('!(\{/?nocache\:[0-9a-f]{32}#\d+\})!', 
        $params['results'], - 1, PREG_SPLIT_DELIM_CAPTURE);
        $level = 0;
        $j = 0;
        for ($i = 0, $results_count = count($results); $i < $results_count &&
         $j < $match_count; $i ++) {
    ...
    if (! empty($smarty->cache_handler_func)) {
        ...
        array('write', &$smarty, &$params['results'], $params['tpl_file'], 
