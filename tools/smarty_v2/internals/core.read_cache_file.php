...
function smarty_core_read_cache_file (&$params, &$smarty)
{
    ...
    $content_cache[$params['tpl_file'] . ',' . $params['cache_id'] . ',' .
     $params['compile_id']])) {
        list ($params['results'], $smarty->_cache_info) = $content_cache[$params['tpl_file'] .
    ...
    if (! empty($smarty->cache_handler_func)) {
        ...
        array('read', &$smarty, &$params['results'], $params['tpl_file'], 
        $params['cache_id'], $params['compile_id'], null));
    } else {
        // use local cache file
        $_auto_id = $smarty->_get_auto_id($params['cache_id'], 
        $params['compile_id']);
        $_cache_file = $smarty->_get_auto_filename($smarty->cache_dir, 
    ...
    if ($smarty->caching == 2 && isset($_cache_info['expires'])) {
        ...
        if ($_cache_info['expires'] > - 1 &&
         (time() > $_cache_info['expires'])) {
        ...
        if ($smarty->cache_lifetime > - 1 &&
         (time() - $_cache_info['timestamp'] > $smarty->cache_lifetime)) {
            // cache expired, regenerate
            return false;
        }
    }
