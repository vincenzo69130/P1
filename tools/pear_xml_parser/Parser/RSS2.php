...
class XML_Feed_Parser_RSS2 extends XML_Feed_Parser_Type
{
     ...
     * @var array
     */
    protected $map = array('ttl' => array('Text'), 'pubDate' => array('Date'), 
    'lastBuildDate' => array('Date'), 'title' => array('Text'), 
    'link' => array('Link'), 'description' => array('Text'), 
    'language' => array('Text'), 'copyright' => array('Text'), 
    'managingEditor' => array('Text'), 'webMaster' => array('Text'), 
    'category' => array('Text'), 'generator' => array('Text'), 
    'docs' => array('Text'), 'ttl' => array('Text'), 'image' => array('Image'), 
    'skipDays' => array('skipDays'), 'skipHours' => array('skipHours'));
    /**
    ...
    'rights' => array('copyright'), 'updated' => array('lastBuildDate'), 
    'subtitle' => array('description'), 'date' => array('pubDate'), 
    'author' => array('managingEditor'));
    protected $namespaces = array('dc' => 'http://purl.org/rss/1.0/modules/dc/', 
    ...
    function getEntryById ($id)
    {
        ...
        if ($entries->length > 0) {
            ...
            if (in_array('evaluate', get_class_methods($this->xpath))) {
                ...
                "count(preceding-sibling::item)", $entries->item(0));
    ...
    function getTextInput ()
    {
        ...
        if ($inputs->length > 0) {
            ...
            'description' => $input->getElementsByTagName('description')->item(
            0)->value, 
