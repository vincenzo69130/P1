...
class XML_Feed_Parser_RSS2Element extends XML_Feed_Parser_RSS2
{
     ...
     * @var array
     */
    protected $map = array('title' => array('Text'), 'guid' => array('Guid'), 
    'description' => array('Text'), 'author' => array('Text'), 
    'comments' => array('Text'), 'enclosure' => array('Enclosure'), 
    'pubDate' => array('Date'), 'source' => array('Source'), 
    'link' => array('Text'), 'content' => array('Content'));
    /**
     ...
     * @var array
     */
    protected $compatMap = array('id' => array('guid'), 
    'updated' => array('lastBuildDate'), 'published' => array('pubdate'), 
    ...
    protected function getEnclosure ($method, $parameters)
    {
        ...
        if ($encs->length > $offset) {
            try {
                ...
                return array('url' => $attrs->getNamedItem('url')->value, 
    ...
    protected function getSource ()
    {
        ...
        if ($get->length) {
            $source = $get->item(0);
            $array = array('content' => $source->nodeValue);
