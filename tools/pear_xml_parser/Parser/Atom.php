...
class XML_Feed_Parser_Atom extends XML_Feed_Parser_Type
{
     ...
     * @var array
     */
    protected $map = array('author' => array('Person'), 
    'contributor' => array('Person'), 'icon' => array('Text'), 
    'logo' => array('Text'), 'id' => array('Text', 'fail'), 
    'rights' => array('Text'), 'subtitle' => array('Text'), 
    'title' => array('Text', 'fail'), 'updated' => array('Date', 'fail'), 
    'link' => array('Link'), 'generator' => array('Text'), 
     ...
     * @var array
     */
    protected $compatMap = array('guid' => array('id'), 
    'links' => array('link'), 'tags' => array('category'), 
    ...
    function getEntryById ($id)
    {
        ...
        if ($entries->length > 0) {
            ...
            if (in_array('evaluate', get_class_methods($this->xpath))) {
                ...
                "count(preceding-sibling::atom:entry)", $entries->item(0));
        ...
        if (! empty($attribute) and
         ! ($method == 'generator' and $attribute == 'name')) {
            if ($content->hasAttribute($attribute)) {
                ...
                if ($attribute == 'href' and $content->hasAttribute('uri')) {
    ...
    function getLink ($offset = 0, $attribute = 'href', $params = false)
    {
        if (is_array($params) and ! empty($params)) {
            ...
            $query = $this->xpathPrefix . 'atom:link' . $alt_predicate .
             $other_predicate;
