...
class XML_Feed_Parser_AtomElement extends XML_Feed_Parser_Atom
{
     ...
     * @var array
     */
    protected $map = array('author' => array('Person', 'fallback'), 
    'contributor' => array('Person'), 'id' => array('Text', 'fail'), 
    'published' => array('Date'), 'updated' => array('Date', 'fail'), 
    'title' => array('Text', 'fail'), 'rights' => array('Text', 'fallback'), 
    'summary' => array('Text'), 'content' => array('Content'), 
    'link' => array('Link'), 'enclosure' => array('Enclosure'), 
    ...
    function getEnclosure ($method, $arguments = array())
    {
        ...
        if ($encs->length > $offset) {
            try {
                ...
                $length = $encs->item($offset)->hasAttribute('length') ? $encs->item(
                $offset)->getAttribute('length') : false;
                return array('url' => $attrs->getNamedItem('href')->value, 
