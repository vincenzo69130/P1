...
class XML_Feed_Parser implements Iterator
{
    ...
    'rss2' => array('http://backend.userland.com/rss', 
    ...
    function __construct ($feed, $strict = false, $suppressWarnings = false, $tidy = false)
    {
        ...
        if (! @$this->model->loadXML($feed)) { /* PrestaShop - no error display */
            if (extension_loaded('tidy') && $tidy) {
                ...
                if (! @$this->model->loadXML((string) $tidy)) { /* PrestaShop - no error display */
                    throw new XML_Feed_Parser_Exception(
                    'Invalid input: this is not ' . 'valid XML');
                }
            } else {
                throw new XML_Feed_Parser_Exception(
                'Invalid input: this is not valid XML');
        ...
        switch (true) {
            case ($doc_element->namespaceURI == 'http://www.w3.org/2005/Atom'):
                ...
                require_once (_PS_PEAR_XML_PARSER_PATH_ .
                 'Parser/AtomElement.php');
            ...
            case ($doc_element->namespaceURI == 'http://purl.org/atom/ns#'):
                ...
                $error = 'Atom 0.3 deprecated, using 1.0 parser which won\'t provide ' .
                 'all options';
                break;
            case ($doc_element->namespaceURI == 'http://purl.org/rss/1.0/' || ($doc_element->hasChildNodes() &&
             $doc_element->childNodes->length > 1 && $doc_element->childNodes->item(
            1)->namespaceURI == 'http://purl.org/rss/1.0/')):
                require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS1.php');
                require_once (_PS_PEAR_XML_PARSER_PATH_ .
                 'Parser/RSS1Element.php');
                $class = 'XML_Feed_Parser_RSS1';
                break;
            case ($doc_element->namespaceURI == 'http://purl.org/rss/1.1/' || ($doc_element->hasChildNodes() &&
             $doc_element->childNodes->length > 1 && $doc_element->childNodes->item(
            1)->namespaceURI == 'http://purl.org/rss/1.1/')):
                require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS11.php');
                require_once (_PS_PEAR_XML_PARSER_PATH_ .
                 'Parser/RSS11Element.php');
                $class = 'XML_Feed_Parser_RSS11';
                break;
            case (($doc_element->hasChildNodes() &&
             $doc_element->childNodes->length > 1 && $doc_element->childNodes->item(
            1)->namespaceURI == 'http://my.netscape.com/rdf/simple/0.9/') ||
             $doc_element->namespaceURI ==
             'http://my.netscape.com/rdf/simple/0.9/'):
                require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS09.php');
                require_once (_PS_PEAR_XML_PARSER_PATH_ .
                 'Parser/RSS09Element.php');
                $class = 'XML_Feed_Parser_RSS09';
                break;
            case ($doc_element->tagName == 'rss' and $doc_element->hasAttribute(
            'version') && $doc_element->getAttribute('version') == 0.91):
                $error = 'RSS 0.91 has been superceded by RSS2.0. Using RSS2.0 parser.';
                require_once (_PS_PEAR_XML_PARSER_PATH_ . 'Parser/RSS2.php');
                require_once (_PS_PEAR_XML_PARSER_PATH_ .
                 'Parser/RSS2Element.php');
                $class = 'XML_Feed_Parser_RSS2';
                break;
            case ($doc_element->tagName == 'rss' and $doc_element->hasAttribute(
            'version') && $doc_element->getAttribute('version') == 0.92):
                ...
                $class = 'XML_Feed_Parser_RSS2';
                break;
            case (in_array($doc_element->namespaceURI, 
            $this->feedNamespaces['rss2']) || $doc_element->tagName == 'rss'):
