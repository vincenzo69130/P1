...
class FPDF
{
    ...
    function FPDF ($orientation = 'P', $unit = 'mm', $format = 'A4')
    {
        ...
        'courierBI' => 'Courier-BoldOblique', 'helvetica' => 'Helvetica', 
        'helveticaB' => 'Helvetica-Bold', 'helveticaI' => 'Helvetica-Oblique', 
        'helveticaBI' => 'Helvetica-BoldOblique', 'times' => 'Times-Roman', 
        'timesB' => 'Times-Bold', 'timesI' => 'Times-Italic', 
    ...
    function SetDisplayMode ($zoom, $layout = 'continuous')
    {
        ...
        if ($zoom == 'fullpage' || $zoom == 'fullwidth' || $zoom ==
         'real' || $zoom == 'default' || ! is_string($zoom))
            $this->ZoomMode = $zoom;
        else
    ...
    function SetDrawColor ($r, $g = null, $b = null)
    {
        ...
        if (($r == 0 && $g == 0 && $b == 0) || $g === null)
            ...
            $this->DrawColor = sprintf('%.3F %.3F %.3F RG', $r / 255, $g / 255, 
            $b / 255);
        if ($this->page > 0)
    ...
    function SetFillColor ($r, $g = null, $b = null)
    {
        ...
        if (($r == 0 && $g == 0 && $b == 0) || $g === null)
            ...
            $this->FillColor = sprintf('%.3F %.3F %.3F rg', $r / 255, $g / 255, 
            $b / 255);
    ...
    function SetTextColor ($r, $g = null, $b = null)
    {
        ...
        if (($r == 0 && $g == 0 && $b == 0) || $g === null)
            ...
            $this->TextColor = sprintf('%.3F %.3F %.3F rg', $r / 255, $g / 255, 
            $b / 255);
        $this->ColorFlag = ($this->FillColor != $this->TextColor);
    }
    ...
    function Line ($x1, $y1, $x2, $y2)
    {
        //Draw a line
        $this->_out(
        sprintf('%.2F %.2F m %.2F %.2F l S', $x1 * $this->k, 
        ($this->h - $y1) * $this->k, $x2 * $this->k, ($this->h - $y2) * $this->k));
    }
    function Rect ($x, $y, $w, $h, $style = '')
    {
        ...
        elseif ($style == 'FD' || $style == 'DF')
            ...
            $op = 'S';
        $this->_out(
        sprintf('%.2F %.2F %.2F %.2F re %s', $x * $this->k, 
        ($this->h - $y) * $this->k, $w * $this->k, - $h * $this->k, $op));
    }
    ...
    function SetFont ($family, $style = '', $size = 0)
    {
        ...
        if ($this->FontFamily == $family && $this->FontStyle ==
         $style && $this->FontSizePt == $size)
            return;
        ...
        if ($this->page > 0)
            $this->_out(
            sprintf('BT /F%d %.2F Tf ET', $this->CurrentFont['i'], 
            $this->FontSizePt));
    }
    ...
    {
        //Output a string
        $s = sprintf('BT %.2F %.2F Td (%s) Tj ET', $x * $this->k, 
    ...
    $fill = false, $link = '')
    {
        ...
        if ($fill || $border == 1) {
            if ($fill)
                ...
                $op = 'S';
            $s = sprintf('%.2F %.2F %.2F %.2F re %s ', $this->x * $k, 
            ($this->h - $this->y) * $k, $w * $k, - $h * $k, $op);
        }
        if (is_string($border)) {
            $x = $this->x;
            $y = $this->y;
            if (strpos($border, 'L') !== false)
                ...
                ($this->h - $y) * $k, $x * $k, ($this->h - ($y + $h)) * $k);
            if (strpos($border, 'T') !== false)
                $s .= sprintf('%.2F %.2F m %.2F %.2F l S ', $x * $k, 
                ($this->h - $y) * $k, ($x + $w) * $k, ($this->h - $y) * $k);
            if (strpos($border, 'R') !== false)
                $s .= sprintf('%.2F %.2F m %.2F %.2F l S ', ($x + $w) * $k, 
                ($this->h - $y) * $k, ($x + $w) * $k, ($this->h - ($y + $h)) * $k);
        ...
        if ($txt !== '') {
            ...
            if ($this->underline)
                $s .= ' ' .
                 $this->_dounderline($this->x + $dx, 
                $this->y + .5 * $h + .3 * $this->FontSize, $txt);
            if ($this->ColorFlag)
                $s .= ' Q';
            if ($link)
                $this->Link($this->x + $dx, $this->y + .5 * $h -
                 .5 * $this->FontSize, $this->GetStringWidth($txt), 
                $this->FontSize, $link);
        }
        if ($s)
            $this->_out($s);
        ...
        while ($i < $nb) {
            ...
            if ($l > $wmax) {
                ...
                if ($sep == - 1) {
                    ...
                    if ($align == 'J') {
                        $this->ws = ($ns > 1) ? ($wmax - $ls) / 1000 *
                         $this->FontSize / ($ns - 1) : 0;
                        $this->_out(sprintf('%.3F Tw', $this->ws * $this->k));
                    }
                    $this->Cell($w, $h, substr($s, $j, $sep - $j), $b, 2, 
                    $align, $fill);
                    $i = $sep + 1;
                }
                $sep = - 1;
    ...
    function Write ($h, $txt, $link = '')
    {
        ...
        while ($i < $nb) {
            ...
            if ($c == "\n") {
                //Explicit line break
                $this->Cell($w, $h, substr($s, $j, $i - $j), 0, 
                2, '', 0, $link);
                $i ++;
                $sep = - 1;
                $j = $i;
    ...
    function Image ($file, $x = null, $y = null, $w = 0, $h = 0, $type = '', $link = '')
    {
        ...
        if ($x === null)
            $x = $this->x;
        $this->_out(
        sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q', $w * $this->k, 
        $h * $this->k, $x * $this->k, ($this->h - ($y + $h)) * $this->k, 
        $info['i']));
        if ($link)
    ...
    function _dounderline ($x, $y, $txt)
    {
        ...
        ($this->h - ($y - $up / 1000 * $this->FontSize)) * $this->k, $w *
    ...
    function _parsepng ($file)
    {
        ...
        do {
            ...
            if ($type == 'PLTE') {
                ...
                elseif ($ct == 2)
                    $trns = array(ord(substr($t, 1, 1)), ord(substr($t, 3, 1)), 
                    ord(substr($t, 5, 1)));
                else {
                    $pos = strpos($t, chr(0));
                    if ($pos !== false)
        ...
        'f' => 'FlateDecode', 'parms' => $parms, 'pal' => $pal, 'trns' => $trns, 
        'data' => $data);
    }
    ...
    function _putpages ()
    {
        ...
        if (! empty($this->AliasNbPages)) {
            ...
            for ($n = 1; $n <= $nb; $n ++)
                $this->pages[$n] = str_replace($this->AliasNbPages, $nb, 
                $this->pages[$n]);
        }
        ...
        for ($n = 1; $n <= $nb; $n ++) {
            ...
            if (isset($this->PageSizes[$n]))
                $this->_out(
                sprintf('/MediaBox [0 0 %.2F %.2F]', $this->PageSizes[$n][0], 
                $this->PageSizes[$n][1]));
    ...
    function _putfonts ()
    {
        ...
        foreach ($this->fonts as $k => $font) {
            ...
            if ($type == 'core') {
                ...
                if ($font['enc']) {
                    if (isset($font['diff']))
                        $this->_out('/Encoding ' . ($nf + $font['diff']) .
                         ' 0 R');
                    else
                        $this->_out('/Encoding /WinAnsiEncoding');
                }
    ...
    function _putimages ()
    {
        ...
        while (list ($file, $info) = each($this->images)) {
            ...
            if ($info['cs'] == 'Indexed')
                $this->_out(
                '/ColorSpace [/Indexed /DeviceRGB ' . (strlen($info['pal']) / 3 -
                 1) . ' ' . ($this->n + 1) . ' 0 R]');
            else {
    ...
    function _putinfo ()
    {
        ...
        $this->_out('/CreationDate ' . $this->_textstring('D:' .
         @date('YmdHis')));
    }
