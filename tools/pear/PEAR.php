...
if (! class_exists('PEAR', false)) {
    ...
    class PEAR
    {
        ...
        function _checkDelExpect ($error_code)
        {
            ...
            foreach ($this->_expected_errors as $key => $error_array) {
                if (in_array($error_code, $error_array)) {
                    unset(
                    $this->_expected_errors[$key][array_search($error_code, 
                    $error_array)]);
                    $deleted = true;
                }
        ...
        function delExpect ($error_code)
        {
            ...
            if ((is_array($error_code) && (0 != sizeof($error_code)))) {
                ...
                return $deleted ? true : PEAR::raiseError(
                "The expected error you submitted does not exist"); // IMPROVE ME
                ...
                // $error_code is empty
                return PEAR::raiseError(
                "The expected error you submitted is empty"); // IMPROVE ME
            }
        }
        // }}}
        // {{{ raiseError()
        ...
        $userinfo = null, $error_class = null, $skipmsg = false)
        {
            ...
            if ($mode === null) {
                // Class error handler
                if (isset($this) &&
                 isset($this->_default_error_mode)) {
    ...
    function _PEAR_call_destructors ()
    {
        ...
        if (is_array($_PEAR_destructor_object_list) &&
         sizeof($_PEAR_destructor_object_list)) {
            ...
            if (@PEAR::getStaticProperty('PEAR', 'destructlifo')) {
                $_PEAR_destructor_object_list = array_reverse(
        ...
        if (is_array($GLOBALS['_PEAR_shutdown_funcs']) and
    ...
    class PEAR_Error
    {
        ...
        $userinfo = null)
        {
            ...
            if ($this->mode & PEAR_ERROR_EXCEPTION) {
                trigger_error(
                "PEAR_ERROR_EXCEPTION is obsolete, use class PEAR_Exception for exceptions", 
                E_USER_WARNING);
                eval(
        ...
        function toString ()
        {
            ...
            E_USER_WARNING => 'warning', E_USER_ERROR => 'error');
            if ($this->mode & PEAR_ERROR_CALLBACK) {
                if (is_array($this->callback)) {
                    $callback = (is_object($this->callback[0]) ? strtolower(
                    get_class($this->callback[0])) : $this->callback[0]) . '::' .
                     $this->callback[1];
                } else {
...
if (! class_exists('PEAR_Exception', false)) {
    class PEAR_Exception extends Exception
    {
        ...
        public function getCauseMessage (&$causes)
        {
            ...
            'message' => $this->message, 'file' => 'unknown', 
            'line' => 'unknown');
            if (isset($trace[0])) {
            ...
            if ($this->cause instanceof PEAR_Exception) {
             ...
             $this->cause instanceof PEAR_Error) {
                $causes[] = array('class' => get_class($this->cause), 
                'message' => $this->cause->getMessage(), 'file' => 'unknown', 
                'line' => 'unknown');
                ...
                foreach ($this->cause as $cause) {
                    if ($cause instanceof PEAR_Exception) {
                     ...
                     $cause instanceof PEAR_Error) {
                        $causes[] = array('class' => get_class($cause), 
                        'message' => $cause->getMessage(), 'file' => 'unknown', 
                        'line' => 'unknown');
        ...
        public function toHtml ()
        {
            ...
            foreach ($trace as $k => $v) {
                ...
                $html .= '(' . implode(', ', $args) . ')' . '</td>' . '<td>' .
                 (isset($v['file']) ? $v['file'] : 'unknown') . ':' .
                 (isset($v['line']) ? $v['line'] : 'unknown') . '</td></tr>' .
                 "\n";
            }
        ...
        public function toText ()
        {
            ...
            foreach ($causes as $i => $cause) {
                $causeMsg .= str_repeat(' ', $i) . $cause['class'] . ': ' .
                 $cause['message'] . ' in ' . $cause['file'] . ' on line ' .
                 $cause['line'] . "\n";
            }
