...
class Smarty_Internal_CacheResource_File
{
    ...
    public function writeCachedContent ($_template, $content)
    {
        if (! $_template->resource_object->isEvaluated) {
            ...
            $_template->getCachedFilepath(), $content, $this->smarty) === true) {
    ...
    {
        $_cache_id = isset($cache_id) ? preg_replace('![^\w\|]+!', '_', 
        $cache_id) : null;
        ...
        if (isset($resource_name)) {
            ...
            $tpl = new $this->smarty->template_class($resource_name, 
            $this->smarty);
            // remove from template cache
            unset(
            $this->smarty->template_objects[crc32(
            $tpl->template_resource . $tpl->cache_id . $tpl->compile_id)]);
            $this->smarty->caching = $_save_stat;
            if ($tpl->isExisting()) {
                $_resourcename_parts = basename(
                str_replace('^', '/', $tpl->getCachedFilepath()));
            } else {
                return 0;
            }
        }
        ...
        if (file_exists($_dir)) {
            ...
            foreach ($_cache as $_file) {
                ...
                if ($_file->isDir()) {
                    ...
                    str_replace('\\', '/', substr((string) $_file, 
                    $_dir_length)));
                    ...
                    if (isset($_compile_id) &&
                     ...
                     $_parts[$_parts_count - 2 - $_compile_id_offset] !=
                     $_compile_id)) {
                        continue;
                    }
                    // check cache id
                    if (isset($_cache_id)) {
                        ...
                        $_parts_count = (isset(
                        $_compile_id)) ? $_parts_count - 2 - $_compile_id_offset : $_parts_count -
                         1 - $_compile_id_offset;
