...
class Smarty_Internal_Template extends Smarty_Internal_Data
{
    ...
    public function mustCompile ()
    {
        ...
        if ($this->mustCompile === null) {
            ...
            ($this->smarty->compile_check &&
             $this->getCompiledTimestamp() < $this->getTemplateTimestamp())));
        }
    ...
    public function compileTemplateSource ()
    {
        if (! $this->resource_object->isEvaluated) {
            ...
            $this->getTemplateFilepath(), $this->getTemplateTimestamp(), 
            $this->resource_type);
        }
        ...
        if (! is_object($this->compiler_object)) {
            ...
            $this->resource_object->template_parser_class, $this->smarty);
        }
        // compile locking
        ...
        try {
            ...
            if ($this->smarty->compile_locking &&
             ! $this->resource_object->isEvaluated && $saved_timestamp) {
        ...
        if (! $this->resource_object->isEvaluated &&
         $this->write_compiled_code) {
            ...
            $this->getCompiledFilepath(), $this->compiled_template, 
            $this->smarty);
        }
        if ($this->smarty->debugging) {
        ...
        $this->compiler_object->parser, $this->compiler_object->lex, 
        $this->compiler_object->template, $this->compiler_object);
    }
    /**
    ...
    public function getCachedFilepath ()
    {
        return $this->cached_filepath === null ? $this->cached_filepath = ($this->resource_object->isEvaluated ||
         ! ($this->caching == Smarty::CACHING_LIFETIME_CURRENT ||
         $this->caching == Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedFilepath(
    ...
    public function getCachedTimestamp ()
    {
        return $this->cached_timestamp === null ? $this->cached_timestamp = ($this->resource_object->isEvaluated ||
         ! ($this->caching == Smarty::CACHING_LIFETIME_CURRENT ||
         $this->caching == Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedTimestamp(
    ...
    public function getCachedContent ()
    {
        return $this->rendered_content === null ? $this->rendered_content = ($this->resource_object->isEvaluated ||
         ! ($this->caching == Smarty::CACHING_LIFETIME_CURRENT ||
         $this->caching == Smarty::CACHING_LIFETIME_SAVED)) ? false : $this->cache_resource_object->getCachedContents(
    ...
    public function writeCachedContent ($content)
    {
        ...
        return $this->cache_resource_object->writeCachedContent($this, 
        $this->createPropertyHeader(true) . $content);
    }
    /**
        ...
        if ($this->isCached === null) {
            ...
            if (($this->caching == Smarty::CACHING_LIFETIME_CURRENT ||
             ...
             ! $this->resource_object->isEvaluated) {
                ...
                if ($cachedTimestamp === false || $this->smarty->force_compile ||
                 $this->smarty->force_cache) {
                    return $this->isCached;
                }
                if ($this->caching === Smarty::CACHING_LIFETIME_SAVED ||
                 ($this->caching == Smarty::CACHING_LIFETIME_CURRENT &&
                 (time() <= ($cachedTimestamp + $this->cache_lifetime) ||
                     ...
                     $this->smarty->compile_check) {
                        ...
                        foreach ($this->properties['file_dependency'] as $_file_to_check) {
                            If ($_file_to_check[2] == 'file' ||
                             $_file_to_check[2] == 'extends' ||
                             $_file_to_check[2] == 'php') {
                                $mtime = filemtime($_file_to_check[0]);
                            } else {
                                $this->getResourceTypeName($_file_to_check[0], 
    ...
    public function renderTemplate ()
    {
        ...
        if (! $this->resource_object->isEvaluated &&
         empty($this->properties['file_dependency'][$this->templateUid])) {
            $this->properties['file_dependency'][$this->templateUid] = array(
            $this->getTemplateFilepath(), $this->getTemplateTimestamp(), 
            $this->resource_type);
        }
        ...
        if (! $this->resource_object->isEvaluated &&
         ...
         $this->caching == Smarty::CACHING_LIFETIME_CURRENT)) {
            ...
            preg_match_all(
            "!/\*%%SmartyNocache:{$this->properties['nocache_hash']}%%\*\/(.+?)/\*/%%SmartyNocache:{$this->properties['nocache_hash']}%%\*/!s", 
            $this->rendered_content, $cache_parts);
            $output = '';
            // loop over items, stitch back together
            foreach ($cache_split as $curr_idx => $curr_split) {
                // escape PHP tags in template content
                $output .= preg_replace(
                '/(<%|%>|<\?php|<\?|\?>)/', '<?php echo \'$1\'; ?>', $curr_split);
            ...
            if (isset($this->smarty->autoload_filters['output']) ||
             isset($this->smarty->registered_filters['output'])) {
                $output = Smarty_Internal_Filter_Handler::runFilter('output', 
                $output, $this);
            }
            ...
            if ($this->has_nocache_code &&
             ! empty($this->properties['nocache_hash']) &&
     ...
     */
    public function parseResourceName ($template_resource, &$resource_type, 
    &$resource_name, &$resource_handler)
    {
        if (empty($template_resource))
            return false;
        $this->getResourceTypeName($template_resource, $resource_type, 
        $resource_name);
        ...
        if ($resource_type != 'eval') {
            ...
            $this->template_resource . $this->cache_id . $this->compile_id)] = $this;
        }
        return true;
    }
    /**
    ...
    public function buildTemplateFilepath ($file = null)
    {
        ...
        if (! preg_match('/^([\/\\\\]|[a-zA-Z]:[\/\\\\])/', $file)) {
            foreach ((array) $this->smarty->template_dir as $_template_dir) {
                ...
                $_template_dir)) {
                    ...
                    if (($_filepath = Smarty_Internal_Get_Include_Path::getIncludePath(
                    $_filepath)) !== false) {
                        return $_filepath;
                    }
                }
        ...
        if (! empty($this->smarty->default_template_handler_func)) {
            if (! is_callable($this->smarty->default_template_handler_func)) {
                ...
                array($this->resource_type, $this->resource_name, 
                &$this->template_source, &$this->template_timestamp, $this));
    ...
    public function updateParentVariables ($scope = Smarty::SCOPE_LOCAL)
    {
        ...
        foreach ($this->tpl_vars as $_key => $_variable) {
            ...
            if ($scope == Smarty::SCOPE_LOCAL &&
             $_variable_scope == Smarty::SCOPE_LOCAL) {
                continue;
            }
            if (isset($this->parent) &&
             ($scope == Smarty::SCOPE_PARENT ||
             $_variable_scope == Smarty::SCOPE_PARENT)) {
            ...
            if ($scope == Smarty::SCOPE_ROOT ||
             $_variable_scope == Smarty::SCOPE_ROOT) {
                if ($this->parent == null) {
                    continue;
                }
                if (! $has_root) {
            ...
            if ($scope == Smarty::SCOPE_GLOBAL ||
             $_variable_scope == Smarty::SCOPE_GLOBAL) {
     ...
     * @param string $resource_name return resource name
     */
    protected function getResourceTypeName ($template_resource, &$resource_type, 
    &$resource_name)
    {
    ...
    protected function loadTemplateResourceHandler ($resource_type)
    {
        ...
        if (isset($this->smarty->registered_resources[$resource_type])) {
            ...
            array('file', 'string', 'extends', 'php', 'stream', 'eval'))) {
                ...
                if ($this->smarty->loadPlugin($_resource_class)) {
                    ...
                    if (in_array($resource_type, $_known_stream)) {
                        ...
                        throw new SmartyException(
                        'Unkown resource type \'' . $resource_type . '\'');
                    }
                }
            }
        }
            ...
            if (! empty($this->smarty->template_functions)) {
                ...
                foreach ($this->smarty->template_functions as $name => $function_data) {
                    if (isset($function_data['called_nocache'])) {
                        unset($function_data['called_nocache'], 
    ...
    public function decodeProperties ($properties)
    {
        ...
        if (! empty($properties['function'])) {
            ...
            $this->properties['function'], $properties['function']);
            $this->smarty->template_functions = array_merge(
            $this->smarty->template_functions, $properties['function']);
        }
    }
    /**
    ...
    $parent = null)
    {
        if ($template == null) {
            ...
            return $this->smarty->display($template, $cache_id, $compile_id, 
            $parent);
        }
    }
    /**
    ...
    public function __set ($property_name, $value)
    {
        if ($property_name == 'resource_object' ||
         $property_name == 'cache_resource_object') {
    ...
    public function __get ($property_name)
    {
        if ($property_name == 'resource_object') {
            ...
            if (! $this->parseResourceName($this->template_resource, 
            $this->resource_type, $this->resource_name, $this->resource_object)) {
                throw new SmartyException(
    ...
    public function __call ($name, $args)
    {
        ...
        if (in_array($first3, array('set', 'get')) && substr($name, 3, 1) !== '_') {
            ...
            substr($name, 3, 1)) . substr($name, 4);
            // convert camel case to underscored name
            $property_name = preg_replace_callback('/([A-Z])/', 
            $camel_func, $property_name);
