...
class Smarty_Internal_Utility
{
    ...
    $time_limit = 0, $max_errors = null)
    {
        ...
        foreach ((array) $this->smarty->template_dir as $_dir) {
            ...
            foreach ($_compile as $_fileinfo) {
                ...
                try {
                    $_tpl = $this->smarty->createTemplate($_template_file, null, 
                    null, null, false);
                    if ($_tpl->mustCompile()) {
     ...
     * @return integer number of template files recompiled
     */
    function compileAllConfig ($extention = '.conf', $force_compile = false, $time_limit = 0, 
    $max_errors = null)
    {
        // switch off time limit
        if (function_exists('set_time_limit')) {
        ...
        foreach ((array) $this->smarty->config_dir as $_dir) {
            ...
            foreach ($_compile as $_fileinfo) {
                ...
                if ($_fileinfo->getPath() == substr($_dir, 0, - 1)) {
                    ...
                    $_config_file = substr($_fileinfo->getPath(), strlen($_dir)) .
                     DS . $_file;
                }
     ...
     */
    function clearCompiledTemplate ($resource_name = null, $compile_id = null, 
    $exp_time = null)
    {
        ...
        foreach ($_compile as $_file) {
            ...
            if ($_file->isDir()) {
                ...
                if ((! isset($_compile_id) ||
                 (strlen((string) $_file) > strlen($_compile_id_part) &&
                 substr_compare((string) $_file, $_compile_id_part, 0, 
                strlen($_compile_id_part)) == 0)) &&
                 (! isset($resource_name) ||
                 (strlen((string) $_file) > strlen($_resource_part_1) &&
                 substr_compare((string) $_file, $_resource_part_1, 
                - strlen($_resource_part_1), strlen($_resource_part_1)) == 0) ||
                 (strlen((string) $_file) > strlen($_resource_part_2) &&
                 substr_compare((string) $_file, $_resource_part_2, 
                - strlen($_resource_part_2), strlen($_resource_part_2)) == 0))) {
                    if (isset($exp_time)) {
