...
class Smarty_Internal_TemplateCompilerBase
{
    ...
    public function compileTag ($tag, $args, $parameter = array())
    {
        ...
        if (($_output = $this->callTagCompiler($tag, $args, $parameter)) ===
         false) {
        ...
        if ($_output !== false) {
            ...
            if (strlen($tag) < 6 || substr($tag, - 5) != 'close') {
                ...
                $this->smarty->registered_objects[$tag]) &&
                 isset($parameter['object_methode'])) {
                    ...
                    $this->smarty->registered_objects[$tag][1]))) {
                        return $this->callTagCompiler('private_object_function', 
                        $args, $parameter, $tag, $methode);
                ...
                Smarty::PLUGIN_FUNCTION, Smarty::PLUGIN_BLOCK) as $type) {
                    if (isset($this->smarty->registered_plugins[$type][$tag])) {
                        ...
                        if ($type ==
                         Smarty::PLUGIN_FUNCTION || $type == Smarty::PLUGIN_BLOCK) {
                            return $this->callTagCompiler(
                            'private_registered_' . $type, $args, $parameter, 
                            $tag);
                        }
                    }
                }
                ...
                foreach ($this->smarty->plugin_search_order as $plugin_type) {
                    if ($plugin_type == Smarty::PLUGIN_BLOCK &&
                     $this->smarty->loadPlugin('smarty_compiler_' . $tag)) {
                        ...
                        if ($function = $this->getPlugin($tag, $plugin_type)) {
                            return $this->callTagCompiler(
                            'private_' . $plugin_type . '_plugin', $args, 
                ...
                $this->smarty->registered_plugins[Smarty::PLUGIN_BLOCK][$base_tag])) {
                    return $this->callTagCompiler('private_registered_block', 
                    $args, $parameter, $tag);
                }
                // block plugin?
                if ($function = $this->getPlugin($base_tag, 
                Smarty::PLUGIN_BLOCK)) {
                    return $this->callTagCompiler('private_block_plugin', $args, 
                    $parameter, $tag, $function);
                }
     ...
     * @return string compiled code
     */
    public function callTagCompiler ($tag, $args, $param1 = null, $param2 = null, 
    $param3 = null)
    {
    ...
    public function getPlugin ($plugin_name, $type)
    {
        ...
        foreach ((array) $this->smarty->plugins_dir as $_plugin_dir) {
            $file = rtrim($_plugin_dir, '/\\') . DS . $type . '.' . $plugin_name .
             '.php';
            if (file_exists($file)) {
                // require_once($file);
                $found = true;
    ...
    public function processNocacheCode ($content, $is_code)
    {
        ...
        if ($is_code && ! empty($content)) {
            ...
            if ((! $this->template->resource_object->isEvaluated ||
             $this->template->forceNocache) && $this->template->caching &&
             ! $this->suppressNocacheProcessing &&
             ($this->nocache || $this->tag_nocache ||
             $this->template->forceNocache == 2)) {
                $this->template->has_nocache_code = true;
                $_output = str_replace("'", "\'", $content);
                $_output = str_replace("^#^", "'", $_output);
                $_output = "<?php echo '/*%%SmartyNocache:{$this->nocache_hash}%%*/" .
                 $_output . "/*/%%SmartyNocache:{$this->nocache_hash}%%*/';?>";
    ...
    public function trigger_template_error ($args = null, $line = null)
    {
        ...
        $error_text = 'Syntax Error in template "' .
         $this->template->getTemplateFilepath() . '"  on line ' . $line . ' "' .
        ...
        if (isset($args)) {
            ...
            $error_text .= ' - Unexpected "' . $this->lex->value .
             '"';
            if (count(
            $this->parser->yy_get_expected_tokens($this->parser->yymajor)) <= 4) {
