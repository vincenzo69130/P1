...
class Smarty extends Smarty_Internal_Data
{
    ...
    $parent = null, $display = false)
    {
        ...
        $template, $cache_id, $compile_id, $parent, false);
        ...
        if (! $this->debugging && $this->debugging_ctrl == 'URL') {
            ...
            if (false !== strpos($_query_string, $this->smarty_debug_id)) {
                if (false !==
                 ...
                 strpos($_query_string, $this->smarty_debug_id . '=off')) {
        ...
        if ((! $this->caching || $_template->resource_object->isEvaluated) &&
         (isset($this->autoload_filters['output']) ||
         isset($this->registered_filters['output']))) {
            $_output = Smarty_Internal_Filter_Handler::runFilter('output', 
            $_template->getRenderedTemplate(), $_template);
        } else {
        ...
        if ($display) {
            if ($this->caching && $this->cache_modified_check) {
                ...
                if ($_isCached && $_gmt_mtime == $_last_modified_date) {
                    ...
                    header(
                    'Last-Modified: ' .
                     gmdate('D, d M Y H:i:s', $_template->getCachedTimestamp()) .
                     ' GMT');
                    echo $_output;
                }
            } else {
    ...
    $parent = null, $do_clone = true)
    {
        ...
        if (! is_object($template)) {
            ...
            if (isset($this->template_objects[$_templateId]) && $this->caching) {
                ...
                if ($do_clone) {
                    $tpl = new $this->template_class($template, clone $this, 
                    $parent, $cache_id, $compile_id);
                } else {
     ...
     * @return integer number of cache files deleted
     */
    function clearCache ($template_name, $cache_id = null, $compile_id = null, 
    $exp_time = null, $type = null)
    {
        // load cache resource and call clear
        return $this->loadCacheResource($type)->clear($template_name, 
    ...
    public function addTemplateDir ($template_dir)
    {
        ...
        array_merge((array) $this->template_dir, (array) $template_dir));
        return;
    }
    /**
    ...
    function addPluginsDir ($plugins_dir)
    {
        ...
        array_merge((array) $this->plugins_dir, (array) $plugins_dir));
        return;
    }
    /**
    ...
    public function __call ($name, $args)
    {
        ...
        foreach (array('filter', 'register') as $external) {
            /* PrestaShop */
            if ((PHP_VERSION_ID <= 50100 and
             is_callable(array("Smarty_Internal_{$external}", $name))) or
             method_exists("Smarty_Internal_{$external}", $name)) {
                if (! isset($this->$external)) {
                    $class = "Smarty_Internal_{$external}";
                    $this->$external = new $class($this);
                }
                return call_user_func_array(array($this->$external, $name), 
                $args);
            }
            /* End */
        }
        if (in_array($name, 
        array('clearCompiledTemplate', 'compileAllTemplates', 
        'compileAllConfig', 'testInstall', 'getTags'))) {
...
function smartyAutoload ($class)
{
    ...
    if (substr($_class, 0, 16) === 'smarty_internal_' ||
     ...
     file_exists(SMARTY_SYSPLUGINS_DIR . $_class . '.php')) {
