...
class Swift_Cache_Disk extends Swift_Cache
{
    ...
    public function write ($key, $data)
    {
        ...
        if (false === fwrite($handle, $data)) {
            ...
            throw new Swift_FileException(
            "Disk Caching failed.  Tried to write to file at [" .
             self::$save_path . "/" . $this->prefix . $key .
             "] but failed.  Check the permissions, or don't use disk caching.");
        }
        fclose($handle);
    }
    /**
    ...
    public function read ($key, $size = null)
    {
        ...
        if (! isset($this->open[$key])) {
            $this->open[$key] = fopen(
            self::$save_path . "/" . $this->prefix . $key, "rb");
        }
