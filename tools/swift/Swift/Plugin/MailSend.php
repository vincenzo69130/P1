 ...
 * @author Chris Corbyn <chris@w3style.co.uk>
 */
class Swift_Plugin_MailSend implements Swift_Events_SendListener, 
Swift_Events_BeforeSendListener
{
    ...
    public function sendPerformed (Swift_Events_SendEvent $e)
    {
        ...
        $to = implode(", ", $to);
        $bcc_orig = $message->headers->has("Bcc") ? $message->headers->get(
        "Bcc") : null;
        $subject_orig = $message->headers->has("Subject") ? $message->headers->get(
        "Subject") : null;
        ...
        $subject_enc = $message->headers->has("Subject") ? $message->headers->getEncoded(
        "Subject") : "";
        ...
        $this->doMail($to, $subject_enc, $message_body, $message->headers, 
        sprintf($this->getAdditionalParams(), $this->returnPath));
