...
class Swift_Plugin_FileEmbedder implements Swift_Events_BeforeSendListener
{
    ...
    "img" => array("attributes" => array("src"), 
    "extensions" => array("gif", "png", "jpg", "jpeg", "pjpeg")), 
    "link" => array("attributes" => array("href"), "extensions" => array("css")), 
    "script" => array("attributes" => array("src"), "extensions" => array("js")));
    /**
     * Protocols which may be used to download a remote file.
     * @var array
     */
    protected $protocols = array("http" => "http", "https" => "https", 
    "ftp" => "ftp");
    /**
     ...
     * @var array
     */
    protected $mimeTypes = array("gif" => "image/gif", "png" => "image/png", 
    "jpeg" => "image/jpeg", "jpg" => "image/jpeg", "pjpeg" => "image/pjpeg", 
    ...
    public function setTagDefinition ($tag, $attributes, $extensions)
    {
        ...
        $this->definitions[$tag] = array("attributes" => $attributes, 
    ...
    public function getRemoteFilePattern ($tag_name)
    {
        ...
        if ($def = $this->getTagDefinition($tag_name)) {
            $pattern = sprintf($pattern_format, $tag_name, 
            implode("|", $def["attributes"]), 
            implode("|", $this->getProtocols()), 
    ...
    public function getLocalFilePattern ($tag_name)
    {
        ...
        if ($def = $this->getTagDefinition($tag_name)) {
            $pattern = sprintf($pattern_format, $tag_name, 
            implode("|", $def["attributes"]), implode("|", $def["extensions"]));
            return $pattern;
        } else
            return null;
    }
    ...
    protected function embedRemoteFile ($matches)
    {
        ...
        $att = new Swift_Message_EmbeddedFile($filedata, $filename, 
        $this->getType($ext));
    ...
    protected function embedLocalFile ($matches)
    {
        ...
        $att = new Swift_Message_EmbeddedFile(new Swift_File($path), $filename, 
        $this->getType($ext));
        $id = $this->message->attach($att);
