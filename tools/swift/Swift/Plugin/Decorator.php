...
class Swift_Plugin_Decorator implements Swift_Events_BeforeSendListener
{
    ...
    public function beforeSendPerformed (Swift_Events_SendEvent $e)
    {
        ...
        $this->store = array("headers" => array(), "body" => false, 
        "children" => array());
     ...
     */
    protected function recursiveReplace (Swift_Message_Mime $mime, $replacements, 
    &$store)
    {
        //Check headers
        if ($this->getPermittedInHeaders()) {
            foreach ($mime->headers->getList() as $name => $value) {
                if (is_string($value) &&
                 ($replaced = $this->replace($replacements, $value)) != $value) {
                ...
                foreach ($mime->headers->listAttributes($name) as $att_name => $att_value) {
                    if (is_string($att_value) &&
                     ($att_replaced = $this->replace($replacements, $att_value)) !=
                     $att_value) {
                        if (! isset($store["headers"][$name])) {
                            $store["headers"][$name] = array("value" => false, 
                            "attributes" => array());
                        }
                        $mime->headers->setAttribute($name, $att_name, 
                        $att_replaced);
        ...
        //Check body
        $body = $mime->getData();
        if ($this->isPermittedType($mime->getContentType()) && is_string($body) &&
         ($replaced = $this->replace($replacements, $body)) != $body) {
            $mime->setData($replaced);
            $store["body"] = $body;
        }
        //Check sub-parts
        foreach ($mime->listChildren() as $id) {
            $store["children"][$id] = array("headers" => array(), "body" => false, 
            "children" => array());
            $child = $mime->getChild($id);
            $this->recursiveReplace($child, $replacements, 
            $store["children"][$id]);
        }
    }
    /**
    ...
    protected function replace ($replacements, $value)
    {
        ...
        array_values($replacements), $value);
    }
    /**
        ...
        if (empty($store)) //3.3.3 bugfix
{
    ...
    public function setReplacements ($replacements)
    {
        if ($replacements === null) {
            ...
            $this->replacements = new Swift_Plugin_Decorator_Replacements(
            $replacements);
