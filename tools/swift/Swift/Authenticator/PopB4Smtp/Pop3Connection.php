...
class Swift_Authenticator_PopB4Smtp_Pop3Connection
{
    ...
    public function assertOk ($line)
    {
        if (substr($line, 0, 3) != "+OK") {
            ...
            "The POP3 server did not suitably respond with a +OK response. " .
             "[" . $line . "]");
        }
    }
    /**
    ...
    public function start ()
    {
        ...
        if ($this->getEncryption() == self::ENC_SSL)
            $url = "ssl://" . $url;
        if ((false ===
         $this->handle = fsockopen($url, $this->getPort(), $errno, $errstr, 
        $timeout))) {
            Swift_ClassLoader::load("Swift_ConnectionException");
            throw new Swift_ConnectionException(
            "The POP3 connection failed to start.  The error string returned from fsockopen() is [" .
             $errstr . "] #" . $errno);
        }
    }
    /**
    ...
    public function stop ()
    {
        if ($this->handle !== null) {
            if (false === fclose($this->handle)) {
                Swift_ClassLoader::load("Swift_ConnectionException");
                throw new Swift_ConnectionException(
                "The POP3 connection did not close successfully.");
            }
        }
        $this->handle = null;
    }
    ...
    public function read ()
    {
        if (false === $response = fgets($this->handle)) {
            Swift_ClassLoader::load("Swift_ConnectionException");
            throw new Swift_ConnectionException(
            "Data could not be read from the POP3 connection.");
        }
