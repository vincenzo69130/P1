...
class Swift_Connection_SMTP extends Swift_ConnectionBase
{
    ...
    public function attachAuthenticator (Swift_Authenticator $auth)
    {
        ...
        if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
            $log->add(
            "Authentication mechanism '" . $auth->getAuthExtensionName() .
             "' attached.");
        }
    }
    /**
    ...
    public function read ()
    {
        ...
        while (! feof($this->handle)) {
            ...
            if ($tmp === false && ! feof($this->handle)) {
                throw new Swift_ConnectionException(
                "There was a problem reading line " . $line .
                 " of an SMTP response. The response so far was:<br />[" . $ret .
    ...
    public function write ($command, $end = "\r\n")
    {
        ...
        if (! @fwrite($this->handle, $command . $end) && ! empty($command))
            throw new Swift_ConnectionException(
            "The SMTP connection did not allow the command '" . $command .
             "' to be sent." . $this->smtpErrors());
    }
    /**
    ...
    public function start ()
    {
        ...
        if ($log->hasLevel(Swift_log::LOG_EVERYTHING)) {
            $log->add(
            "Trying to connect to SMTP server at '" . $server . ":" . $this->port);
        }
        if (! $this->handle = @fsockopen($server, $this->port, $errno, $errstr, 
        $this->timeout)) {
            $error_msg = "The SMTP connection failed to start [" . $server . ":" .
             $this->port . "]: fsockopen returned Error Number " . $errno .
             " and Error String '" . $errstr . "'";
            if ($log->isEnabled()) {
    ...
    public function postConnect (Swift $instance)
    {
        if ($this->getUsername() && $this->getPassword()) {
            $this->runAuthenticators($this->getUsername(), $this->getPassword(), 
            $instance);
        }
    }
    /**
    ...
    public function runAuthenticators ($user, $pass, Swift $swift)
    {
        ...
        foreach ($this->authenticators as $name => $obj) {
            ...
            if (in_array($name, $this->getAttributes("AUTH")) ||
             $name{0} == "*") {
                $tried ++;
                if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
                    $log->add("Trying '" . $name . "' authentication...");
                }
                if ($this->authenticators[$name]->isAuthenticated($user, $pass, 
                $swift)) {
                    if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
        ...
        if ($tried == 0)
            ...
            "No authentication mechanisms were tried since the server did not support any of the ones loaded. " .
             "Loaded authenticators: [" .
             implode(", ", array_keys($this->authenticators)) . "]");
        else
            throw new Swift_ConnectionException(
            "Authentication failed using username '" . $user . "' and password '" .
