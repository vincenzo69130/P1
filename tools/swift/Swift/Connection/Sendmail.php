...
class Swift_Connection_Sendmail extends Swift_ConnectionBase
{
    ...
    public function findSendmail ()
    {
        ...
        if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
            $log->add(
            "Sendmail path auto-detection in progress.  Trying `which sendmail`");
        }
        ...
        if (! is_executable($path)) {
            ...
            $common_locations = array('/usr/bin/sendmail', '/usr/lib/sendmail', 
            '/var/qmail/bin/sendmail', '/bin/sendmail', '/usr/sbin/sendmail', 
            '/sbin/sendmail');
            ...
            if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
                $log->add(
                "Falling back to /usr/sbin/sendmail (but it doesn't look good)!");
            }
    ...
    protected function pipeIn ($command, $end = "\r\n")
    {
        ...
        if (! @fwrite($this->pipes[0], $command . $end) && ! empty($command))
            throw new Swift_ConnectionException(
            "The sendmail process did not allow the command '" . $command .
             "' to be sent.");
        fflush($this->pipes[0]);
    }
    /**
        ...
        while (true) {
            ...
            if ($tmp === false) {
                ...
                "There was a problem reading line " . $line .
                 " of a sendmail SMTP response. The response so far was:<br />[" .
                 $ret . "].  It appears the process has died.");
            }
            $ret .= trim($tmp) . "\r\n";
            if ($tmp{3} == " ")
                break;
    ...
    public function start ()
    {
        ...
        if ($log->hasLevel(Swift_Log::LOG_EVERYTHING)) {
            $log->add(
            "Trying to stat the executable '" . $this->getPath() . "'.");
        }
        if (! @lstat($this->getPath())) {
            throw new Swift_ConnectionException(
            "Sendmail cannot be seen with lstat().  The command given [" .
             $this->getCommand() . "] does not appear to be valid.");
        }
        $pipes_spec = array(array("pipe", "r"), array("pipe", "w"), 
        array("pipe", "w"));
    ...
    {
        return ($this->proc !== false && is_resource($this->proc) &&
         is_resource($this->pipes[0]) && is_resource($this->pipes[1]) &&
         $this->proc !== null);
    }
    /**
     * Destructor.
