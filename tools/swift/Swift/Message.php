...
class Swift_Message extends Swift_Message_Mime
{
    ...
    "alternative" => array(), "mixed" => array(), "related" => array());
    /**
     * Ctor.
     ...
     */
    public function __construct ($subject = "", $body = null, $type = "text/plain", $encoding = null, 
    $charset = null)
    {
        parent::__construct();
    ...
    {
        $midparams = array("utctime" => gmstrftime("%Y%m%d%H%M%S"), 
        "pid" => getmypid(), "randint" => mt_rand(), 
        "customstr" => (preg_match("/^(?<!\\.)[a-z0-9\\.]+(?!\\.)\$/iD", 
        $idstring) ? $idstring : "swift"), 
        "hostname" => (isset($_SERVER["SERVER_NAME"]) ? $_SERVER["SERVER_NAME"] : php_uname(
        "n")));
    ...
    {
        return $this->headers->has("Message-ID") ? $this->headers->get(
        "Message-ID") : null;
    }
    /**
        ...
        if ($this->headers->has("Return-Path")) {
            if (! $smtp)
                ...
                if (strpos($path, ">") > strpos($path, "<"))
                    return substr($path, ($start = strpos($path, "<")), 
    ...
    public function setCharset ($charset)
    {
        ...
        if (($this->getEncoding() == "7bit") &&
         (strtolower($charset) == "utf-8" || strtolower($charset) == "utf8"))
    ...
    public function setPriority ($priority)
    {
        ...
        $label = array(1 => "High", 2 => "High", 3 => "Normal", 4 => "Low", 
        5 => "Low");
        $this->headers->set("X-Priority", $priority);
        $this->headers->set("X-MSMail-Priority", $label[$priority]);
        $this->headers->set("X-MimeOLE", 
        "Produced by SwiftMailer " . $this->libVersion);
    }
    /**
        ...
        try {
            switch (true) {
                case array_key_exists($id, $this->references["alternative"]):
                    $this->getReference("parent", "alternative")->removeChild(
                    $id);
                ...
                default:
                    throw new Swift_Message_MimeException(
                    "Unable to detach part identified by ID '" . $id .
     ...
     */
    protected function moveBranchIn ($type, $nested_type, $from, $old_branch, 
    $new_branch, $tag)
    {
        ...
        foreach ($from as $id => $ref) {
            ...
            switch ($new_branch) {
                case "related":
                    $this->getReference("related", $tag)->addChild($ref, $id, 
                    $sign);
                    break;
                case "mixed":
                    $this->getReference("mixed", $tag)->addChild($ref, $id, 
                    $sign);
                    break;
            }
    ...
    protected function postAttachFixStructure ()
    {
        switch (true) {
            case (! empty($this->references["mixed"]) &&
             ! empty($this->references["related"]) &&
             ! empty($this->references["alternative"])):
                if (! isset($this->references["related"]["_alternative"])) {
                    $this->moveBranchIn("multipart/alternative", 
                    "multipart/alternative", $this->references["alternative"], 
                    "alternative", "related", "_alternative");
                }
                if (! isset($this->references["mixed"]["_related"])) {
                    $this->moveBranchIn("multipart/related", 
                    "multipart/alternative", $this->references["related"], 
                    "related", "mixed", "_related");
                }
                break;
            case (! empty($this->references["mixed"]) &&
             ! empty($this->references["related"])):
                if (! isset($this->references["mixed"]["_related"])) {
                    $this->moveBranchIn("multipart/related", 
                    "multipart/related", $this->references["related"], "related", 
                    "mixed", "_related");
                }
                break;
            case (! empty($this->references["mixed"]) &&
             ! empty($this->references["alternative"])):
                if (! isset($this->references["mixed"]["_alternative"])) {
                    $this->moveBranchIn("multipart/alternative", null, 
                    $this->references["alternative"], "alternative", "mixed", 
                    "_alternative");
                }
                break;
    ...
    protected function moveBranchOut ($from, $old_branch, $new_branch, $tag)
    {
        foreach ($from as $id => $ref) {
            ...
            $this->getReference("parent", $new_branch)->addChild($ref, $id, 
            $sign);
            switch ($new_branch) {
                case "related":
    ...
    protected function postDetachFixStructure ()
    {
        switch (true) {
            case (! empty($this->references["mixed"]) &&
             ! empty($this->references["related"]) &&
             ! empty($this->references["alternative"])):
                if (array_keys($this->references["related"]) ==
                 array("_alternative")) {
                    $alt = $this->getReference("parent", "related")->getChild(
                    "_alternative");
                    $this->getReference("parent", "mixed")->addChild($alt, 
                    "_alternative", - 1);
                    $this->setReference("mixed", "_alternative", $alt); //mixedRefs["_alternative"] = $alt;
                    $this->getReference("parent", "related")->removeChild(
                    "_alternative");
                    unset($this->references["related"]["_alternative"]);
                    $this->getReference("parent", "mixed")->removeChild(
                    "_related");
                    unset($this->references["mixed"]["_related"]);
                }
                if (array_keys($this->references["mixed"]) == array("_related")) {
                    $this->moveBranchOut($this->references["related"], 
                    "related", "mixed", "_related");
                }
                break;
            case (! empty($this->references["mixed"]) &&
            ...
            case (! empty($this->references["mixed"]) &&
             ! empty($this->references["alternative"])):
                if (array_keys($this->references["mixed"]) ==
                 array("_alternative")) {
                    $this->moveBranchOut($this->references["alternative"], 
                    "alternative", "mixed", "_alternative");
                }
                break;
            case (! empty($this->references["related"]) &&
             ! empty($this->references["alternative"])):
            ...
            case (! empty($this->references["related"])):
                if (isset($this->references["related"]["_alternative"]) ||
                 isset($this->references["mixed"]["_alternative"]))
                    $this->detach("_alternative");
                break;
        }
    }
    ...
    public function preBuild ()
    {
        ...
        if ($this->getCharset() === null && ! $this->numChildren()) {
            Swift_ClassLoader::load("Swift_Message_Encoder");
            if (is_string($data) &&
             Swift_Message_Encoder::instance()->isUTF8($data)) {
                $this->setCharset("utf-8");
            } elseif (is_string($data) &&
             Swift_Message_Encoder::instance()->is7BitAscii($data)) {
