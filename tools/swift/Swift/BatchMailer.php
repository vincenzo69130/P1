...
class Swift_BatchMailer
{
    ...
    {
        $this->headers["To"] = $message->headers->has("To") ? $message->headers->get(
        "To") : null;
        $this->headers["Reply-To"] = $message->headers->has("Reply-To") ? $message->headers->get(
        "Reply-To") : null;
        $this->headers["Return-Path"] = $message->headers->has("Return-Path") ? $message->headers->get(
        "Return-Path") : null;
        $this->headers["From"] = $message->headers->has("From") ? $message->headers->get(
        "From") : null;
    }
    /**
    ...
    Swift_RecipientList $recipients, $sender)
    {
        $sent = 0;
        $successive_fails = 0;
        ...
        while ($it->hasNext()) {
            ...
            while ($loop && $tried < $this->getMaxTries()) {
                try {
                    ...
                    $sent += ($n = $this->swift->send($message, $recipient, 
                    $sender));
                    if (! $n)
                    ...
                    if (($max = $this->getMaxSuccessiveFailures()) &&
                     $successive_fails > $max) {
                        throw new Exception(
                        "Too many successive failures. BatchMailer is configured to allow no more than " .
                         $max . " successive failures.");
                    }
