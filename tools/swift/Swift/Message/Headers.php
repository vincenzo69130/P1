...
class Swift_Message_Headers
{
     ...
     * @var array headers
     */
    protected $emailContainingHeaders = array("To", "From", "Reply-To", "Cc", 
    "Bcc", "Return-Path", "Sender");
    /**
    ...
    public function set ($name, $value)
    {
        ...
        if (is_array($value)) {
            foreach ($value as $v) {
                if (! $this->getCharset() &&
                 Swift_Message_Encoder::instance()->isUTF8($v)) {
            ...
            if (! $this->getCharset() &&
             Swift_Message_Encoder::instance()->isUTF8($value)) {
                $this->setCharset("utf-8");
            }
        }
    ...
    {
        $lname = strtolower($name);
        return (array_key_exists($lname, $this->lowerHeaders) &&
         $this->lowerHeaders[$lname] !== null);
    }
    /**
    ...
    public function setAttribute ($header, $name, $value)
    {
        ...
        if (! $this->has($header)) {
            ...
            "Cannot set attribute '" . $name . "' for header '" . $header .
             "' as the header does not exist. " .
    ...
    public function hasAttribute ($header, $name)
    {
        ...
        if (! $this->has($header)) {
            ...
            return (isset($this->attributes[$lheader]) &&
             isset($this->attributes[$lheader][$name]) &&
             ($this->attributes[$lheader][$name] !== null));
        }
    }
    /**
    ...
    public function getAttribute ($header, $name)
    {
        if (! $this->has($header)) {
            throw new Swift_Message_MimeException(
            "Cannot locate attribute '" . $name . "' for header '" . $header .
             "' as the header does not exist. " .
             "Consider using Swift_Message_Headers-&gt;has() to check.");
        }
        $name = strtolower($name);
    ...
    public function getEncoded ($name)
    {
        ...
        foreach ($encoded_value as $key => $row) {
            ...
            if (Swift_Message_Encoder::instance()->isHeaderSafe(
            $row) && ! $this->forceEncoding) {
            ...
            } elseif ($this->encoding == "Q") //QP encode required
{
                ...
                $used_length = strlen($name) + 2 + strlen($spec) +
                 2;
                //Encode to QP, excluding the specification for now but keeping the lines short enough to be compliant
                $encoded_value[$key] = str_replace(" ", "_", 
                Swift_Message_Encoder::instance()->QPEncode($row, 
                (75 - (strlen($spec) + 6)), ($key > 0 ? 0 : (75 - $used_length)), 
                true, $this->LE));
            } elseif ($this->encoding == "B") //Need to Base64 encode
{
                ...
                $row, (75 - (strlen($spec) + 5)), 
                ($key > 0 ? 0 : (76 - ($used_length + 3))), true, $this->LE);
            }
            if (false !== $p = strpos($encoded_value[$key], $this->LE)) {
                $cb = 'str_replace("' . $this->LE . '", "", "<$1>");';
                $encoded_value[$key] = preg_replace("/<([^>]+)>/e", $cb, 
                $encoded_value[$key]);
            }
            ...
            for ($i = 0, $len = count($lines); $i < $len; $i ++) {
                ...
                if (empty($spec) && $is_email &&
                 (false !== $p = strpos($lines[$i], ","))) {
                    $s = strpos($lines[$i], " <");
                    $e = strpos($lines[$i], ">");
                    if ($s < $e) {
                        $addr = substr($lines[$i], $s);
                        $lines[$i] = "\"" . substr($lines[$i], 0, $s) . "\"" .
                         $addr;
                    } else {
                        $lines[$i] = "\"" . $lines[$i] . "\"";
                    }
        ...
        $this->cached[$lname] = implode("," . $this->LE . " ", 
        $encoded_value);
        //Append attributes if there are any
        if (! empty($this->attributes[$lname]))
            $this->cached[$lname] .= $this->buildAttributes(
            $this->cached[$lname], $lname);
        return $this->cached[$lname];
    }
    /**
    ...
    protected function buildAttributes ($header_line, $header_name)
    {
        ...
        foreach ($this->attributes[$header_name] as $attribute => $att_value) {
            ...
            $avail_len = 70 -
             (strlen($attribute) + 4);
            $encoded = Swift_Message_Encoder::instance()->rfc2047Encode(
            $att_value, $this->charset, $this->language, $avail_len, $this->LE);
            ...
            if ((strlen($encoded) + $used_len + strlen($attribute) +
             4) < 74) {
        ...
        foreach ($this->headers as $name => $value) //Look at each header
{
            if ($value === null)
                continue;
            $ret .= ltrim($name, ".") . ": " . $this->getEncoded($name) .
             $this->LE;
        }
        return trim($ret);
    }
