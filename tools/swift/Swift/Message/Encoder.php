...
class Swift_Message_Encoder
{
    ...
    {
        $elements = 0;
        while (preg_match('/^(.*?)(<' . self::CHEAP_ADDRESS_RE . '>)/s', $input, 
        $matches)) {
            if (! empty($matches[1]))
    ...
    {
        $elements = 0;
        while (preg_match('/^(.*?)(<[\x20-\x3A\x3C-\x7E]*>)/s', $input, 
        $matches)) {
            if (! empty($matches[1]))
                $ret[($elements ++)] = $matches[1];
            $ret[('a' . ($elements ++))] = $matches[2];
     ...
     * @return string
     */
    public function base64Encode ($data, $chunk = 76, $init_chunk = 0, 
    $headers = false, $le = "\r\n")
    {
        $ret = "";
        $chunk -= 2;
            ...
            if ($key{0} == 'a') //This is an address
{
     ...
     * @return string
     */
    public function QPEncode ($data, $chunk = 76, $init_chunk = 0, $headers = false, 
    $le = "\r\n")
    {
        $ret = "";
        if ($headers)
            ...
            if ($key{0} == 'a') //An address
{
                ...
                foreach ($lines as $n => $line)
                    $lines[$n] = $this->rawQPEncode(
                    rtrim($line, $trailing_spaces));
                $string = implode($le, $lines);
                if ($init_chunk > 3) {
                    if (preg_match(
                    '/^.{1,' . ($init_chunk - 5) . '}[^=]{2}(?!=[A-F0-9]{2})/', 
                    $string, $matches) ||
                     preg_match('/^.{1,' . ($init_chunk - 6) . '}([^=]{0,3})?/', 
                    $string, $matches)) {
                ...
                '/^.{1,' . ($init_chunk - 5) . '}[^=]{2}(?!=[A-F0-9]{2})/', 
                $string, $matches) ||
                 preg_match('/^.{1,' . ($chunk - 6) . '}([^=]{0,3})?/', $string, 
                $matches) || (strlen($string) > 0 && $matches = array($string))) {
    ...
    public function QPEncodeFile (Swift_File $file, $chunk = 76, $le = "\r\n")
    {
        ...
        while (false !== $bytes = $file->readln()) {
            ...
            preg_match_all('/.{1,' . ($chunk - 6) . '}([^=]{0,3})?/', $next, 
            $next);
            if (count($next[0]))
                $cache->write("qp", 
                $this->fixLE(implode("=" . $le, $next[0]), $le));
        }
    ...
    public function encode7BitFile (Swift_File $file, $chunk = 76, $le = "\r\n")
    {
        ...
        while (false !== $bytes = $file->read(8192))
            $ret .= $bytes;
        $cache->write("7b", $this->fixLE(wordwrap($ret, $chunk - 2, $le, 1), 
        $le));
    ...
    public function encode8BitFile (Swift_File $file, $chunk = 76, $le = "\r\n")
    {
        ...
        while (false !== $bytes = $file->read(8192))
            $ret .= $bytes;
        $cache->write("8b", $this->fixLE(wordwrap($ret, $chunk - 2, $le, 1), 
        $le));
     ...
     * @return string
     */
    public function header7BitEncode ($data, $chunk = 76, $init_chunk = 0, 
    $le = "\r\n")
    {
     ...
     * @return string
     */
    public function rfc2047Encode ($str, $charset = "iso-8859-1", $language = "en-us", $chunk = 76, 
    $le = "\r\n")
    {
        $lang_spec = "";
