...
abstract class Swift_Message_Mime
{
    ...
    public function setEncoding ($encoding, $recursive = false, $non_ascii = false)
    {
        ...
        if ($non_ascii && is_string($data) && strlen($data) > 0 &&
        ...
        if ($recursive) {
            foreach ($this->children as $id => $child) {
                if (! $child->getEncoding())
                    $this->children[$id]->setEncoding($encoding, $recursive, 
                    $non_ascii);
            }
        }
    }
        ...
        if (! empty($this->children)) //If we've got some mime parts we need to stick them onto the end of the message
{
            ...
            $this->headers->setAttribute("Content-Type", "boundary", 
            $this->boundary);
            $this->cache->clear("append");
            foreach ($this->children as $part) {
                $this->cache->write("append", 
                $this->LE . "--" . $this->boundary . $this->LE);
        ...
        switch ($this->getEncoding()) {
            case "quoted-printable":
                if ($is_file) {
                    $qp_os = Swift_Message_Encoder::instance()->QPEncodeFile(
                    $this->getData(), 76, $this->LE);
                    while (false !== $bytes = $qp_os->read())
                        $this->cache->write("body", $bytes);
                } else {
                    $this->cache->write("body", 
                    Swift_Message_Encoder::instance()->QPEncode(
                    $this->getData(), 76, 0, false, $this->LE));
                }
                break;
            case "base64":
                if ($is_file) {
                    $b64_os = Swift_Message_Encoder::instance()->base64EncodeFile(
                    $this->getData(), 76, $this->LE);
                    while (false !== $bytes = $b64_os->read())
                        $this->cache->write("body", $bytes);
                } else {
                    $this->cache->write("body", 
                    Swift_Message_Encoder::instance()->base64Encode(
                    $this->getData(), 76, 0, false, $this->LE));
                }
                break;
            case "binary":
            ...
            case "7bit":
                if ($is_file) {
                    $os = Swift_Message_Encoder::instance()->encode7BitFile(
                    $this->getData(), $this->wrap, $this->LE);
                    while (false !== $bytes = $os->read())
                        $this->cache->write("body", $bytes);
                } else {
                    $this->cache->write("body", 
                    Swift_Message_Encoder::instance()->encode7Bit(
                    $this->getData(), $this->wrap, $this->LE));
                }
                break;
            case "8bit":
            default:
                if ($is_file) {
                    $os = Swift_Message_Encoder::instance()->encode8BitFile(
                    $this->getData(), $this->wrap, $this->LE);
                    while (false !== $bytes = $os->read())
                        $this->cache->write("body", $bytes);
                } else {
                    $this->cache->write("body", 
                    Swift_Message_Encoder::instance()->encode8Bit(
                    $this->getData(), $this->wrap, $this->LE));
                }
                break;
        }
    ...
    public function getChild ($id)
    {
        if ($this->hasChild($id)) {
            ...
            "Cannot retrieve child part identified by '" . $id .
             "' as it does not exist.  Consider using hasChild() to check.");
        }
    }
    /**
    ...
    public function removeChild ($id)
    {
        ...
        if (! $this->hasChild($id)) {
            throw new Swift_Message_MimeException(
            "Cannot remove child part identified by '" . $id .
             "' as it does not exist. Consider using hasChild() to check.");
        } else {
