...
class Swift
{
     ...
     */
    public function __construct (Swift_Connection $conn, $domain = false, 
    $options = null)
    {
        ...
        if (! $domain)
            $domain = ! empty($_SERVER["SERVER_ADDR"]) ? "[" .
    ...
    public function notifyListeners ($e, $type)
    {
        ...
        if (! empty($this->listeners[$type]) &&
         $notifyMethod = Swift_Events_ListenerMapper::getNotifyMethod($type)) {
            $e->setSwift($this);
    ...
    public function disconnect ()
    {
        ...
        new Swift_Events_DisconnectEvent($this->connection), 
        "DisconnectListener");
    }
    /**
    ...
    Swift_Events_ResponseEvent $response, $codes)
    {
        $codes = (array) $codes;
    ...
    {
        if ($this->connection->getRequiresEHLO() ||
         strpos($greeting->getString(), "ESMTP"))
            $this->setConnectionExtensions(
            $this->command("EHLO " . $this->domain, 250));
        else
    ...
    protected function setConnectionExtensions (Swift_Events_ResponseEvent $list)
    {
        ...
        for ($i = 1, $len = count($list); $i < $len; $i ++) {
            ...
            $this->connection->setExtension($attributes[0], 
            (isset($attributes[1]) ? array_slice($attributes, 1) : array()));
        }
    }
    /**
    ...
    public function command ($command, $code = null)
    {
        ...
        if ($command !== "") {
            ...
            $this->notifyListeners($command_event, 
        ...
        $response_event = new Swift_Events_ResponseEvent(
        ...
        if ($command !== "" && $command_event->getCode() !== null)
            $this->assertCorrectResponse($response_event, 
            $command_event->getCode());
    ...
    public function send (Swift_Message $message, $recipients, $from)
    {
        ...
        if (is_string($recipients) &&
         preg_match("/^" . Swift_Message_Encoder::CHEAP_ADDRESS_RE . "\$/", 
        $recipients)) {
        ...
        if (! $has_message_id)
            $message->setId(null);
        if ($log->hasLevel(Swift_Log::LOG_NETWORK))
            $log->add("Message sent to " . $sent . "/" . $total . " recipients", 
            Swift_Log::NORMAL);
        return $sent;
    }
    /**
     ...
     * @return int The number of successful recipients
     */
    public function batchSend (Swift_Message $message, Swift_RecipientList $to, 
    $from)
    {
