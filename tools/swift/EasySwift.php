...
class EasySwift
{
    ...
    public function __construct (Swift_Connection $connection, $domain = null)
    {
        try {
            ...
            $this->swift->attachPlugin(
            new Swift_Plugin_EasySwiftResponseTracker($this), "_ResponseTracker");
        } catch (Swift_ConnectionException $e) {
            $this->failed = true;
            $this->setError(
            "The connection failed to start.  An exception was thrown:<br />" .
             $e->getMessage());
        }
        $this->newMessage();
    ...
    public function flushParts ()
    {
        ...
        foreach ($this->partIds as $id) {
            try {
                ...
                $success = false;
                $this->setError(
                "A MIME part failed to detach due to the error:<br />" .
                 $e->getMessage());
            }
        }
        $this->partIds = array();
    ...
    public function flushAttachments ()
    {
        ...
        foreach ($this->attachmentIds as $id) {
            try {
                ...
                $success = false;
                $this->setError(
                "An attachment failed to detach due to the error:<br />" .
                 $e->getMessage());
            }
        }
        $this->attachmentIds = array();
    ...
    public function connect ()
    {
        if (! $this->isConnected()) {
            try {
                ...
                $this->failed = true;
                $this->setError(
                "Swift failed to run the connection process:<br />" .
                 $e->getMessage());
            }
        }
        return false;
    ...
    public function loadAuthenticator (Swift_Authenticator $auth)
    {
        if (method_exists($this->swift->connection, "attachAuthenticator")) {
            ...
            throw new Exception(
            "SMTP authentication cannot be used with connection class '" .
             get_class($this->connection) . "'. Swift_Connection_SMTP is needed");
    }
    /**
    ...
    public function authenticate ($username, $password)
    {
        if (method_exists($this->swift->connection, "runAuthenticators")) {
            try {
                $this->swift->connection->runAuthenticators($username, 
                $password, $this->swift);
                return true;
            } catch (Swift_ConnectionException $e) {
                $this->setError(
                "Authentication failed:<br />" . $e->getMessage());
                return false;
            }
        } else
            throw new Exception(
    ...
    public function stringToAddress ($string)
    {
        ...
        if (preg_match(
        "/^\\s*(\"?)(.*?)\\1 *<(" . Swift_Message_Encoder::CHEAP_ADDRESS_RE .
         ")>\\s*\$/", $string, $matches)) {
            if (! empty($matches[2]))
                $name = $matches[2];
            $address = $matches[3];
        } elseif (preg_match(
        "/^\\s*" . Swift_Message_Encoder::CHEAP_ADDRESS_RE . "\\s*\$/", $string)) {
            $address = trim($string);
        } else
            return false;
    ...
    public function setCharset ($charset)
    {
        try {
            ...
            $this->setError(
            "Unable to set the message charset:<br />" . $e->getMessage());
            return false;
        }
    }
    /**
    ...
    public function addPart ($body, $type = "text/plain", $encoding = null, $charset = null)
    {
        if ($body instanceof Swift_Message_Mime) {
            try {
                ...
                $this->setError(
                "A MIME part failed to attach:<br />" . $e->getMessage());
                return false;
            }
        } else {
            try {
                $this->partIds[] = $this->message->attach(
                new Swift_Message_Part($body, $type, $encoding, $charset));
            } catch (Swift_Message_MimeException $e) {
                $this->setError(
                "A MIME part failed to attach:<br />" . $e->getMessage());
                return false;
    ...
    $type = "application/octet-stream", $encoding = null)
    {
        if ($data instanceof Swift_Message_Mime) {
            try {
                ...
                $this->setError(
                "An attachment failed to attach:<br />" . $e->getMessage());
                return false;
            }
        } else {
            try {
                $this->attachmentIds[] = $this->message->attach(
                new Swift_Message_Attachment($data, $filename, $type, $encoding));
            } catch (Swift_Message_MimeException $e) {
                $this->setError(
                "An attachment failed to attach<br />" . $e->getMessage());
                return false;
    ...
    public function addImage ($input)
    {
        ...
        if ($input instanceof Swift_Message_Image) {
            ...
            try {
                $ret = $this->message->attach(
                new Swift_Message_Image(new Swift_File($input)));
                $this->attachmentIds[] = $ret;
                return $ret;
            } catch (Swift_Message_MimeException $e) {
                $this->setError(
    ...
    public function embedFile ($data, $type = "application/octet-stream", $filename = null, $cid = null)
    {
        ...
        if ($data instanceof Swift_Message_EmbeddedFile) {
            ...
            try {
                $ret = $this->message->attach(
                new Swift_Message_EmbeddedFile($data, $filename, $type, $cid));
                $this->attachmentIds[] = $ret;
                return $ret;
            } catch (Swift_Message_MimeException $e) {
                $this->setError(
    ...
    public function addHeaders ($string)
    {
        ...
        foreach ($headers as $header) {
            ...
            foreach ($attribute_pairs as $pair) {
                ...
                $pair, $matches)) {
                    try {
                        $this->message->headers->setAttribute($header_name, 
                        $matches[1], $matches[3]);
                    } catch (Swift_Message_MimeException $e) {
                        $this->setError(
                        "There was a problem parsing or setting a header attribute:<br />" .
                         $e->getMessage());
        ...
        try {
            if (! $this->exactCopy && ! $this->recipients->getCc() &&
             ! $this->recipients->getBcc()) {
                $sent = $this->swift->batchSend($this->message, 
                $this->recipients, $sender);
            } else {
