...
if ($_POST['type'] == 'navinfo') {
    ...
    if (sha1(
    $_POST['id_connections'] . $_POST['id_page'] . $_POST['time_start'] .
     _COOKIE_KEY_) != $_POST['token'])
        die();
    if (! Validate::isInt($_POST['time']) or $_POST['time'] <= 0)
        die();
    Connection::setPageTime((int) $_POST['id_connections'], 
    (int) $_POST['id_page'], substr($_POST['time_start'], 0, 19), 
    intval($_POST['time']));
}