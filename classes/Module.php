...
abstract class ModuleCore
{
    ...
    public function __construct ($name = NULL)
    {
        ...
        if ($this->name != NULL) {
            if (self::$modulesCache == NULL and ! is_array(self::$modulesCache)) {
                ...
                'SELECT * FROM `' . pSQL(_DB_PREFIX_ . $this->table) . '`');
    ...
    public function install ()
    {
        ...
        if ($result)
            return false;
        $result = Db::getInstance()->AutoExecute(_DB_PREFIX_ . $this->table, 
        array('name' => $this->name, 'active' => 1), 'INSERT');
        if (! $result)
            return false;
    ...
    public function registerHook ($hook_name)
    {
		...
		VALUES (' . (int) ($this->id) . ', ' . (int) ($result['id_hook']) . ', ' .
         (int) ($result2['position'] + 1) . ')');
    ...
    public function registerExceptions ($id_hook, $excepts)
    {
        foreach ($excepts as $except) {
            if (! empty($except)) {
				...
				VALUES (' . (int) ($this->id) . ', ' . (int) ($id_hook) . ', \'' .
                 pSQL(strval($except)) . '\')');
                if (! $result)
                    return false;
            }
    ...
    public function editExceptions ($id_hook, $excepts)
    {
        ...
        '
				...
				WHERE `id_module` = ' . (int) ($this->id) . ' AND `id_hook` =' .
         (int) ($id_hook));
    ...
    public static function getModuleNameFromClass ($currentClass)
    {
        ...
        if (! isset(self::$classInModule[$currentClass])) {
            ...
            if (substr(realpath($filePath), 0, strlen($realpathModuleDir)) ==
             $realpathModuleDir) {
                self::$classInModule[$currentClass] = substr(dirname($filePath), 
                strlen($realpathModuleDir) + 1);
                $id_lang = (! isset($cookie) or ! is_object($cookie)) ? (int) (Configuration::get(
                'PS_LANG_DEFAULT')) : (int) ($cookie->id_lang);
                $file = _PS_MODULE_DIR_ . self::$classInModule[$currentClass] .
                 '/' . Language::getIsoById($id_lang) . '.php';
                if (Tools::file_exists_cache($file) and include_once ($file))
                    $_MODULES = ! empty($_MODULES) ? array_merge($_MODULES, 
                    $_MODULE) : $_MODULE;
            } else
    ...
    public static function getInstanceByName ($moduleName)
    {
        ...
        _PS_MODULE_DIR_ . $moduleName . '/' . $moduleName . '.php'))
            return false;
    ...
    public static function getModulesOnDisk ($useConfig = false)
    {
        ...
        foreach ($modules_dir as $module) {
            ...
            if ($xml_exist)
                $needNewConfigFile = (filemtime($configFile) <
                 filemtime(_PS_MODULE_DIR_ . $module . '/' . $module . '.php'));
            else
            ...
            if ($useConfig and $xml_exist) {
                ...
                foreach (libxml_get_errors() as $error)
                    $errors[] = '[' . $module . '] ' .
                     Tools::displayError('Error found in config file:') . ' ' .
                     htmlentities($error->message);
                libxml_clear_errors();
                if (! count($errors) and (int) $xml_module->need_instance == 0 and
                 ! $needNewConfigFile) {
                    $file = _PS_MODULE_DIR_ . $module . '/' .
                     Language::getIsoById($cookie->id_lang) . '.php';
                    if (Tools::file_exists_cache($file) and include_once ($file))
                        if (isset($_MODULE) and is_array($_MODULE))
                            $_MODULES = ! empty($_MODULES) ? array_merge(
                            $_MODULES, $_MODULE) : $_MODULE;
                    $xml_module->displayName = Module::findTranslation(
                    $xml_module->name, 
                    self::configXmlStringFormat($xml_module->displayName), 
                    (string) $xml_module->name);
                    $xml_module->description = Module::findTranslation(
                    $xml_module->name, 
                    self::configXmlStringFormat($xml_module->description), 
                    (string) $xml_module->name);
                    $xml_module->author = Module::findTranslation(
                    $xml_module->name, 
                    self::configXmlStringFormat($xml_module->author), 
                    (string) $xml_module->name);
                    if (isset($xml_module->confirmUninstall))
                        $xml_module->confirmUninstall = Module::findTranslation(
                        $xml_module->name, 
                        self::configXmlStringFormat(
                        $xml_module->confirmUninstall), 
                        (string) $xml_module->name);
                    $moduleList[$moduleListCursor] = $xml_module;
                    $moduleNameList[$moduleListCursor] = '\'' .
                     strval($xml_module->name) . '\'';
                    $modulesNameToCursor[strval($xml_module->name)] = $moduleListCursor;
                    $moduleListCursor ++;
                }
            }
            if (! $useConfig or ! $xml_exist or (isset(
            $xml_module->need_instance) and (int) $xml_module->need_instance == 1) or
             $needNewConfigFile) {
                // If class already exists, don't include the file
                if (! class_exists($module, false)) {
                    $filepath = _PS_MODULE_DIR_ . $module . '/' . $module .
                     '.php';
                    $file = trim(
                    file_get_contents(
                    _PS_MODULE_DIR_ . $module . '/' . $module . '.php'));
                    ...
                    if (eval(
                    'if (false){	' . $file . ' }') !== false)
                        require_once (_PS_MODULE_DIR_ . $module . '/' . $module .
                         '.php');
                    else
                        $errors[] = sprintf(
                        Tools::displayError('%1$s (parse error in %2$s)'), 
                        $module, substr($filepath, strlen(_PS_ROOT_DIR_)));
                }
                if (class_exists($module, false)) {
                    ...
                    $errors[] = sprintf(
                    Tools::displayError('%1$s (class missing in %2$s)'), $module, 
        ...
        if (! empty($moduleNameList)) {
            ...
            'SELECT `id_module`, `active`, `name` FROM `' . _DB_PREFIX_ .
             'module` WHERE `name` IN (' . join(',', $moduleNameList) . ')');
            foreach ($results as $result) {
    ...
    public static function getModulesDirOnDisk ()
    {
        ...
        foreach ($modules as $name) {
            if (is_dir(_PS_MODULE_DIR_ . $name) && Tools::file_exists_cache(
            _PS_MODULE_DIR_ . $name . '/' . $name . '.php')) {
    ...
    {
        $db = Db::getInstance();
        $module_list_xml = _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'config' .
         DIRECTORY_SEPARATOR . 'modules_list.xml';
    ...
    public static function getModulesInstalled ($position = 0)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ . 'hook` k ON hm.`id_hook` = k.`id_hook`
		WHERE k.`position` = 1' : ''));
    }
    /*
    ...
    {
        global $cookie;
        if ((! empty($id_module) and ! Validate::isUnsignedId($id_module)) or
        ...
        if (! isset(self::$_hookModulesCache)) {
            ...
            if ($result)
                while ($row = $db->nextRow()) {
                    ...
                    self::$_hookModulesCache[$row['hook']][] = array(
                    'id_hook' => $row['id_hook'], 'module' => $row['module'], 
        ...
        foreach (self::$_hookModulesCache[$hook_name] as $array) {
            ...
            if (! ($moduleInstance = Module::getInstanceByName($array['module'])))
                continue;
            $exceptions = $moduleInstance->getExceptions(
            (int) $array['id_hook'], (int) $array['id_module']);
            foreach ($exceptions as $exception)
                if (strstr(
                basename($_SERVER['PHP_SELF']) . '?' . $_SERVER['QUERY_STRING'], 
                $exception['file_name']) &&
                 ! strstr($_SERVER['QUERY_STRING'], $exception['file_name']))
                    continue 2;
            if (is_callable(array($moduleInstance, 'hook' . $hook_name))) {
                $hookArgs['altern'] = ++ $altern;
                $display = call_user_func(
                array($moduleInstance, 'hook' . $hook_name), $hookArgs);
                if ($array['live_edit'] && ((Tools::isSubmit('live_edit') and
                 Tools::getValue('ad') and (Tools::getValue('liveToken') ==
                 sha1(Tools::getValue('ad') . _COOKIE_KEY_))))) {
                    $live_edit = true;
                    $output .= '<script type="text/javascript"> modules_list.push(\'' .
                     $moduleInstance->name . '\');</script>
								<div id="hook_' . $array['id_hook'] . '_module_' .
                     $moduleInstance->id . '_moduleName_' . $moduleInstance->name . '"
								class="dndModule" style="border: 1px dotted red;' .
                     (! strlen($display) ? 'height:50px;' : '') . '">
                     ...
                     $moduleInstance->displayName . '<span style="float:right">
							 	...
							 	<a href="#" id="' . $array['id_hook'] . '_' . $moduleInstance->id . '" class="unregisterHook">
							 		<img src="' .
                     _PS_ADMIN_IMG_ . 'delete.gif"></span></a>
							 	</span>' . $display . '</div>';
                } else
                    $output .= $display;
            }
        }
        return ($live_edit ? '<script type="text/javascript">hooks_list.push(\'' .
         $hook_name . '\'); </script><!--<div id="add_' . $hook_name . '" class="add_module_live_edit">
				<a class="exclusive" href="#">Add a module</a></div>--><div id="' .
         $hook_name . '" class="dndHook" style="min-height:50px">' : '') .
    ...
    public static function hookExecPayment ()
    {
        ...
        if ($result)
            foreach ($result as $module)
                ...
                $module['name'])) and
                 is_callable(array($moduleInstance, 'hookpayment')))
                    ...
                    Currency::checkPaymentCurrencies($moduleInstance->id))))
                        ...
                        array($moduleInstance, 'hookpayment'), $hookArgs);
        return $output;
    }
    /**
    ...
    public static function getPaymentModules ()
    {
		...
		INNER JOIN `' . _DB_PREFIX_ .
         'customer_group` cg on (cg.`id_group` = mg.`id_group` AND cg.`id_customer` = ' .
         (int) ($id_customer) . ')
		LEFT JOIN `' . _DB_PREFIX_ . 'hook_module` hm ON hm.`id_module` = m.`id_module`
		LEFT JOIN `' .
         _DB_PREFIX_ . 'hook` h ON hm.`id_hook` = h.`id_hook`
    ...
    public static function findTranslation ($name, $string, $source)
    {
        ...
        if (! isset(self::$l_cache[$cache_key])) {
            ...
            $currentKey = '<{' . strtolower($name) . '}' .
             strtolower(_THEME_NAME_) . '>' . strtolower($source) . '_' .
             md5($string);
            $defaultKey = '<{' . strtolower($name) . '}prestashop>' .
             strtolower($source) . '_' . md5($string);
    ...
    public function updatePosition ($id_hook, $way, $position = NULL)
    {
		...
		WHERE position between ' .
         (int) (min(array($from['position'], $to['position']))) . ' AND ' .
         (int) (max(array($from['position'], $to['position']))) . '
		AND `id_hook`=' . (int) ($from['id_hook'])) and Db::getInstance()->Execute(
        '
		UPDATE `' . _DB_PREFIX_ . 'hook_module`
		SET `position`=' . (int) ($to['position']) . '
		WHERE `' . pSQL($this->identifier) . '` = ' .
         (int) ($from[$this->identifier]) . ' AND `id_hook`=' .
         (int) ($to['id_hook'])));
    }
    /*
    ...
    public function getPosition ($id_hook)
    {
        if (isset(Hook::$preloadModulesFromHooks))
            if (isset(Hook::$preloadModulesFromHooks[$id_hook]))
                if (isset(
                Hook::$preloadModulesFromHooks[$id_hook]['module_position'][$this->id]))
    ...
    public function displayConfirmation ($string)
    {
		...
		<div class="module_confirmation conf confirm">
			<img src="' . _PS_IMG_ . 'admin/ok.gif" alt="" title="" /> ' .
         $string . '
		</div>';
        return $output;
    }
    ...
    {
        if (self::$exceptionsCache == NULL and ! is_array(
        self::$exceptionsCache)) {
        ...
        return (array_key_exists((int) ($id_hook) . '-' . (int) ($this->id), 
        self::$exceptionsCache) ? self::$exceptionsCache[(int) ($id_hook) . '-' .
         (int) ($this->id)] : array());
    }
	...
	*/
    protected static function _isTemplateOverloadedStatic ($moduleName, 
    $template)
    {
        if (Tools::file_exists_cache(
        ...
        _PS_MODULE_DIR_ . $moduleName . '/' . $template))
            return false;
        return NULL;
    }
    ...
    public static function display ($file, $template, $cacheId = NULL, 
    $compileId = NULL)
    {
        global $smarty;
        ...
        $smarty->assign('module_dir', 
        __PS_BASE_URI__ . 'modules/' . basename($file, '.php') . '/');
        if (($overloaded = self::_isTemplateOverloadedStatic(
        basename($file, '.php'), $template)) === NULL)
            $result = Tools::displayError('No template found for module') . ' ' .
             basename($file, '.php');
        else {
            $smarty->assign('module_template_dir', 
            ($overloaded ? _THEME_DIR_ : __PS_BASE_URI__) . 'modules/' .
             basename($file, '.php') . '/');
            $result = $smarty->fetch(
            ($overloaded ? _PS_THEME_DIR_ . 'modules/' . basename($file, '.php') : _PS_MODULE_DIR_ .
             basename($file, '.php')) . '/' . $template, $cacheId, $compileId);
        }
        if (Configuration::get('PS_FORCE_SMARTY_2')) /* Keep a backward compatibility for Smarty v2 */
    ...
    {
        return $this->_isTemplateOverloaded($template) ? _PS_THEME_DIR_ : _PS_MODULE_DIR_ .
         $this->name . '/';
    }
    public function isCached ($template, $cacheId = NULL, $compileId = NULL)
    {
        ...
        if (! Configuration::get('PS_FORCE_SMARTY_2')) /* PHP version > 5.1.2 */
			return $smarty->isCached(
            $this->_getApplicableTemplateDir($template) . $template, $cacheId, 
            $compileId);
            /* or keep a backward compatibility if PHP version < 5.1.2 */
        else
            return $smarty->is_cached(
            $this->_getApplicableTemplateDir($template) . $template, $cacheId, 
            $compileId);
    }
...
<module>
	...
	<tab><![CDATA[' . Tools::htmlentitiesUTF8($this->tab) . ']]></tab>' . (isset(
	...
	<need_instance>' . (int) $this->need_instance . '</need_instance>' . (isset(
        $this->limited_countries) ? "\n\t" . '<limited_countries>' .
         (sizeof($this->limited_countries) == 1 ? $this->limited_countries[0] : '') .
         '</limited_countries>' : '') . '
</module>';
        if (is_writable(_PS_MODULE_DIR_ . $this->name . '/'))
            file_put_contents(_PS_MODULE_DIR_ . $this->name . '/config.xml', 
            $xml);
    }
    /**
