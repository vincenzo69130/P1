...
class DateRangeCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('time_start' => 'isDate', 
    'time_end' => 'isDate');
    ...
    public static function getCurrentRange ()
    {
		...
		WHERE `time_end` = (SELECT MAX(`time_end`) FROM `' . _DB_PREFIX_ .
         'date_range`)');
        if (! $result['id_date_range'] or
         strtotime($result['time_end']) < strtotime(date('Y-m-d H:i:s'))) {
            ...
            $dateRange->time_end = strftime('%Y-%m-%d %H:%M:%S', 
            strtotime($dateRange->time_start) + $rangeSize);
            $dateRange->add();
            return $dateRange->id;
        }
