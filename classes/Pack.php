...
class PackCore extends Product
{
    ...
    public static function isPack ($id_product)
    {
        if (! array_key_exists($id_product, self::$cacheIsPack)) {
            ...
            'SELECT COUNT(*) FROM ' . _DB_PREFIX_ .
             'pack WHERE id_product_pack = ' . (int) ($id_product));
    ...
    public static function isPacked ($id_product)
    {
        if (! array_key_exists($id_product, self::$cacheIsPacked)) {
            $result = Db::getInstance()->getValue(
            'SELECT COUNT(*) FROM ' . _DB_PREFIX_ .
             'pack WHERE id_product_item = ' . (int) ($id_product));
    ...
    public static function noPackPrice ($id_product)
    {
        ...
        $items = self::getItems($id_product, 
    ...
    public static function getItems ($id_product, $id_lang)
    {
        ...
        'SELECT id_product_item, quantity FROM ' . _DB_PREFIX_ .
         'pack where id_product_pack = ' . (int) ($id_product));
        $arrayResult = array();
    ...
    {
        $items = self::getItems((int) ($id_product), 
        Configuration::get('PS_LANG_DEFAULT'));
        foreach ($items as $item)
            if ($item->quantity < $item->pack_quantity and
    ...
    public static function getPacksTable ($id_product, $id_lang, $full = false, 
    $limit = NULL)
    {
    ...
    public static function deleteItems ($id_product)
    {
        ...
        'UPDATE ' . _DB_PREFIX_ .
         'product SET cache_is_pack = 0 WHERE id_product = ' .
    ...
    public static function addItem ($id_product, $id_item, $qty)
    {
        Db::getInstance()->Execute(
        'UPDATE ' . _DB_PREFIX_ .
         'product SET cache_is_pack = 1 WHERE id_product = ' .
         (int) ($id_product) . ' LIMIT 1');
        return Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'pack', 
        array('id_product_pack' => (int) ($id_product), 
        'id_product_item' => (int) ($id_item), 'quantity' => (int) ($qty)), 
        'INSERT');
    }
    public static function duplicate ($id_product_old, $id_product_new)
    {
		...
		(SELECT ' . (int) ($id_product_new) . ', id_product_item, quantity FROM ' .
         _DB_PREFIX_ . 'pack WHERE id_product_pack = ' . (int) ($id_product_old) .
         ')');
