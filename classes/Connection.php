...
class ConnectionCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_guest' => 'isUnsignedId', 
    'id_page' => 'isUnsignedId', 'ip_address' => 'isInt', 
    'http_referer' => 'isAbsoluteUrl');
    ...
    public static function setPageConnection ($cookie, $full = true)
    {
        ...
        if (! isset($cookie->id_connections) or ! strstr(
    ...
    public static function setNewConnection ($cookie)
    {
        ...
        if (! $result['id_guest'] and (int) ($cookie->id_guest)) {
            ...
            if (! isset($arrayUrl['host']) or preg_replace('/^www./', '', 
            $arrayUrl['host']) ==
             preg_replace('/^www./', '', Tools::getHttpHost(false, false)))
                $referer = '';
            $connection = new Connection();
            $connection->id_guest = (int) ($cookie->id_guest);
            $connection->id_page = Page::getCurrentId();
            $connection->ip_address = Tools::getRemoteAddr() ? ip2long(
            Tools::getRemoteAddr()) : '';
    ...
    public static function setPageTime ($id_connections, $id_page, $time_start, 
    $time)
    {
        if (! Validate::isUnsignedId($id_connections) or
         ! Validate::isUnsignedId($id_page) or ! Validate::isDate($time_start))
            return;
