...
class MessageCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('message' => 'isCleanHtml', 
    'id_cart' => 'isUnsignedId', 'id_order' => 'isUnsignedId', 
    'id_customer' => 'isUnsignedId', 'id_employee' => 'isUnsignedId', 
    'private' => 'isBool');
    ...
    public static function getMessagesByOrderId ($id_order, $private = false)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'message_readed` mr ON (mr.id_message = m.id_message AND mr.id_employee = ' .
    ...
    {
        if (! Validate::isUnsignedId($id_message) or
         ! Validate::isUnsignedId($id_employee))
            die(Tools::displayError());
