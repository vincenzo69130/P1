...
class ProductSaleCore
{
	...
	*/
    public static function fillProductSales ()
    {
		...
		SELECT od.product_id, COUNT(od.product_id), SUM(od.product_quantity), NOW()
					FROM ' . _DB_PREFIX_ .
         'order_detail od GROUP BY od.product_id');
    }
    /*
	...
	*/
    public static function getBestSales ($id_lang, $pageNumber = 0, 
    $nbProducts = 10, $orderBy = NULL, $orderWay = NULL)
    {
        if ($pageNumber < 0)
            $pageNumber = 0;
        if ($nbProducts < 1)
		...
		ORDER BY ' . (isset($orderByPrefix) ? $orderByPrefix . '.' : '') . '`' .
	...
	*/
    public static function getBestSalesLight ($id_lang, $pageNumber = 0, 
    $nbProducts = 10)
    {
        global $link;
        if ($pageNumber < 0)
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         ...
         (int) $id_lang . ')
		LEFT JOIN `' . _DB_PREFIX_ .
         'category_lang` cl ON (cl.`id_category` = p.`id_category_default` AND cl.`id_lang` = ' .
         (int) $id_lang . ')
		WHERE p.`active` = 1
		AND p.`id_product` IN (
			SELECT cp.`id_product`
        ...
        foreach ($result as &$row) {
            $row['link'] = $link->getProductLink($row['id_product'], 
    ...
    public static function getNbrSales ($id_product)
    {
        ...
        'SELECT `sale_nbr` FROM ' . _DB_PREFIX_ .
         'product_sale WHERE `id_product` = ' . (int) ($id_product));
    ...
    public static function removeProductSale ($id_product, $qty = 1)
    {
        ...
        if ($nbrSales > 1)
            return Db::getInstance()->Execute(
            'UPDATE ' . _DB_PREFIX_ .
             'product_sale SET `quantity` = `quantity` - ' . (int) ($qty) .
             ', `sale_nbr` = `sale_nbr` - 1, `date_upd` = NOW() WHERE `id_product` = ' .
             (int) ($id_product));
        elseif ($nbrSales == 1)
            return Db::getInstance()->Execute(
