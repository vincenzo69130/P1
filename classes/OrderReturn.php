...
class OrderReturnCore extends ObjectModel
{
    ...
    'id_order' => 'isUnsignedId', 'question' => 'isMessage');
    ...
    $customizationIds, $customizationQtyInput)
    {
        ...
        if ($orderDetailList)
            foreach ($orderDetailList as $key => $orderDetail)
                if ($qty = (int) ($productQtyList[$key]))
                    ...
                    array('id_order_return' => (int) ($this->id), 
                    'id_order_detail' => (int) ($orderDetail), 
                    'product_quantity' => $qty, 'id_customization' => 0), 
                    'INSERT');
            ...
            foreach ($customizationIds as $orderDetailId => $customizations)
                foreach ($customizations as $customizationId)
                    if ($quantity = (int) ($customizationQtyInput[(int) ($customizationId)]))
                        ...
                        'id_customization' => (int) ($customizationId)), 'INSERT');
    }
    ...
    $customizationIds, $customizationQtyInput)
    {
        ...
        $order_return = self::getOrdersReturn($order->id_customer, $order->id, 
        true);
        foreach ($order_return as $or) {
            $order_return_products = self::getOrdersReturnProducts(
            $or['id_order_return'], $order);
        ...
        if ($customizationIds) {
            $orderedCustomizations = Customization::getOrderedCustomizations(
            (int) ($order->id_cart));
            foreach ($customizationIds as $customizations)
                foreach ($customizations as $customizationId) {
                    ...
                    if ((int) ($orderedCustomizations[$customizationId]['quantity']) -
                     $quantity < 0)
                        return false;
                }
        }
    ...
    public static function getOrdersReturn ($customer_id, $order_id = false, 
    $no_denied = false)
    {
        global $cookie;
    ...
    public static function getOrdersReturnProducts ($orderReturnId, $order)
    {
        ...
        foreach ($productsRet as $return_detail) {
            $tmp[$return_detail['id_order_detail']]['quantity'] = isset(
            $tmp[$return_detail['id_order_detail']]['quantity']) ? $tmp[$return_detail['id_order_detail']]['quantity'] +
             (int) ($return_detail['product_quantity']) : (int) ($return_detail['product_quantity']);
    ...
    public static function deleteOrderReturnDetail ($id_order_return, 
    $id_order_detail, $id_customization = 0)
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'order_return_detail` WHERE `id_order_detail` = ' .
         (int) ($id_order_detail) . ' AND `id_order_return` = ' .
         (int) ($id_order_return) . ' AND `id_customization` = ' .
