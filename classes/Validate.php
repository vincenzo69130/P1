...
class ValidateCore
{
    ...
    {
        return empty($email) or preg_match(
    ...
    public static function isModuleUrl ($url, &$errors)
    {
        ...
        elseif (substr($url, - 4) != '.tar' and substr($url, - 4) != '.zip' and
         substr($url, - 4) != '.tgz' and substr($url, - 7) != '.tar.gz')
    ...
    {
        return preg_match('/^[a-zA-Z0-9]{2,3}((-)[a-zA-Z0-9]{1,3})?$/', 
        $isoCode);
    }
    ...
    public static function isDateFormat ($date)
    {
        ...
        '/^([0-9]{4})-((0?[0-9])|(1[0-2]))-((0?[1-9])|([0-2][0-9])|(3[01]))( [0-9]{2}:[0-9]{2}:[0-9]{2})?$/', 
        $date);
    }
    /**
    ...
    public static function isDate ($date)
    {
        ...
        $date, $matches))
            return false;
        return checkdate((int) $matches[2], (int) $matches[5], 
        (int) $matches[0]);
    }
    /**
    ...
    {
        return ($orderWay === 'ASC' | $orderWay === 'DESC' | $orderWay === 'asc' |
         $orderWay === 'desc');
    }
    /**
     * Check for table or identifier validity
    ...
    {
        return (preg_match('#^[0-9]+$#', (string) $value) and $value < 4294967296 and
         $value >= 0);
    }
    /**
     * Check for an integer validity (unsigned)
    ...
    {
        return empty($dni) or (bool) preg_match('/^[0-9A-Za-z-.]{1,16}$/U', 
        $dni);
    }
    /**
    ...
    public static function isDni ($dni)
    {
		...
		*/
        ...
        if (preg_match('/(^[0-9]{8}[A-Z]{1}$)/', $dni))
            if ($char[8] ==
             substr('TRWAGMYFPDXBNJZSQVHLCKE', substr($dni, 0, 8) % 23, 1))
                return 1;
            else
                return - 2;
        $sum = $char[2] + $char[4] + $char[6];
        for ($i = 1; $i < 8; $i += 2)
            $sum += substr((2 * $char[$i]), 0, 1) + substr((2 * $char[$i]), 1, 
            1);
        ...
        if (preg_match('/^[XYZ]{1}/', $dni))
            if ($char[8] == substr('TRWAGMYFPDXBNJZSQVHLCKE', 
            substr(
            str_replace(array('X', 'Y', 'Z'), array('0', '1', '2'), $dni), 0, 8) %
             23, 1))
                return 1;
            else
                return - 4;
    ...
    {
        return ($data === 'states' or $data === 'taxes' or $data === 'currencies' or
         $data === 'languages' or $data === 'units');
    }
    /**
    ...
    {
        return ($data == NULL) or
         (bool) (is_string($data) and preg_match('/^a:[0-9]+:{.*;}$/s', $data));
    }
    /**
    ...
    {
        return ($data == NULL) or
         (bool) (preg_match('/^\-?[0-9]{1,8}\.[0-9]{1,8}$/s', $data));
    }
    /**
    ...
    public static function isSceneZones ($zones)
    {
        foreach ($zones as $zone) {
            ...
            if (! isset($zone['width']) || ! self::isUnsignedInt($zone['width']))
                return false;
            if (! isset($zone['height']) || ! self::isUnsignedInt(
            $zone['height']))
                return false;
            if (! isset($zone['id_product']) ||
             ! self::isUnsignedInt($zone['id_product']))
                return false;
        }
        return true;
    }
