...
class FeatureValueCore extends ObjectModel
{
    ...
    public static function getFeatureValuesWithLang ($id_lang, $id_feature)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'feature_value_lang` vl ON (v.`id_feature_value` = vl.`id_feature_value` AND vl.`id_lang` = ' .
         (int) $id_lang . ')
    ...
    public static function addFeatureValueImport ($id_feature, $name)
    {
        ...
        if (! isset($rq[0]['id_feature_value']) or
    ...
    public function add ($autodate = true, $nullValues = false)
    {
        ...
        if ($return)
            Module::hookExec('afterSaveFeatureValue', 
            array('id_feature_value' => $this->id));
        return $return;
    }
    public function delete ()
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'feature_product` WHERE `id_feature_value` = ' . (int) $this->id);
        $return = parent::delete();
        if ($return)
            Module::hookExec('afterDeleteFeatureValue', 
            array('id_feature_value' => $this->id));
        return $return;
    }
