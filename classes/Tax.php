...
class TaxCore extends ObjectModel
{
    ...
    public static function getTaxes ($id_lang = false, $active = 1)
    {
        ...
        '
		SELECT t.id_tax, t.rate' .
         ((int) ($id_lang) ? ', tl.name, tl.id_lang ' : '') . '
		FROM `' . _DB_PREFIX_ . 'tax` t
		' . ((int) ($id_lang) ? 'LEFT JOIN `' .
         _DB_PREFIX_ .
         'tax_lang` tl ON (t.`id_tax` = tl.`id_tax` AND tl.`id_lang` = ' .
         (int) ($id_lang) . ')' . ($active == 1 ? 'WHERE t.`active` = 1' : '') . '
		ORDER BY `name` ASC' : ''));
    }
     ...
     */
    public static function getApplicableTax ($id_tax, $productTax, 
    $id_address = NULL)
    {
        Tools::displayAsDeprecated();
     ...
     * @return float taxe_rate
     */
    public static function getApplicableTaxRate ($id_tax, $productTax, 
    $id_address = NULL)
    {
        Tools::displayAsDeprecated();
        return $productTax;
    ...
    public static function getProductTaxRate ($id_product, $id_address = NULL)
    {
        ...
        if (! empty($id_address)) {
            ...
            if ($address_infos['id_country']) {
                ...
                $id_county = (int) County::getIdCountyByZipCode(
                $address_infos['id_state'], $address_infos['postcode']);
            }
            if (! empty($address_infos['vat_number']) and $address_infos['id_country'] !=
             Configuration::get('VATNUMBER_COUNTRY') and
             Configuration::get('VATNUMBER_MANAGEMENT'))
                return 0;
        }
        if ($rate = Tax::getProductTaxRateViaRules((int) $id_product, 
    ...
    public static function getProductEcotaxRate ($id_address = NULL)
    {
        ...
        if ($rate = TaxRulesGroup::getTaxesRate(
     ...
     * @return Tax
     */
    public static function getProductTaxRateViaRules ($id_product, $id_country, 
    $id_state, $id_county)
    {
        if (! isset(
        self::$_product_tax_via_rules[$id_product . '-' . $id_country . '-' .
         $id_state . '-' . $id_county])) {
            $tax_rate = TaxRulesGroup::getTaxesRate(
            (int) Product::getIdTaxRulesGroupByIdProduct((int) $id_product), 
            (int) $id_country, (int) $id_state, (int) $id_county);
            self::$_product_tax_via_rules[$id_product . '-' . $id_country . '-' .
             $id_county] = $tax_rate;
        }
    ...
    public static function getCarrierTaxRate ($id_carrier, $id_address = NULL)
    {
        ...
        return TaxRulesGroup::getTaxesRate(
        (int) Carrier::getIdTaxRulesGroupByIdCarrier((int) $id_carrier), 
        (int) $id_country, (int) $id_state, (int) $id_county);
    }
