...
class ManufacturerCore extends ObjectModel
{
    ...
    protected $fieldsSizeLang = array('short_description' => 254, 
    'meta_title' => 128, 'meta_description' => 255, 'meta_description' => 255);
    protected $fieldsValidateLang = array('description' => 'isString', 
    'short_description' => 'isString', 'meta_title' => 'isGenericName', 
    'meta_description' => 'isGenericName', 'meta_keywords' => 'isGenericName');
    protected $table = 'manufacturer';
    protected $identifier = 'id_manufacturer';
    protected $webserviceParameters = array(
    'fields' => array('active' => array(), 
    'link_rewrite' => array('getter' => 'getLink', 'setter' => false)), 
    'associations' => array(
    'addresses' => array('resource' => 'address', 'setter' => false, 
    'fields' => array('id' => array('xlink_resource' => 'addresses')))));
    ...
    {
        $fieldsArray = array('description', 'short_description', 'meta_title', 
        ...
        foreach ($languages as $language) {
            ...
            foreach ($fieldsArray as $field) {
                ...
                if (isset($this->{$field}[$language['id_lang']]) and
                 ! empty($this->{$field}[$language['id_lang']]))
                    ...
                    $this->{$field}[$language['id_lang']], true);
            ...
            $fields[$language['id_lang']]['short_description'] = (isset(
            $this->short_description[$language['id_lang']])) ? pSQL(
            $this->short_description[$language['id_lang']], true) : '';
        }
        return $fields;
    }
    ...
    protected function getManufacturerAddress ()
    {
        ...
        'SELECT `id_address` FROM ' . _DB_PREFIX_ .
         'address WHERE `id_manufacturer` = ' . (int) ($this->id));
        if (! $result)
            return false;
     ...
     */
    public static function getManufacturers ($getNbProducts = false, $id_lang = 0, 
    $active = true, $p = false, $n = false, $all_group = false)
    {
        if (! $id_lang)
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'manufacturer_lang` ml ON (m.`id_manufacturer` = ml.`id_manufacturer` AND ml.`id_lang` = ' .
         (int) ($id_lang) . ')
        ...
        if ($getNbProducts) {
            ...
            if (! $all_group) {
                ...
                $sqlGroups = (count($groups) ? 'IN (' . implode(',', $groups) .
                 ')' : '= 1');
            }
            foreach ($manufacturers as $key => $manufacturer) {
				...
				WHERE m.`id_manufacturer` = ' . (int) ($manufacturer['id_manufacturer']) .
                 ($active ? ' AND p.`active` = 1 ' : '') . ($all_group ? '' : ' AND p.`id_product` IN (
        ...
        for ($i = 0; $i < sizeof($manufacturers); $i ++)
            if ((int) (Configuration::get('PS_REWRITING_SETTINGS')))
                $manufacturers[$i]['link_rewrite'] = Tools::link_rewrite(
                $manufacturers[$i]['name'], false);
            else
    ...
    public static function getNameById ($id_manufacturer)
    {
        if (! isset(self::$cacheName[$id_manufacturer]))
            self::$cacheName[$id_manufacturer] = Db::getInstance(
            _PS_USE_SQL_SLAVE_)->getValue(
            '
			SELECT `name` FROM `' . _DB_PREFIX_ .
             'manufacturer` WHERE `id_manufacturer` = ' .
             (int) ($id_manufacturer) . ' AND `active` = 1');
    ...
    public static function getProducts ($id_manufacturer, $id_lang, $p, $n, 
    $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $active_category = true)
    {
        if ($p < 1)
            $p = 1;
            ...
            $sql = '
             ...
             ($active ? ' AND p.`active` = 1' : '') . '
				AND p.`id_product` IN (
					...
					FROM `' . _DB_PREFIX_ . 'category_group` cg
					LEFT JOIN `' . _DB_PREFIX_ .
             'category_product` cp ON (cp.`id_category` = cg.`id_category`)' .
             ($active_category ? ' INNER JOIN `' . _DB_PREFIX_ .
             'category` ca ON cp.`id_category` = ca.`id_category` AND ca.`active` = 1' : '') . '
					WHERE cg.`id_group` ' . $sqlGroups . '
				)';
        ...
        $sql = '
		SELECT p.*, pa.`id_product_attribute`, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, m.`name` AS manufacturer_name, tl.`name` AS tax_name, t.`rate`, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
	...
	*/
    ...
    public function getAddresses ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'country_lang` AS cl ON (cl.`id_country` = a.`id_country` AND cl.`id_lang` = ' .
         (int) ($id_lang) . ')
    ...
    public function setWsAddresses ($id_addresses)
    {
        ...
        if (count($ids))
            ...
            '
			UPDATE `' . _DB_PREFIX_ . 'address` 
			SET id_customer = 0, id_supplier = 0, id_manufacturer = ' .
             (int) $this->id . ' 
