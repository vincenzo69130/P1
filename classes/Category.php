...
class CategoryCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('nleft' => 'isUnsignedInt', 
    'nright' => 'isUnsignedInt', 'level_depth' => 'isUnsignedInt', 
    'active' => 'isBool', 'id_parent' => 'isUnsignedInt', 
    'groupBox' => 'isArrayWithIds');
    protected $fieldsRequiredLang = array('name', 'link_rewrite');
    protected $fieldsSizeLang = array('name' => 64, 'link_rewrite' => 64, 
    'meta_title' => 128, 'meta_description' => 255, 'meta_keywords' => 255);
    protected $fieldsValidateLang = array('name' => 'isCatalogName', 
    'link_rewrite' => 'isLinkRewrite', 'description' => 'isCleanHtml', 
    'meta_title' => 'isGenericName', 'meta_description' => 'isGenericName', 
    ...
    protected $webserviceParameters = array('objectsNodeName' => 'categories', 
    'hidden_fields' => array('nleft', 'nright', 'groupBox'), 
    'fields' => array('id_parent' => array('xlink_resource' => 'categories'), 
    'level_depth' => array('setter' => false), 
    'nb_products_recursive' => array('getter' => 'getWsNbProductsRecursive', 
    'setter' => false)), 
    'associations' => array(
    ...
    public function __construct ($id_category = NULL, $id_lang = NULL)
    {
        ...
        $this->id_image = ($this->id and
         file_exists(_PS_CAT_IMG_DIR_ . (int) ($this->id) . '.jpg')) ? (int) ($this->id) : false;
    ...
    public function getTranslationsFieldsChild ()
    {
        ...
        return parent::getTranslationsFields(
        array('name', 'description', 'link_rewrite', 'meta_title', 
     ...
     */
    function recurseLiteCategTree ($maxDepth = 3, $currentDepth = 0, 
    $id_lang = NULL, $excludedIdsArray = NULL)
    {
        global $link;
        if (! (int) $id_lang)
            $id_lang = _USER_ID_LANG_;
        $children = array();
        if (($maxDepth == 0 or $currentDepth < $maxDepth) and
         $subcats = $this->getSubCategories((int) $id_lang, true) and
         sizeof($subcats))
            foreach ($subcats as &$subcat) {
                if (! $subcat['id_category'])
                    break;
                elseif (! is_array($excludedIdsArray) ||
                 ! in_array($subcat['id_category'], $excludedIdsArray)) {
                    $categ = new Category((int) $subcat['id_category'], 
                    (int) $id_lang);
                    $children[] = $categ->recurseLiteCategTree($maxDepth, 
                    $currentDepth + 1, (int) $id_lang, $excludedIdsArray);
                }
            }
        return array('id' => (int) $this->id_category, 
        'link' => $link->getCategoryLink((int) $this->id, $this->link_rewrite), 
        'name' => $this->name, 'desc' => $this->description, 
        'children' => $children);
    }
    public static function recurseCategory ($categories, $current, 
    $id_category = 1, $id_selected = 1)
    {
        echo '<option value="' . $id_category . '"' .
         (($id_selected == $id_category) ? ' selected="selected"' : '') . '>' .
         str_repeat('&nbsp;', $current['infos']['level_depth'] * 5) .
         stripslashes($current['infos']['name']) . '</option>';
        if (isset($categories[$id_category]))
            foreach (array_keys($categories[$id_category]) as $key)
                self::recurseCategory($categories, 
                $categories[$id_category][$key], $key, $id_selected);
    }
    /**
    ...
    public function delete ()
    {
        ...
        $list = sizeof($toDelete) > 1 ? implode(',', 
        array_map('intval', $toDelete)) : (int) ($this->id);
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'category` WHERE `id_category` IN (' .
         $list . ')');
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'category_lang` WHERE `id_category` IN (' .
         $list . ')');
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'category_product` WHERE `id_category` IN (' . $list . ')');
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'category_group` WHERE `id_category` IN (' . $list . ')');
		...
		WHERE `id_product` NOT IN (SELECT `id_product` FROM `' . _DB_PREFIX_ .
         'category_product`)');
        foreach ($result as $p) {
		...
		NOT IN (SELECT `id_category` FROM `' . _DB_PREFIX_ .
         'category`)');
    ...
    public static function regenerateEntireNtree ()
    {
        ...
        'SELECT id_category, id_parent FROM ' . _DB_PREFIX_ .
         'category ORDER BY id_category ASC');
    ...
    protected static function _subTree (&$categories, $id_category, &$n)
    {
        ...
        if (isset($categories[(int) $id_category]['subcategories']))
            foreach (array_keys(
            $categories[(int) $id_category]['subcategories']) as $id_subcategory)
                self::_subTree($categories, (int) $id_subcategory, $n);
        $right = (int) $n ++;
        Db::getInstance()->Execute(
        'UPDATE ' . _DB_PREFIX_ . 'category SET nleft = ' . (int) $left .
         ', nright = ' . (int) $right . ' WHERE id_category = ' .
         (int) $id_category . ' LIMIT 1');
    }
    /**
     ...
     */
    public static function getCategories ($id_lang = false, $active = true, $order = true, 
    ...
    public function getSubCategories ($id_lang, $active = true)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'category_lang` cl ON (c.`id_category` = cl.`id_category` AND `id_lang` = ' .
         (int) ($id_lang) . ')
        ...
        foreach ($result as &$row) {
            $row['id_image'] = (file_exists(
            _PS_CAT_IMG_DIR_ . $row['id_category'] . '.jpg')) ? (int) ($row['id_category']) : Language::getIsoById(
            $id_lang) . '-default';
     ...
     */
    public function getProducts ($id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, 
    $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, 
    $checkAccess = true)
    {
        global $cookie;
        ...
        if ($orderBy == 'price')
            $orderBy = 'orderprice';
        if (! Validate::isBool($active) or ! Validate::isOrderBy($orderBy) or
         ! Validate::isOrderWay($orderWay))
            die(Tools::displayError());
        ...
        if ($random === true) {
            ...
            $sql .= ' ORDER BY ' .
             (isset($orderByPrefix) ? $orderByPrefix . '.' : '') . '`' .
    ...
    public static function getRootCategory ($id_lang = NULL)
    {
        ...
        is_null($id_lang) ? (int) _USER_ID_LANG_ : (int) ($id_lang));
    }
    /**
     *
     * @param int $id_parent
     ...
     * @return array
     */
    public static function getChildrenWithNbSelectedSubCat ($id_parent, 
    $selectedCat, $id_lang)
    {
		...
		SELECT c.`id_category`, c.`level_depth`, cl.`name`, IF((
			...
			AND c3.`id_category`  IN (' .
         implode(',', array_map('intval', $selectedCat)) . ')
    ...
    public static function duplicateProductCategories ($id_old, $id_new)
    {
        ...
        if ($result)
            foreach ($result as $i)
                $row[] = '(' . implode(', ', 
                array((int) ($id_new), $i['id_category'], 
                '(SELECT tmp.max + 1 FROM (SELECT MAX(cp.`position`) AS max FROM `' .
                 _DB_PREFIX_ . 'category_product` cp WHERE cp.`id_category`=' .
                 (int) ($i['id_category']) . ') AS tmp)')) . ')';
        $flag = Db::getInstance()->Execute(
        'INSERT INTO `' . _DB_PREFIX_ .
         'category_product` (`id_product`, `id_category`, `position`) VALUES ' .
         implode(',', $row));
        return $flag;
    }
    /**
    ...
    public static function checkBeforeMove ($id_category, $id_parent)
    {
        ...
        while (42) {
            ...
            'SELECT `id_parent` FROM `' . _DB_PREFIX_ .
             'category` WHERE `id_category` = ' . (int) ($i));
    ...
    {
        if (! Validate::isUnsignedId($id_category) or
     ...
     */
    public static function searchByNameAndParentCategoryId ($id_lang, 
    $category_name, $id_parent_category)
    {
        return Db::getInstance()->getRow(
        '
		SELECT c.*, cl.*
    ...
    {
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'category_group` WHERE `id_category` = ' .
         (int) ($this->id));
    }
    public function addGroups ($groups)
    {
        foreach ($groups as $group) {
            $row = array('id_category' => (int) ($this->id), 
            'id_group' => (int) ($group));
            Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'category_group', $row, 
            'INSERT');
        }
    }
    ...
    public function checkAccess ($id_customer)
    {
        if (! $id_customer) {
			...
			INNER JOIN ' . _DB_PREFIX_ .
             'customer_group cg on (cg.`id_group` = ctg.`id_group` AND cg.`id_customer` = ' .
             (int) ($id_customer) . ')
			WHERE ctg.`id_category` = ' . (int) ($this->id));
        }
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
			...
			WHERE `position`
			' . ($way ? '> ' . (int) ($movedCategory['position']) .
         ' AND `position` <= ' . (int) ($position) : '< ' .
         (int) ($movedCategory['position']) . ' AND `position` >= ' .
         (int) ($position)) . '
    ...
    public static function getLastPosition ($id_category_parent)
    {
        ...
        'SELECT MAX(position)+1 FROM `' . _DB_PREFIX_ .
         'category` WHERE `id_parent` = ' . (int) ($id_category_parent)));
    }
    ...
    public function getWsNbProductsRecursive ()
    {
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
        '
		SELECT count(distinct(id_product)) as nb_product_recursive FROM  `' .
         _DB_PREFIX_ . 'category_product`
		WHERE id_category IN (SELECT id_category
		FROM `' . _DB_PREFIX_ . 'category`
		WHERE nleft > ' .
         (int) $this->nleft . ' AND nright < ' . (int) $this->nright .
         ' AND active = 1 UNION SELECT ' . (int) $this->id . ')');
        if (! $result)
            return - 1;
