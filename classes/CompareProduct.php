...
class CompareProductCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_product' => 'isUnsignedInt', 
    'id_guest' => 'isUnsignedInt', 'id_customer' => 'isUnsignedInt');
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'compare_product` WHERE `id_guest` = ' .
         (int) ($id_guest) . ' AND `id_product` = ' . (int) ($id_product));
    }
    /**
    ...
    public static function addCustomerCompareProduct ($id_customer, $id_product)
    {
        ...
        '
			INSERT INTO `' . _DB_PREFIX_ . 'compare_product` (`id_product`, `id_guest`, `id_customer`, `date_add`, `date_upd`)
			VALUES (' . (int) ($id_product) . ', 0, ' . (int) ($id_customer) .
         ', NOW(), NOW())');
    }
    /**
     ...
     * @return boolean
     */
    public static function removeCustomerCompareProduct ($id_customer, 
    $id_product)
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'compare_product` WHERE `id_customer` = ' .
         (int) ($id_customer) . ' AND `id_product` = ' . (int) ($id_product));
    }
    /**
