...
abstract class ModuleGraphCore extends Module
{
    ...
    protected function setDateGraph ($layers, $legend = false)
    {
        ...
        $fromArray = getdate(
        strtotime($this->_employee->stats_date_from));
        $toArray = getdate(strtotime($this->_employee->stats_date_to));
        // If the granularity is inferior to 1 day
        if ($this->_employee->stats_date_from ==
         $this->_employee->stats_date_to) {
            if ($legend)
                for ($i = 0; $i < 24; $i ++) {
                    if ($layers == 1)
                        $this->_values[$i] = 0;
...
elseif (strtotime($this->_employee->stats_date_to) -
         strtotime($this->_employee->stats_date_from) <= 2678400) {
            if ($legend) {
                $days = array();
                if ($fromArray['mon'] == $toArray['mon'])
                    ...
                    $imax = date('t', 
                    mktime(0, 0, 0, $fromArray['mon'], 1, $fromArray['year']));
...
elseif (strtotime('-1 year', 
        strtotime($this->_employee->stats_date_to)) <
         strtotime($this->_employee->stats_date_from)) {
            if ($legend) {
                $months = array();
        ...
        if (sizeof($this->_legend)) {
            ...
            if ($datas['type'] == 'pie')
                foreach ($this->_legend as $key => $legend)
                    for ($i = 0; $i <
                     (is_array($this->_titles['main']) ? sizeof($this->_values) : 1); ++ $i)
    ...
    protected function _displayCsv ()
    {
        ...
        header(
        'Content-Disposition: attachment; filename="' . $this->displayName .
         ' - ' . time() . '.csv"');
        echo $this->_csv;
        exit();
    }
    public function create ($render, $type, $width, $height, $layers)
    {
        ...
        $file = dirname(__FILE__) . '/../modules/' . $render . '/' . $render .
         '.php'))
    ...
    public static function engine ($params)
    {
        ...
        if (! file_exists(
        dirname(__FILE__) . '/../modules/' . $render . '/' . $render . '.php'))
        ...
        global $cookie;
        $id_employee = (int) ($cookie->id_employee);
        $drawer = 'drawer.php?render=' . $render . '&module=' .
         Tools::safeOutput(Tools::getValue('module')) . '&type=' .
         Tools::safeOutput($params['type']) . '&layers=' .
         Tools::safeOutput($params['layers']) . '&id_employee=' . $id_employee .
         '&id_lang=' . $id_lang;
        if (isset($params['option']))
            $drawer .= '&option=' . $params['option'];
        require_once (dirname(__FILE__) . '/../modules/' . $render . '/' .
         $render . '.php');
        return call_user_func(array($render, 'hookGraphEngine'), $params, 
        $drawer);
    }
    protected static function getEmployee ($employee = null)
    {
        ...
        if (empty($employee->stats_date_from) or empty($employee->stats_date_to) or
         $employee->stats_date_from == '0000-00-00' or
         $employee->stats_date_to == '0000-00-00') {
            if (empty($employee->stats_date_from) or
             $employee->stats_date_from == '0000-00-00')
                $employee->stats_date_from = date('Y') . '-01-01';
            if (empty($employee->stats_date_to) or
             $employee->stats_date_to == '0000-00-00')
    ...
    public static function getDateBetween ($employee = null)
    {
        ...
        return ' \'' . $employee->stats_date_from . ' 00:00:00\' AND \'' .
         $employee->stats_date_to . ' 23:59:59\' ';
    }
