...
class ConfigurationCore extends ObjectModel
{
    ...
    public static function deleteByName ($key)
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'configuration_lang` WHERE `id_configuration` = (SELECT `id_configuration` FROM `' .
         _DB_PREFIX_ . 'configuration` WHERE `name` = \'' . pSQL($key) . '\')') and Db::getInstance()->Execute(
    ...
    public static function getInt ($key)
    {
        ...
        foreach ($languages as $language)
            $resultsArray[$language['id_lang']] = self::get($key, 
            $language['id_lang']);
        return $resultsArray;
    }
    /**
    ...
    {
        if (! is_array($keys) or ! is_array(self::$_CONF) or
         ($id_lang and ! is_array(self::$_CONF_LANG)))
            die(Tools::displayError());
        $resTab = array();
        if (! $id_lang) {
            foreach ($keys as $key)
    ...
    public static function getMultipleInt ($keys)
    {
        ...
        $resultsArray = array();
        foreach ($languages as $language)
            $resultsArray[$language['id_lang']] = self::getMultiple($keys, 
            $language['id_lang']);
        return $resultsArray;
    }
    /**
    ...
    public static function updateValue ($key, $values, $html = false)
    {
        ...
        if (! is_array($values)) {
            ...
            $conf = $db->getRow(
            'SELECT `id_configuration` FROM `' . _DB_PREFIX_ .
             'configuration` WHERE `name` = \'' . pSQL($key) . '\'');
            if (! is_array($conf) or ! array_key_exists('id_configuration', 
            $conf)) {
            ...
            if (! array_key_exists('id_configuration', $conf) or
             ! (int) ($conf['id_configuration']))
                return false;
            foreach ($values as $id_lang => $value) {
                $value = pSQL($value, $html);
                $result &= $db->Execute(
                'INSERT INTO `' . _DB_PREFIX_ . 'configuration_lang` (`id_configuration`, `id_lang`, `value`, `date_upd`)
										VALUES (' . $conf['id_configuration'] . ', ' . (int) ($id_lang) .
                 ', \'' . $value . '\', NOW())
    ...
    public static function loadConfiguration ()
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'configuration_lang` cl ON (c.id_configuration = cl.id_configuration)');
        if (is_array($result))
