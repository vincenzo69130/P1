...
class CountryCore extends ObjectModel
{
    ...
    protected $fieldsRequired = array('id_zone', 'id_currency', 'iso_code', 
    'contains_states', 'need_identification_number', 'display_tax_label');
    protected $fieldsSize = array('iso_code' => 3);
    protected $fieldsValidate = array('id_zone' => 'isUnsignedId', 
    'id_currency' => 'isUnsignedId', 'call_prefix' => 'isInt', 
    'iso_code' => 'isLanguageIsoCode', 'active' => 'isBool', 
    'contains_states' => 'isBool', 'need_identification_number' => 'isBool', 
    'need_zip_code' => 'isBool', 'zip_code_format' => 'isZipCodeFormat', 
    'display_tax_label' => 'isBool');
    protected $fieldsRequiredLang = array('name');
    protected $fieldsSizeLang = array('name' => 64);
    protected $fieldsValidateLang = array('name' => 'isGenericName');
    protected $webserviceParameters = array('objectsNodeName' => 'countries', 
    'fields' => array(
     ...
     */
    public static function getCountries ($id_lang, $active = false, 
    $containStates = NULL)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         ...
         (int) ($id_lang) . ')
		LEFT JOIN `' .
         _DB_PREFIX_ . 'zone` z ON z.`id_zone` = c.`id_zone`
		WHERE 1
		' .
    ...
    {
        global $cookie;
        if (Configuration::get('PS_GEOLOCATION_ENABLED') && $cookie &&
         isset($cookie->iso_code_country) &&
    ...
    public static function getCountriesByZoneId ($id_zone, $id_lang)
    {
        ...
        WHERE (c.`id_zone` = ' . (int) $id_zone . ' OR s.`id_zone` = ' .
         (int) $id_zone . ')
