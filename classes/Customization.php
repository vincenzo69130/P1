...
class CustomizationCore
{
    public static function getReturnedCustomizations ($id_order)
    {
        ...
        '
         ...
         ' AND ord.`id_customization` != 0')) === false)
            return false;
    ...
    {
        if (! $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
        'SELECT `id_customization`, `quantity` FROM `' . _DB_PREFIX_ .
         'customization` WHERE `id_cart` = ' . (int) ($id_cart)))
            return false;
        $customizations = array();
        foreach ($result as $row)
            $customizations[(int) ($row['id_customization'])] = $row;
    ...
    public static function countCustomizationQuantityByProduct ($customizations)
    {
        ...
        foreach ($customizations as $customization)
            $total[(int) ($customization['id_order_detail'])] = ! isset(
            $total[(int) ($customization['id_order_detail'])]) ? (int) ($customization['quantity']) : $total[(int) ($customization['id_order_detail'])] +
