...
class AttributeCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_attribute_group' => 'isUnsignedId', 
    'color' => 'isColor');
    ...
    public function delete ()
    {
        ...
        'SELECT `id_product_attribute` FROM `' . _DB_PREFIX_ .
         'product_attribute_combination` WHERE `' . $this->identifier . '` = ' .
         (int) ($this->id))) === false)
            return false;
        ...
        if (Db::getInstance()->numRows()) {
            ...
            'DELETE FROM `' . _DB_PREFIX_ .
             ...
             implode(', ', $combinationIds) . ')') === false)
                return false;
        }
        $return = parent::delete();
        if ($return)
            Module::hookExec('afterDeleteAttribute', 
            array('id_attribute' => $this->id));
        return $return;
    }
    public function update ($nullValues = false)
    {
        ...
        if ($return)
            Module::hookExec('afterSaveAttribute', 
            array('id_attribute' => $this->id));
        return $return;
    }
    ...
    public function isColorAttribute ()
    {
        ...
        '
			SELECT `is_color_group` FROM `' . _DB_PREFIX_ . 'attribute_group` WHERE `id_attribute_group` = (
				SELECT `id_attribute_group` FROM `' . _DB_PREFIX_ .
         'attribute` WHERE `id_attribute` = ' . (int) ($this->id) . ')
