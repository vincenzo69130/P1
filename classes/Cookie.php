...
class CookieCore
{
    ...
    public function __set ($key, $value)
    {
        ...
        if (! $this->_modified and (! isset($this->_content[$key]) or
         (isset($this->_content[$key]) and $this->_content[$key] != $value)))
            $this->_modified = true;
    ...
    public function isLogged ($withGuest = false)
    {
        ...
        if ($this->logged == 1 and $this->id_customer and
         Validate::isUnsignedId($this->id_customer) and
    ...
    public function isLoggedBack ()
    {
        ...
        return ($this->id_employee and Validate::isUnsignedId(
        $this->id_employee) and
         Employee::checkPassword((int) $this->id_employee, $this->passwd) and (! isset(
        $this->_content['remote_addr']) or
         $this->_content['remote_addr'] == ip2long(Tools::getRemoteAddr()) or
    ...
    function update ($nullValues = false)
    {
        if (isset($_COOKIE[$this->_name])) {
            ...
            /* Get cookie checksum */
            $checksum = crc32(
            $this->_iv . substr($content, 0, strrpos($content, '¤') + 2));
            ...
            if (! isset($this->_content['checksum']) or
             $this->_content['checksum'] != $checksum)
                $this->logout();
