...
class TagCore extends ObjectModel
{
    ...
    public function __construct ($id = NULL, $name = NULL, $id_lang = NULL)
    {
        ...
        elseif ($name and Validate::isGenericName($name) and $id_lang and
         Validate::isUnsignedId($id_lang)) {
            $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
			SELECT *
			FROM `' . _DB_PREFIX_ . 'tag` t
			WHERE `name` LIKE \'' . pSQL($name) . '\' AND `id_lang` = ' .
             (int) ($id_lang));
            if ($row) {
    ...
    {
        if (! Validate::isUnsignedId($id_lang) or ! Validate::isTagsList(
        $string))
            return false;
        $tmpTab = array_unique(
        array_map('trim', preg_split('/,/', $string, NULL, PREG_SPLIT_NO_EMPTY)));
        $list = array();
    ...
    public function getProducts ($associated = true)
    {
		...
		AND p.active = 1
		' .
         ($this->id ? ('AND p.id_product ' . ($associated ? 'IN' : 'NOT IN') .
         ' (SELECT pt.id_product FROM `' . _DB_PREFIX_ .
         'product_tag` pt WHERE pt.id_tag = ' . (int) ($this->id) . ')') : '') . '
		ORDER BY pl.name');
    }
    public function setProducts ($array)
    {
        Db::getInstance()->Execute(
        'DELETE FROM ' . _DB_PREFIX_ . 'product_tag WHERE id_tag = ' .
         (int) $this->id);
        if (is_array($array)) {
            $array = array_map('intval', $array);
            $result1 = Db::getInstance()->Execute(
            'UPDATE ' . _DB_PREFIX_ .
             'product SET indexed = 0 WHERE id_product IN (' .
            ...
            'INSERT INTO ' . _DB_PREFIX_ .
             'product_tag (id_product, id_tag) VALUES ' . implode(',', $ids)) &&
