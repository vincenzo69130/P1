...
class LocalizationPackCore
{
    ...
    $install_mode = false)
    {
        ...
        if (empty($selection)) {
            ...
            if ($res and isset($this->iso_code_lang))
                Configuration::updateValue('PS_LANG_DEFAULT', 
                (int) Language::getIdByIso($this->iso_code_lang));
            if ($install_mode and $res and isset($this->iso_currency)) {
                $res &= Configuration::updateValue('PS_CURRENCY_DEFAULT', 
                (int) Currency::getIdByIsoCode($this->iso_currency));
                Currency::refreshCurrencies();
            }
            return $res;
        }
        foreach ($selection as $selected)
            if (! Validate::isLocalizationPackSelection($selected) or
             ! $this->{'_install' . ucfirst($selected)}($xml))
                return false;
        return true;
    }
        ...
        if (isset($xml->states->state))
            foreach ($xml->states->state as $data) {
                ...
                if (! $id_state = State::getIdByName($attributes['name'])) {
                    ...
                    $state->id_country = Country::getByIso(
                    strval($attributes['country']));
                    $state->id_zone = (int) (Zone::getIdByName(
                    strval($attributes['zone'])));
                    ...
                    if (! $country->contains_states) {
                        ...
                        if (! $country->update())
                            $this->_errors[] = Tools::displayError(
                            'Cannot update the associated country: ') .
                             $country->name;
                    }
                    if (! $state->add()) {
                        $this->_errors[] = Tools::displayError(
                ...
                foreach ($data->county as $xml_county) {
                    ...
                    if (! $id_county = County::getIdCountyByNameAndIdState(
                    $county_attributes['name'], $state->id)) {
                        $county = new County();
                        ...
                        if (! $county->validateFields()) {
                            $this->_errors[] = Tools::displayError(
                            'Invalid County properties');
                            return false;
                        }
                        if (! $county->save()) {
                            $this->_errors[] = Tools::displayError(
                            'An error has occurred while adding the county');
                            return false;
                        }
                    } else {
                        ...
                        if (! Validate::isLoadedObject($county)) {
                            $this->_errors[] = Tools::displayError(
                            'An error occurred while fetching the county.');
                            return false;
                        }
                    }
                    // add zip codes
                    foreach ($xml_county->zipcode as $xml_zipcode) {
                        ...
                        if (! $county->addZipCodes($zipcodes)) {
                            $this->_errors[] = Tools::displayError(
                            'An error has occurred while adding zipcodes');
                            return false;
                        }
                    }
                }
    ...
    protected function _installTaxes ($xml)
    {
        if (isset($xml->taxes->tax)) {
            $available_behavior = array(PS_PRODUCT_TAX, PS_STATE_TAX, 
            PS_BOTH_TAX);
            $assoc_taxes = array();
            foreach ($xml->taxes->tax as $taxData) {
                ...
                if (Tax::getTaxIdByName($attributes['name']))
                    continue;
                $tax = new Tax();
                $tax->name[(int) (Configuration::get('PS_LANG_DEFAULT'))] = strval(
                $attributes['name']);
                ...
                if (! $tax->add()) {
                    $this->_errors[] = Tools::displayError(
                    'An error occurred while importing the tax: ') .
                     strval($attributes['name']);
                    return false;
                }
            ...
            foreach ($xml->taxes->taxRulesGroup as $group) {
                ...
                foreach ($group->taxRule as $rule) {
                    ...
                    // Validation
                    if (! isset(
                    $rule_attributes['iso_code_country']))
                        continue;
                    $id_country = Country::getByIso(
                    strtoupper($rule_attributes['iso_code_country']));
                    if (! $id_country)
                        continue;
                    if (! isset($rule_attributes['id_tax']) || ! array_key_exists(
                    strval($rule_attributes['id_tax']), $assoc_taxes))
                        continue;
                         // Default values
                    $id_state = (int) isset(
                    $rule_attributes['iso_code_state']) ? State::getIdByIso(
                    strtoupper($rule_attributes['iso_code_state'])) : 0;
                    $id_county = 0;
                    $state_behavior = 0;
                    $county_behavior = 0;
                    if ($id_state) {
                        if (isset($rule_attributes['state_behavior']) && in_array(
                        $rule_attributes['state_behavior'], $available_behavior))
                            $state_behavior = (int) $rule_attributes['state_behavior'];
                        if (isset($rule_attributes['county_name'])) {
                            $id_county = County::getIdCountyByNameAndIdState(
                            $rule_attributes['county_name'], (int) $id_state);
                            if (! $id_county)
                                continue;
                        }
                        if (isset($rule_attributes['county_behavior']) && in_array(
                        $rule_attributes['state_behavior'], $available_behavior))
                    ...
                    $tr->id_tax = $assoc_taxes[strval(
                    $rule_attributes['id_tax'])];
                    $tr->save();
                }
            }
        }
    ...
    protected function _installCurrencies ($xml, $install_mode = false)
    {
        if (isset($xml->currencies->currency)) {
            ...
            'http://www.prestashop.com/xml/currencies.xml') and ! $feed = @simplexml_load_file(
            dirname(__FILE__) . '/../localization/currencies.xml')) {
                $this->_errors[] = Tools::displayError(
                'Cannot parse the currencies XML feed.');
                return false;
            }
            foreach ($xml->currencies->currency as $data) {
                ...
                if (! Currency::exists($currency->iso_code)) {
                    if (! $currency->add()) {
                        $this->_errors[] = Tools::displayError(
                        'An error occurred while importing the currency: ') .
                         strval($attributes['name']);
                        return false;
                    }
            ...
            Currency::refreshCurrencies();
            if (! sizeof($this->_errors) and $install_mode and
             isset($attributes['iso_code']) and
             sizeof($xml->currencies->currency) == 1)
    ...
    protected function _installLanguages ($xml, $install_mode = false)
    {
        ...
        if (isset($xml->languages->language))
            foreach ($xml->languages->language as $data) {
                ...
                if ((in_array((string) $attributes['iso_code'], 
                $native_iso_code) and ! $install_mode) or
                ...
                if (@fsockopen('www.prestashop.com', 80, $errno, $errstr, 10)) {
                    ...
                    'http://www.prestashop.com/download/lang_packs/get_language_pack.php?version=' .
                     _PS_VERSION_ . '&iso_lang=' . $attributes['iso_code']))) {
                        if ($content = Tools::file_get_contents(
                        'http://www.prestashop.com/download/lang_packs/gzip/' .
                         $lang_pack->version . '/' . $attributes['iso_code'] .
                         '.gzip')) {
                            $file = _PS_TRANSLATIONS_DIR_ .
                             $attributes['iso_code'] . '.gzip';
                            if (file_put_contents($file, $content)) {
                                $gz = new Archive_Tar($file, true);
                                if (! $gz->extract(
                                _PS_TRANSLATIONS_DIR_ . '../', false)) {
                                    $this->_errors[] = Tools::displayError(
                                    'Cannot decompress the translation file of the language: ') .
                        ...
                        $this->_errors[] = Tools::displayError(
                        'Error occurred when language was checked according to your Prestashop version.');
                } else
                    $this->_errors[] = Tools::displayError(
        ...
        if (! sizeof($this->_errors) and $install_mode and
         isset($attributes['iso_code']) and
         sizeof($xml->languages->language) == 1)
            $this->iso_code_lang = $attributes['iso_code'];
        return true;
    }
    protected function _installUnits ($xml)
    {
        $varNames = array('weight' => 'PS_WEIGHT_UNIT', 
        'volume' => 'PS_VOLUME_UNIT', 'short_distance' => 'PS_DIMENSION_UNIT', 
        ...
        if (isset($xml->units->unit))
            foreach ($xml->units->unit as $data) {
                ...
                if (! isset($varNames[strval($attributes['type'])])) {
                    $this->_errors[] = Tools::displayError(
                    'Pack corrupted: wrong unit type.');
                    return false;
                }
                if (! Configuration::updateValue(
                $varNames[strval($attributes['type'])], 
                strval($attributes['value']))) {
                    $this->_errors[] = Tools::displayError(
                ...
                if (isset($name) && $module = Module::getInstanceByName($name)) {
                    ...
                    if ($install) {
                        if (! Module::isInstalled($name))
                            if (! $module->install())
                                $this->_errors[] = Tools::displayError(
                                'An error has occured during the module installation: ') .
                                 $name;
                    } else 
                        if (Module::isInstalled($name))
                            if (! $module->uninstall())
                                $this->_errors[] = Tools::displayError(
                                'An error has occured during the module uninstall: ') .
                                 $name;
                    unset($module);
                } else
                    $this->_errors[] = Tools::displayError(
                    'An error has occured, this module doesnt exists: ') . $name;
            }
        return true;
    }
    /**
    ...
    protected function installConfiguration ($xml)
    {
        if (isset($xml->configurations))
            foreach ($xml->configurations->configuration as $data) {
                $attributes = $data->attributes();
                $name = (string) $attributes['name'];
                if (isset($name) && isset($attributes['value']) &&
                 Configuration::get($name) !== false)
                    if (! Configuration::updateValue($name, 
                    (string) $attributes['value']))
                        $this->_errors[] = Tools::displayError(
                        'An error has occured during the configuration setup: ' .
                         $name);
            }
        return true;
    }
    protected function updateDefaultGroupDisplayMethod ($xml)
    {
        if (isset($xml->group_default)) {
            ...
            if (isset($attributes['price_display_method']) &&
             in_array((int) $attributes['price_display_method'], array(0, 1))) {
                $group = new Group((int) _PS_DEFAULT_CUSTOMER_GROUP_);
                $group->price_display_method = (int) $attributes['price_display_method'];
                if (! $group->save())
                    $this->_errors[] = Tools::displayError(
                    'An error has occured during the default group update');
            } else
                $this->_errors[] = Tools::displayError(
                'An error has occured during the default group update');
        }
