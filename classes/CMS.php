...
class CMSCore extends ObjectModel
{
    ...
    public function getTranslationsFieldsChild ()
    {
        ...
        $fieldsArray = array('meta_title', 'meta_description', 'meta_keywords', 
        'link_rewrite');
        $fields = array();
        ...
        foreach ($languages as $language) {
            ...
            $fields[$language['id_lang']]['content'] = (isset(
            $this->content[$language['id_lang']])) ? pSQL(
            $this->content[$language['id_lang']], true) : '';
    ...
    public static function getLinks ($id_lang, $selection = NULL, $active = true)
    {
		...
		LEFT JOIN ' . _DB_PREFIX_ .
         'cms_lang cl ON (c.id_cms = cl.id_cms AND cl.id_lang = ' .
         (int) ($id_lang) . ')
		WHERE 1
		' . (($selection !== NULL) ? ' AND c.id_cms IN (' .
         implode(',', array_map('intval', $selection)) . ')' : '') .
         ($active ? ' AND c.`active` = 1 ' : '') . 'ORDER BY c.`position`');
        $link = new Link();
        $links = array();
        if ($result)
    ...
    public static function listCms ($id_lang = NULL, $id_block = false, $active = true)
    {
		...
		' . (($id_block) ? 'JOIN ' . _DB_PREFIX_ .
         'block_cms b ON (c.id_cms = b.id_cms)' : '') . '
    ...
    public static function updateCmsToBlock ($cms, $id_block)
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ . 'block_cms` WHERE `id_block` = ' .
         (int) ($id_block));
        $list = '';
        ...
        if (! empty($list))
            Db::getInstance()->Execute(
            'INSERT INTO ' . _DB_PREFIX_ . 'block_cms (id_block, id_cms) VALUES ' .
             pSQL($list));
        return true;
    }
    public function updatePosition ($way, $position)
    {
        ...
        '
			...
			WHERE `position` 
			' . ($way ? '> ' . (int) ($movedCms['position']) . ' AND `position` <= ' .
         (int) ($position) : '< ' . (int) ($movedCms['position']) .
    ...
    public static function getLastPosition ($id_category)
    {
        ...
        'SELECT MAX(position)+1 FROM `' . _DB_PREFIX_ .
         'cms` WHERE `id_cms_category` = ' . (int) ($id_category)));
    }
    public static function getCMSPages ($id_lang = NULL, $id_cms_category = NULL, 
    $active = true)
    {
		...
		FROM `' . _DB_PREFIX_ . 'cms` c
		JOIN `' . _DB_PREFIX_ .
         'cms_lang` l ON (c.id_cms = l.id_cms)' .
         (isset($id_cms_category) ? 'WHERE `id_cms_category` = ' .
         (int) ($id_cms_category) : '') . ($active ? ' AND c.`active` = 1 ' : '') . '
