...
class GroupReductionCore extends ObjectModel
{
    ...
    'id_category' => 'isUnsignedId', 'reduction' => 'isPrice');
    ...
    public function delete ()
    {
        ...
        while ($row = Db::getInstance()->nextRow($resource)) {
            $query = 'DELETE FROM `' . _DB_PREFIX_ .
             'product_group_reduction_cache` WHERE `id_product` = ' .
             (int) ($row['id_product']);
    ...
    protected function _clearCache ()
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'product_group_reduction_cache` WHERE `id_group` = ' .
         (int) ($this->id_group));
    }
    protected function _setCache ()
    {
        ...
        $query = 'INSERT INTO `' . _DB_PREFIX_ .
         'product_group_reduction_cache` (`id_product`, `id_group`, `reduction`) VALUES ';
        $updated = false;
        while ($row = Db::getInstance()->nextRow($resource)) {
            $query .= '(' . (int) ($row['id_product']) . ', ' .
             (int) ($this->id_group) . ', ' . (float) ($this->reduction) . '), ';
            $updated = true;
        }
        if ($updated)
        ...
        while ($row = Db::getInstance()->nextRow($resource)) {
            $query = 'UPDATE `' . _DB_PREFIX_ . 'product_group_reduction_cache`
             ...
             (float) ($this->reduction) . '
                                  WHERE `id_product` = ' .
             (int) ($row['id_product']) . ' AND `id_group` = ' .
             (int) ($this->id_group);
            if (Db::getInstance()->Execute($query) === false)
                return false;
        }
    ...
    public static function getGroupReductions ($id_group, $id_lang)
    {
        ...
        '
         ...
         'category_lang` cl ON (cl.`id_category` = gr.`id_category` AND cl.`id_lang` = ' .
         (int) ($id_lang) . ')
        ...
        if (! isset(self::$reductionCache[$id_product . '-' . $id_group]))
            ...
            'SELECT `reduction`
             ...
             'product_group_reduction_cache`
                                                                                                       WHERE `id_product` = ' .
             (int) ($id_product) . ' AND `id_group` = ' . (int) ($id_group));
    ...
    public static function doesExist ($id_group, $id_category)
    {
        ...
        'SELECT `id_group`
         ...
         'group_reduction`
                                                                           WHERE `id_group` = ' .
         (int) ($id_group) . ' AND `id_category` = ' . (int) ($id_category));
    }
    ...
    public static function duplicateReduction ($id_product_old, $id_product)
    {
        ...
        $query = 'INSERT INTO `' . _DB_PREFIX_ .
         'product_group_reduction_cache` (`id_product`, `id_group`, `reduction`) VALUES ';
        $query .= '(' . (int) ($id_product) . ', ' . (int) ($row['id_group']) .
         ', ' . (float) ($row['reduction']) . ')';
        return (Db::getInstance()->Execute($query));
    }
    ...
    {
        $query = 'DELETE FROM `' . _DB_PREFIX_ .
         'group_reduction` WHERE `id_category` = ' . (int) ($id_category);
        if (Db::getInstance()->Execute($query) === false)
            return false;
        return true;
    }
