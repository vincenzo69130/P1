...
class SearchCore
{
    public static function sanitize ($string, $id_lang, $indexation = false)
    {
        ...
        $string = preg_replace(
        '/([' . PREG_CLASS_NUMBERS . ']+)[' . PREG_CLASS_PUNCTUATION . ']+(?=[' .
         PREG_CLASS_NUMBERS . '])/u', '\1', $string);
        $string = preg_replace('/[' . PREG_CLASS_SEARCH_EXCLUDE . ']+/u', ' ', 
        $string);
        if ($indexation)
        ...
        if (! empty($blacklist)) {
            $string = preg_replace('/(?<=\s)(' . $blacklist . ')(?=\s)/Su', '', 
            $string);
            $string = preg_replace('/^(' . $blacklist . ')(?=\s)/Su', '', 
            $string);
            $string = preg_replace('/(?<=\s)(' . $blacklist . ')$/Su', '', 
            $string);
        ...
        if ($indexation) {
            ...
            if ($minWordLen > 1) {
                $minWordLen -= 1;
                $string = preg_replace(
                '/(?<=\s)[^\s]{1,' . $minWordLen . '}(?=\s)/Su', ' ', $string);
                $string = preg_replace('/^[^\s]{1,' . $minWordLen . '}(?=\s)/Su', 
                '', $string);
                $string = preg_replace('/(?<=\s)[^\s]{1,' . $minWordLen .
                 '}$/Su', '', $string);
                $string = preg_replace('/^[^\s]{1,' . $minWordLen . '}$/Su', '', 
                $string);
            }
        }
    ...
    public static function find ($id_lang, $expr, $pageNumber = 1, $pageSize = 1, 
    $orderBy = 'position', $orderWay = 'desc', $ajax = false, $useCookie = true)
    {
        global $cookie;
        ...
        foreach ($words as $key => $word)
            if (! empty($word) and
             strlen($word) >= (int) Configuration::get('PS_SEARCH_MINWORDLEN')) {
                $word = str_replace('%', '\\%', $word);
                $word = str_replace('_', '\\_', $word);
                $intersectArray[] = 'SELECT id_product
					FROM ' . _DB_PREFIX_ . 'search_word sw
					LEFT JOIN ' .
                 _DB_PREFIX_ . 'search_index si ON sw.id_word = si.id_word
        ...
        foreach ($intersectArray as $query) {
            ...
            $eligibleProducts = array_intersect($eligibleProducts, 
            $eligibleProducts2);
        ...
        if (empty($productPool))
            return ($ajax ? array() : array('total' => 0, 'result' => array()));
        $productPool = ((strpos($productPool, ',') === false) ? (' = ' .
         (int) $productPool . ' ') : (' IN (' . rtrim($productPool, ',') . ') '));
        if ($ajax) {
            return $db->ExecuteS(
            '
		...
		SELECT p.*, pl.`description_short`, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`name`,
         ...
         (Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get(
        'PS_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY)) > 0 new
		FROM ' . _DB_PREFIX_ .
         'product p
		INNER JOIN `' .
         _DB_PREFIX_ .
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'tax_rule` tr ON (p.`id_tax_rules_group` = tr.`id_tax_rules_group`
		                                           AND tr.`id_country` = ' .
         (int) Country::getDefaultCountryId() . '
	                                           	   AND tr.`id_state` = 0)
	    LEFT JOIN `' .
         _DB_PREFIX_ .
         'tax` tax ON (tax.`id_tax` = tr.`id_tax`)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'manufacturer` m ON m.`id_manufacturer` = p.`id_manufacturer`
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'image` i ON (i.`id_product` = p.`id_product` AND i.`cover` = 1)
		LEFT JOIN `' .
         _DB_PREFIX_ .
        ...
        if (! $result)
            ...
            $resultProperties = Product::getProductsProperties((int) $id_lang, 
            $result);
    ...
    public static function getTags ($db, $id_product, $id_lang)
    {
        ...
        '
		SELECT t.name FROM ' . _DB_PREFIX_ .
         'product_tag pt
		LEFT JOIN ' .
         _DB_PREFIX_ . 'tag t ON (pt.id_tag = t.id_tag AND t.id_lang = ' .
         (int) $id_lang . ')
    ...
    public static function getAttributes ($db, $id_product, $id_lang)
    {
        ...
        '
		SELECT al.name FROM ' . _DB_PREFIX_ .
    ...
    public static function getFeatures ($db, $id_product, $id_lang)
    {
        ...
        '
		SELECT fvl.value FROM ' . _DB_PREFIX_ .
         'feature_product fp
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'feature_value_lang fvl ON (fp.id_feature_value = fvl.id_feature_value AND fvl.id_lang = ' .
         (int) $id_lang . ')
    ...
    protected static function getProductsToIndex ($nbLanguages, $limit = 50)
    {
		...
		LEFT JOIN ' .
         _DB_PREFIX_ .
         'category_lang cl ON (cl.id_category = p.id_category_default AND pl.id_lang = cl.id_lang)
		LEFT JOIN ' .
    ...
    public static function indexation ($full = false, $id_product = false)
    {
        ...
        if ($full) {
			...
			WHERE ' .
             ($id_product ? 'id_product = ' . (int) $id_product : 'indexed = 0'));
            $ids = array();
            if ($products)
            ...
            if (sizeof($ids))
                ...
                'DELETE FROM ' . _DB_PREFIX_ .
                 'search_index WHERE id_product IN (' . implode(',', $ids) . ')');
        }
        ...
        $nbLanguages = Db::getInstance()->getValue(
        'SELECT COUNT(*) FROM ' . _DB_PREFIX_ . 'lang');
        // Products are processed 50 by 50 in order to avoid overloading MySQL
        while ($products = Search::getProductsToIndex($nbLanguages, 50) and
         count($products) > 0) {
            // Now each non-indexed product is processed one by one, langage by langage 
            foreach ($products as $product) {
                $product['tags'] = Search::getTags($db, 
                (int) $product['id_product'], (int) $product['id_lang']);
                $product['attributes'] = Search::getAttributes($db, 
                (int) $product['id_product'], (int) $product['id_lang']);
                $product['features'] = Search::getFeatures($db, 
                (int) $product['id_product'], (int) $product['id_lang']);
                ...
                foreach ($product as $key => $value)
                    if (strncmp($key, 'id_', 3)) {
                        $words = explode(' ', 
                        Search::sanitize($value, (int) $product['id_lang'], 
                        true));
                        foreach ($words as $word)
                            if (! empty($word)) {
                                $word = Tools::substr($word, 0, 
                                PS_SEARCH_MAX_WORD_LENGTH);
                                // Remove accents
                                $word = Tools::replaceAccentedChars(
                                $word);
                ...
                if (sizeof($pArray)) {
                    ...
                    foreach ($pArray as $word => $weight)
                        if ($weight and ! isset($wordIdsByWord['_' . $word])) {
                            $queryArray[$word] = '(' . (int) $product['id_lang'] .
                             ',\'' . pSQL($word) . '\')';
					...
					WHERE word IN (' . implode(',', 
                    $queryArray2) . ')
					AND id_lang = ' . (int) $product['id_lang']);
                    foreach ($existingWords as $data)
                        unset(
                        $queryArray[Tools::replaceAccentedChars($data['word'])]);
                    if (count($queryArray)) {
                        // The words are inserted...
                        $db->Execute(
                        '
						INSERT IGNORE INTO ' .
                         _DB_PREFIX_ . 'search_word (id_lang, word)
                    ...
                    if (count($queryArray2)) {
                        ...
                        foreach ($addedWords as $wordId)
                            $wordIdsByWord[$product['id_lang']]['_' .
                             Tools::replaceAccentedChars($wordId['word'])] = (int) $wordId['id_word'];
                    }
                }
                foreach ($pArray as $word => $weight) {
                    if (! $weight)
                    ...
                    $queryArray3[] = '(' . (int) $product['id_product'] . ',' .
                     (int) $wordIdsByWord[$product['id_lang']]['_' . $word] . ',' .
                     (int) $weight . ')';
    ...
    protected static function setProductsAsIndexed (&$productsArray)
    {
        if ($i = count($productsArray))
            ...
            'UPDATE ' . _DB_PREFIX_ .
             'product SET indexed = 1 WHERE id_product IN (' .
             implode(',', $productsArray) . ') LIMIT ' . (int) $i);
        $productsArray = array();
    }
    ...
    protected static function saveIndex (&$queryArray3)
    {
        if (count($queryArray3))
            ...
            'INSERT INTO ' . _DB_PREFIX_ .
             'search_index (id_product, id_word, weight) VALUES ' .
             implode(',', $queryArray3) .
             ' ON DUPLICATE KEY UPDATE weight = weight + VALUES(weight)');
        $queryArray3 = array();
    }
    public static function searchTag ($id_lang, $tag, $count = false, $pageNumber = 0, 
    $pageSize = 10, $orderBy = false, $orderWay = false, $useCookie = true)
    {
        global $cookie;
        // Only use cookie if id_customer is not present
        if ($useCookie)
            ...
            $id_customer = 0;
        if (! is_numeric($pageNumber) or ! is_numeric($pageSize) or
         ! Validate::isBool($count) or ! Validate::isValidSearch($tag) or
         $orderBy and ! $orderWay or
         ($orderBy and ! Validate::isOrderBy($orderBy)) or
         ($orderWay and ! Validate::isOrderBy($orderWay)))
            return false;
        if ($pageNumber < 1)
            $pageNumber = 1;
        if ($pageSize < 1)
        ...
        if ($count) {
			...
			LEFT JOIN `' .
             _DB_PREFIX_ .
             'product_tag` pt ON (p.`id_product` = pt.`id_product`)
			LEFT JOIN `' .
             _DB_PREFIX_ .
             'tag` t ON (pt.`id_tag` = t.`id_tag` AND t.`id_lang` = ' .
             (int) $id_lang .
             ')
			LEFT JOIN `' .
             _DB_PREFIX_ .
             'category_product` cp ON (cp.`id_product` = p.`id_product`)
			LEFT JOIN `' .
             _DB_PREFIX_ . 'category_group` cg ON (cg.`id_category` = cp.`id_category`)
			WHERE p.`active` = 1
			AND cg.`id_group` ' . (! $id_customer ? '= 1' : 'IN (
				SELECT id_group FROM ' . _DB_PREFIX_ . 'customer_group
				WHERE id_customer = ' . (int) $id_customer . ')') . '
