...
class ProductCore extends ObjectModel
{
    ...
    protected $fieldsSize = array('reference' => 32, 'supplier_reference' => 32, 
    'location' => 64, 'ean13' => 13, 'upc' => 12, 'unity' => 10);
    protected $fieldsValidate = array('id_tax_rules_group' => 'isUnsignedId', 
    'id_manufacturer' => 'isUnsignedId', 'id_supplier' => 'isUnsignedId', 
    ...
    'price' => 'isPrice', 'additional_shipping_cost' => 'isPrice', 
    'wholesale_price' => 'isPrice', 'on_sale' => 'isBool', 
    'online_only' => 'isBool', 'ecotax' => 'isPrice', 'unit_price' => 'isPrice', 
    'unity' => 'isString', 'reference' => 'isReference', 
    'supplier_reference' => 'isReference', 'location' => 'isReference', 
    'width' => 'isUnsignedFloat', 'height' => 'isUnsignedFloat', 
    'depth' => 'isUnsignedFloat', 'weight' => 'isUnsignedFloat', 
    'out_of_stock' => 'isUnsignedInt', 'quantity_discount' => 'isBool', 
    'customizable' => 'isUnsignedInt', 'uploadable_files' => 'isUnsignedInt', 
    'text_fields' => 'isUnsignedInt', 'active' => 'isBool', 
    'available_for_order' => 'isBool', 'condition' => 'isGenericName', 
    'show_price' => 'isBool', 'ean13' => 'isEan13', 'upc' => 'isUpc', 
    'indexed' => 'isBool', 'cache_is_pack' => 'isBool', 
    ...
    protected $fieldsSizeLang = array('meta_description' => 255, 
    'meta_keywords' => 255, 'meta_title' => 128, 'link_rewrite' => 128, 
    'name' => 128, 'available_now' => 255, 'available_later' => 255);
    protected $fieldsValidateLang = array('meta_description' => 'isGenericName', 
    'meta_keywords' => 'isGenericName', 'meta_title' => 'isGenericName', 
    'link_rewrite' => 'isLinkRewrite', 'name' => 'isCatalogName', 
    'description' => 'isString', 'description_short' => 'isString', 
    'available_now' => 'isGenericName', 'available_later' => 'IsGenericName');
    protected $table = 'product';
    protected $identifier = 'id_product';
    protected $webserviceParameters = array('objectNodeNames' => 'products', 
    'fields' => array(
    ...
    'out_of_stock' => array('required' => true), 'new' => array(), 
    'cache_default_attribute' => array(), 
    'id_default_image' => array('getter' => 'getCoverWs', 
    'setter' => 'setCoverWs', 
    'xlink_resource' => array('resourceName' => 'images', 
    'subResourceName' => 'products')), 
    'id_default_combination' => array('getter' => 'getWsDefaultCombination', 
    'setter' => 'setWsDefaultCombination', 
    'xlink_resource' => array('resourceName' => 'combinations')), 
    'position_in_category' => array('getter' => 'getWsPositionInCategory', 
    'setter' => false), 
    ...
    'combinations' => array('resource' => 'combinations', 
    'fields' => array('id' => array('required' => true))), 
    'product_option_values' => array('resource' => 'product_options_values', 
    'fields' => array('id' => array('required' => true))), 
    'product_features' => array('resource' => 'product_feature', 
    'fields' => array('id' => array('required' => true), 
    'id_feature_value' => array('required' => true, 
    ...
    public function __construct ($id_product = NULL, $full = false, $id_lang = NULL)
    {
        ...
        if ($full and $this->id) {
            ...
            $this->manufacturer_name = Manufacturer::getNameById(
            (int) $this->id_manufacturer);
            $this->supplier_name = Supplier::getNameById(
            (int) $this->id_supplier);
            self::$_tax_rules_group[$this->id] = $this->id_tax_rules_group;
            if (is_object($cart) and
             $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')} != NULL)
                $this->tax_rate = Tax::getProductTaxRate($this->id, 
                $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')});
            else
                $this->tax_rate = Tax::getProductTaxRate($this->id, NULL);
            $this->new = $this->isNew();
            $this->price = Product::getPriceStatic((int) ($this->id), false, 
            NULL, 6, NULL, false, true, 1, false, NULL, NULL, NULL, 
            $this->specificPrice);
            $this->unit_price = ($this->unit_price_ratio != 0 ? $this->price /
             $this->unit_price_ratio : 0);
            if ($this->id)
                $this->tags = Tag::getProductTags((int) $this->id);
        }
        if ($this->id_category_default)
            $this->category = Category::getLinkRewrite(
            (int) $this->id_category_default, (int) $id_lang);
    }
    public function getFields ()
    {
        ...
        $fields['unit_price_ratio'] = (float) ($this->unit_price > 0 ? $this->price /
         $this->unit_price : 0);
    ...
    public function getTranslationsFieldsChild ()
    {
        ...
        $fieldsArray = array('meta_description', 'meta_keywords', 'meta_title', 
        ...
        foreach ($languages as $language) {
            ...
            $fields[$language['id_lang']]['description'] = (isset(
            $this->description[$language['id_lang']])) ? pSQL(
            $this->description[$language['id_lang']], true) : '';
            $fields[$language['id_lang']]['description_short'] = (isset(
            $this->description_short[$language['id_lang']])) ? pSQL(
            $this->description_short[$language['id_lang']], true) : '';
            foreach ($fieldsArray as $field) {
                if (! Validate::isTableOrIdentifier($field))
                    die(Tools::displayError());
                    /* Check fields validity */
                if (isset($this->{$field}[$language['id_lang']]) and
                 ! empty($this->{$field}[$language['id_lang']]))
                    $fields[$language['id_lang']][$field] = pSQL(
                    $this->{$field}[$language['id_lang']]);
                elseif (in_array($field, $this->fieldsRequiredLang)) {
                    if ($this->{$field} != '')
                        $fields[$language['id_lang']][$field] = pSQL(
                        $this->{$field}[$defaultLanguage]);
                } else
    ...
    public static function initPricesComputation ($id_customer = NULL)
    {
        ...
        if ($id_customer) {
            ...
            if (! Validate::isLoadedObject($customer))
                die(Tools::displayError());
            self::$_taxCalculationMethod = Group::getPriceDisplayMethod(
    ...
    public function updatePosition ($way, $position)
    {
        ...
        '
			...
			WHERE `position`
			' . ($way ? '> ' . (int) ($movedProduct['position']) .
         ' AND `position` <= ' . (int) ($position) : '< ' .
         (int) ($movedProduct['position']) . ' AND `position` >= ' .
         (int) ($position)) . '
    ...
    $minimumQuantity = 0)
    {
		...
		WHERE `default_on` = 1 ' . ((int) ($minimumQuantity) > 0 ? 'AND `quantity` >= ' .
         (int) ($minimumQuantity) . ' ' : '') . 'AND `id_product` = ' .
         (int) ($id_product);
        ...
        if (! $result)
			...
			FROM `' . _DB_PREFIX_ . 'product_attribute`
			WHERE ' . ((int) ($minimumQuantity) > 0 ? '`quantity` >= ' .
             (int) ($minimumQuantity) . ' AND ' : '') . '`id_product` = ' .
             (int) ($id_product));
        if (! $result)
            $result = Db::getInstance()->getRow(
    ...
    public static function updateDefaultAttribute ($id_product)
    {
        ...
        'UPDATE ' . _DB_PREFIX_ . 'product SET cache_default_attribute = ' .
         (int) ($id_product_attribute) . ' WHERE id_product = ' .
         (int) ($id_product) . ' LIMIT 1');
    }
    public function validateFieldsLang ($die = true, $errorReturn = false)
    {
        ...
        foreach ($this->description_short as $k => $value)
            if (Tools::strlen(strip_tags($value)) > $limit) {
                if ($die)
                    die(
                    Tools::displayError() . ' (' . get_class($this) .
                     '->description_short: length > ' . $limit . ' for language ' .
                     $k . ')');
                return $errorReturn ? get_class($this) . '->' .
                 Tools::displayError('description_short: length >') . ' ' .
                 $limit . ' ' . Tools::displayError('for language') . ' ' . $k : false;
            }
    ...
    public function delete ()
    {
        ...
        if (! parent::delete() or ! $this->deleteCategories(true) or
         ! $this->deleteImages() or ! $this->deleteProductAttributes() or
         ! $this->deleteProductFeatures() or ! $this->deleteTags() or
         ! $this->deleteCartProducts() or ! $this->deleteAttributesImpacts() or
         ! $this->deleteAttachments() or ! $this->deleteCustomization() or
         ! SpecificPrice::deleteByProductId((int) ($this->id)) or
         ! $this->deletePack() or ! $this->deleteProductSale() or
         ! $this->deleteSceneProducts() or ! $this->deleteSearchIndexes() or
         ! $this->deleteAccessories() or ! $this->deleteFromAccessories())
            return false;
        if ($id = ProductDownload::getIdFromIdProduct($this->id))
            if ($productDownload = new ProductDownload($id) and
    ...
    public function addToCategories ($categories = array())
    {
        ...
        foreach ($categories as $newIdCateg)
            if (! in_array($newIdCateg, $currentCategories))
                $productCats[] = '(' . $newIdCateg . ', ' . $this->id . ', ' .
                 $newCategPos[$newIdCateg] . ')';
    ...
    {
        $result = Db::getInstance()->ExecuteS(
        'SELECT `id_category` FROM `' . _DB_PREFIX_ .
         'category_product` WHERE `id_product` = ' . (int) ($this->id) .
    ...
    {
        return (Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'product_tag` WHERE `id_product` = ' .
         (int) ($this->id)) and Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'tag` WHERE `id_tag` NOT IN (SELECT `id_tag` FROM `' . _DB_PREFIX_ .
         'product_tag`)'));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'cart_product` WHERE `id_product` = ' .
         (int) ($this->id));
    }
    /**
     ...
     */
    public static function getProducts ($id_lang, $start, $limit, $orderBy, 
    $orderWay, $id_category = false, $only_active = false)
    {
        if (! Validate::isOrderBy($orderBy) or ! Validate::isOrderWay($orderWay))
            die(Tools::displayError());
        if ($orderBy == 'id_product' or $orderBy == 'price' or
         $orderBy == 'date_add')
            $orderByPrefix = 'p';
        elseif ($orderBy == 'name')
            $orderByPrefix = 'pl';
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'supplier` s ON (s.`id_supplier` = p.`id_supplier`)' . ($id_category ? 'LEFT JOIN `' .
         _DB_PREFIX_ . 'category_product` c ON (c.`id_product` = p.`id_product`)' : '') . '
		WHERE pl.`id_lang` = ' . (int) ($id_lang) .
         ($id_category ? ' AND c.`id_category` = ' . (int) ($id_category) : '') .
         ($only_active ? ' AND p.`active` = 1' : '') . '
		ORDER BY ' . (isset($orderByPrefix) ? pSQL($orderByPrefix) . '.' : '') .
    ...
    public function isNew ()
    {
        ...
        '
			...
			AND DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
        Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get(
        'PS_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY)) > 0
		');
        return sizeof($result) > 0;
    }
    public function productAttributeExists ($attributesList, 
    $currentProductAttribute = false)
    {
        $result = Db::getInstance()->ExecuteS(
        ...
        foreach ($productAttributes as $key => $productAttribute)
            if (sizeof($productAttribute) == sizeof($attributesList)) {
                ...
                for ($i = 0; $diff == false and isset($productAttribute[$i]); $i ++)
                    if (! in_array($productAttribute[$i], $attributesList) or
                     $key == $currentProductAttribute)
                        $diff = true;
                if (! $diff)
                    return true;
            }
     ...
     */
    public function addProductAttribute ($price, $weight, $unit_impact, $ecotax, 
    $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, 
    $location = NULL, $upc = NULL)
    {
        $price = str_replace(',', '.', $price);
        $weight = str_replace(',', '.', $weight);
        Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'product_attribute', 
        array('id_product' => (int) ($this->id), 'price' => (float) ($price), 
        'ecotax' => (float) ($ecotax), 'quantity' => 0, 
        ...
        'location' => pSQL($location), 'ean13' => pSQL($ean13), 
        'upc' => pSQL($upc), 'default_on' => (int) ($default)), 'INSERT');
        ...
        $query = 'INSERT INTO `' . _DB_PREFIX_ .
         'product_attribute_image` (`id_product_attribute`, `id_image`) VALUES ';
        foreach ($id_images as $id_image)
            $query .= '(' . (int) ($id_product_attribute) . ', ' .
             (int) ($id_image) . '), ';
        $query = trim($query, ', ');
    ...
    public function addCombinationEntity ($wholesale_price, $price, $weight, 
    $unit_impact, $ecotax, $quantity, $id_images, $reference, 
    $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL)
    {
        if (! $id_product_attribute = $this->addProductAttribute($price, 
        $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, 
        $supplier_reference, $ean13, $default, $location, $upc) or ! Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ . 'product_attribute` SET `wholesale_price` = ' .
         (float) ($wholesale_price) . ' WHERE `id_product_attribute` = ' .
    ...
    public function addProductAttributeMultiple ($attributes, $setDefault = true)
    {
        ...
        Db::getInstance()->Execute(
        'INSERT INTO `' . _DB_PREFIX_ . 'product_attribute` (' . $keys[0] .
         ') VALUES ' . implode(', ', $values));
        return (array_map(
        create_function('$elem', 'return $elem[\'id_product_attribute\'];'), 
        Db::getInstance()->ExecuteS(
        '
			SELECT `id_product_attribute` FROM `' . _DB_PREFIX_ .
         'product_attribute` WHERE `id_product_attribute` >= ' .
    ...
    $wholesale_price, $price, $weight, $unit, $ecotax, $quantity, $id_images, 
    $reference, $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL, 
    $minimal_quantity)
    {
        Db::getInstance()->Execute(
        '
        ...
        $price = str_replace(',', '.', $price);
        $weight = str_replace(',', '.', $weight);
        $data = array('wholesale_price' => (float) ($wholesale_price), 
        'price' => (float) ($price), 'ecotax' => (float) ($ecotax), 
        'weight' => ($weight ? (float) ($weight) : 0), 
        'unit_price_impact' => ($unit ? (float) ($unit) : 0), 
        'reference' => pSQL($reference), 
        'supplier_reference' => pSQL($supplier_reference), 
        'location' => pSQL($location), 'ean13' => pSQL($ean13), 
        'upc' => pSQL($upc), 'default_on' => (int) ($default), 
        'minimal_quantity' => (int) ($minimal_quantity));
        if ($quantity)
            $data['quantity'] = (int) $quantity;
        if (! Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'product_attribute', 
        $data, 'UPDATE', 
        '`id_product_attribute` = ' . (int) ($id_product_attribute)) or ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'product_attribute_image` WHERE `id_product_attribute` = ' .
         (int) ($id_product_attribute)))
            return false;
        if ($quantity)
    ...
    {
        Module::hookExec('deleteProductAttribute', 
        array('id_product_attribute' => 0, 'id_product' => $this->id, 
        'deleteAllAttributes' => true));
        $result = Db::getInstance()->Execute(
        '
		DELETE FROM `' . _DB_PREFIX_ . 'product_attribute_combination`
		WHERE `id_product_attribute` IN (SELECT `id_product_attribute` FROM `' .
         _DB_PREFIX_ . 'product_attribute` WHERE `id_product` = ' .
         (int) ($this->id) . ')');
        $result2 = Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'product_attribute` WHERE `id_product` = ' . (int) ($this->id));
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'attribute_impact` WHERE `id_product` = ' .
         (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM ' . _DB_PREFIX_ . 'product_attachment WHERE id_product = ' .
         (int) ($this->id));
    }
    /**
    ...
    public function deleteCustomization ()
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id)) && Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT id_customization_field FROM `' .
    ...
    public function deleteQuantityDiscounts ()
    {
        ...
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'discount_quantity` WHERE `id_product` = ' . (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'pack` WHERE `id_product_pack` = ' .
         (int) ($this->id) . ' OR `id_product_item` = ' . (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'product_sale` WHERE `id_product` = ' .
         (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'scene_products` WHERE `id_product` = ' .
         (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'search_index` WHERE `id_product` = ' .
         (int) ($this->id)) && Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'search_word` WHERE `id_word` NOT IN (SELECT id_word FROM `' .
    ...
    public function addAttributeCombinaison ($id_product_attribute, $attributes)
    {
        ...
        foreach ($attributes as $id_attribute)
            $attributesList .= '(' . (int) ($id_product_attribute) . ',' .
             (int) ($id_attribute) . '),';
        ...
        'INSERT INTO `' . _DB_PREFIX_ .
         'product_attribute_combination` (`id_product_attribute`, `id_attribute`) VALUES ' .
         $attributesList);
        return $result;
    }
    public function addAttributeCombinationMultiple ($id_attributes, 
    $combinations)
    {
        $attributesList = '';
    ...
    public function deleteAttributeCombinaison ($id_product_attribute)
    {
        if (! $id_product_attribute or ! is_numeric($id_product_attribute))
            return false;
        Module::hookExec('deleteProductAttribute', 
        array('id_product_attribute' => $id_product_attribute, 
        'id_product' => $this->id, 'deleteAllAttributes' => false));
        $result = Db::getInstance()->Execute(
        '
    ...
    {
        if (! $productAttributes = Db::getInstance()->ExecuteS(
        'SELECT `id_product_attribute` FROM `' . _DB_PREFIX_ .
         'product_attribute` WHERE `id_product` = ' . (int) ($this->id)))
            return false;
        $ids = array();
        ...
        '
			...
			WHERE pai.`id_product_attribute` IN (' . implode(', ', $ids) .
         ') AND il.`id_lang` = ' . (int) ($id_lang) . ' ORDER by i.`position`'))
            return false;
        $images = array();
        foreach ($result as $row)
     ...
     */
    public static function getNewProducts ($id_lang, $pageNumber = 0, 
    $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL)
    {
        if ($pageNumber < 0)
            $pageNumber = 0;
        if ($nbProducts < 1)
		...
		SELECT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, p.`ean13`, p.`upc`,
			i.`id_image`, il.`legend`, t.`rate`, m.`name` AS manufacturer_name, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
        Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get(
        'PS_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY)) > 0 AS new,
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = ' .
         (int) ($id_lang) . ')
		LEFT JOIN `' . _DB_PREFIX_ . 'tax_rule` tr ON (p.`id_tax_rules_group` = tr.`id_tax_rules_group`
		   AND tr.`id_country` = ' . (int) Country::getDefaultCountryId() . '
		   AND tr.`id_state` = 0)
    ...
    {
        global $cookie, $cart;
        $id_group = $cookie->id_customer ? (int) (Customer::getDefaultGroupId(
        (int) ($cookie->id_customer))) : _PS_DEFAULT_CUSTOMER_GROUP_;
        $id_address = $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')};
        $ids = Address::getCountryAndState($id_address);
        $id_country = (int) ($ids['id_country'] ? $ids['id_country'] : Configuration::get(
        'PS_COUNTRY_DEFAULT'));
        return SpecificPrice::getProductIdByDate((int) (Shop::getCurrentShop()), 
        (int) ($cookie->id_currency), $id_country, $id_group, $beginning, 
        $ending);
    }
    /**
     ...
     */
    public static function getRandomSpecial ($id_lang, $beginning = false, 
    $ending = false)
    {
        $currentDate = date('Y-m-d H:i:s');
        $ids_product = self::_getProductIdByDate(
        (! $beginning ? $currentDate : $beginning), 
        (! $ending ? $currentDate : $ending));
        $groups = FrontController::getCurrentCustomerGroups();
        $sqlGroups = (count($groups) ? 'IN (' . implode(',', $groups) . ')' : '= 1');
     ...
     */
    public static function getPricesDrop ($id_lang, $pageNumber = 0, 
    $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL, $beginning = false, 
    $ending = false)
    {
        ...
        if ($count) {
			...
			WHERE p.`active` = 1
			AND p.`show_price` = 1
			' . ((! $beginning and ! $ending) ? ' AND p.`id_product` IN(' . ((is_array(
            $ids_product) and sizeof($ids_product)) ? implode(', ', 
            array_map('intval', $ids_product)) : 0) . ')' : '') . '
			AND p.`id_product` IN (
				SELECT cp.`id_product`
				FROM `' . _DB_PREFIX_ . 'category_group` cg
				LEFT JOIN `' . _DB_PREFIX_ . 'category_product` cp ON (cp.`id_category` = cg.`id_category`)
		...
		DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
        Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get(
        'PS_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY)) > 0 AS new
		FROM `' . _DB_PREFIX_ . 'product` p
		LEFT JOIN `' . _DB_PREFIX_ .
		...
		AND p.`show_price` = 1
		' . ((! $beginning and ! $ending) ? ' AND p.`id_product` IN (' . ((is_array(
        $ids_product) and sizeof($ids_product)) ? implode(', ', $ids_product) : 0) .
         ')' : '') . '
		AND p.`id_product` IN (
			SELECT cp.`id_product`
			FROM `' . _DB_PREFIX_ . 'category_group` cg
     ...
     */
    /*  */
    public static function getReductionValue ($reduction_price, 
    $reduction_percent, $date_from, $date_to, $product_price, $usetax, $taxrate)
    {
        Tools::displayAsDeprecated();
        // Avoid an error with 1970-01-01
        if (! Validate::isDate($date_from) or ! Validate::isDate(
        $date_to))
            return 0;
        $currentDate = date('Y-m-d H:i:s');
        if ($date_from != $date_to and
         ($currentDate > $date_to or $currentDate < $date_from))
            return 0;
             // reduction values
        if (! $usetax)
     ...
     */
    public static function getPriceStatic ($id_product, $usetax = true, 
    $id_product_attribute = NULL, $decimals = 6, $divisor = NULL, $only_reduc = false, 
    $usereduc = true, $quantity = 1, $forceAssociatedTax = false, $id_customer = NULL, 
    $id_cart = NULL, $id_address = NULL, &$specificPriceOutput = NULL, $with_ecotax = true, 
    $use_groupReduction = true)
    {
        ...
        if (! Validate::isBool($usetax) or ! Validate::isUnsignedId($id_product))
            die(Tools::displayError());
             // Initializations
        if (! $id_customer)
            $id_customer = ((Validate::isCookie($cookie) and
             isset($cookie->id_customer) and $cookie->id_customer) ? (int) ($cookie->id_customer) : NULL);
        $id_group = $id_customer ? (int) (Customer::getDefaultGroupId(
        $id_customer)) : _PS_DEFAULT_CUSTOMER_GROUP_;
        if (! is_object($cur_cart) or
         (Validate::isUnsignedInt($id_cart) and $id_cart)) {
            /*
			...
			*/
            if (! $id_cart and ! Validate::isCookie($cookie))
                die(Tools::displayError());
            $cur_cart = $id_cart ? new Cart((int) ($id_cart)) : new Cart(
            (int) ($cookie->id_cart));
        }
        $cart_quantity = 0;
        if ((int) ($id_cart)) {
            ...
            if (Configuration::get('PS_QTY_DISCOUNT_ON_COMBINATION')) {
                $cache_name = (int) ($id_cart) . '_' . (int) ($id_product) . '_' .
                 (int) ($id_product_attribute);
                $condition = ' AND `id_product_attribute` = ' .
                 (int) ($id_product_attribute);
            }
            if (! isset(self::$_cart_quantity[$cache_name]) or
             self::$_cart_quantity[$cache_name] != (int) ($quantity)) {
                self::$_cart_quantity[$cache_name] = Db::getInstance(
        ...
        $id_currency = (int) (Validate::isLoadedObject($cur_cart) ? $cur_cart->id_currency : ((isset(
        $cookie->id_currency) and (int) ($cookie->id_currency)) ? $cookie->id_currency : Configuration::get(
        'PS_CURRENCY_DEFAULT')));
        ...
        if ($id_address) {
            ...
            if ($address_infos['id_country']) {
                ...
                $id_county = (int) County::getIdCountyByZipCode($id_state, 
                $postcode);
            }
        ...
        if (Tax::excludeTaxeOption())
            $usetax = false;
        if ($usetax != false and ! empty($address_infos['vat_number']) and
         $address_infos['id_country'] != Configuration::get('VATNUMBER_COUNTRY') and
         Configuration::get('VATNUMBER_MANAGEMENT'))
            $usetax = false;
        $id_shop = (int) (Shop::getCurrentShop());
        return Product::priceCalculation($id_shop, $id_product, 
        $id_product_attribute, $id_country, $id_state, $id_county, $id_currency, 
        $id_group, $quantity, $usetax, $decimals, $only_reduc, $usereduc, 
        $with_ecotax, $specificPriceOutput, $use_groupReduction);
    }
    /**
     ...
     * @return float Product price
     **/
    public static function priceCalculation ($id_shop, $id_product, 
    $id_product_attribute, $id_country, $id_state, $id_county, $id_currency, 
    $id_group, $quantity, $use_tax, $decimals, $only_reduc, $use_reduc, 
    $with_ecotax, &$specific_price, $use_groupReduction)
    {
        ...
        $cacheId = $id_product . '-' . $id_shop . '-' . $id_currency . '-' .
         $id_country . '-' . $id_state . '-' . $id_county . '-' . $id_group . '-' .
         $quantity . '-' . $product_attribute_label . '-' .
         ($use_tax ? '1' : '0') . '-' . $decimals . '-' .
         ($only_reduc ? '1' : '0') . '-' . ($use_reduc ? '1' : '0') . '-' .
         $with_ecotax;
        // reference parameter is filled before any returns
        $specific_price = SpecificPrice::getSpecificPrice(
        (int) ($id_product), $id_shop, $id_currency, $id_country, $id_group, 
        $quantity);
        ...
        if (! isset(self::$_pricesLevel2[$cacheId2]))
            self::$_pricesLevel2[$cacheId2] = Db::getInstance(
            _PS_USE_SQL_SLAVE_)->getRow(
            '
			SELECT p.`price`,
			' .
             ($id_product_attribute ? 'pa.`price`' : 'IFNULL((SELECT pa.price FROM `' .
             _DB_PREFIX_ . 'product_attribute` pa WHERE id_product = ' .
             (int) ($id_product) . ' AND default_on = 1), 0)') . ' AS attribute_price,
			p.`ecotax`
			' . ($id_product_attribute ? ', pa.`ecotax` AS attribute_ecotax' : '') . '
			FROM `' . _DB_PREFIX_ . 'product` p
			' . ($id_product_attribute ? 'LEFT JOIN `' . _DB_PREFIX_ .
             'product_attribute` pa ON pa.`id_product_attribute` = ' .
             (int) ($id_product_attribute) : '') . '
			WHERE p.`id_product` = ' . (int) ($id_product));
        ...
        if (! $specific_price or
         ! ($specific_price['price'] > 0 and $specific_price['id_currency']))
            $price = Tools::convertPrice($price, $id_currency);
             // Attribute price
        $attribute_price = Tools::convertPrice(
        array_key_exists('attribute_price', $result) ? (float) ($result['attribute_price']) : 0, 
        $id_currency);
        if ($id_product_attribute !== false) // If you want the default combination, please use NULL value instead
            $price += $attribute_price;
             // TaxRate calculation
        $tax_rate = Tax::getProductTaxRateViaRules(
        (int) $id_product, (int) $id_country, (int) $id_state, (int) $id_county);
        if ($tax_rate === false)
            $tax_rate = 0;
             // Add Tax
        if ($use_tax)
        ...
        if (($only_reduc or $use_reduc) and $specific_price) {
            if ($specific_price['reduction_type'] == 'amount') {
                ...
                if (! $specific_price['id_currency'])
                    $reduction_amount = Tools::convertPrice($reduction_amount, 
                    $id_currency);
                $reduc = Tools::ps_round(
                ! $use_tax ? $reduction_amount / (1 + $tax_rate / 100) : $reduction_amount, 
                $decimals);
            } else
                $reduc = Tools::ps_round($price * $specific_price['reduction'], 
                $decimals);
        }
        if ($only_reduc)
            return $reduc;
        ...
        if ($use_groupReduction) {
            if ($reductionFromCategory = (float) (GroupReduction::getValueForProduct(
            $id_product, $id_group)))
                $price -= $price * $reductionFromCategory;
            else // apply group reduction if there is no group reduction for this category
                $price *= ((100 -
                 Group::getReductionByIdGroup($id_group)) / 100);
        }
        $price = Tools::ps_round($price, $decimals);
        // Eco Tax
        if (($result['ecotax'] or isset($result['attribute_ecotax'])) and
         $with_ecotax) {
            $ecotax = $result['ecotax'];
            if (isset($result['attribute_ecotax']) &&
             $result['attribute_ecotax'] > 0)
                $ecotax = $result['attribute_ecotax'];
            if ($id_currency)
                $ecotax = Tools::convertPrice($ecotax, $id_currency);
            if ($use_tax) {
                $taxRate = TaxRulesGroup::getTaxesRate(
                (int) Configuration::get('PS_ECOTAX_TAX_RULES_GROUP_ID'), 
                (int) $id_country, (int) $id_state, (int) $id_county);
    ...
    public static function convertAndFormatPrice ($price, $currency = false)
    {
        ...
        return Tools::displayPrice(Tools::convertPrice($price, $currency), 
        $currency);
    }
    public static function isDiscounted ($id_product, $quantity = 1)
    {
        ...
        '
			SELECT SUM(`quantity`)
			FROM `' . _DB_PREFIX_ . 'cart_product`
			WHERE `id_product` = ' . (int) ($id_product) . ' AND `id_cart` = ' .
         (int) ($cart->id));
        $quantity = $cart_quantity ? $cart_quantity : $quantity;
        $id_currency = (int) (Validate::isLoadedObject($cart) ? $cart->id_currency : ((isset(
        $cookie->id_currency) and (int) ($cookie->id_currency)) ? $cookie->id_currency : Configuration::get(
        'PS_CURRENCY_DEFAULT')));
        $ids = Address::getCountryAndState(
        (int) ($cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')}));
        $id_country = (int) ($ids['id_country'] ? $ids['id_country'] : Configuration::get(
        'PS_COUNTRY_DEFAULT'));
        $id_shop = (int) (Shop::getCurrentShop());
        return (bool) SpecificPrice::getSpecificPrice((int) $id_product, 
        $id_shop, $id_currency, $id_country, $id_group, $quantity);
    }
    /**
     * Get product price
    ...
    $divisor = NULL, $only_reduc = false, $usereduc = true, $quantity = 1)
    {
        ...
        $id_product_attribute, $decimals, $divisor, $only_reduc, $usereduc, 
        $quantity);
    }
    /**
     * @deprecated
    ...
    public function getIdProductAttributeMostExpensive ()
    {
		...
		WHERE `id_product` = ' . (int) ($this->id) . '
		ORDER BY `price` DESC');
        return (isset($row['id_product_attribute']) and
         $row['id_product_attribute']) ? (int) $row['id_product_attribute'] : 0;
    }
    public function getPriceWithoutReduct ($notax = false, 
    $id_product_attribute = false)
    {
        return Product::getPriceStatic((int) ($this->id), ! $notax, 
        $id_product_attribute, 6, NULL, false, false);
    }
    /**
    ...
    static function productPrice ($params, &$smarty)
    {
        ...
        if (isset($params['p']['reduction']) and $params['p']['reduction'])
            $ret .= '<span class="discounted">' . Tools::displayPrice(
            $params['p']['price_without_reduction'], $smarty->ps_currency) .
             '</span><br />';
     ...
     */
    public static function getQuantity ($id_product, 
    $id_product_attribute = NULL, $cache_is_pack = NULL)
    {
        $lang = Configuration::get('PS_LANG_DEFAULT');
        if (((int) $cache_is_pack or ($cache_is_pack === NULL and Pack::isPack(
        (int) $id_product, (int) $lang))) and
		...
		WHERE p.`id_product` = ' . (int) ($id_product) . '
		' . (isset($id_product_attribute) ? 'AND `id_product_attribute` = ' .
         (int) ($id_product_attribute) : '') . '
    ...
    public static function updateQuantity ($product, $id_order = NULL)
    {
        ...
        if (Pack::isPack((int) ($product['id_product']))) {
            $products_pack = Pack::getItems((int) ($product['id_product']), 
            (int) (Configuration::get('PS_LANG_DEFAULT')));
            foreach ($products_pack as $product_pack) {
                $tab_product_pack['id_product'] = (int) ($product_pack->id);
                $tab_product_pack['id_product_attribute'] = self::getDefaultAttribute(
                $tab_product_pack['id_product'], 1);
                $tab_product_pack['cart_quantity'] = (int) ($product_pack->pack_quantity *
                 $product['cart_quantity']);
                self::updateQuantity($tab_product_pack);
            }
        }
        $productObj = new Product((int) $product['id_product'], false, 
        (int) Configuration::get('PS_LANG_DEFAULT'));
        return $productObj->addStockMvt(- (int) $product['cart_quantity'], 
        (int) _STOCK_MOVEMENT_ORDER_REASON_, 
        (int) $product['id_product_attribute'], (int) $id_order, NULL);
    }
    public static function reinjectQuantities (&$orderDetail, $quantity)
    {
        ...
        if (Pack::isPack((int) ($orderDetail->product_id))) {
            $products_pack = Pack::getItems((int) ($orderDetail->product_id), 
            (int) (Configuration::get('PS_LANG_DEFAULT')));
            foreach ($products_pack as $product_pack)
                if (! $product_pack->addStockMvt(
                (int) ($product_pack->pack_quantity * $quantity), 
                _STOCK_MOVEMENT_ORDER_REASON_, 
                (int) $product_pack->id_product_attribute, 
                (int) $orderDetail->id_order, (int) $cookie->id_employee))
                    return false;
        }
        $product = new Product((int) $orderDetail->product_id);
        if (! $product->addStockMvt((int) $quantity, 
        _STOCK_MOVEMENT_ORDER_REASON_, (int) $orderDetail->product_attribute_id, 
        (int) $orderDetail->id_order, (int) $cookie->id_employee))
            return false;
    ...
    {
        return ! Configuration::get('PS_STOCK_MANAGEMENT') ? true : ((int) ($oos) ==
         2 ? (int) (Configuration::get('PS_ORDER_OUT_OF_STOCK')) : (int) ($oos));
    }
    /**
    ...
    {
        if (Pack::isPack((int) ($this->id)) and
         ! Pack::isInStock((int) ($this->id)))
            return false;
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'accessory` WHERE `id_product_1` = ' .
         (int) ($this->id));
    }
    /**
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'accessory` WHERE `id_product_2` = ' .
         (int) ($this->id));
    }
    /**
    ...
    public function getAccessories ($id_lang, $active = true)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         ...
         (int) ($id_lang) . ')
		LEFT JOIN `' . _DB_PREFIX_ .
         'category_lang` cl ON (p.`id_category_default` = cl.`id_category` AND cl.`id_lang` = ' .
         (int) ($id_lang) . ')
		LEFT JOIN `' . _DB_PREFIX_ . 'image` i ON (i.`id_product` = p.`id_product` AND i.`cover` = 1)
		LEFT JOIN `' . _DB_PREFIX_ .
         'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = ' .
    ...
    {
        return Db::getInstance()->getRow(
        'SELECT `id_product`, `name` FROM `' . _DB_PREFIX_ .
         'product_lang` WHERE `id_product` = ' . (int) ($accessoryId));
    }
    /**
    ...
    public function changeAccessories ($accessories_id)
    {
        foreach ($accessories_id as $id_product_2)
            Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'accessory', 
            array('id_product_1' => (int) ($this->id), 
            'id_product_2' => (int) ($id_product_2)), 'INSERT');
    }
    /**
    ...
    {
        $row = array('id_feature_value' => (int) ($id_value), 
        'id_lang' => (int) ($lang), 'value' => pSQL($cust));
        return Db::getInstance()->autoExecute(
        _DB_PREFIX_ . 'feature_value_lang', $row, 'INSERT');
    }
    public function addFeaturesToDB ($id_feature, $id_value, $cust = 0)
    {
        if ($cust) {
            $row = array('id_feature' => (int) ($id_feature), 'custom' => 1);
            Db::getInstance()->autoExecute(_DB_PREFIX_ . 'feature_value', $row, 
            'INSERT');
            $id_value = Db::getInstance()->Insert_ID();
        }
        $row = array('id_feature' => (int) ($id_feature), 
        'id_product' => (int) ($this->id), 
        'id_feature_value' => (int) ($id_value));
        Db::getInstance()->autoExecute(_DB_PREFIX_ . 'feature_product', $row, 
        'INSERT');
        if ($id_value)
            return ($id_value);
    }
    public static function addFeatureProductImport ($id_product, $id_feature, 
    $id_feature_value)
    {
        return Db::getInstance()->Execute(
        '
			INSERT INTO `' . _DB_PREFIX_ . 'feature_product` (`id_feature`, `id_product`, `id_feature_value`)
			VALUES (' . (int) ($id_feature) . ', ' . (int) ($id_product) . ', ' .
    ...
    public static function getFeaturesStatic ($id_product)
    {
        if (! array_key_exists($id_product, self::$_cacheFeatures))
            self::$_cacheFeatures[$id_product] = Db::getInstance(
    ...
    public static function cacheProductsFeatures ($productIds)
    {
        ...
        foreach ($productIds as $id_product)
            if ((int) $id_product and
             ! array_key_exists($id_product, self::$_cacheFeatures))
    ...
    public static function cacheFrontFeatures ($productIds, $id_lang)
    {
        $productImplode = array();
        foreach ($productIds as $id_product)
            if ((int) $id_product and ! array_key_exists(
            $id_product . '-' . $id_lang, self::$_cacheFeatures))
                $productImplode[] = (int) $id_product;
        if (! count($productImplode))
            return;
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
		...
		LEFT JOIN ' . _DB_PREFIX_ .
         'feature_lang fl ON (fl.id_feature = pf.id_feature AND fl.id_lang = ' .
         (int) $id_lang . ')
		LEFT JOIN ' . _DB_PREFIX_ .
         'feature_value_lang fvl ON (fvl.id_feature_value = pf.id_feature_value AND fvl.id_lang = ' .
         (int) $id_lang . ')
		WHERE `id_product` IN (' . implode($productImplode, ',') . ')');
        foreach ($result as $row) {
            if (! array_key_exists($row['id_product'] . '-' . $id_lang, 
            self::$_frontFeaturesCache))
    ...
    public static function searchByName ($id_lang, $query)
    {
		...
		WHERE pl.`name` LIKE \'%' . pSQL($query) . '%\' OR p.`reference` LIKE \'%' .
         pSQL($query) . '%\' OR p.`supplier_reference` LIKE \'%' . pSQL($query) .
        ...
        $resultsArray = array();
        foreach ($result as $row) {
            $row['price'] = Product::getPriceStatic($row['id_product'], true, 
            NULL, 2);
    ...
    public static function duplicateAttributes ($id_product_old, $id_product_new)
    {
        ...
        foreach ($result as $row) {
            ...
            $return &= Db::getInstance()->AutoExecute(
            _DB_PREFIX_ . 'product_attribute', $row, 'INSERT');
            $id_product_attribute_new = (int) (Db::getInstance()->Insert_ID());
            if ($resultImages = self::_getAttributeImageAssociations(
            $id_product_attribute_old)) {
            ...
            foreach ($result2 as $row2) {
                ...
                $return &= Db::getInstance()->AutoExecute(
                _DB_PREFIX_ . 'product_attribute_combination', $row2, 'INSERT');
            }
        }
    ...
    public static function duplicateAccessories ($id_product_old, 
    $id_product_new)
    {
        $return = true;
        $result = Db::getInstance()->ExecuteS(
		...
		WHERE `id_product_1` = ' .
         (int) ($id_product_old));
        foreach ($result as $row) {
            $data = array('id_product_1' => (int) ($id_product_new), 
            'id_product_2' => (int) ($row['id_product_2']));
            $return &= Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'accessory', 
            $data, 'INSERT');
        }
        return $return;
    }
    public static function duplicateTags ($id_product_old, $id_product_new)
    {
        ...
        'SELECT `id_tag` FROM `' . _DB_PREFIX_ .
         'product_tag` WHERE `id_product` = ' . (int) ($id_product_old));
        if (! Db::getInstance()->NumRows())
            return true;
        $query = 'INSERT INTO `' . _DB_PREFIX_ .
         'product_tag` (`id_product`, `id_tag`) VALUES';
        foreach ($tags as $tag)
            $query .= ' (' . (int) ($id_product_new) . ', ' .
             (int) ($tag['id_tag']) . '),';
        $query = rtrim($query, ',');
        return Db::getInstance()->Execute($query);
    }
    public static function duplicateDownload ($id_product_old, $id_product_new)
    {
        ...
        'SELECT `display_filename`, `physically_filename`, `date_deposit`, `date_expiration`,
			`nb_days_accessible`, `nb_downloadable`, `active`
			FROM `' . _DB_PREFIX_ . 'product_download` WHERE `id_product` = ' .
         (int) ($id_product_old), false);
        if (! Db::getInstance()->NumRows())
            return true;
        $query = 'INSERT INTO `' . _DB_PREFIX_ .
         'product_download` (`id_product`, `display_filename`, `physically_filename`, `date_deposit`, `date_expiration`, `nb_days_accessible`, `nb_downloadable`, `active`) VALUES';
        while ($row = Db::getInstance()->nextRow($resource))
            $query .= ' (' . (int) ($id_product_new) . ', \'' .
             pSQL($row['display_filename']) . '\', \'' .
             pSQL($row['physically_filename']) . '\', \'' .
             pSQL($row['date_deposit']) . '\', \'' .
             pSQL($row['date_expiration']) . '\', ' .
             (int) ($row['nb_days_accessible']) . ', ' .
             (int) ($row['nb_downloadable']) . ', ' . (int) ($row['active']) .
             '),';
        $query = rtrim($query, ',');
        return Db::getInstance()->Execute($query);
    }
    ...
    public static function duplicateFeatures ($id_product_old, $id_product_new)
    {
        ...
        foreach ($result as $row) {
            ...
            if ($result2['custom']) {
                ...
                $return &= Db::getInstance()->AutoExecute(
                _DB_PREFIX_ . 'feature_value', $result2, 'INSERT');
                ...
                foreach ($languages as $language) {
                    ...
                    $return &= Db::getInstance()->AutoExecute(
                    _DB_PREFIX_ . 'feature_value_lang', $result3, 'INSERT');
                }
                $row['id_feature_value'] = $new_id_feature_value;
            }
            $row['id_product'] = $id_product_new;
            $return &= Db::getInstance()->AutoExecute(
            _DB_PREFIX_ . 'feature_product', $row, 'INSERT');
        }
        return $return;
    }
    ...
    public static function duplicateCustomizationFields ($oldProductId, 
    $productId)
    {
        if (($customizations = self::_getCustomizationFieldsNLabels(
        $oldProductId)) === false)
            return false;
        ...
        foreach ($customizations['fields'] as $customizationField) {
            ...
            _DB_PREFIX_ . 'customization_field', $customizationField, 'INSERT') or
             ! $customizationFieldId = Db::getInstance()->Insert_ID())
                return false;
            if (isset($customizations['labels'])) {
                $query = 'INSERT INTO `' . _DB_PREFIX_ .
                 'customization_field_lang` (`id_customization_field`, `id_lang`, `name`) VALUES ';
                foreach ($customizations['labels'][$oldCustomizationFieldId] as $customizationLabel)
                    $query .= '(' . (int) ($customizationFieldId) . ', ' .
                     (int) ($customizationLabel['id_lang']) . ', \'' .
                     pSQL($customizationLabel['name']) . '\'), ';
                $query = rtrim($query, ', ');
                if (! Db::getInstance()->Execute($query))
                    return false;
            }
    ...
    public static function getProductProperties ($id_lang, $row)
    {
        ...
        $row['allow_oosp'] = Product::isAvailableWhenOutOfStock(
        $row['out_of_stock']);
        if ((! isset($row['id_product_attribute']) or
         ! $row['id_product_attribute']) and ((isset(
        $row['cache_default_attribute']) and
         ($ipa_default = $row['cache_default_attribute']) !== NULL) or ($ipa_default = Product::getDefaultAttribute(
        $row['id_product'], ! $row['allow_oosp']))))
            $row['id_product_attribute'] = $ipa_default;
        if (! isset($row['id_product_attribute']))
            $row['id_product_attribute'] = 0;
             // Tax
        $usetax = Tax::excludeTaxeOption();
        $cacheKey = $row['id_product'] . '-' . $row['id_product_attribute'] . '-' .
         $id_lang . '-' . (int) ($usetax);
        if (array_key_exists($cacheKey, self::$producPropertiesCache))
            return self::$producPropertiesCache[$cacheKey];
             // Datas
        $link = new Link();
        $row['category'] = Category::getLinkRewrite(
        (int) $row['id_category_default'], (int) ($id_lang));
        $row['link'] = $link->getProductLink((int) $row['id_product'], 
        $row['link_rewrite'], $row['category'], $row['ean13']);
        $row['attribute_price'] = (isset($row['id_product_attribute']) and
         $row['id_product_attribute']) ? (float) (Product::getProductAttributePrice(
        $row['id_product_attribute'])) : 0;
        $row['price_tax_exc'] = Product::getPriceStatic(
        (int) $row['id_product'], false, 
        ((isset($row['id_product_attribute']) and
         ! empty($row['id_product_attribute'])) ? (int) ($row['id_product_attribute']) : NULL), 
        (self::$_taxCalculationMethod == PS_TAX_EXC ? 2 : 6));
        if (self::$_taxCalculationMethod == PS_TAX_EXC) {
            $row['price_tax_exc'] = Tools::ps_round($row['price_tax_exc'], 2);
            $row['price'] = Product::getPriceStatic((int) $row['id_product'], 
            true, 
            ((isset($row['id_product_attribute']) and
             ! empty($row['id_product_attribute'])) ? (int) ($row['id_product_attribute']) : NULL), 
            6);
            ...
            Product::getPriceStatic((int) $row['id_product'], true, 
            ((isset($row['id_product_attribute']) and
             ! empty($row['id_product_attribute'])) ? (int) ($row['id_product_attribute']) : NULL), 
            2), 2);
            $row['price_without_reduction'] = Product::getPriceStatic(
            (int) $row['id_product'], true, 
            ((isset($row['id_product_attribute']) and
             ! empty($row['id_product_attribute'])) ? (int) ($row['id_product_attribute']) : NULL), 
            6, NULL, false, false);
        }
        $row['reduction'] = Product::getPriceStatic((int) ($row['id_product']), 
        (bool) $usetax, (int) ($row['id_product_attribute']), 6, NULL, true, 
        true, 1, true, NULL, NULL, NULL, $specific_prices);
        ...
        $row['features'] = Product::getFrontFeaturesStatic((int) $id_lang, 
        $row['id_product']);
        $row['attachments'] = ((! isset($row['cache_has_attachments']) or
         $row['cache_has_attachments']) ? Product::getAttachmentsStatic(
        (int) ($id_lang), $row['id_product']) : array());
        // Pack management
        $row['pack'] = (! isset($row['cache_is_pack']) ? Pack::isPack(
        $row['id_product']) : (int) $row['cache_is_pack']);
        $row['packItems'] = $row['pack'] ? Pack::getItemTable(
        $row['id_product'], $id_lang) : array();
        $row['nopackprice'] = $row['pack'] ? Pack::noPackPrice(
        $row['id_product']) : 0;
	...
	*/
    ...
    {
        if (! array_key_exists($id_product . '-' . $id_lang, 
        self::$_frontFeaturesCache)) {
            self::$_frontFeaturesCache[$id_product . '-' . $id_lang] = Db::getInstance(
            _PS_USE_SQL_SLAVE_)->ExecuteS(
            '
    ...
    public static function getAttachmentsStatic ($id_lang, $id_product)
    {
		...
		LEFT JOIN ' . _DB_PREFIX_ .
         'attachment_lang al ON (a.id_attachment = al.id_attachment AND al.id_lang = ' .
         (int) ($id_lang) . ')
	...
	*/
    public static function getAllCustomizedDatas ($id_cart, $id_lang = null)
    {
        ...
        '
			...
			LEFT JOIN `' . _DB_PREFIX_ .
         'customization_field_lang` cfl ON (cfl.id_customization_field = cd.`index` AND id_lang = ' .
         (int) ($id_lang) . ')
        ...
        if (! $result = Db::getInstance()->ExecuteS(
        'SELECT `id_product`, `id_product_attribute`, `id_customization`, `quantity`, `quantity_refunded`, `quantity_returned` FROM `' .
         _DB_PREFIX_ . 'customization` WHERE `id_cart` = ' . (int) ($id_cart)))
            return false;
        foreach ($result as $row) {
    ...
    public function deleteCustomizedDatas ($id_customization)
    {
        ...
        'SELECT `value` FROM `' . _DB_PREFIX_ .
         'customized_data` WHERE `id_customization` = ' .
         (int) ($id_customization) . ' AND `type` = ' . _CUSTOMIZE_FILE_)) ===
         false)
            return false;
        foreach ($result as $row)
            if (! @unlink(_PS_UPLOAD_DIR_ . $row['value']) or
             ! @unlink(_PS_UPLOAD_DIR_ . $row['value'] . '_small'))
                return false;
        return (Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization` WHERE `id_customization` = ' . (int) ($id_customization)) and Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customized_data` WHERE `id_customization` = ' .
    ...
    public static function addCustomizationPrice (&$products, &$customizedDatas)
    {
        foreach ($products as &$productUpdate) {
            ...
            $productAttributeId = (int) (isset(
            $productUpdate['id_product_attribute']) ? $productUpdate['id_product_attribute'] : $productUpdate['product_attribute_id']);
            ...
            $priceWt = $price * (1 + ((isset($productUpdate['tax_rate']) ? $productUpdate['tax_rate'] : $productUpdate['rate']) *
             0.01));
            ...
            if ($customizationQuantity) {
                $productUpdate['total_wt'] = $priceWt *
                 ($productQuantity - $customizationQuantity);
                $productUpdate['total_customization_wt'] = $priceWt *
                 $customizationQuantity;
                $productUpdate['total'] = $price *
                 ($productQuantity - $customizationQuantity);
                $productUpdate['total_customization'] = $price *
                 $customizationQuantity;
            }
        }
    }
	...
	*/
    ...
    {
        $max = array(
        _CUSTOMIZE_FILE_ => (int) (Tools::getValue('uploadable_files')), 
        _CUSTOMIZE_TEXTFIELD_ => (int) (Tools::getValue('text_fields')));
        /* Get customization field ids */
        if (($result = Db::getInstance()->ExecuteS(
        'SELECT `id_customization_field`, `type` FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id) .
         ' ORDER BY `id_customization_field`')) === false)
            return false;
        if (empty($result))
            return true;
        $customizationFields = array(_CUSTOMIZE_FILE_ => array(), 
        _CUSTOMIZE_TEXTFIELD_ => array());
        foreach ($result as $row)
            $customizationFields[(int) ($row['type'])][] = (int) ($row['id_customization_field']);
        $extraFile = count($customizationFields[_CUSTOMIZE_FILE_]) -
         $max[_CUSTOMIZE_FILE_];
        $extraText = count($customizationFields[_CUSTOMIZE_TEXTFIELD_]) -
         $max[_CUSTOMIZE_TEXTFIELD_];
        /* If too much inside the database, deletion */
        if ($extraFile > 0 and
         count($customizationFields[_CUSTOMIZE_FILE_]) - $extraFile >= 0 and (! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id) .
         ' AND `type` = ' . _CUSTOMIZE_FILE_ .
         ' AND `id_customization_field` >= ' . (int) ($customizationFields[_CUSTOMIZE_FILE_][count(
        $customizationFields[_CUSTOMIZE_FILE_]) - $extraFile])) or ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT `id_customization_field` FROM `' .
         _DB_PREFIX_ . 'customization_field`)')))
            return false;
        if ($extraText > 0 and
         count($customizationFields[_CUSTOMIZE_TEXTFIELD_]) - $extraText >= 0 and (! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field` WHERE `id_product` = ' . (int) ($this->id) .
         ' AND `type` = ' . _CUSTOMIZE_TEXTFIELD_ .
         ' AND `id_customization_field` >= ' . (int) ($customizationFields[_CUSTOMIZE_TEXTFIELD_][count(
        $customizationFields[_CUSTOMIZE_TEXTFIELD_]) - $extraText])) or ! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT `id_customization_field` FROM `' .
         _DB_PREFIX_ . 'customization_field`)')))
            return false;
    ...
    protected function _createLabel (&$languages, $type)
    {
        ...
        if (! Db::getInstance()->Execute(
        'INSERT INTO `' . _DB_PREFIX_ .
         'customization_field` (`id_product`, `type`, `required`) VALUES (' .
         (int) ($this->id) . ', ' . (int) ($type) . ', 0)') or
        ...
        foreach ($_POST as $field => $value)
			...
			if (strncmp($field, 'label_', 6) == 0) {
                ...
                '
					...
					(`id_customization_field`, `id_lang`, `name`) VALUES (' .
                 (int) ($tmp[2]) . ', ' . (int) ($tmp[3]) . ', \'' . pSQL(
                $value) . '\')
					ON DUPLICATE KEY UPDATE `name` = \'' . pSQL($value) .
                 '\''))
                    return false;
                $isRequired = isset(
                $_POST['require_' . (int) ($tmp[1]) . '_' . (int) ($tmp[2])]) ? 1 : 0;
                $hasRequiredFields |= $isRequired;
                /* Require option update */
                if (! Db::getInstance()->Execute(
                'UPDATE `' . _DB_PREFIX_ .
                 'customization_field` SET `required` = ' . (int) ($isRequired) .
                 ' WHERE `id_customization_field` = ' . (int) ($tmp[2])))
                    return false;
            }
        if ($hasRequiredFields and ! Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ .
         'product` SET `customizable` = 2 WHERE `id_product` = ' .
         (int) ($this->id)))
            return false;
    ...
    public function getCustomizationFields ($id_lang = false)
    {
        ...
        '
         ...
         ($id_lang ? ' AND cfl.`id_lang` = ' . (int) ($id_lang) : '') . '
    ...
    public function hasAllRequiredCustomizableFields ()
    {
        global $cookie;
        $fields = array_merge(
        $cookie->getFamily('pictures_' . (int) ($this->id)), 
        $cookie->getFamily('textFields_' . (int) ($this->id)));
        if (($requiredFields = $this->getRequiredCustomizableFields()) === false)
            return false;
        $prefix = array(
        _CUSTOMIZE_FILE_ => 'pictures_' . (int) ($this->id) . '_', 
        _CUSTOMIZE_TEXTFIELD_ => 'textFields_' . (int) ($this->id) . '_');
        foreach ($requiredFields as $field)
            if (! isset(
            $fields[$prefix[$field['type']] . $field['id_customization_field']]) or
             empty(
            $fields[$prefix[$field['type']] . $field['id_customization_field']]))
                return false;
        return true;
    }
    /**
    ...
    {
        if (! ((int) $id_product > 0) || ! is_array($categories) ||
         empty($categories))
            return false;
        ...
        if (! Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS($sql))
            return false;
        self::$_incat[md5($sql)] = (Db::getInstance(_PS_USE_SQL_SLAVE_)->NumRows() >
         0 ? true : false);
        return self::$_incat[md5($sql)];
    }
    ...
    public function checkAccess ($id_customer)
    {
        if (! $id_customer)
			...
			WHERE cp.`id_product` = ' . (int) $this->id . ' AND cg.`id_customer` = ' .
             (int) $id_customer);
    }
    /**
    ...
    public function addStockMvt ($quantity, $id_reason, 
    $id_product_attribute = null, $id_order = null, $id_employee = null)
    {
    ...
    public function getStockMvts ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'product_lang` pl ON (sm.id_product = pl.id_product AND pl.id_lang = ' .
         (int) $id_lang . ')
		LEFT JOIN `' . _DB_PREFIX_ .
         'stock_mvt_reason_lang` mrl ON (sm.id_stock_mvt_reason = mrl.id_stock_mvt_reason AND mrl.id_lang = ' .
         (int) $id_lang . ')
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'attribute_lang` al ON (al.id_attribute = pac.id_attribute AND al.id_lang = ' .
         (int) $id_lang . ')
    ...
    public function getBasicPrice ()
    {
        Tools::displayAsDeprecated();
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
        'SELECT `price` FROM `' . _DB_PREFIX_ . 'product` WHERE `id_product` = ' .
         (int) ($this->id));
    }
    ...
    public function setWsProductFeatures ($productFeatures)
    {
        ...
        foreach ($productFeatures as $productFeature)
            $this->addFeaturesToDB($productFeature['id'], 
    ...
    {
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
        'SELECT `id_category` AS id FROM `' . _DB_PREFIX_ .
         'category_product` WHERE `id_product` = ' . (int) $this->id);
        return $result;
    }
    /**
    ...
    {
        $result = Db::getInstance()->ExecuteS(
        'SELECT `id_product_attribute` as id FROM `' . _DB_PREFIX_ .
         'product_attribute` WHERE `id_product` = ' . (int) ($this->id));
        return $result;
    }
    /**
    ...
    public function setWsCombinations ($combinations)
    {
        ...
        if (count($toDelete) > 0)
            Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ .
             'product_attribute` WHERE `id_product_attribute` IN (' .
             implode(',', $toDelete) . ')');
        foreach ($toAdd as $id) {
            // Update id_product if exists else create
            if (in_array($id, $all_ids))
                Db::getInstance()->Execute(
                'UPDATE `' . _DB_PREFIX_ . 'product_attribute` SET id_product = ' .
                 (int) $this->id . ' WHERE id_product_attribute=' . $id);
            else
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'product_attribute` (`id_product`) VALUES (' . $this->id . ')');
        }
        return true;
    }
    /**
    ...
    public function setCoverWs ($id_image)
    {
        ...
        Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ . 'image`
									SET `cover` = 1 WHERE `id_product` = ' . (int) ($this->id) .
    ...
    {
        $row = GroupReduction::getGroupByCategoryId(
        ...
        if (! $row) // Remove
{
            ...
            if (! GroupReduction::setProductReduction((int) $this->id, 
            $row['id_group'], (int) $this->id_category_default, 
            (float) $row['reduction']))
                return false;
        return true;
    }
