...
class TaxRulesGroupCore extends ObjectModel
{
    ...
    {
        $tax_rules[] = array('id_tax_rules_group' => 0, 
    ...
    $id_county)
    {
        ...
        if (isset(
        self::$_taxes[$id_tax_rules_group . '-' . $id_country . '-' . $id_state .
         '-' . $id_county]))
            return self::$_taxes[$id_tax_rules_group . '-' . $id_country . '-' .
             $id_state . '-' . $id_county];
    ...
    public static function getTaxesRate ($id_tax_rules_group, $id_country, 
    $id_state, $id_county)
    {
        $rate = 0;
        foreach (TaxRulesGroup::getTaxes($id_tax_rules_group, $id_country, 
        $id_state, $id_county) as $tax)
            $rate += (float) $tax->rate;
        return $rate;
    }
