...
class FeatureCore extends ObjectModel
{
    ...
    'objectNodeName' => 'product_feature', 'fields' => array());
    ...
    public static function getFeature ($id_lang, $id_feature)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'feature_lang` fl ON ( f.`id_feature` = fl.`id_feature` AND fl.`id_lang` = ' .
         (int) ($id_lang) . ')
    ...
    public function delete ()
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'feature_value_lang` WHERE `id_feature_value` IN (SELECT id_feature_value FROM `' .
         _DB_PREFIX_ . 'feature_value` WHERE `id_feature` = ' . (int) ($this->id) .
         ')');
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'feature_value` WHERE `id_feature` = ' .
         (int) ($this->id));
        /* Also delete related products */
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'feature_product` WHERE `id_feature` = ' .
         (int) ($this->id));
        $return = parent::delete();
        if ($return)
            Module::hookExec('afterDeleteFeature', 
            array('id_feature' => $this->id));
        return $return;
    }
    public function update ($nullValues = false)
    {
        ...
        foreach ($fields as $field) {
            ...
            'SELECT `id_lang` FROM `' . pSQL(_DB_PREFIX_ . $this->table) .
             '_lang` WHERE `' . pSQL($this->identifier) . '` = ' .
             (int) ($this->id) . ' AND `id_lang` = ' . (int) ($field['id_lang']));
            $result *= (! Db::getInstance()->NumRows()) ? Db::getInstance()->AutoExecute(
            _DB_PREFIX_ . $this->table . '_lang', $field, 'INSERT') : Db::getInstance()->AutoExecute(
    ...
    public static function nbFeatures ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'feature_lang` agl ON (ag.`id_feature` = agl.`id_feature` AND `id_lang` = ' .
         (int) ($id_lang) . ')
		ORDER BY `name` ASC');
        return ($result['nb']);
    }
    ...
    public static function addFeatureImport ($name)
    {
        ...
        'SELECT `id_feature` FROM ' . _DB_PREFIX_ .
         'feature_lang WHERE `name` = \'' . pSQL($name) .
         '\' GROUP BY `id_feature`');
        if (! empty($rq))
