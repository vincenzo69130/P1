...
class SearchCore
{
    public static function sanitize ($string, $id_lang, $indexation = false)
    {
        ...
        if ($indexation) {
            ...
            if ($minWordLen > 1) {
                ...
                $string = preg_replace(
                '/^[^\s]{1,' . $minWordLen . '}(?=\s)/Su', '', $string);
                $string = preg_replace(
                '/(?<=\s)[^\s]{1,' . $minWordLen . '}$/Su', '', $string);
    ...
    $orderBy = 'position', $orderWay = 'desc', $ajax = false, $useCookie = true)
    {
		...
		SELECT p.*, pl.`description_short`, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`name`,
         ...
         $score . ', DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
    ...
    public static function getTags ($db, $id_product, $id_lang)
    {
		...
		LEFT JOIN ' . _DB_PREFIX_ .
         'tag t ON (pt.id_tag = t.id_tag AND t.id_lang = ' . (int) $id_lang . ')
            ...
            foreach ($products as $product) {
                ...
                foreach ($pArray as $word => $weight) {
                    if (! $weight)
                        continue;
                    if (! isset(
                    $wordIdsByWord[$product['id_lang']]['_' . $word]))
                        continue;
    ...
    $pageSize = 10, $orderBy = false, $orderWay = false, $useCookie = true)
    {
		...
		AND cg.`id_group` ' . (! $id_customer ? '= 1' : 'IN (
         ...
         ')') . '
		AND t.`name` LIKE \'%' . pSQL($tag) . '%\'
		ORDER BY position DESC' . ($orderBy ? ', ' . $orderBy : '') .
         ($orderWay ? ' ' . $orderWay : '') . '
		LIMIT ' . (int) (($pageNumber - 1) * $pageSize) . ',' . (int) $pageSize);
        if (! $result)
            return false;
