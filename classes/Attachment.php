...
class AttachmentCore extends ObjectModel
{
    ...
    protected $fieldsSize = array('file' => 40, 'mime' => 128, 
    'file_name' => 128);
    protected $fieldsValidate = array('file' => 'isGenericName', 
    'mime' => 'isCleanHtml', 'file_name' => 'isGenericName');
    ...
    public function delete ()
    {
        ...
        'DELETE FROM ' . _DB_PREFIX_ .
         'product_attachment WHERE id_attachment = ' . (int) ($this->id));
    ...
    public static function getAttachments ($id_lang, $id_product, 
    $include = true)
    {
		...
		WHERE a.id_attachment ' . ($include ? 'IN' : 'NOT IN') .
         ' (SELECT pa.id_attachment FROM ' . _DB_PREFIX_ .
         'product_attachment pa WHERE id_product = ' . (int) ($id_product) . ')');
    }
        ...
        if (is_array($array)) {
            ...
            foreach ($array as $id_attachment)
                $ids[] = '(' . (int) ($id_product) . ',' . (int) ($id_attachment) .
                 ')';
            Db::getInstance()->Execute(
            'UPDATE ' . _DB_PREFIX_ . 'product SET cache_has_attachments = ' .
             (count($ids) ? '1' : '0') . ' WHERE id_product = ' .
             (int) ($id_product) . ' LIMIT 1');
            return ($result1 && Db::getInstance()->Execute(
            'INSERT INTO ' . _DB_PREFIX_ .
             'product_attachment (id_product, id_attachment) VALUES ' .
             implode(',', $ids)));
        }
        return $result1;
    }
