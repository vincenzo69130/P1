...
class OrderCore extends ObjectModel
{
    ...
    protected $fieldsRequired = array('conversion_rate', 'id_address_delivery', 
    'id_address_invoice', 'id_cart', 'id_currency', 'id_lang', 'id_customer', 
    'id_carrier', 'payment', 'total_paid', 'total_paid_real', 'total_products', 
    'total_products_wt');
    protected $fieldsValidate = array('id_address_delivery' => 'isUnsignedId', 
    'id_address_invoice' => 'isUnsignedId', 'id_cart' => 'isUnsignedId', 
    'id_currency' => 'isUnsignedId', 'id_lang' => 'isUnsignedId', 
    'id_customer' => 'isUnsignedId', 'id_carrier' => 'isUnsignedId', 
    'secure_key' => 'isMd5', 'payment' => 'isGenericName', 
    'recyclable' => 'isBool', 'gift' => 'isBool', 'gift_message' => 'isMessage', 
    'total_discounts' => 'isPrice', 'total_paid' => 'isPrice', 
    'total_paid_real' => 'isPrice', 'total_products' => 'isPrice', 
    'total_products_wt' => 'isPrice', 'total_shipping' => 'isPrice', 
    'carrier_tax_rate' => 'isFloat', 'total_wrapping' => 'isPrice', 
    'shipping_number' => 'isUrl', 'conversion_rate' => 'isFloat');
    protected $webserviceParameters = array(
    'objectMethods' => array('add' => 'addWs'), 'objectNodeName' => 'order', 
    ...
    'module' => array('required' => true), 'invoice_number' => array(), 
    'invoice_date' => array(), 'delivery_number' => array(), 'delivery_date' => array(), 
    'valid' => array(), 
    'current_state' => array('getter' => 'getCurrentState', 
    'setter' => 'setCurrentState', 'xlink_resource' => 'order_states'), 
    'date_add' => array(), 'date_upd' => array()), 
    'associations' => array(
    'order_rows' => array('resource' => 'order_row', 'setter' => false, 
    'virtual_entity' => true, 
    'fields' => array('id' => array(), 'product_id' => array('required' => true), 
    ...
    protected function _deleteProduct ($orderDetail, $quantity)
    {
        ...
        elseif ($orderDetail->reduction_amount != '0.000000')
            $reduction_amount = Tools::ps_round($orderDetail->reduction_amount, 
            2);
        if (isset($reduction_amount) and $reduction_amount)
            $price = Tools::ps_round($price - $reduction_amount, 2);
        $productPriceWithoutTax = number_format(
        $price / (1 + $orderDetail->tax_rate * 0.01), 2, '.', '');
        $price += Tools::ps_round(
        $orderDetail->ecotax * (1 + $orderDetail->ecotax_tax_rate / 100), 2);
        $productPrice = number_format($quantity * $price, 2, '.', '');
        /* Update cart */
        $cart = new Cart($this->id_cart);
        $cart->updateQty($quantity, $orderDetail->product_id, 
        $orderDetail->product_attribute_id, false, 'down'); // customization are deleted in deleteCustomization
        $cart->update();
        /* Update order */
        ...
        if (! $orderDetail->product_quantity) {
            ...
            if (count($this->getProductsDetail()) == 0) {
                ...
                Configuration::get('PS_OS_CANCELED'), (int) ($this->id));
    ...
    public function deleteCustomization ($id_customization, $quantity, 
    $orderDetail)
    {
        if (! (int) ($this->getCurrentState()))
            return false;
        if ($this->hasBeenDelivered())
            ...
            'UPDATE `' . _DB_PREFIX_ .
             'customization` SET `quantity_returned` = `quantity_returned` + ' .
             (int) ($quantity) . ' WHERE `id_customization` = ' .
             (int) ($id_customization) . ' AND `id_cart` = ' .
             (int) ($this->id_cart) . ' AND `id_product` = ' .
             (int) ($orderDetail->product_id));
        elseif ($this->hasBeenPaid())
            return Db::getInstance()->Execute(
            'UPDATE `' . _DB_PREFIX_ .
             'customization` SET `quantity_refunded` = `quantity_refunded` + ' .
             (int) ($quantity) . ' WHERE `id_customization` = ' .
             (int) ($id_customization) . ' AND `id_cart` = ' .
             (int) ($this->id_cart) . ' AND `id_product` = ' .
             (int) ($orderDetail->product_id));
        ...
        'UPDATE `' . _DB_PREFIX_ .
         'customization` SET `quantity` = `quantity` - ' . (int) ($quantity) .
         ' WHERE `id_customization` = ' . (int) ($id_customization) .
         ' AND `id_cart` = ' . (int) ($this->id_cart) . ' AND `id_product` = ' .
         (int) ($orderDetail->product_id)))
            return false;
        if (! Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'customization` WHERE `quantity` = 0'))
            return false;
        return $this->_deleteProduct($orderDetail, (int) ($quantity));
    }
    /**
    ...
    public function getHistory ($id_lang, $id_order_state = false, $no_hidden = false)
    {
        if (! $id_order_state)
            $id_order_state = 0;
        if (! isset(self::$_historyCache[$this->id . '_' . $id_order_state]) or
         $no_hidden) {
			...
			' . ((int) ($id_order_state) ? ' AND oh.`id_order_state` = ' .
             (int) ($id_order_state) : '') . '
    ...
    public function getLastMessage ()
    {
        ...
        $sql = 'SELECT `message` FROM `' . _DB_PREFIX_ .
         'message` WHERE `id_order` = ' . (int) ($this->id) .
    ...
    public function setProductPrices (&$row)
    {
        if ($this->_taxCalculationMethod == PS_TAX_EXC)
            ...
            $row['product_price_wt'] = Tools::ps_round(
            $row['product_price'] * (1 + $row['tax_rate'] / 100), 2);
        $group_reduction = 1;
        ...
        if ($row['reduction_percent'] != 0) {
            if ($this->_taxCalculationMethod == PS_TAX_EXC)
                $row['product_price'] = ($row['product_price'] -
                 $row['product_price'] * ($row['reduction_percent'] * 0.01));
            else {
                $reduction = Tools::ps_round(
                $row['product_price_wt'] * ($row['reduction_percent'] * 0.01), 2);
                $row['product_price_wt'] = Tools::ps_round(
                ($row['product_price_wt'] - $reduction), 2);
            }
        }
        if ($row['reduction_amount'] != 0) {
            if ($this->_taxCalculationMethod == PS_TAX_EXC)
                $row['product_price'] = ($row['product_price'] -
                 ($row['reduction_amount'] / (1 + $row['tax_rate'] / 100)));
            else
                $row['product_price_wt'] = Tools::ps_round(
                ($row['product_price_wt'] - $row['reduction_amount']), 2);
        }
        if ($row['group_reduction'] > 0) {
            if ($this->_taxCalculationMethod == PS_TAX_EXC)
                ...
                $row['product_price_wt'] = Tools::ps_round(
                $row['product_price_wt'] * $group_reduction, 2);
        }
        if (($row['reduction_percent'] or $row['reduction_amount'] or
         $row['group_reduction']) and $this->_taxCalculationMethod == PS_TAX_EXC)
            $row['product_price'] = Tools::ps_round($row['product_price'], 2);
        if ($this->_taxCalculationMethod == PS_TAX_EXC)
            $row['product_price_wt'] = Tools::ps_round(
            $row['product_price'] * (1 + ($row['tax_rate'] * 0.01)), 2) + Tools::ps_round(
            $row['ecotax'] * (1 + $row['ecotax_tax_rate'] / 100), 2);
        else {
            $row['product_price_wt_but_ecotax'] = $row['product_price_wt'];
            $row['product_price_wt'] = Tools::ps_round(
            $row['product_price_wt'] +
             $row['ecotax'] * (1 + $row['ecotax_tax_rate'] / 100), 2);
        }
     ...
     */
    public function getProducts ($products = false, $selectedProducts = false, 
    $selectedQty = false)
    {
        if (! $products)
        ...
        foreach ($products as $row) {
            ...
            if ($row['download_hash'] and ! empty($row['download_hash']))
                $row['filename'] = ProductDownload::getFilenameFromIdProduct(
                $row['product_id']);
                /* Stock product */
    ...
    public function isVirtual ($strict = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if ($pd and Validate::isUnsignedInt($pd) and
             $product['download_hash']) {
                if ($strict === false)
                    return true;
            } else
                $virtual &= false;
    ...
    public function getDiscounts ($details = false)
    {
        ...
        '
		SELECT *
		FROM `' . _DB_PREFIX_ . 'order_discount` od ' . ($details ? 'LEFT JOIN `' .
         _DB_PREFIX_ . 'discount` d ON (d.`id_discount` = od.`id_discount`)' : '') . '
		WHERE od.`id_order` = ' . (int) ($this->id));
    }
    ...
    public function getCurrentStateFull ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'order_state` os ON (os.`id_order_state` = oh.`id_order_state`)
		WHERE osl.`id_lang` = ' . (int) ($id_lang) . ' AND oh.`id_order` = ' .
         (int) ($this->id) . '
    ...
    {
        return sizeof(
        $this->getHistory((int) ($this->id_lang), 
    ...
    $showHiddenStatus = false)
    {
        ...
        foreach ($res as $key => $val) {
			...
			WHERE oh.`id_order` = ' . (int) ($val['id_order']) .
             (! $showHiddenStatus ? ' AND os.`hidden` != 1' : '') . '
			ORDER BY oh.`date_add` DESC, oh.`id_order_history` DESC
			LIMIT 1');
            if ($res2)
    ...
    $id_customer = NULL, $type = NULL)
    {
        $sql = '
		SELECT `id_order`
		FROM `' . _DB_PREFIX_ . 'orders`
		WHERE DATE_ADD(date_upd, INTERVAL -1 DAY) <= \'' . pSQL($date_to) .
         '\' AND date_upd >= \'' . pSQL($date_from) . '\'' .
	...
	*/
    ...
    {
        return Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'order_discount', 
        array('id_order' => (int) ($this->id), 
        'id_discount' => (int) ($id_discount), 'name' => pSQL($name), 
        'value' => (float) ($value)), 'INSERT');
    }
    /**
    ...
    public function getNumberOfDays ()
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
        '
		SELECT TO_DAYS(NOW()) - TO_DAYS(`delivery_date`)  AS days FROM `' .
         _DB_PREFIX_ . 'orders`
    ...
    {
        $payment = $this->getHistory((int) ($this->id_lang), 
        Configuration::get('PS_OS_PAYMENT'));
        $delivred = $this->getHistory((int) ($this->id_lang), 
        Configuration::get('PS_OS_DELIVERED'));
        if ($payment and $delivred and strtotime($delivred[0]['date_add']) < strtotime(
        $payment[0]['date_add']))
            return ((int) (Configuration::get('PS_ORDER_RETURN')) == 1 and
             $this->getNumberOfDays());
        else
            return ((int) (Configuration::get('PS_ORDER_RETURN')) == 1 and
             (int) ($this->getCurrentState()) ==
             Configuration::get('PS_OS_DELIVERED') and $this->getNumberOfDays());
    }
    ...
    public function setInvoice ()
    {
		...
		SET `invoice_number` = ' . $number . ', `invoice_date` = \'' .
         date('Y-m-d H:i:s') . '\'
		WHERE `id_order` = ' . (int) $this->id);
    ...
    public static function printPDFIcons ($id_order, $tr)
    {
        ...
        if (! Validate::isLoadedObject($orderState) or
         ! Validate::isLoadedObject($order))
            die(Tools::displayError('Invalid objects'));
        echo '<span style="width:20px; margin-right:5px;">';
        if (($orderState->invoice and $order->invoice_number) and
         (int) ($tr['product_number']))
            echo '<a href="pdf.php?id_order=' . (int) ($order->id) .
             '&pdf"><img src="../img/admin/tab-invoice.gif" alt="invoice" /></a>';
        else
            echo '&nbsp;';
        echo '</span>';
        echo '<span style="width:20px;">';
        if ($orderState->delivery and $order->delivery_number)
            echo '<a href="pdf.php?id_delivery=' .
             (int) ($order->delivery_number) .
    ...
    public function addWs ($autodate = true, $nullValues = false)
    {
        ...
        $paymentModule->validateOrder($this->id_cart, 
        Configuration::get('PS_OS_WS_PAYMENT'), $this->total_paid, 
        $this->payment, NULL, array(), null, false, $customer->secure_key);
