...
class AttributeGroupCore extends ObjectModel
{
    ...
    'objectsNodeName' => 'product_options', 'objectNodeName' => 'product_option', 
    'fields' => array(), 
    'associations' => array(
    ...
    public function add ($autodate = true, $nullValues = false)
    {
        ...
        Module::hookExec('afterSaveAttributeGroup', 
        array('id_attribute_group' => $this->id));
        return $return;
    }
    ...
    public static function cleanDeadCombinations ()
    {
        ...
        'SELECT pac.`id_attribute`, pa.`id_product_attribute` FROM `' .
         _DB_PREFIX_ . 'product_attribute` pa LEFT JOIN `' . _DB_PREFIX_ .
    ...
    public function delete ()
    {
        ...
        'SELECT `id_attribute` FROM `' . _DB_PREFIX_ .
         'attribute` WHERE `id_attribute_group` = ' . (int) ($this->id));
        ...
        if (! empty($toRemove) and Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'product_attribute_combination` WHERE `id_attribute` IN (' .
         implode(', ', $toRemove) . ')') === false)
            return false;
        ...
        'DELETE FROM `' . _DB_PREFIX_ .
         'attribute_lang` WHERE `id_attribute` IN (SELECT id_attribute FROM `' .
         _DB_PREFIX_ . 'attribute` WHERE `id_attribute_group` = ' .
         (int) ($this->id) . ')') === false or
         Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'attribute` WHERE `id_attribute_group` = ' . (int) ($this->id)) === false)
            return false;
        $return = parent::delete();
        if ($return)
            Module::hookExec('afterDeleteAttributeGroup', 
            array('id_attribute_group' => $this->id));
        return $return;
    }
    /**
    ...
    public static function getAttributesGroups ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'attribute_group_lang` agl ON (ag.`id_attribute_group` = agl.`id_attribute_group` AND `id_lang` = ' .
         (int) ($id_lang) . ')
		ORDER BY `name` ASC');
    }
    /**
    ...
    public function getWsProductOptionValues ()
    {
        ...
        'SELECT id_attribute AS id from `' . _DB_PREFIX_ .
         'attribute` WHERE id_attribute_group = ' . (int) $this->id);
        return $result;
    }
}
