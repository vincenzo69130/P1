...
class OrderSlipCore extends ObjectModel
{
    ...
    'id_order' => 'isUnsignedId', 'conversion_rate' => 'isFloat');
    ...
    public function addSlipDetail ($orderDetailList, $productQtyList)
    {
        foreach ($orderDetailList as $key => $orderDetail) {
            if ($qty = (int) ($productQtyList[$key]))
                ...
                _DB_PREFIX_ . 'order_slip_detail', 
                array('id_order_slip' => (int) ($this->id), 
                'id_order_detail' => (int) ($orderDetail), 
                'product_quantity' => $qty), 'INSERT');
        }
    }
    ...
    public static function getOrdersSlipDetail ($id_order_slip = true, 
    $id_order_detail = false)
    {
        ...
        ($id_order_detail ? 'SELECT SUM(`product_quantity`) AS `total`' : 'SELECT *') .
         ...
         ($id_order_slip ? ' WHERE `id_order_slip` = ' . (int) ($id_order_slip) : '') . ($id_order_detail ? ' WHERE `id_order_detail` = ' .
         (int) ($id_order_detail) : ''));
    }
                ...
                if (sizeof($discounts)) {
                    ...
                    foreach ($discounts as $discount) {
                        if ($discount['id_discount_type'] == 1)
                            $resTab[$key]['product_price'] -= $realProductPrice *
                             ($discount['value'] / 100);
                        elseif ($discount['id_discount_type'] == 2)
                            $resTab[$key]['product_price'] -= (($discount['value'] * ($product['product_price_wt'] /
                             $order->total_products_wt)) /
                             (1.00 + ($product['tax_rate'] / 100)));
                    }
                }
            }
    ...
    public static function getSlipsIdByDate ($dateFrom, $dateTo)
    {
		...
		FROM `' . _DB_PREFIX_ . 'order_slip`
		WHERE `date_add` BETWEEN \'' . pSQL($dateFrom) . ' 00:00:00\' AND \'' .
    ...
    public static function createOrderSlip ($order, $productList, $qtyList, 
    $shipping_cost = false)
    {
