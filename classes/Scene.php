...
class SceneCore extends ObjectModel
{
    ...
    'zones' => 'isSceneZones', 'categories' => 'isArrayWithIds');
    protected $fieldsRequiredLang = array('name');
    protected $fieldsSizeLang = array('name' => 100);
    protected $fieldsValidateLang = array('name' => 'isGenericName');
    public function __construct ($id = NULL, $id_lang = NULL, $liteResult = true, 
    $hideScenePosition = false)
    {
    ...
    public function deleteImage ()
    {
        if (parent::deleteImage()) {
            if (file_exists(
            $this->image_dir . 'thumbs/' . $this->id . '-thumb_scene.' .
    ...
    public function addCategories ($categories)
    {
        ...
        foreach ($categories as $category) {
            ...
            'INSERT INTO `' . _DB_PREFIX_ .
             'scene_category` ( `id_scene` , `id_category`) VALUES (' .
    ...
    public function addZoneProducts ($zones)
    {
        ...
        foreach ($zones as $zone) {
            $sql = 'INSERT INTO `' . _DB_PREFIX_ . 'scene_products` ( `id_scene` , `id_product` , `x_axis` , `y_axis` , `zone_width` , `zone_height`) VALUES
				 (' . (int) ($this->id) . ', ' . (int) ($zone['id_product']) . ', ' .
     ...
     */
    public static function getScenes ($id_category, $id_lang = NULL, 
    $onlyActive = true, $liteResult = true, $hideScenePosition = true)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ . 'scene` s ON (sc.id_scene = s.id_scene)
		LEFT JOIN `' . _DB_PREFIX_ . 'scene_lang` sl ON (sl.id_scene = s.id_scene)
		WHERE sc.id_category = ' . (int) ($id_category) . '	AND sl.id_lang = ' .
         (int) ($id_lang) . ($onlyActive ? ' AND s.active = 1' : '') . '
		ORDER BY sl.name ASC');
        if (! $liteResult and $scenes)
            foreach ($scenes as &$scene)
                $scene = new Scene((int) ($scene['id_scene']), (int) ($id_lang), 
                false, $hideScenePosition);
        return $scenes;
    }
    /**
    ...
    public function getProducts ($onlyActive = true, $id_lang = NULL, $liteResult = true)
    {
		...
		WHERE s.id_scene = ' . (int) ($this->id) .
         ($onlyActive ? ' AND p.active = 1' : ''));
        if (! $liteResult and $products)
            foreach ($products as &$product) {
                $product['details'] = new Product((int) ($product['id_product']), 
                ! $liteResult, (int) ($id_lang));
                $product['link'] = $link->getProductLink(
                (int) ($product['details']->id), 
                $product['details']->link_rewrite, $product['details']->category, 
