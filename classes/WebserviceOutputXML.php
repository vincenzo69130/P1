...
class WebserviceOutputXMLCore implements WebserviceOutputInterface
{
        ...
        if (isset($field['i18n']) && $field['i18n']) {
            foreach ($this->languages as $language) {
                ...
                if (isset($field['synopsis_details']) ||
                 (isset($field['value']) and is_array($field['value']))) {
                    $more_attr .= ' xlink:href="' . $this->getWsUrl() .
                     'languages/' . $language . '"';
                    if (isset($field['synopsis_details']) &&
                ...
                $node_content .= '<language id="' . $language . '"' . $more_attr .
                 '>';
                if (isset($field['value']) and is_array($field['value']) and
                 isset($field['value'][$language]))
            ...
            if (array_key_exists('xlink_resource', $field) &&
             $this->schemaToDisplay != 'blank') {
                if (! is_array($field['xlink_resource']))
                    $ret .= ' xlink:href="' . $this->getWsUrl() .
                     $field['xlink_resource'] . '/' . $field['value'] . '"';
                else
                    $ret .= ' xlink:href="' . $this->getWsUrl() .
                     $field['xlink_resource']['resourceName'] . '/' . (isset(
                    $field['xlink_resource']['subResourceName']) ? $field['xlink_resource']['subResourceName'] .
                     '/' . $field['object_id'] . '/' : '') . $field['value'] .
                     '"';
            }
    ...
    public function renderNodeHeader ($node_name, $params, $more_attr = null, 
    $has_child = true)
    {
        $string_attr = '';
    ...
    $closed_tags = false)
    {
        ...
        if ($this->schemaToDisplay != 'blank') {
            $more .= (isset(
            $params['associations'][$assoc_name]['virtual_entity']) &&
             $params['associations'][$assoc_name]['virtual_entity'] ? ' virtual_entity="true"' : '');
            $more .= ' node_type="' .
             $params['associations'][$assoc_name]['resource'] . '"';
        }
