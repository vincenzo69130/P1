...
class PageCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_page_type' => 'isUnsignedId', 
    'id_object' => 'isUnsignedId');
    ...
    {
        $phpSelf = isset($_SERVER['PHP_SELF']) ? substr($_SERVER['PHP_SELF'], 
        strlen(__PS_BASE_URI__)) : '';
        // Some pages must be distinguished in order to record exactly what is being seen
        $specialArray = array('product.php' => 'id_product', 
        'category.php' => 'id_category', 'order.php' => 'step', 
        ...
        if (array_key_exists($phpSelf, $specialArray)) {
            ...
            if ($result['id_page'])
				...
				VALUES ((SELECT pt.`id_page_type` FROM `' . _DB_PREFIX_ .
                 'page_type` pt WHERE pt.`name` = \'' . pSQL($phpSelf) . '\'),
						' . (int) ($id_object) . ')');
