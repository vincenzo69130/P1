...
class DiscountCore extends ObjectModel
{
    ...
    public $date_upd;
    protected $fieldsRequired = array('id_discount_type', 'name', 'value', 
    'quantity', 'quantity_per_user', 'date_from', 'date_to');
    protected $fieldsSize = array('name' => '32', 'date_from' => '32', 
    'date_to' => '32');
    protected $fieldsValidate = array('id_customer' => 'isUnsignedId', 
    'id_group' => 'isUnsignedId', 'id_discount_type' => 'isUnsignedId', 
    'id_currency' => 'isUnsignedId', 'name' => 'isDiscountName', 
    'value' => 'isPrice', 'quantity' => 'isUnsignedInt', 
    'quantity_per_user' => 'isUnsignedInt', 'cumulable' => 'isBool', 
    'cumulable_reduction' => 'isBool', 'date_from' => 'isDate', 
    ...
    'description' => 'isVoucherDescription');
    protected $table = 'discount';
    protected $identifier = 'id_discount';
    protected $webserviceParameters = array(
    'fields' => array(
    'id_discount_type' => array('sqlId' => 'id_discount_type', 
    'xlink_resource' => 'discount_types'), 
    'id_customer' => array('sqlId' => 'id_customer', 
    'xlink_resource' => 'customers'), 
    'id_group' => array('sqlId' => 'id_group', 'xlink_resource' => 'groups'), 
    'id_currency' => array('sqlId' => 'id_currency', 
    'xlink_resource' => 'currencies'), 'name' => array('sqlId' => 'name'), 
    ...
    public function delete ()
    {
        ...
        'DELETE FROM ' . _DB_PREFIX_ . 'discount_category WHERE id_discount = ' .
         (int) ($this->id)));
    }
     ...
     */
    public static function getCustomerDiscounts ($id_lang, $id_customer, 
    $active = false, $includeGenericOnes = true, $stock = false)
    {
        global $cart;
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'discount_lang` dl ON (d.`id_discount` = dl.`id_discount` AND dl.`id_lang` = ' .
         (int) ($id_lang) . ')
		...
		OR `id_group` IN (SELECT `id_group` FROM `' . _DB_PREFIX_ .
         'customer_group` WHERE `id_customer` = ' . (int) ($id_customer) . ')' .
        ...
        foreach ($res as &$discount)
            if ($discount['quantity_per_user']) {
                $quantity_used = Order::getDiscountsCustomer(
                (int) ($id_customer), (int) ($discount['id_discount']));
                if (isset($cart) and isset($cart->id))
                    $quantity_used += $cart->getDiscountsCustomer(
                    (int) ($discount['id_discount']));
                $discount['quantity_for_user'] = $discount['quantity_per_user'] -
                 $quantity_used;
            } else
     ...
     */
    public function getValue ($nb_discounts = 0, $order_total_products = 0, 
    $shipping_fees = 0, $idCart = false, $useTax = true)
    {
        $totalAmount = 0;
        $cart = new Cart((int) ($idCart));
        if (! Validate::isLoadedObject($cart))
            return 0;
        if ((! $this->cumulable and (int) ($nb_discounts) > 1) or ! $this->active or
         (! $this->quantity and ! $cart->OrderExists()))
            return 0;
        if ($this->usedByCustomer((int) ($cart->id_customer)) >=
         $this->quantity_per_user and ! $cart->OrderExists())
            return 0;
        $date_start = strtotime($this->date_from);
        $date_end = strtotime($this->date_to);
        if ((time() < $date_start or time() > $date_end) and
         ! $cart->OrderExists())
            return 0;
        ...
        switch ($this->id_discount_type) {
            ...
            case 1:
                ...
                foreach ($products as $product)
                    ...
                    $categories))
                        if ($this->cumulable_reduction or (! $product['reduction_applies'] and
                         ! $product['on_sale']))
                            $amount += ($useTax ? $product['total_wt'] : $product['total']) *
                             $percentage;
                return $amount;
            /* Absolute value */
            case 2:
                // An "absolute" voucher is available in one currency only
                $currency = ((int) $cart->id_currency ? Currency::getCurrencyInstance(
     ...
     * @deprecated
     */
    public static function isParentCategoryProductDiscount ($id_category_product, 
    $id_category_discount)
    {
    ...
    public function updateCategories ($categories)
    {
        ...
        foreach ($categories as $category) {
			...
			SELECT `id_discount`
			FROM `' . _DB_PREFIX_ . 'discount_category`
			WHERE `id_discount`=' . (int) ($this->id) . ' AND `id_category`=' .
             (int) ($category));
            if (Db::getInstance()->NumRows() == 0)
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'discount_category` (`id_discount`, `id_category`) VALUES(' .
                 (int) ($this->id) . ',' . (int) ($category) . ')');
        }
    }
    public static function discountExists ($discountName, $id_discount = 0)
    {
        ...
        'SELECT `id_discount` FROM ' . _DB_PREFIX_ .
         'discount WHERE `name` LIKE \'' . pSQL($discountName) .
         '\' AND `id_discount` != ' . (int) ($id_discount));
    }
    public static function createOrderDiscount ($order, $productList, $qtyList, 
    $name, $shipping_cost = false, $id_category = 0, $subcategory = 0)
    {
        ...
        foreach ($discounts as $discount) {
            ...
            elseif ($discount['id_discount_type'] == 2)
                $total -= ($discount['value'] *
                 ($total_tmp / $order->total_products_wt));
        }
        if ($shipping_cost)
        ...
        $voucher->name = 'V0C' . (int) ($order->id_customer) . 'O' .
         (int) ($order->id);
        ...
        if (! $voucher->validateFieldsLang(false) or ! $voucher->add())
            return false;
             // set correct name
        $voucher->name = 'V' . (int) ($voucher->id) . 'C' .
         (int) ($order->id_customer) . 'O' . $order->id;
        if (! $voucher->update())
            return false;
        return $voucher;
    }
    public static function display ($discountValue, $discountType, 
    $currency = false)
    {
    ...
    public static function getVouchersToCartDisplay ($id_lang, $id_customer)
    {
		...
		WHERE d.`active` = 1
		AND d.`date_from` <= \'' . pSQL(date('Y-m-d H:i:s')) .
         '\' AND d.`date_to` >= \'' . pSQL(date('Y-m-d H:i:s')) . '\'
		AND dl.`id_lang` = ' . (int) ($id_lang) . '
		AND d.`cart_display` = 1 AND d.`quantity` > 0
		AND ((d.`id_customer` = 0 AND d.`id_group` = 0) ' . ($id_customer ? 'OR (d.`id_customer` = ' .
         $id_customer . '
		OR d.`id_group` IN (SELECT `id_group` FROM `' . _DB_PREFIX_ .
         'customer_group` WHERE `id_customer` = ' . (int) ($id_customer) . ')))' : 'OR d.`id_group` = 1)'));
    }
    public static function deleteByIdCustomer ($id_customer)
    {
        $discounts = Db::getInstance()->ExecuteS(
        'SELECT `id_discount` FROM `' . _DB_PREFIX_ .
         'discount` WHERE `id_customer` = ' . (int) ($id_customer));
        foreach ($discounts as $discount) {
            Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'discount` WHERE `id_discount` = ' .
             (int) ($discount['id_discount']));
            Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ .
             'discount_category` WHERE `id_discount` = ' .
             (int) ($discount['id_discount']));
            Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ .
             'discount_lang` WHERE `id_discount` = ' .
             (int) ($discount['id_discount']));
        }
        return true;
    }
    public static function deleteByIdGroup ($id_group)
    {
        $discounts = Db::getInstance()->ExecuteS(
        'SELECT `id_discount` FROM `' . _DB_PREFIX_ .
         'discount` WHERE `id_group` = ' . (int) ($id_group));
        foreach ($discounts as $discount) {
            Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'discount` WHERE `id_discount` = ' .
             (int) ($discount['id_discount']));
    ...
    {
        return Db::getInstance()->getRow(
        'SELECT * FROM `' . _DB_PREFIX_ . 'discount` WHERE `id_discount` = ' .
         (int) $id_discount);
    }
}