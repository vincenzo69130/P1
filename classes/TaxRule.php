...
class TaxRuleCore extends ObjectModel
{
    ...
    protected $fieldsRequired = array('id_tax_rules_group', 'id_country', 
    'id_tax');
    protected $fieldsValidate = array('id_tax_rules_group' => 'isUnsignedId', 
    'id_country' => 'isUnsignedId', 'id_state' => 'isUnsignedId', 
    'id_county' => 'isUnsignedId', 'id_tax' => 'isUnsignedId', 
    'state_behavior' => 'isUnsignedInt', 'county_behavior' => 'isUnsignedInt');
    ...
    public static function getTaxRulesByGroupId ($id_group)
    {
        ...
        foreach ($results as $row)
            $res[$row['id_country']][$row['id_state']][$row['id_county']] = array(
            'id_tax' => $row['id_tax'], 
            'state_behavior' => $row['state_behavior'], 
            'county_behavior' => $row['county_behavior']);
        return $res;
    }
    ...
    public static function isTaxInUse ($id_tax)
    {
        ...
        '
        SELECT COUNT(*) FROM `' . _DB_PREFIX_ . 'tax_rule` WHERE `id_tax` = ' .
         (int) $id_tax);
    }
}
