...
class ImageCore extends ObjectModel
{
    ...
    'position' => 'isUnsignedInt', 'cover' => 'isBool');
    ...
    public function delete ()
    {
        ...
        if ($result)
            foreach ($result as $row) {
                $row['position'] = $i ++;
                Db::getInstance()->AutoExecute(_DB_PREFIX_ . $this->table, $row, 
                'UPDATE', '`id_image` = ' . (int) ($row['id_image']), 1);
            }
        return true;
    }
    /**
    ...
    public static function getImages ($id_lang, $id_product)
    {
		...
		WHERE i.`id_product` = ' . (int) $id_product . ' AND il.`id_lang` = ' .
         (int) $id_lang . '
     ...
     */
    public static function duplicateProductImages ($id_product_old, 
    $id_product_new, $combinationImages)
    {
        ...
        foreach ($result as $row) {
            ...
            if ($imageNew->add()) {
                ...
                foreach ($imagesTypes as $imageType) {
                    if (file_exists(
                    _PS_PROD_IMG_DIR_ . $imageOld->getExistingImgPath() . '-' .
                     $imageType['name'] . '.jpg')) {
                ...
                if (file_exists(
                _PS_PROD_IMG_DIR_ . $imageOld->getExistingImgPath() . '.jpg'))
                    copy(
                    _PS_PROD_IMG_DIR_ . $imageOld->getExistingImgPath() . '.jpg', 
                    $new_path . '.jpg');
                self::replaceAttributeImageAssociationId($combinationImages, 
                (int) ($imageOld->id), (int) ($imageNew->id));
            } else
                return false;
        }
        return self::duplicateAttributeImageAssociations($combinationImages);
    }
    static protected function replaceAttributeImageAssociationId (
    &$combinationImages, $saved_id, $id_image)
    {
        if (! isset($combinationImages['new']) or
         ! is_array($combinationImages['new']))
            return;
     ...
     * @return boolean
     */
    public static function duplicateAttributeImageAssociations (
    $combinationImages)
    {
        if (! isset($combinationImages['new']) or
         ! is_array($combinationImages['new']))
    ...
    {
        if (! isset(self::$_cacheGetSize[$type]) or
         self::$_cacheGetSize[$type] === NULL)
            self::$_cacheGetSize[$type] = Db::getInstance()->getRow(
            'SELECT `width`, `height` FROM ' . _DB_PREFIX_ .
             'image_type WHERE `name` = \'' . pSQL($type) . '\'');
    ...
    public function deleteImage ()
    {
        if (! $this->id)
            return false;
             // Delete base image
        if (file_exists(
        $this->image_dir . $this->getExistingImgPath() . '.' .
         $this->image_format))
            unlink(
            $this->image_dir . $this->getExistingImgPath() . '.' .
             $this->image_format);
        ...
        foreach ($imageTypes as $imageType)
            $files_to_delete[] = $this->image_dir . $this->getExistingImgPath() .
             '-' . $imageType['name'] . '.' . $this->image_format;
             // Delete watermark image
        $files_to_delete[] = $this->image_dir .
         $this->getExistingImgPath() . '-watermark.' . $this->image_format;
        // delete index.php
        $files_to_delete[] = $this->image_dir . $this->getImgFolder() .
         'index.php';
        // Delete tmp images
        $files_to_delete[] = _PS_TMP_IMG_DIR_ . 'product_' .
         $this->id_product . '.' . $this->image_format;
        $files_to_delete[] = _PS_TMP_IMG_DIR_ . 'product_mini_' .
         $this->id_product . '.' . $this->image_format;
    ...
    public function getExistingImgPath ()
    {
        ...
        if (! $this->existing_path) {
            ...
            _PS_PROD_IMG_DIR_ . $this->id_product . '-' . $this->id . '.' .
             $this->image_format))
    ...
    public function createImgFolder ()
    {
        ...
        if (! file_exists(_PS_PROD_IMG_DIR_ . $this->getImgFolder())) {
            ...
            $success = @mkdir(
            _PS_PROD_IMG_DIR_ . $this->getImgFolder(), 0755, true) ||
             @chmod(_PS_PROD_IMG_DIR_ . $this->getImgFolder(), 0755);
            // Create an index.php file in the new folder
            if ($success && ! file_exists(
            _PS_PROD_IMG_DIR_ . $this->getImgFolder() . 'index.php') &&
             file_exists($this->source_index))
                return @copy($this->source_index, 
                _PS_PROD_IMG_DIR_ . $this->getImgFolder() . 'index.php');
        }
        return true;
    }
        ...
        foreach (scandir(_PS_PROD_IMG_DIR_) as $file) {
            ...
            $file, $matches)) {
                ...
                if (Validate::isLoadedObject($image) &&
                 $image->id_product == (int) rtrim($matches[1], "-")) {
                    ...
                    $new_path = _PS_PROD_IMG_DIR_ .
                     $image->getImgPath() .
                     (isset($matches[3]) ? $matches[3] : '') . '.jpg';
                    if (file_exists($new_path)) {
                        ...
                        $tmp_path = _PS_PROD_IMG_DIR_ . $tmp_folder .
                         basename($file);
                        if (! @rename($new_path, $tmp_path) ||
                         ! file_exists($tmp_path))
                            return false;
                    }
                    // move the image
                    if (! @rename(_PS_PROD_IMG_DIR_ . $file, 
                    $new_path) || ! file_exists($new_path))
                        return false;
                }
            }
            if ((int) $max_execution_time != 0 &&
             (time() - $start_time > (int) $max_execution_time - 4))
                return 'timeout';
        }
        return true;
    }
