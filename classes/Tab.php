...
class TabCore extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_parent' => 'isInt', 
    'position' => 'isUnsignedInt', 'module' => 'isTabName');
    ...
    public static function initAccess ($id_tab)
    {
        ...
        'SELECT `id_profile` FROM ' . _DB_PREFIX_ .
         'profile where `id_profile` != 1');
        if (! $profiles or empty($profiles))
            return false;
            /* Query definition */
        // note : insert ignore should be avoided
        $query = 'INSERT IGNORE INTO `' . _DB_PREFIX_ .
         'access` (`id_profile`, `id_tab`, `view`, `add`, `edit`, `delete`) VALUES ';
        // default admin 
        ...
        foreach ($profiles as $profile) {
            ...
            $rights = $profile['id_profile'] ==
             $cookie->profile ? 1 : 0;
            $query .= '(' . $profile['id_profile'] . ', ' . (int) $id_tab . ', ' .
    ...
    public static function getCurrentParentId ()
    {
        ...
        'SELECT `id_parent` FROM `' . _DB_PREFIX_ .
         'tab` WHERE LOWER(class_name) = \'' .
         pSQL(Tools::strtolower(Tools::getValue('tab'))) . '\''))
    ...
    public static function getIdFromClassName ($class_name)
    {
        if (self::$_getIdFromClassName === null) {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS(
            'SELECT id_tab, class_name FROM `' . _DB_PREFIX_ . 'tab`');
            foreach ($result as $row)
    ...
    public static function getClassNameFromID ($id_tab)
    {
        ...
        'SELECT class_name FROM `' . _DB_PREFIX_ . 'tab` t WHERE t.`id_tab` = \'' .
         (int) $id_tab . '\'');
    }
    ...
    public static function getNewLastPosition ($id_parent)
    {
        ...
        'SELECT MAX(position)+1 FROM `' . _DB_PREFIX_ .
         'tab` WHERE `id_parent` = ' . (int) ($id_parent)));
    }
    public function move ($direction)
    {
        ...
        if ($direction == 'r' and $this->position >= $nbTabs)
            return false;
        $newPosition = ($direction == 'l') ? $this->position - 1 : $this->position +
         1;
        Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ . 'tab` t SET position = ' .
         (int) ($this->position) . ' WHERE id_parent = ' .
         (int) ($this->id_parent) . ' AND position = ' . (int) ($newPosition));
