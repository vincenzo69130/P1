...
class CarrierCore extends ObjectModel
{
    ...
    'name' => 'isCarrierName', 'active' => 'isBool', 'is_free' => 'isBool', 
    'url' => 'isAbsoluteUrl', 'shipping_handling' => 'isBool', 
    'range_behavior' => 'isBool', 'shipping_method' => 'isUnsignedInt');
    ...
    'fields' => array('id_tax_rules_group' => array(), 'deleted' => array(), 
    'is_module' => array()));
    ...
    {
        if (! parent::add($autodate, $nullValues) or
         ! Validate::isLoadedObject($this))
            return false;
        if (! Db::getInstance()->ExecuteS(
        'SELECT `id_carrier` FROM `' . _DB_PREFIX_ . $this->table .
         '` WHERE `deleted` = 0'))
            return false;
    ...
    public function setConfiguration ($id_old)
    {
        ...
        'UPDATE `' . _DB_PREFIX_ . 'delivery` SET `id_carrier` = ' .
         (int) ($this->id) . ' WHERE `id_carrier` = ' . (int) ($id_old));
    }
    /**
    ...
    public function getDeliveryPriceByWeight ($totalWeight, $id_zone)
    {
        ...
        if (! isset(self::$priceByWeight[$cache_key])) {
            ...
            if (! isset($result['price']))
                self::$priceByWeight[$cache_key] = $this->getMaxDeliveryPriceByWeight(
                $id_zone);
            else
    ...
    public static function checkDeliveryPriceByWeight ($id_carrier, $totalWeight, 
    $id_zone)
    {
    ...
    $id_currency = NULL)
    {
        $cache_key = $this->id . '_' . $orderTotal . '_' . $id_zone . '_' .
         $id_currency;
        if (! isset(self::$priceByPrice[$cache_key])) {
            if (! empty($id_currency))
                $orderTotal = Tools::convertPrice($orderTotal, $id_currency, 
                false);
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
			SELECT d.`price`
            ...
            if (! isset($result['price']))
                self::$priceByPrice[$cache_key] = $this->getMaxDeliveryPriceByPrice(
                $id_zone);
            else
     ...
     * @return float Delivery price
     */
    public static function checkDeliveryPriceByPrice ($id_carrier, $orderTotal, 
    $id_zone, $id_currency = NULL)
    {
        $cache_key = $id_carrier . '_' . $orderTotal . '_' . $id_zone . '_' .
         $id_currency;
    ...
    public static function getDeliveryPriceByRanges ($rangeTable, $id_carrier)
    {
		...
		FROM `' . _DB_PREFIX_ . 'delivery` d
		LEFT JOIN `' . _DB_PREFIX_ . $rangeTable . '` r ON r.`id_' . $rangeTable .
         '` = d.`id_' . $rangeTable . '`
		WHERE (d.`id_' . $rangeTable . '` IS NOT NULL AND d.`id_' . $rangeTable .
         '` != 0 AND d.`id_carrier` = ' . (int) ($id_carrier) . ')
		ORDER BY r.`delimiter1` ASC');
    }
    /**
     ...
     */
    public static function getCarriers ($id_lang, $active = false, $delete = false, 
    $id_zone = false, $ids_group = NULL, $modules_filters = self::PS_CARRIERS_ONLY)
    {
        ...
        $sql = '
         ...
         'carrier_lang` cl ON (c.`id_carrier` = cl.`id_carrier` AND cl.`id_lang` = ' .
         (int) ($id_lang) . ')
			LEFT JOIN `' . _DB_PREFIX_ .
         'carrier_zone` cz  ON (cz.`id_carrier` = c.`id_carrier`)' . ($id_zone ? 'LEFT JOIN `' .
         _DB_PREFIX_ . 'zone` z  ON (z.`id_zone` = ' . (int) ($id_zone) . ')' : '') . '
			WHERE c.`deleted` ' .
         ($delete ? '= 1' : ' = 0') . ($active ? ' AND c.`active` = 1' : '') . ($id_zone ? ' AND cz.`id_zone` = ' .
         (int) ($id_zone) . '
			AND z.`active` = 1 ' : ' ');
        ...
        $sql .= ($ids_group ? ' AND c.id_carrier IN (SELECT id_carrier FROM ' .
         _DB_PREFIX_ . 'carrier_group WHERE id_group IN (' . $ids . ')) ' : '') . '
    ...
    public static function getDeliveredCountries ($id_lang, 
    $activeCountries = false, $activeCarriers = false, $containStates = NULL)
    {
        if (! Validate::isBool($activeCountries) or
         ! Validate::isBool($activeCarriers))
            die(Tools::displayError());
        ...
        '
         ...
         ($activeCarriers ? 'AND cr.active = 1) ' : ') ') . '
			...
			' . (! is_null($containStates) ? 'AND c.`contains_states` = ' .
         (int) ($containStates) : '') . '
     ...
     */
    public static function getDefaultCarrierSelection ($carriers, 
    $defaultCarrier = 0)
    {
        if (empty($carriers))
            return 0;
    ...
    public static function getCarriersForOrder ($id_zone, $groups = NULL)
    {
        ...
        if (is_array($groups) and ! empty($groups))
            $result = Carrier::getCarriers((int) $cookie->id_lang, true, false, 
            (int) $id_zone, $groups, 
        ...
        foreach ($result as $k => $row) {
            ...
            if ($shippingMethod != Carrier::SHIPPING_METHOD_FREE) {
                ...
                if (($shippingMethod ==
                 ...
                 $carrier->getMaxDeliveryPriceByWeight($id_zone) === false) or ($shippingMethod ==
                ...
                if ($row['range_behavior']) {
                    ...
                    $row['id_carrier'], $cart->getTotalWeight(), $id_zone))) or ($shippingMethod ==
            ...
            $row['name'] = (strval($row['name']) != '0' ? $row['name'] : Configuration::get(
            'PS_SHOP_NAME'));
            $row['price'] = ($shippingMethod == Carrier::SHIPPING_METHOD_FREE ? 0 : $cart->getOrderShippingCost(
            (int) $row['id_carrier']));
            $row['price_tax_exc'] = ($shippingMethod ==
             Carrier::SHIPPING_METHOD_FREE ? 0 : $cart->getOrderShippingCost(
            (int) $row['id_carrier'], false));
            $row['img'] = file_exists(
            _PS_SHIP_IMG_DIR_ . (int) ($row['id_carrier']) . '.jpg') ? _THEME_SHIP_DIR_ .
    ...
    public static function getCarriersOpc ($id_country, $groups = NULL)
    {
        ...
        return self::getCarriersForOrder(
        (int) Country::getIdZone((int) ($id_country)), $groups);
    }
    public static function checkCarrierZone ($id_carrier, $id_zone)
    {
        ...
        '
			...
			LEFT JOIN `' . _DB_PREFIX_ . 'zone` z  ON (z.`id_zone` = ' .
         (int) ($id_zone) . ')
    ...
    public function getZones ()
    {
        ...
        '
			...
			LEFT JOIN `' .
         _DB_PREFIX_ . 'zone` z ON cz.`id_zone` = z.`id_zone`
    ...
    {
        return Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ . 'delivery` WHERE `id_carrier` = ' .
         (int) ($this->id) . ' AND (`id_' . $rangeTable . '` IS NOT NULL OR `id_' .
         $rangeTable . '` = 0)');
    }
    /**
    ...
    public function addDeliveryPrice ($priceList)
    {
        ...
        'INSERT INTO `' . _DB_PREFIX_ .
         'delivery` (`id_range_price`, `id_range_weight`, `id_carrier`, `id_zone`, `price`) VALUES ' .
         $priceList);
    }
    /**
    ...
    public function copyCarrierData ($oldId)
    {
        ...
        foreach ($res as $val) {
			...
			VALUES (' . (int) ($this->id) . ',' . (float) ($val['delimiter1']) . ',' .
             (float) ($val['delimiter2']) . ')');
            ...
            foreach ($res2 as $val2)
				...
				VALUES (' . (int) ($this->id) . ',' . (int) ($maxRangePrice) . ',NULL,' .
                 (int) ($val2['id_zone']) . ',' . (float) ($val2['price']) . ')');
        }
        ...
        foreach ($res as $val) {
            ...
            foreach ($res2 as $val2)
                ...
                '
				INSERT INTO `' . _DB_PREFIX_ . 'delivery` (`id_carrier`,`id_range_price`,`id_range_weight`,`id_zone`, `price`)
				VALUES (' . (int) ($this->id) . ',NULL,' . (int) ($maxRangeWeight) . ',' .
                 (int) ($val2['id_zone']) . ',' . (float) ($val2['price']) . ')');
        }
        // Copy existing zones
        $res = Db::getInstance()->ExecuteS(
        ...
        if ((int) (Configuration::get('PS_CARRIER_DEFAULT')) ==
         $oldId)
            Configuration::updateValue('PS_CARRIER_DEFAULT', (int) ($this->id));
    }
    /**
