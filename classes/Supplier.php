...
class SupplierCore extends ObjectModel
{
    ...
    protected $fieldsSizeLang = array('meta_title' => 128, 
    'meta_description' => 255, 'meta_keywords' => 255);
    ...
    'fields' => array('link_rewrite' => array('sqlId' => 'link_rewrite')));
    ...
    public function getTranslationsFieldsChild ()
    {
        ...
        array('description', 'meta_title', 'meta_keywords', 'meta_description'));
    }
    /**
     * Return suppliers
     *
     ...
     */
    public static function getSuppliers ($getNbProducts = false, $id_lang = 0, 
    $active = true, $p = false, $n = false, $all_groups = false)
    {
        if (! $id_lang)
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'supplier_lang` sl ON (s.`id_supplier` = sl.`id_supplier` AND sl.`id_lang` = ' .
         (int) ($id_lang) . ')
        ...
        if ($getNbProducts) {
            ...
            foreach ($suppliers as $key => $supplier) {
                $sql = '
                 ...
                 ($active ? ' AND p.`active` = 1' : '') . ($all_groups ? '' : '
					AND p.`id_product` IN (
        ...
        for ($i = 0; $i < sizeof($suppliers); $i ++)
            if ((int) (Configuration::get('PS_REWRITING_SETTINGS')))
                $suppliers[$i]['link_rewrite'] = Tools::link_rewrite(
                $suppliers[$i]['name'], false);
            else
    ...
    public static function getNameById ($id_supplier)
    {
        if (! isset(self::$cacheName[$id_supplier]))
            ...
            '
			SELECT `name` FROM `' . _DB_PREFIX_ . 'supplier` WHERE `id_supplier` = ' .
             (int) ($id_supplier));
    ...
    $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $active_category = true)
    {
        ...
        $sql = '
		SELECT p.*, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, s.`name` AS supplier_name, tl.`name` AS tax_name, t.`rate`, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL ' . (Validate::isUnsignedInt(
