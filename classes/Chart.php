...
class ChartCore
{
    ...
    public static function init ()
    {
        if (! self::$poolId) {
		    ...
		    <script type="text/javascript" src="' . PS_BASE_URI .
             'js/jquery/jquery.flot.min.js"></script>';
        }
    }
    ...
    public function display ()
    {
        $options = '';
        if ($this->timeMode) {
            $options = 'xaxis:{mode:"time",timeformat:\'' .
             addslashes($this->format) . '\',min:' . $this->from . '000,max:' .
             $this->to . '000}';
        ...
        if (count($jsCurves))
            echo '
			<div id="flot' . self::$poolId . '" style="width:' . $this->width .
             'px;height:' . $this->height . 'px"></div>
			<script type="text/javascript">
				$(function () {
					$.plot($(\'#flot' . self::$poolId . '\'), [' . implode(',', $jsCurves) .
             '], {' . $options . '});
				});
			</script>';
        else
            echo ErrorFacade::Display(PS_ERROR_UNDEFINED, 
            'No values for this chart.');
    }
}
class Curve
{
    ...
    public function getValues ($timeMode = false)
    {
        ...
        foreach ($this->values as $key => $value)
            $string .= '[' . addslashes((string) $key) . ($timeMode ? '000' : '') .
             ',' . (float) $value . '],';
