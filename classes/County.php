...
class CountyCore extends ObjectModel
{
    ...
    'id_state' => 'isUnsignedId', 'active' => 'isBool');
    ...
    'fields' => array('id_state' => array('xlink_resource' => 'states')));
    ...
    public function delete ()
    {
        ...
        return (County::deleteZipCodeByIdCounty($id) and
         TaxRule::deleteTaxRuleByIdCounty($id));
    }
    ...
    public function isZipCodeRangePresent ($zip_codes)
    {
        ...
        return ($res or County::isZipCodePresent($from) or
         County::isZipCodePresent($to));
    }
    public function isZipCodePresent ($zip_code)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ . 'state` s ON (s.`id_state` = c.`id_state`)
		WHERE
		(`from_zip_code` <= ' . (int) $zip_code . ' AND `to_zip_code` >= ' .
         (int) $zip_code . ')
		OR
