...
class MetaCore extends ObjectModel
{
    ...
    protected $fieldsSizeLang = array('title' => 128, 'description' => 255, 
    'keywords' => 255, 'url_rewrite' => 255);
    protected $fieldsValidateLang = array('title' => 'isGenericName', 
    'description' => 'isGenericName', 'keywords' => 'isGenericName', 
    'url_rewrite' => 'isLinkRewrite');
    ...
    public function getTranslationsFieldsChild ()
    {
        ...
        array('title', 'description', 'keywords', 'url_rewrite'));
    }
    public static function getPages ($excludeFilled = false, $addPage = false)
    {
        ...
        $exludePages = array('category', 'changecurrency', 'cms', 
        'footer', 'header', 'images.inc', 'init', 'pagination', 'product', 
        'product-sort', 'statistics');
        foreach ($files as $file)
            if (preg_match('/^[a-z0-9_.-]*\.php$/i', $file) and
             ! in_array(str_replace('.php', '', $file), $exludePages))
        ...
        if ($excludeFilled) {
            ...
            foreach ($metas as $meta)
                if (in_array($meta['page'], $selectedPages))
                    unset(
                    $selectedPages[array_search($meta['page'], $selectedPages)]);
        }
        // Add selected page
        if ($addPage) {
    ...
    public static function getMetaByPage ($page, $id_lang)
    {
		...
		FROM ' . _DB_PREFIX_ . 'meta m
		LEFT JOIN ' . _DB_PREFIX_ . 'meta_lang ml on (m.id_meta = ml.id_meta)
		WHERE m.page = \'' . pSQL($page) . '\' AND ml.id_lang = ' .
         (int) ($id_lang));
    }
    ...
    public static function getEquivalentUrlRewrite ($new_id_lang, $id_lang, 
    $url_rewrite)
    {
		...
		WHERE id_meta = (
			SELECT id_meta
			FROM `' . _DB_PREFIX_ . 'meta_lang`
			WHERE url_rewrite = \'' . pSQL($url_rewrite) . '\' AND id_lang = ' .
         (int) ($id_lang) . '
		)
