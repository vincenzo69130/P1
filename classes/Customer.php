...
class CustomerCore extends ObjectModel
{
    ...
    protected $fieldsRequired = array('lastname', 'passwd', 'firstname', 
    'email');
    protected $fieldsSize = array('lastname' => 32, 'passwd' => 32, 
    'firstname' => 32, 'email' => 128, 'note' => 65000);
    protected $fieldsValidate = array('secure_key' => 'isMd5', 
    'lastname' => 'isName', 'firstname' => 'isName', 'email' => 'isEmail', 
    'passwd' => 'isPasswd', 'id_gender' => 'isUnsignedId', 
    'birthday' => 'isBirthDate', 'newsletter' => 'isBool', 'optin' => 'isBool', 
    'active' => 'isBool', 'note' => 'isCleanHtml', 'is_guest' => 'isBool');
    protected $webserviceParameters = array(
    'fields' => array('id_default_group' => array('xlink_resource' => 'groups'), 
    'newsletter_date_add' => array(), 'ip_registration_newsletter' => array(), 
    'last_passwd_gen' => array('setter' => null), 
    'secure_key' => array('setter' => null), 'deleted' => array(), 
    ...
    public function getFields ()
    {
        ...
        $fields['ip_registration_newsletter'] = pSQL(
    ...
    {
        $this->birthday = (empty($this->years) ? $this->birthday : (int) ($this->years) .
         '-' . (int) ($this->months) . '-' . (int) ($this->days));
        $this->secure_key = md5(uniqid(rand(), true));
        $this->last_passwd_gen = date('Y-m-d H:i:s', 
        strtotime('-' . Configuration::get('PS_PASSWD_TIME_FRONT') . 'minutes'));
        if (empty($this->id_default_group))
            $this->id_default_group = 1;
            /* Can't create a guest customer, if this feature is disabled */
        if ($this->is_guest and ! Configuration::get(
        'PS_GUEST_CHECKOUT_ENABLED'))
            return false;
        if (! parent::add($autodate, $nullValues))
            return false;
        $row = array('id_customer' => (int) ($this->id), 
        'id_group' => (int) $this->id_default_group);
        return Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'customer_group', 
        $row, 'INSERT');
    }
    ...
    public function delete ()
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ . 'customer_group` WHERE `id_customer` = ' .
         (int) ($this->id));
    ...
    {
        if (! Validate::isEmail($email) or
         ($passwd and ! Validate::isPasswd($passwd)))
		...
		' . (isset($passwd) ? 'AND `passwd` = \'' . md5(
        pSQL(_COOKIE_KEY_ . $passwd)) . '\'' : '') . '
		AND `deleted` = 0
		AND `is_guest` = 0');
        if (! $result)
            return false;
     ...
     */
    public static function customerExists ($email, $return_id = false, 
    $ignoreGuest = true)
    {
    ...
    public function cantChangeemail ()
    {
		...
		FROM `' . _DB_PREFIX_ . 'customer`
		WHERE `email` = \'' . pSQL($this->email) . '\' AND `id_customer` != ' .
         (int) ($this->id));
        return $result['total'];
    }
    /**
    ...
    public static function customerHasAddress ($id_customer, $id_address)
    {
        if (! array_key_exists($id_customer, self::$_customerHasAddress)) {
            self::$_customerHasAddress[$id_customer] = (bool) Db::getInstance(
    ...
    public function getAddresses ($id_lang)
    {
		...
		WHERE `id_lang` = ' . (int) ($id_lang) . ' AND `id_customer` = ' .
         (int) ($this->id) . ' AND a.`deleted` = 0');
    }
    /**
    ...
    public function getConnections ($nb = 10)
    {
		...
		WHERE `id_guest` IN (SELECT `id_guest` FROM `' . _DB_PREFIX_ .
         'guest` WHERE `id_customer` = ' . (int) ($this->id) . ')
	...
	*/
    ...
    public function addGroups ($groups)
    {
        foreach ($groups as $group) {
            $row = array('id_customer' => (int) ($this->id), 
            'id_group' => (int) ($group));
            Db::getInstance()->AutoExecute(_DB_PREFIX_ . 'customer_group', $row, 
            'INSERT');
        }
    }
    ...
    public static function getDefaultGroupId ($id_customer)
    {
        if (! isset(self::$_defaultGroupId[(int) ($id_customer)]))
            ...
            'SELECT `id_default_group` FROM `' . _DB_PREFIX_ .
             'customer` WHERE `id_customer` = ' . (int) $id_customer);
    ...
    public static function getCurrentCountry ($id_customer)
    {
        ...
        if (! $cart or ! $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')})
            ...
            'SELECT `id_address` FROM `' . _DB_PREFIX_ .
             'address` WHERE `id_customer` = ' . (int) ($id_customer) .
             ' AND `deleted` = 0 ORDER BY `id`'));
        else
            $id_address = $cart->{Configuration::get('PS_TAX_ADDRESS_TYPE')};
        $ids = Address::getCountryAndState($id_address);
        return (int) ($ids['id_country'] ? $ids['id_country'] : Configuration::get(
        'PS_COUNTRY_DEFAULT'));
    }
    ...
    public function transformToCustomer ($id_lang, $password = NULL)
    {
        ...
        if ($this->update()) {
            $vars = array('{firstname}' => $this->firstname, 
            '{lastname}' => $this->lastname, '{email}' => $this->email, 
            '{passwd}' => $password);
            Mail::Send((int) $id_lang, 'guest_to_customer', 
            Mail::l(
            'Your guest account has been transformed to customer account'), $vars, 
    ...
    public static function printNewsIcon ($id_customer, $tr)
    {
        ...
        echo '<a href="index.php?tab=AdminCustomers&id_customer=' .
         (int) ($customer->id) . '&changeNewsletterVal&token=' .
    ...
    public static function printOptinIcon ($id_customer, $tr)
    {
        ...
        if (! Validate::isLoadedObject($customer))
            die(Tools::displayError('Invalid objects'));
        echo '<a href="index.php?tab=AdminCustomers&id_customer=' .
         (int) ($customer->id) . '&changeOptinVal&token=' .
         Tools::getAdminTokenLite('AdminCustomers') . '">' .
