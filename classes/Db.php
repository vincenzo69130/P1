...
abstract class DbCore
{
    ...
    'password' => _DB_PASSWD_, 'database' => _DB_NAME_)); /* MySQL Master server */
	...
	*/
    ...
    public static function getInstance ($master = 1)
    {
        if ($master or ($nServers = sizeof(self::$_servers)) == 1)
            ...
            $idServer = ($nServers > 2 and
             ($id = ++ self::$_idServer % (int) $nServers) !== 0) ? $id : 1;
        if (! isset(self::$_instance[$idServer]))
            self::$_instance[(int) ($idServer)] = new MySQL(
            self::$_servers[(int) ($idServer)]['server'], 
            self::$_servers[(int) ($idServer)]['user'], 
            self::$_servers[(int) ($idServer)]['password'], 
            self::$_servers[(int) ($idServer)]['database']);
     ...
     */
    public function autoExecute ($table, $values, $type, $where = false, 
    $limit = false, $use_cache = 1)
    {
        if (! sizeof($values))
            return true;
     ...
     * @return mixed|boolean SQL query result
     */
    public function autoExecuteWithNullValues ($table, $values, $type, 
    $where = false, $limit = false)
    {
        if (! sizeof($values))
            return true;
        if (strtoupper($type) == 'INSERT') {
            ...
            $query = rtrim($query, ',') . ') VALUES (';
            foreach ($values as $key => $value)
                $query .= (($value === '' or $value === NULL) ? 'NULL' : '\'' .
                 $value . '\'') . ',';
            $query = rtrim($query, ',') . ')';
            if ($limit)
                $query .= ' LIMIT ' . (int) ($limit);
...
function pSQL ($string, $htmlOK = false)
{
    ...
    if (! is_numeric($string)) {
        ...
        $string = _PS_MYSQL_REAL_ESCAPE_STRING_ ? mysql_real_escape_string(
        $string, $link) : addslashes($string);
        if (! $htmlOK)
