...
class ProfileCore extends ObjectModel
{
    ...
    public static function getProfiles ($id_lang)
    {
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'profile_lang` pl ON (p.`id_profile` = pl.`id_profile` AND `id_lang` = ' .
         (int) ($id_lang) . ')
		ORDER BY `name` ASC');
    }
    /**
    ...
    public function add ($autodate = true, $nullValues = false)
    {
        if (parent::add($autodate, true))
            ...
            'INSERT INTO ' . _DB_PREFIX_ . 'access (SELECT ' . (int) ($this->id) .
             ', id_tab, 0, 0, 0, 0 FROM ' . _DB_PREFIX_ . 'tab)');
        return false;
    }
    public function delete ()
    {
        if (parent::delete())
            return Db::getInstance()->Execute(
            'DELETE FROM `' . _DB_PREFIX_ . 'access` WHERE `id_profile` = ' .
             (int) ($this->id));
        return false;
    }
