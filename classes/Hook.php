...
class HookCore extends ObjectModel
{
    ...
    public static function getModuleFromHook ($id_hook, $id_module)
    {
		...
		FROM `' . _DB_PREFIX_ . 'module` m
		LEFT JOIN `' . _DB_PREFIX_ . 'hook_module` hm ON (hm.id_module = m.id_module)
		WHERE hm.id_hook = ' . (int) ($id_hook) . ' AND m.id_module = ' .
         (int) ($id_module));
    }
    public static function newOrder ($cart, $order, $customer, $currency, 
    $orderStatus)
    {
        return Module::hookExec('newOrder', 
        array('cart' => $cart, 'order' => $order, 'customer' => $customer, 
    ...
    public static function updateOrderStatus ($newOrderStatusId, $id_order)
    {
        ...
        $return = ((int) ($newOS->id) == Configuration::get('PS_OS_PAYMENT')) ? Module::hookExec(
        'paymentConfirm', array('id_order' => (int) ($order->id))) : true;
        $return = Module::hookExec('updateOrderStatus', 
        array('newOrderStatus' => $newOS, 'id_order' => (int) ($order->id))) and
         $return;
        return $return;
    }
    public static function postUpdateOrderStatus ($newOrderStatusId, $id_order)
    {
        $order = new Order((int) ($id_order));
        $newOS = new OrderState((int) ($newOrderStatusId), $order->id_lang);
        $return = Module::hookExec('postUpdateOrderStatus', 
        array('newOrderStatus' => $newOS, 'id_order' => (int) ($order->id)));
        return $return;
    }
    /**
    ...
    {
        return Module::hookExec('updateQuantity', 
        array('product' => $product, 'order' => $order));
    }
    public static function productFooter ($product, $category)
    {
        return Module::hookExec('productFooter', 
        array('product' => $product, 'category' => $category));
    }
    public static function productOutOfStock ($product)
    {
        return Module::hookExec('productOutOfStock', 
    ...
    {
        return Module::hookExec('updateProductAttribute', 
    ...
    {
        if (Validate::isUnsignedId($id_order) and
         Validate::isUnsignedId($id_module)) {
            $params = array();
            $order = new Order((int) ($id_order));
            $currency = new Currency((int) ($order->id_currency));
            if (Validate::isLoadedObject($order)) {
                ...
                $params['objOrder'] = $order;
                $params['currencyObj'] = $currency;
                return Module::hookExec('paymentReturn', $params, 
                (int) ($id_module));
            }
        }
        return false;
    ...
    public static function PDFInvoice ($pdf, $id_order)
    {
        if (! is_object($pdf) or ! Validate::isUnsignedId($id_order))
            return false;
        return Module::hookExec('PDFInvoice', 
        array('pdf' => $pdf, 'id_order' => $id_order));
    }
    ...
    public static function updateCarrier ($id_carrier, $carrier)
    {
        if (! Validate::isUnsignedId($id_carrier) or ! is_object($carrier))
            return false;
        return Module::hookExec('updateCarrier', 
        array('id_carrier' => $id_carrier, 'carrier' => $carrier));
    }
}
