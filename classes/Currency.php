...
class CurrencyCore extends ObjectModel
{
    ...
    /** @var int bool active */
    public $active;
    protected $fieldsRequired = array('name', 'iso_code', 'sign', 
    'conversion_rate', 'format', 'decimals');
    protected $fieldsSize = array('name' => 32, 'iso_code' => 3, 
    'iso_code_num' => 3, 'sign' => 8);
    protected $fieldsValidate = array('name' => 'isGenericName', 
    'iso_code' => 'isLanguageIsoCode', 'iso_code_num' => 'isNumericIsoCode', 
    'blank' => 'isInt', 'sign' => 'isGenericName', 'format' => 'isUnsignedId', 
    'decimals' => 'isBool', 'conversion_rate' => 'isFloat', 
    'deleted' => 'isBool', 'active' => 'isBool');
    ...
    public function delete ()
    {
        if ($this->id == Configuration::get('PS_CURRENCY_DEFAULT')) {
            ...
            'SELECT `id_currency` FROM ' . _DB_PREFIX_ .
             'currency WHERE `id_currency` != ' . (int) ($this->id) .
             ' AND `deleted` = 0');
            if (! $result['id_currency'])
                return false;
            Configuration::updateValue('PS_CURRENCY_DEFAULT', 
            $result['id_currency']);
        }
        $this->deleted = 1;
        return $this->update();
    ...
    public function getSign ($side = NULL)
    {
        if (! $side)
            return $this->sign;
        $formated_strings = array('left' => $this->sign . ' ', 
        'right' => ' ' . $this->sign);
        $formats = array(
    ...
    public static function getCurrencies ($object = false, $active = 1)
    {
        ...
        if ($object)
            foreach ($tab as $key => $currency)
                $tab[$key] = Currency::getCurrencyInstance(
     ...
     * @deprecated
     **/
    public static function refreshCurrenciesGetDefault ($data, $isoCodeSource, 
    $idCurrency)
    {
        ...
        if ($defaultCurrency->iso_code != $isoCodeSource)
            foreach ($data->currency as $obj)
                if ($defaultCurrency->iso_code == strval($obj['iso_code']))
                    $defaultCurrency->conversion_rate = round(
                    (float) ($obj['rate']), 6);
    ...
    public static function refreshCurrencies ()
    {
        ...
        foreach ($currencies as $currency)
            $currency->refreshCurrency($feed->list, $isoCodeSource, 
            $default_currency);
    }
