...
class importerosc extends ImportModule
{
    public function __construct ()
    {
        ...
        'This module allows you to import from osCommerce to Prestashop.');
    }
    ...
    public function displaySpecificOptions ()
    {
        ...
        'SELECT * FROM  `' . bqSQL($this->prefix) . 'languages`');
        $curencies = $this->ExecuteS(
        'SELECT * FROM  `' . bqSQL($this->prefix) . 'currencies`');
        $html = '<label style=\'width:220px\'>' .
         $this->l('Default osCommerce language  : ') . '</label>
				<div class="margin-form">
				<select name=\'defaultOscLang\'><option value=\'0\'>------</option>';
        foreach ($langagues as $lang)
            $html .= '<option value="' . $lang['languages_id'] . '">' .
             $lang['name'] . '</option>';
        $html .= '</select></div>
				<label style=\'width:220px\'>' .
         $this->l('Default osCommerce currency : ') . '</label>
				<div class=\'margin-form\'>
				<select name=\'defaultOscCurrency\'><option value=\'0\'>------</option>';
        foreach ($curencies as $curency)
            $html .= '<option value=\'' . $curency['currencies_id'] . '\'>' .
             $curency['title'] . '</option>';
        $html .= '</select></div>';
        $html .= '<label style=\'width:220px\'>' . $this->l('Shop url : ') . '</label>
				<div class=\'margin-form\'>
					http://<input type="text" name="shop_url">/
					<p>' .
         $this->l('Specify the root URL of your site oscommerce') . '</p>
				</div>';
        return $html;
    }
    ...
    public function getLangagues ($limit = 0, $nrb_import = 100)
    {
        $identifier = 'id_lang';
        $langagues = $this->ExecuteS(
        'SELECT languages_id as id_lang, name as name, code as iso_code, 1 as active FROM  `' .
         bqSQL($this->prefix) . 'languages` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
    ...
    public function getCurrencies ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									FROM  `' .
         bqSQL($this->prefix) . 'currencies` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
    ...
    public function getZones ($limit = 0, $nrb_import = 100)
    {
        $identifier = 'id_zone';
        $zones = $this->ExecuteS(
        'SELECT geo_zone_id as id_zone, geo_zone_name as name, 1 as active FROM  `' .
         bqSQL($this->prefix) . 'geo_zones` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
    ...
    public function getCountries ($limit = 0, $nrb_import = 100)
    {
        ...
        '
										SELECT countries_id as id_country, countries_name as name, countries_iso_code_2 as iso_code, `' .
         bqSQL($defaultIdLang) .
         '̀  as id_lang,
										1 as id_zone, 0 as id_currency, 1 as contains_states, 1 as need_identification_number, 1 as active, 1 as display_tax_label
										FROM  `' .
         bqSQL($this->prefix) . 'countries` as c  LIMIT ' . (int) ($limit) .
         ' , ' . (int) $nrb_import);
    ...
    public function getStates ($limit = 0, $nrb_import = 100)
    {
        $identifier = 'id_state';
        $states = array(
        0 => array('id_state' => 0, 'id_country' => 0, 'id_zone' => 0, 
        'iso_code' => 999, 'name' => 'osc', 'active' => 0));
    ...
    public function getGroups ()
    {
        ...
        return array(
        1 => array('id_group' => 1, 'price_display_method' => 0, 
    ...
    public function getCustomers ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									FROM  `' . bqSQL($this->prefix) .
         'customers` c
									LEFT JOIN `' .
         bqSQL($this->prefix) . 'customers_info` ci  ON (c.`customers_id` = ci.`customers_info_id`)
									LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
        foreach ($customers as &$customer)
            if (isset($customer['id_gender']) &&
             array_key_exists($customer['id_gender'], $genderMatch))
    ...
    public function getAddresses ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									FROM  `' .
         bqSQL($this->prefix) . 'address_book` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
    ...
    public function getCategories ($limit = 0, $nrb_import = 100)
    {
        ...
        '
         ...
         'categories` c 
									LEFT JOIN `' .
         bqSQL($this->prefix) . 'categories_description` cd ON (c.categories_id = cd.categories_id)
        ...
        foreach ($categories as & $cat) {
            ...
            $cat['images'] = array(
            Tools::getProtocol() . Tools::getValue('shop_url') . '/images/' .
             $cat['images']);
        }
        return $this->autoFormat($categories, $identifier, $keyLanguage, 
        $multiLangFields);
    }
    public function getAttributesGroups ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									FROM  `' .
         bqSQL($this->prefix) . 'products_options` 
									LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
        return $this->autoFormat($countries, $identifier, $keyLanguage, 
        $multiLangFields);
    ...
    public function getAttributes ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									FROM  `' .
         bqSQL($this->prefix) .
         'products_options_values` p
									LEFT JOIN `' .
         bqSQL($this->prefix) . 'products_options_values_to_products_options` po ON (po.products_options_values_id = p.products_options_values_id)
									LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
        return $this->autoFormat($countries, $identifier, $keyLanguage, 
        $multiLangFields);
    ...
    public function getProducts ($limit = 0, $nrb_import = 100)
    {
        ...
        '
									...
									pd.products_description as description, CONCAT(\'' .
         pSQL(Tools::getProtocol()) . pSQL(Tools::getValue('shop_url')) .
         '\/images/\',p.`products_image`) as images,
									(SELECT ptc.categories_id FROM `' .
         bqSQL($this->prefix) . 'products_to_categories` ptc WHERE ptc.`products_id` = p.`products_id` LIMIT 1) as id_category_default,
									p.`products_date_added` as date_add
									FROM	`' . bqSQL($this->prefix) .
         'products` p 
									LEFT JOIN `' .
         bqSQL($this->prefix) . 'products_description` pd ON (p.products_id = pd.products_id)
        ...
        foreach ($products as & $product) {
            ...
            'SELECT `image` FROM `' . bqSQL($this->prefix) .
             'products_images` WHERE products_id = ' .
             (int) $product['id_product']);
            $images = array();
            foreach ($result as $res)
                $images[] = Tools::getProtocol() . Tools::getValue('shop_url') .
                 '/images/' . $res['image'];
            ...
            $product['id_category_default'] => $product['id_product']));
        }
        return $this->autoFormat($products, $identifier, $keyLanguage, 
        $multiLangFields);
    }
    public function getProductsCombination ($limit = 0, $nrb_import = 100)
    {
        ...
        '
										...
										FROM  `' .
         bqSQL($this->prefix) . 'products_attributes` LIMIT ' . (int) ($limit) .
         ' , ' . (int) $nrb_import);
        foreach ($combinations as & $combination) {
            $combination['association'] = array(
            'product_attribute_combination' => array(
            $combination['options_values_id'] => $combination['id_product_attribute']));
    ...
    public function getManufacturers ($limit = 0, $nrb_import = 100)
    {
        ...
        '
										...
										FROM  `' .
         bqSQL($this->prefix) . 'manufacturers` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
    ...
    public function getOrdersStates ($limit = 0, $nrb_import = 100)
    {
        ...
        '
         ...
         'orders_status`
									LIMIT ' .
         (int) ($limit) . ' , ' . (int) $nrb_import); //IF(`public_flag` = 0, 1, 0) as hidden
        return $this->autoFormat($ordersStates, $identifier, 
        $keyLanguage, $multiLangFields);
    }
    public function getOrders ($limit = 0, $nrb_import = 100)
    {
        $orders = array();
        $addresses = $this->ExecuteS(
        'SELECT customers_id as id_customer, address_book_id as id_address FROM  `' .
         bqSQL($this->prefix) . 'address_book` GROUP BY customers_id');
        ...
        '
								SELECT orders_id as id_cart, ' .
         $psCarrierDefault .
         ' as id_carrier, 1 as id_lang, currency as id_currency, customers_id as id_customer, payment_method as payment, 1 as valid,
								date_purchased as date_add, last_modified as date_upd
								FROM  `' .
         bqSQL($this->prefix) . 'orders` LIMIT ' . (int) ($limit) . ' , ' .
         (int) $nrb_import);
        foreach ($orders as $key => $order) {
            $orders[$key]['id_currency'] = (array_key_exists(
            $order['id_currency'], $psCurrency) ? $psCurrency[$order['id_currency']] : 0);
            $orders[$key]['id_address_delivery'] = (array_key_exists(
            $order['id_customer'], $matchAddresses) ? $matchAddresses[$order['id_customer']] : 0);
            $orders[$key]['id_address_invoice'] = (array_key_exists(
            $order['id_customer'], $matchAddresses) ? $matchAddresses[$order['id_customer']] : 0);
            $orders[$key]['total_paid'] = $this->getValue(
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
             ' AND class=\'ot_total\'');
            $orders[$key]['total_paid_real'] = $this->getValue(
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
             ' AND class=\'ot_total\'');
            $orders[$key]['total_products'] = $this->getValue(
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
            ...
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
             ' AND class=\'ot_tax\'');
            $orders[$key]['total_products_wt'] = $this->getValue(
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
             ' AND class=\'ot_total\'') - $tax;
            $orders[$key]['total_shipping'] = $this->getValue(
            'SELECT value FROM `' . bqSQL($this->prefix) .
             'orders_total` WHERE `orders_id` = ' . (int) $order['id_cart'] .
             ' AND class=\'ot_shipping\'');
            ...
            '
														...
														FROM  `' .
             bqSQL($this->prefix) . 'orders_products` WHERE `orders_id` = ' .
             (int) $order['id_cart']);
            ...
            '
														...
														FROM  `' .
             bqSQL($this->prefix) . 'orders_status_history` WHERE `orders_id` = ' .
             (int) $order['id_cart']);
        }
        return $orders;
    }
    private function autoFormat ($items, $identifier, $keyLanguage = NULL, 
    $multiLangFields = array())
    {
        $array = array();
        foreach ($items as $item)
            if (sizeof($multiLangFields) && is_array($multiLangFields) &&
             isset($array[$item[$identifier]][$multiLangFields[0]]))
                foreach ($multiLangFields as $key)
                    $array[$item[$identifier]][$key][$item[$keyLanguage]] = $item[$key];
            else
                foreach ($item as $key => $value)
                    if (sizeof($multiLangFields) and
                     in_array($key, $multiLangFields))
                        $array[$item[$identifier]][$key] = array(
                        $item[$keyLanguage] => $value);
        ...
        if ($result && ! empty($result['passwd_' . $this->name])) {
            if ($this->checkPwd($passwd, $result['passwd_' . pSQL($this->name)])) {
				...
				UPDATE `' . _DB_PREFIX_ .
                 'customer`
				SET `passwd` = \'' .
                 pSQL($ps_passwd) . '\', `passwd_' . bqSQL($this->name) .
                 '` = \'\'
				WHERE `' .
                 _DB_PREFIX_ . 'customer`.`id_customer` =' .
                 (int) $result['id_customer'] . ' LIMIT 1');
            }
        }
    }
    private function checkPwd ($passwd, $encrypt_pwd)
    {
        ...
        if (preg_match('/^[A-Z0-9]{32}\:[A-Z0-9]{2}$/i', $encrypt_pwd) ===
         1) {
            //salt
    ...
    public function displayConfigConnector ()
    {
        $content = '<label>' . $this->l('Server') . ' : </label>
						<div class="margin-form">
							...
							<p>' .
         $this->l('(eg : mysql.mydomain.com)') . '</p>
						</div>
					...
					<label>' . $this->l('Password') . ' : </label>
						<div class="margin-form">
							...
							<p>' .
         $this->l('(password can be empty)') . '</p>
						</div>
					...
					<label>' .
         $this->l('Database prefix') .
         ' : </label>
						<div class="margin-form" style="">
							<input type="text" name="prefix" id="prefix" value="">
							<p>' .
         $this->l(
        '(The prefix is optional. If all of your database tables start with "pref_", your prefix is "pref_")') . '</p>
						</div>';
        return $content;
    }
}
