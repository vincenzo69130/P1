...
if (! defined('_PS_USE_SQL_SLAVE_'))
    ...
    // dir = admin-dev
if (_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . $_POST['dir'] !==
 realpath(_PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . $_POST['dir']))
    die('not allowed');
...
$adminObj->ajax = true;
{
    if (is_object($adminObj)) {
        if ($adminObj->checkToken()) {
            ...
            'ajaxProcess' . Tools14::toCamelCase($action)))
            ...
            if (! empty($action) and method_exists($adminObj, 
            'displayAjax' . Tools14::toCamelCase($action)))
