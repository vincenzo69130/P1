...
class BackupCore
{
    ...
    public function getRealBackupPath ($filename = NULL)
    {
        ...
        if (! empty($this->customBackupDir)) {
            $backupDir = str_replace(_PS_ADMIN_DIR_ . self::$backupDir, 
            _PS_ADMIN_DIR_ . $this->customBackupDir, $backupDir);
    ...
    public static function getBackupPath ($filename)
    {
        ...
        if ($backupfile === false or
         strncmp($backupdir, $backupfile, strlen($backupdir)) != 0)
    ...
    {
        $adminDir = __PS_BASE_URI__ .
         substr($_SERVER['SCRIPT_NAME'], strlen(__PS_BASE_URI__));
    ...
    public function delete ()
    {
        if (! $this->id || ! unlink($this->id)) {
            $this->error = Tools::displayError('Error deleting') . ' ' . ($this->id ? '"' .
             $this->id . '"' : Tools::displayError('Invalid ID'));
            return false;
        }
        return true;
    }
    ...
    {
        if (_DB_TYPE_ !== 'MySQL') {
            $this->error = Tools::displayError(
            'Sorry, backup currently only supports MySQL database types. You are using') .
             ' "' . _DB_TYPE_ . '"';
            return false;
        }
        if (! $this->psBackupAll)
            $ignore_insert_table = array(_DB_PREFIX_ . 'connections', 
            _DB_PREFIX_ . 'connections_page', _DB_PREFIX_ . 'connections_source', 
            _DB_PREFIX_ . 'guest', _DB_PREFIX_ . 'statssearch');
        else
            $ignore_insert_table = array();
             // Generate some random number, to make it extra hard to guess backup file names
        $rand = dechex(
        mt_rand(0, min(0xffffffff, mt_getrandmax())));
        $date = time();
        ...
        if ($fp === false) {
            echo Tools::displayError('Unable to create backup file') . ' "' .
        ...
        foreach ($tables as $table) {
            ...
            if (strlen($table) < strlen(_DB_PREFIX_) ||
             strncmp($table, _DB_PREFIX_, strlen(_DB_PREFIX_)) != 0)
                continue;
                 // Export the table schema
            $schema = Db::getInstance()->ExecuteS(
            'SHOW CREATE TABLE `' . $table . '`');
            if (count($schema) != 1 || ! isset($schema[0]['Table']) ||
             ! isset($schema[0]['Create Table'])) {
                fclose($fp);
                $this->delete();
                echo Tools::displayError(
                'An error occurred while backing up. Unable to obtain the schema of') .
                 ' "' . $table;
                return false;
            }
            ...
            if ($this->psBackupDropTable)
                fwrite($fp, 
                'DROP TABLE IF EXISTS `' . $schema[0]['Table'] . '`;' . "\n");
            fwrite($fp, $schema[0]['Create Table'] . ";\n\n");
            if (! in_array($schema[0]['Table'], $ignore_insert_table)) {
                $data = Db::getInstance()->ExecuteS(
                'SELECT * FROM `' . $schema[0]['Table'] . '`', false);
