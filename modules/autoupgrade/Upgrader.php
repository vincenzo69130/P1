...
class UpgraderCore
{
            ...
            if ($force || ($last_check <
             time() - (3600 * Upgrader::DEFAULT_CHECK_VERSION_DELAY_HOURS))) {
                ...
                if ($feed = @simplexml_load_file($this->rss_version_link)) {
                    ...
                    $config_last_version = array('name' => $this->version_name, 
                    'num' => $this->version_num, 'link' => $this->link, 
                    'md5' => $this->md5, 'autoupgrade' => $this->autoupgrade, 
                    'autoupgrade_module' => $this->autoupgrade_module, 
                    'changelog' => $this->changelog, 'desc' => $this->desc);
                    if (class_exists('Configuration')) {
                        Configuration::updateValue('PS_LAST_VERSION', 
                        serialize($config_last_version));
                        Configuration::updateValue('PS_LAST_VERSION_CHECK', 
                        time());
                    }
                }
            } else {
                $last_version_check = @unserialize(
                Configuration::get('PS_LAST_VERSION'));
    ...
    public function getChangedFilesList ()
    {
        if (count($this->changed_files) == 0) {
            $checksum = @simplexml_load_file(
            $this->rss_md5file_link_dir . _PS_VERSION_ . '.xml');
            if ($checksum === false)
                return false;
            else
        ...
        foreach ($node as $key => $child) {
            if (is_object($child) && $child->getName() == 'dir') {
                ...
                if (is_object($child) && $child->getName() == 'md5file') {
                    ...
                    $fullpath = _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR .
                     $relative_path;
                    $fullpath = str_replace('ps_root_dir', _PS_ROOT_DIR_, 
                    $fullpath);
                    // replace default admin dir by current one 
                    $fullpath = str_replace(
                    _PS_ROOT_DIR_ . '/admin', _PS_ADMIN_DIR_, $fullpath);
                    if (! file_exists($fullpath))
                        $this->addMissingFile($relative_path);
                    else 
                        if (! $this->compareChecksum($fullpath, (string) $child))
