...
if (! version_compare(_PS_VERSION_, '1.4.5.0', '<') &&
 file_exists(_PS_ROOT_DIR_ . '/classes/Upgrader.php'))
...
class AdminSelfUpgrade extends AdminSelfTab
{
     ...
     */
    public $ajaxParams = array(// autoupgrade options
'dontBackupImages', 'keepDefaultTheme', 
    'keepTrad', 'manualMode', 'desactivateCustomModule', //
'backupDbFilename', 
    'backupFilesFilename');
  ...
  * value = the next step you want instead
	...
	*/
    ...
    public function checkToken ()
    {
        ...
        if ($this->ajax)
            return ($_COOKIE['autoupgrade'] ==
             $this->encrypt($_COOKIE['id_employee']));
        else
    ...
    public function createCustomToken ()
    {
        ...
        $cookiePath = __PS_BASE_URI__ . str_replace($this->prodRootDir, '', 
        trim($this->adminDir, DIRECTORY_SEPARATOR));
        setcookie('id_employee', $id_employee, time() + 3600, $cookiePath);
        setcookie('id_tab', $this->id, time() + 3600, $cookiePath);
        setcookie('autoupgrade', $this->encrypt($id_employee), time() + 3600, 
        $cookiePath);
        return false;
    }
    ...
    $htmlentities = TRUE)
    {
        if (version_compare(_PS_VERSION_, '1.4.3.0', '<')) {
            ...
            return SelfModule::findTranslation('autoupgrade', 
            $string, 'AdminSelfUpgrade');
        } else
    ...
    private function _setFields ()
    {
        ...
        'title' => $this->l('Also save images'), 'cast' => 'intval', 
        'validation' => 'isBool', 'type' => 'bool', 
        'desc' => $this->l(
        'You can exclude the image directory from backup if you already saved it by another method (not recommended)'));
        $this->_fieldsAutoUpgrade['PS_AUTOUP_KEEP_DEFAULT_THEME'] = array(
        'title' => $this->l('Keep theme "prestashop"'), 'cast' => 'intval', 
        'validation' => 'isBool', 'type' => 'bool', 
        'desc' => $this->l(
        'If you have customized PrestaShop default theme, you can protect it from upgrade (not recommended)'));
        $this->_fieldsAutoUpgrade['PS_AUTOUP_KEEP_TRAD'] = array(
        'title' => $this->l('Keep translations'), 'cast' => 'intval', 
        'validation' => 'isBool', 'type' => 'bool', 
        'desc' => $this->l('If set to yes, you will keep all your translations'));
        $this->_fieldsAutoUpgrade['PS_AUTOUP_CUSTOM_MOD_DESACT'] = array(
        'title' => $this->l('Deactivate custom modules'), 'cast' => 'intval', 
        'validation' => 'isBool', 'type' => 'bool', 
        'desc' => $this->l(
        'If you don\'t deactivate your modules, you can have some compatibility problems and the Modules page might not load correctly.'));
        // allow manual mode only for dev
        if (defined('_PS_MODE_DEV_') and _PS_MODE_DEV_)
            $this->_fieldsAutoUpgrade['PS_AUTOUP_MANUAL_MODE'] = array(
            'title' => $this->l('Manual mode'), 'cast' => 'intval', 
            'validation' => 'isBool', 'type' => 'bool', 
            'desc' => $this->l('Check this if you want to stop after each step'));
        if (defined('_PS_ALLOW_UPGRADE_UNSTABLE_') and
         _PS_ALLOW_UPGRADE_UNSTABLE_ and function_exists('svn_checkout')) {
            $this->_fieldsAutoUpgrade['PS_AUTOUP_USE_SVN'] = array(
            'title' => $this->l('Use Subversion'), 'cast' => 'intval', 
            'validation' => 'isBool', 'type' => 'bool', 
            'desc' => $this->l(
            'check this if you want to use unstable svn instead of official release'));
        }
    }
    ...
    public function init ()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.4.5.0', '<')) {
            ...
            if (! class_exists('ConfigurationTest', false) and
             class_exists('ConfigurationTestCore'))
                eval('class ConfigurationTest extends ConfigurationTestCore{}');
        }
        if (ConfigurationTest::test_dir($this->prodRootDir, true))
            $this->rootWritable = true;
        if (! in_array($this->action, 
        array('upgradeFile', 'upgradeDb', 'upgradeComplete', 'rollback', 
        'restoreFiles', 'restoreDb'))) {
        ...
        if (defined('_PS_ALLOW_UPGRADE_UNSTABLE_') and
         _PS_ALLOW_UPGRADE_UNSTABLE_ and function_exists('svn_checkout')) {
            if (version_compare(_PS_VERSION_, '1.4.5.0', '<') or
             class_exists('Configuration', false))
        ...
        $this->autoupgradePath = $this->adminDir .
         DIRECTORY_SEPARATOR . $this->autoupgradeDir;
        if (! file_exists($this->autoupgradePath))
            if (! @mkdir($this->autoupgradePath, 0777))
                $this->_errors[] = Tools::displayError(
                sprintf($this->l('unable to create directory %s'), 
                $this->autoupgradePath));
        $latest = $this->autoupgradePath . DIRECTORY_SEPARATOR . 'latest';
        if (! file_exists($latest))
            if (! @mkdir($latest, 0777))
                $this->_errors[] = Tools::displayError(
                sprintf($this->l('unable to create directory %s'), $latest));
        if (class_exists('Configuration', false)) {
            $time = time();
            $this->backupDbFilename = Configuration::get(
            ...
            $this->backupFilesFilename = Configuration::get(
            'UPGRADER_BACKUPFILES_FILENAME');
            if (! file_exists($this->backupFilesFilename)) {
                $this->backupFilesFilename = $this->autoupgradePath .
                 DIRECTORY_SEPARATOR . 'backupfile-' . date('Y-m-d') . '-' .
                 $time . '.zip';
        ...
        /* option */
        if (class_exists('Configuration', false)) {
            $this->dontBackupImages = ! Configuration::get(
            'PS_AUTOUP_DONT_SAVE_IMAGES');
            $this->keepDefaultTheme = Configuration::get(
            'PS_AUTOUP_KEEP_DEFAULT_THEME');
            $this->keepTrad = Configuration::get('PS_AUTOUP_KEEP_TRAD');
            $this->manualMode = Configuration::get('PS_AUTOUP_MANUAL_MODE');
            $this->desactivateCustomModule = Configuration::get(
    ...
    {
        return $this->autoupgradePath . DIRECTORY_SEPARATOR .
         $this->destDownloadFilename;
    }
    ...
    {
        $this->nextDesc = $this->l(
        'Upgrade process done. Congratulations ! You can now reactive your shop.');
        $this->next = '';
    }
    public function ajaxProcessCheckFilesVersion ()
    {
        ...
        if ($changedFileList === false) {
            ...
            $this->nextParams['msg'] = ($testOrigCore ? $this->l(
            'Core files are ok') : sprintf(
            $this->l('%1$s core files have been modified (%2$s total)'), 
            count($changedFileList['core']), 
            count(
            array_merge($changedFileList['core'], $changedFileList['mail'], 
    ...
    public function ajaxProcessUpgradeNow ()
    {
        ...
        if ($this->useSvn) {
            ...
            $this->nextDesc = $this->l(
            'switching to svn checkout (useSvn set to true)');
        } else {
            $this->next = 'download';
            $this->nextDesc = $this->l(
            'Shop desactivated. Now downloading (this can takes some times )...');
        }
    }
    public function ajaxProcessSvnExport ()
    {
        if ($this->useSvn) {
            ...
            if (svn_export(
            $this->autoupgradePath . DIRECTORY_SEPARATOR . $this->svnDir, 
            $this->latestRootDir)) {
                ...
                $adminDir = str_replace(
                $this->prodRootDir, '', $this->adminDir);
                rename(
                ...
                $this->latestRootDir . DIRECTORY_SEPARATOR . $adminDir);
                // Unsetting to force listing
                unset($this->nextParams['removeList']);
                $this->next = "removeSamples";
                $this->nextDesc = $this->l(
                'Export svn complete. removing sample files...');
                return true;
            } else {
                $this->next = 'error';
    ...
    {
        if (version_compare(_PS_VERSION_, '1.4.5.0', '<') and
         ! class_exists('Tools', false))
        ...
        if (self::ZipExtract($filepath, $destExtract)) {
            ...
            $this->next = "removeSamples";
            $this->nextDesc = $this->l(
            'Extract complete. removing sample files...');
            return true;
        } else {
            $this->next = "error";
            $this->nextDesc = sprintf(
            $this->l('unable to extract %1$s into %2$s ...'), $filepath, 
            $destExtract);
            return true;
        }
    }
            ...
            if ($file[0] != '.') {
                ...
                if (! $this->_skipFile($file, $fullPath, $way)) {
                    if (is_dir($fullPath))
                        ...
                        $this->_listFilesInDir($fullPath, $way));
                    else
                        $list[] = $fullPath;
                } else
                    $list[] = $fullPath;
    ...
    public function _listFilesToUpgrade ($dir)
    {
        ...
        foreach ($allFiles as $file) {
            ...
            if (! $this->_skipFile($file, $fullPath, "upgrade")) {
                ...
                if (is_dir($fullPath))
                    if (strpos($dir . DIRECTORY_SEPARATOR . $file, 'install') ===
                     false)
    ...
    public function ajaxProcessUpgradeFiles ()
    {
        ...
        $filesToUpgrade = @unserialize(
        file_get_contents($this->nextParams['filesToUpgrade']));
        ...
        for ($i = 0; $i < self::$loopUpgradeFiles; $i ++) {
            ...
            if (! $this->upgradeThisFile($file)) {
                ...
                $totalFiles = array_unshift($filesToUpgrade, 
                $file);
                $this->next = 'error';
                $this->nextQuickInfo[] = sprintf(
                $this->l('error when trying to upgrade %s'), $file);
                break;
            } else {
                $this->nextQuickInfo[] = sprintf(
                $this->l('copied %1$s. %2$s files left to upgrade.'), $file, 
                sizeof($filesToUpgrade));
                // @TODO : maybe put several files at the same times ?
                $this->nextDesc = sprintf(
                $this->l('%2$s files left to upgrade.'), $file, 
                sizeof($filesToUpgrade));
            }
        }
        file_put_contents($this->nextParams['filesToUpgrade'], 
        serialize($filesToUpgrade));
        return true;
    }
    public function _modelDo ($method)
    {
        ...
        if (substr($memory_limit, - 1) != 'G' and ((substr($memory_limit, - 1) ==
         'M' and substr($memory_limit, 0, - 1) < 128) or
        ...
        define('INSTALL_PATH', 
        realpath($this->latestRootDir . DIRECTORY_SEPARATOR . 'install'));
        ...
        define('INSTALLER__PS_BASE_URI', 
        substr($_SERVER['REQUEST_URI'], 0, 
        - 1 * (strlen($_SERVER['REQUEST_URI']) - strrpos(
        $_SERVER['REQUEST_URI'], '/')) - strlen(
        substr(dirname($_SERVER['REQUEST_URI']), 
        strrpos(dirname($_SERVER['REQUEST_URI']), '/') + 1))));
        define('INSTALLER__PS_BASE_URI_ABSOLUTE', 
        'http://' . ToolsInstall::getHttpHost(false, true) .
         INSTALLER__PS_BASE_URI);
        // XML Header
        header('Content-Type: text/xml');
        // Switching method
        if (in_array($method, 
        array('doUpgrade', 'createDB', 'checkShopInfos'))) {
            global $logger;
            $logger = new FileLogger();
            $logger->setFilename(
            $this->prodRootDir . DIRECTORY_SEPARATOR . 'log' .
             DIRECTORY_SEPARATOR . @date('Ymd') . '_installation.log');
        }
        switch ($method) {
            case 'checkConfig':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'checkConfig.php');
                die();
                break;
            case 'checkDB':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'checkDB.php');
                break;
            case 'createDB':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'createDB.php');
                break;
            case 'checkMail':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'checkMail.php');
                break;
            case 'checkShopInfos':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'checkShopInfos.php');
                break;
            case 'doUpgrade':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'doUpgrade.php');
                break;
            case 'getVersionFromDb':
                require_once (INSTALL_PATH . DIRECTORY_SEPARATOR . 'xml' .
                 DIRECTORY_SEPARATOR . 'getVersionFromDb.php');
                break;
        }
    }
        ...
        switch ($this->currentParams['upgradeDbStep']) {
            ...
            case '3':
                if (! $this->_modelDo('doUpgrade')) {
                    $this->next = 'error';
                    $this->nextDesc = $this->l(
                    'error during upgrade Db. You may need to restore your database');
                }
                break;
            case '4':
                ...
                $this->nextDesc = $this->l(
                'Way to go ! Upgrade complete. You can now reactivate your shop.');
                break;
        }
    ...
    public function upgradeThisFile ($file)
    {
        ...
        if ($this->_skipFile('', $file, 'upgrade')) {
            ...
            $dest = str_replace($this->latestRootDir, $this->destUpgradePath, 
            $file);
            if (is_dir($file)) {
                ...
                if (! file_exists($dest)) {
                    if (@mkdir($dest))
                        ...
                        $this->next = 'error';
                        $this->nextQuickInfo[] = sprintf(
                        $this->l('error when creating directory %s'), $dest);
                        $this->nextDesc = sprintf(
                        $this->l('error when creating directory %s'), $dest);
                        return false;
                    }
                ...
                if (copy($file, $dest))
                    ...
                    $this->next = 'error';
                    $this->nextQuickInfo[] = sprintf(
                    $this->l('error for copy %1$s in %2$s'), $file, $dest);
                    $this->nextDesc = sprintf(
                    $this->l('error for copy %1$s in %2$s'), $file, $dest);
                    return false;
                }
    ...
    public function ajaxProcessRollback ()
    {
        ...
        $this->nextParams = $this->currentParams;
        if (! empty($this->backupFilesFilename) and
         file_exists($this->backupFilesFilename)) {
            $this->next = 'restoreFiles';
            $this->status = 'ok';
            $this->nextDesc = $this->l('Restoring files...');
        } else {
            if (! empty($this->backupDbFilename) and
             file_exists($this->backupDbFilename)) {
                $this->next = 'restoreDb';
                $this->status = 'ok';
    ...
    public function ajaxProcessRestoreFiles ()
    {
        ...
        if (! empty($this->backupFilesFilename) and
         file_exists($this->backupFilesFilename)) {
            ...
            file_put_contents(
            $this->autoupgradePath . DIRECTORY_SEPARATOR .
             $this->fromArchiveFileList, serialize($fromArchive));
            //$this->_cleanUp($this->prodRootDir.'/');
            $this->nextQuickInfo[] = $this->l(
            ...
            if (self::ZipExtract($filepath, $destExtract)) {
                ...
                // get new file list 
                $this->nextDesc = $this->l(
                'Files restored. Removing files added by upgrade ...');
        ...
        $this->autoupgradePath . DIRECTORY_SEPARATOR . $this->toRemoveFileList)) {
            if (! isset($fromArchive))
                $fromArchive = unserialize(
                file_get_contents($this->fromArchiveFileList));
            $toRemove = array_diff($this->_listFilesInDir($this->prodRootDir), 
            $fromArchive);
            file_put_contents(
            $this->autoupgradePath . DIRECTORY_SEPARATOR .
             $this->toRemoveFileList, serialize($toRemove));
        }
        if (! isset($toRemove))
                ...
                if (in_array($checkFile, $toRemove) &&
                 ...
                 ! $this->_skipFile('', $path . $file, 'upgrade')) {
                    if (file_exists($file) && @unlink($file)) {
                        $this->nextQuickInfo[] = sprintf($this->l('%s removed'), 
                        $file);
                    } else {
                        $this->next = 'error';
                        $this->nextDesc = sprintf(
                        $this->l('error when removing %1$s'), $file);
                        $this->nextQuickInfo[] = sprintf(
                        $this->l('%s not removed'), $file);
                        return false;
                    }
                }
            }
        ...
        $this->autoupgradePath . DIRECTORY_SEPARATOR . $this->toRemoveFileList, 
        serialize($toRemove));
        $this->nextDesc = sprintf($this->l('%s left to remove'), 
        count($toRemove));
        $checkFile = array_shift($toRemove);
        return true;
    }
    ...
    public function ajaxProcessRestoreDb ()
    {
        ...
        switch ($fileext) {
            ...
            case 'bz2':
                if ($fp = bzopen($this->backupDbFilename, 'r')) {
                    while (! feof($fp))
                        $content .= bzread($fp, 
                        filesize($this->backupDbFilename));
                    bzclose($fp);
                }
                break;
            case 'gz':
        ...
        // Configuration::updateValue('UPGRADER_BACKUPDB_FILENAME','');
            ...
            $this->nextQuickInfo[] = $this->l(
            'Nothing to restore (no request found)');
        $this->next = 'rollback';
    ...
    private function _loadDbRelatedClasses ()
    {
        ...
        if (! defined('_PS_MYSQL_REAL_ESCAPE_STRING_'))
            define('_PS_MYSQL_REAL_ESCAPE_STRING_', 
    ...
    public function ajaxProcessBackupDb ()
    {
        ...
        if ($res) {
            ...
            $this->next = 'upgradeFiles';
            $this->nextDesc = sprintf(
            $this->l('Database backup done in %s. Now updating files'), 
            $backup->id);
        }
    ...
    public function ajaxProcessBackupFiles ()
    {
        ...
        if (! isset($this->nextParams['filesForBackup'])) {
            ...
            file_put_contents(
            $this->autoupgradePath . DIRECTORY_SEPARATOR .
             $this->toBackupFileList, serialize($list));
            $this->nextQuickInfo[] = sprintf($this->l('%s Files to backup.'), 
            sizeof($this->toBackupFileList));
            $this->nextParams['filesForBackup'] = $this->toBackupFileList;
            // delete old backup, create new
            if (file_exists($this->backupFilesFilename))
                unlink($this->backupFilesFilename);
            $this->nextQuickInfo[] = sprintf(
            $this->l('backup files initialized in %s'), 
            $this->backupFilesFilename);
        }
        ...
        $this->nextDesc = sprintf(
        $this->l('Backup files in progress. %s files left'), 
        sizeof($filesToBackup));
            ...
            if ($zip->open($this->backupFilesFilename, ZIPARCHIVE::CREATE)) {
                ...
                for ($i = 0; $i < self::$loopBackupFiles; $i ++) {
                    if (sizeof($filesToBackup) <= 0) {
                        ...
                        $this->next = 'backupDb';
                        $this->nextDesc = $this->l(
                        'All files saved. Now backup Database');
                        $this->nextQuickInfo[] = $this->l(
                    ...
                    $archiveFilename = ltrim(
                    str_replace($this->prodRootDir, '', $file), 
                    DIRECTORY_SEPARATOR);
                    // @TODO : maybe put several files at the same times ?
                    if ($zip->addFile($file, 
                    $archiveFilename))
                        $this->nextQuickInfo[] = sprintf(
                        $this->l('%1$s added to archive. %2$s left.'), $file, 
                        sizeof($filesToBackup));
                    else {
                        ...
                        $this->nextDesc = sprintf(
                        $this->l(
                        'error when trying to add %1$s to archive %2$s.'), $file, 
                        $backupFilePath);
                        break;
                    }
                }
                ...
                file_put_contents(
                $this->autoupgradePath . DIRECTORY_SEPARATOR .
                 $this->toBackupFileList, serialize($filesToBackup));
                return true;
            } else {
                $this->next = 'error';
    ...
    private function _removeOneSample ($removeList)
    {
        if (is_array($removeList) and sizeof($removeList) > 0) {
            if (file_exists($removeList[0]) and unlink($removeList[0])) {
                ...
                $this->nextQuickInfo[] = sprintf(
                $this->l('%1$s removed. %2$s items left'), $item, 
                sizeof($removeList));
            } else {
                $this->next = 'error';
                $this->nextParams['removeList'] = $removeList;
                $this->nextQuickInfo[] = sprintf(
                $this->l('error when removing %1$s, %2$s items left'), 
                $removeList[0], sizeof($removeList));
                return false;
            }
        }
        return true;
    ...
    public function ajaxProcessRemoveSamples ()
    {
        ...
        if (! isset(
        $this->currentParams['removeList'])) {
            $this->_listSampleFiles(
            $this->autoupgradePath . '/latest/prestashop/img', 'jpg');
            $this->_listSampleFiles(
            $this->autoupgradePath . '/latest/prestashop/modules/editorial/', 
            'homepage_logo.jpg');
            // @TODO handle this bad thing
            $this->nextQuickInfo[] = sprintf(
            $this->l('Starting to remove %1$s sample files'), 
            sizeof($this->sampleFileList));
        ...
        for ($i = 0; $i < self::$loopRemoveSamples; $i ++) {
            ...
            $resRemove &= $this->_removeOneSample(
            $this->nextParams['removeList']);
            if (! $resRemove)
                break;
        }
        return $resRemove;
        ...
        if ($this->useSvn) {
            ...
            if (is_array($svnStatus)) {
                if (sizeof($svnStatus) == 0) {
                    ...
                    $this->nextDesc = sprintf(
                    $this->l(
                    'working copy already %s up-to-date. now exporting it into latest dir'), 
                    $dest);
                } else {
                    // we assume no modification has been done
                    // @TODO a svn revert ?
                    if ($svnUpdate = svn_update(
                    $dest)) {
                        $this->next = 'svnExport';
                        $this->nextDesc = sprintf(
                        $this->l(
                        'SVN Update done for working copy %s . now exporting it into latest...'), 
                        $dest);
                    }
                }
            } else {
                ...
                if (! file_exists($dest))
                    if (! @mkdir($dest, 0777)) {
                        $this->next = 'error';
                        $this->nextDesc = sprintf(
                        $this->l('unable to create directory %s'), $dest);
                        return false;
                    }
                if (svn_checkout($this->svn_link, $dest)) {
                    ...
                    $this->nextDesc = sprintf(
                    $this->l(
                    'SVN Checkout done from %s . now exporting it into latest...'), 
                    $this->svn_link);
                    return true;
                } else {
                    $this->next = 'error';
    ...
    public function ajaxProcessDownload ()
    {
        if (@ini_get('allow_url_fopen')) {
            $res = $this->upgrader->downloadLast($this->autoupgradePath, 
            $this->destDownloadFilename);
            if ($res) {
                if (md5_file(
                realpath($this->autoupgradePath) . DIRECTORY_SEPARATOR .
                 $this->destDownloadFilename) == $this->upgrader->md5) {
                    $this->next = 'unzip';
                    $this->nextDesc = $this->l(
                    ...
                    $this->next = 'error';
                    $this->nextDesc = $this->l(
                    'Download complete but md5sum does not match. Operation aborted.');
                }
            } else {
                $this->next = 'error';
            ...
            $this->nextDesc = sprintf(
            $this->l(
            'you need allow_url_fopen for automatic download. You can also manually upload it in %s'), 
    ...
    public function displayConf ()
    {
        if (! $this->standalone) {
            if (version_compare(_PS_VERSION_, '1.4.5.0', '<') and false)
                $this->_errors[] = Tools::displayError(
                'This class depends of several files modified in 1.4.5.0 version and should not be used in an older version');
        }
        parent::displayConf();
    }
    public function ajaxPreProcess ()
    {
        ...
        if (! empty($_POST['action'])) {
            ...
            if (isset(self::$skipAction[strtolower($action)])) {
                ...
                $this->nextDesc = sprintf($this->l('action %s skipped'), 
                $action);
                $this->nextQuickInfo[] = sprintf($this->l('action %s skipped'), 
                $action);
                unset($_POST['action']);
            } else 
                if (! method_exists(get_class($this), 'ajaxProcess' . $action)) {
                    $this->nextDesc = sprintf(
                    $this->l('action "%1$s" not found'), $action);
                    $this->next = 'error';
                    $this->error = '1';
                }
        }
        if (! method_exists('Tools', 'apacheModExists') ||
         Tools::apacheModExists('evasive'))
            sleep(1);
    }
...
txtError[6] = "' .
         $this->l(
        'Can\'t write settings file, please create a file named settings.inc.php in config directory.') . '";
txtError[7] = "' . $this->l('Impossible to upload the file!') . '";
txtError[8] = "' . $this->l('Data integrity is not valided. Hack attempt?') . '";
txtError[9] = "' .
         $this->l('Impossible to read the content of a MySQL content file.') . '";
txtError[10] = "' .
         $this->l('Impossible the access the a MySQL content file.') . '";
txtError[11] = "' . $this->l('Error while inserting data in the database:') . '";
txtError[12] = "' .
         $this->l(
        'The password is incorrect (alphanumeric string at least 8 characters).') . '";
txtError[14] = "' .
         $this->l(
...
txtError[17] = "' .
         $this->l('Error while creating the /config/settings.inc.php file.') . '";
txtError[18] = "' . $this->l('Error:') . '";
txtError[19] = "' .
         $this->l(
        'This PrestaShop database already exists. Please revalidate your authentication informations to the database.') . '";
txtError[22] = "' . $this->l(
        'An error occurred while resizing the picture.') . '";
txtError[23] = "' . $this->l('Database connection is available!') . '";
txtError[24] = "' .
         $this->l('Database Server is available but database is not found') . '";
txtError[25] = "' .
         $this->l(
...
txtError[45] = "' .
         $this->l('Cannot convert your database\'s data to utf-8.') . '";
txtError[46] = "' . $this->l('Invalid shop name') . '";
txtError[47] = "' .
         $this->l('Your firstname contains some invalid characters') . '";
txtError[48] = "' .
         $this->l('Your lastname contains some invalid characters') . '";
txtError[49] = "' .
         $this->l('Your database server does not support the utf-8 charset.') . '";
txtError[50] = "' .
         $this->l(
        'Your MySQL server doesn\'t support this engine, please use another one like MyISAM') . '";
txtError[51] = "' .
         $this->l(
...
txtError[28] = "' .
         sprintf($this->l('You already have the %s version.'), $INSTALL_VERSION) . '";
txtError[29] = "' .
         $this->l(
        'There is no older version. Did you delete or rename the config/settings.inc.php file?') . '";
txtError[30] = "' .
         $this->l(
        'The config/settings.inc.php file was not found. Did you delete or rename this file?') . '";
txtError[31] = "' .
         $this->l(
        'Can\'t find the sql upgrade files. Please verify that the /install/sql/upgrade folder is not empty)') . '";
txtError[32] = "' . $this->l('No upgrade is possible.') . '";
txtError[33] = "' . $this->l('Error while loading sql upgrade file.') . '";
txtError[34] = "' .
         $this->l('Error while inserting content into the database') . '";
txtError[35] = "' .
         $this->l('Unfortunately,') . '";
...
txtError[37] = "' .
    ...
    private function _displayRollbackForm ()
    {
        ...
        echo '<p>' .
         $this->l(
        'After upgrading your shop, you can rollback to the previously database and files. Use this function if your theme or an essential module is not working correctly.') .
         '</p><br/>';
        if (empty($this->backupFilesFilename) and empty($this->backupDbFilename))
            echo $this->l('No rollback available');
        else 
            if (! empty($this->backupFilesFilename) or
             ! empty($this->backupDbFilename)) {
                echo '<div id="rollbackContainer"><a class="upgradestep button" href="" id="rollback">' .
                 $this->l('rollback') . '</a></div><br/>';
            }
        if (! empty($this->backupFilesFilename) and
         file_exists($this->backupFilesFilename))
            echo '<div id="restoreFilesContainer"><a href="" class="upgradestep button" id="restoreFiles">restoreFiles</a> ' . sprintf(
            $this->l('click to restore %s'), $this->backupFilesFilename) .
             '</div><br/>';
        if (! empty($this->backupDbFilename) and
         file_exists($this->backupDbFilename))
            echo '<div id="restoreDbContainer"><a href="" class="upgradestep button" id="restoreDb">restoreDb</a> ' .
             sprintf($this->l('click to restore %s'), $this->backupDbFilename) .
             '</div><br/>';
        echo '</div></fieldset>';
    }
    ...
    private function getCurrentConfiguration ()
    {
        ...
        $content .= '<legend><a href="#" id="currentConfigurationToggle">' .
         $this->l('Your current configuration') . '</a></legend>';
        $content .= '<div id="currentConfiguration">
		<p>' .
         $this->l(
        'All the following points must be ok in order to allow the upgrade.') . '</p>
		<b>' . $this->l('Root directory') . ' : </b>' . $this->prodRootDir .
         '<br/><br/>';
        ...
        $content .= '<b>' . $this->l('Root directory status') . ' : </b>' .
         '<img src="' . $srcRootWritable . '" /> ' . ($this->rootWritable ? $this->l(
        'fully writable') : $this->l('not writable recursively')) . '<br/><br/>';
        if ($this->upgrader->need_upgrade) {
            ...
            $content .= '<b>' . $this->l('Upgrade available') . ' : </b>' .
             '<img src="' . $srcAutoupgrade . '" /> ' . ($this->upgrader->autoupgrade ? $this->l(
            'This release allows autoupgrade.') : $this->l(
            'This release does not allow autoupgrade')) . ' <br/><br/>';
        } else {
            $srcAutoupgrade = '../img/admin/disabled.gif';
            $content .= '<b>' . $this->l('Upgrade available') . ' : </b>' .
        ...
        $content .= '<b>' . $this->l('Shop desactivated') . ' : </b>' .
         '<img src="' . $srcShopStatus .
         '" /><a href="index.php?tab=AdminPreferences&token=' .
         $token_preferences . '" class="button">' . $label . '</a><br/><br/>';
        $max_exec_time = ini_get('max_execution_time');
        if ($max_exec_time == 0)
            $srcExecTime = '../img/admin/enabled.gif';
        else
            $srcExecTime = '../img/admin/warning.gif';
        $content .= '<b>' . $this->l('PHP time limit') . ' : </b>' . '<img src="' .
         $srcExecTime . '" />' . ($max_exec_time == 0 ? $this->l('disabled') : $max_exec_time .
         ' ' . $this->l('seconds')) . ' <br/><br/>';
        if ($testConfigDone = (Configuration::get('PS_AUTOUP_DONT_SAVE_IMAGES') !==
         false))
            $configurationDone = '../img/admin/enabled.gif';
        else
            $configurationDone = '../img/admin/disabled.gif';
        $content .= '<b>' . $this->l('Options chosen') . ' : </b>' . '<img src="' .
         $configurationDone .
         '" /> 
		<a class="button" id="scrollToOptions" href="#options">' . ($testConfigDone ? $this->l(
        'autoupgrade configuration ok') . ' - ' . $this->l('Modify your options') : $this->l(
        'Please configure autoupgrade options')) . '</a><br/><br/>';
    ...
    private function _displayUpgraderForm ()
    {
        ...
        $content .= '<br/>';
        $content .= '<fieldset class=""><legend>' . $this->l('Update') .
         '</legend>';
        ...
        $content .= '<script type="text/javascript">
         ...
         ($this->configOk() ? '$("#currentConfiguration").hide();$("#currentConfigurationToggle").after("<img src=\"../img/admin/enabled.gif\" />");' : '') .
         '</script>';
        $content .= '<div style="float:left">
		<h1>' . sprintf($this->l('Your current prestashop version : %s '), 
        _PS_VERSION_) . '</h1>';
        $content .= '<p>' . sprintf($this->l('Last version is %1$s (%2$s) '), 
        $this->upgrader->version_name, $this->upgrader->version_num) . '</p>';
        // @TODO : this should be checked when init()
        if ($this->isUpgradeAllowed()) {
            if ($pleaseUpdate) {
                $content .= '<li><img src="' . _PS_ADMIN_IMG_ .
                 'information.png" alt="information"/> ' .
                 $this->l('Latest Prestashop version available is:') . ' <b>' .
                 $pleaseUpdate['name'] . '</b></li>';
            }
            ...
            if ($this->upgrader->need_upgrade) {
                if ($this->configOk()) {
                    $content .= '<p><a href="" id="upgradeNow" class="button-autoupgrade upgradestep">' .
                     $this->l('Upgrade PrestaShop now !') . '</a></p>';
                    $content .= '<small>' . sprintf(
                    $this->l('PrestaShop will be downloaded from %s'), 
                    $this->upgrader->link) . '</small><br/>';
                    $content .= '<small><a href="' . $this->upgrader->changelog .
                     '">' . $this->l('see CHANGELOG') . '</a></small>';
                } else
                    $content .= $this->displayWarning(
                    'Your current configuration does not allow upgrade.');
            } else {
                $content .= '<span class="button-autoupgrade upgradestep" >' .
                 $this->l('Your shop is already up to date.') . '</span> ';
            }
            $content .= '<br/><br/><small>' .
             sprintf($this->l('last datetime check : %s '), 
            date('Y-m-d H:i:s', Configuration::get('PS_LAST_VERSION_CHECK'))) . '</span> 
		<a class="button" href="index.php?tab=AdminSelfUpgrade&token=' . Tools::getAdminToken(
            'AdminSelfUpgrade' . (int) (Tab::getIdFromClassName(
            get_class($this))) . (int) $cookie->id_employee) .
             '&refreshCurrentVersion=1">' . $this->l('Please click to refresh') . '</a>
		</small>';
            $content .= '</div>
		<div id="currentlyProcessing" style="display:none;float:right"><h4>Currently processing <img id="pleaseWait" src="' .
             __PS_BASE_URI__ . 'img/loader.gif"/></h4>
			
		<div id="infoStep" class="processing" style=height:50px;width:400px;" >' .
             $this->l('I\'m waiting for your command, sir') . '</div>';
            $content .= '</div>';
            $content .= '</fieldset>';
            if (defined('_PS_MODE_DEV_') and _PS_MODE_DEV_ and $this->manualMode) {
                $content .= '<br class="clear"/>';
                $content .= '<fieldset class="autoupgradeSteps"><legend>' .
                 $this->l('Step') . '</legend>';
            ...
            $content .= '<p>' .
             ...
             '</p>';
        $content .= '<br/><br/><small>' . sprintf(
        $this->l('last datetime check : %s '), 
        date('Y-m-d H:i:s', Configuration::get('PS_LAST_VERSION_CHECK'))) . '</span> 
		<a class="button" href="index.php?tab=AdminSelfUpgrade&token=' . Tools::getAdminToken(
        'AdminSelfUpgrade' . (int) (Tab::getIdFromClassName(get_class($this))) .
         (int) $cookie->id_employee) . '&refreshCurrentVersion=1">' .
         $this->l('Please click to refresh') . '</a>
		</small>';
        $content .= '</fieldset>';
...
.changedImportant li{color:red;font-weight:bold}
        ...
        if ($this->useSvn)
            echo '<div class="error"><h1>' . $this->l('Unstable upgrade') . '</h1>
				<p class="warning">' .
             $this->l(
            'Your current configuration indicate you want to upgrade your system from the unstable development branch, with no version number. If you upgrade, you will not be able to follow the official release process anymore') . '.</p>
				</div>';
        $this->_displayUpgraderForm();
        echo '<br/>';
        $this->_displayRollbackForm();
        echo '<br/>';
        $this->_displayForm('autoUpgradeOptions', $this->_fieldsAutoUpgrade, 
        '<a href="" name="options" id="options">' . $this->l('Options') . '</a>', 
        '', 'prefs');
        ...
        echo '<script type="text/javascript">
         ...
         'modules/autoupgrade/jquery-1.6.2.min.js"></script>';
        echo '<script type="text/javascript" src="' . __PS_BASE_URI__ .
         'modules/autoupgrade/jquery.xml2json.js"></script>';
        echo '<script type="text/javascript">' . $this->_getJsInit() .
         '</script>';
    }
...
{
	$("#upgradeNow").unbind();
	$("#upgradeNow").replaceWith("<span class=\"button-autoupgrade\">' .
         $this->l('Upgrading PrestaShop') . '</span>");
}
...
function afterUpgradeComplete()
{
	...
	$("#dbResultCheck")
		...
		.html("<p>' .
         $this->l(
        'upgrade complete. Please check your front-office theme is functionnal (try to make an order, check theme)') . '</p>")
		.show("slow")
		.append("<a href=\"index.php?tab=AdminPreferences&token=' .
         $token_preferences . '\" class=\"button\">' .
         $this->l('activate your shop here') . '</a>");
	$("#dbCreateResultCheck")
		.hide("slow");
	$("#infoStep").html("<h3>' . $this->l('Upgrade Complete ! ') .
         '</h3>");
}
...
{
	$("#restoreDbContainer").html("<a href=\"\" class=\"upgradestep button\" id=\"restoreDb\">restoreDb</a> ' .
         $this->l('click to restore database') .
         '");
...
{
	$("#restoreFilesContainer").html("<div id=\"restoreFilesContainer\"><a href=\"\" class=\"upgradestep button\" id=\"restoreFiles\">restoreFiles</a> ' .
         $this->l('click to restore files') . '");
...
function doAjaxRequest(action, nextParams){
		...
		req = $.ajax({
			type:"POST",
			url : "' . ($this->standalone ? __PS_BASE_URI__ .
         trim($this->adminDir, DIRECTORY_SEPARATOR) .
         '/autoupgrade/ajax-upgradetab.php' : str_replace('index', 'ajax-tab', 
        $currentIndex)) . '",
			async: true,
			data : {
			...
			error: function(res,textStatus,jqXHR)
			{
				...
				if (textStatus == "timeout" && action == "download")
				{
					updateInfoStep("' .
         $this->l(
        'Your server can\'t download the file. Please upload it first by ftp in your admin/autoupgrade directory') . '");
				}
				else
				{
        ...
        $js .= 'function addModifiedFileList(title, fileList, css_class)
{
	...
	$.ajax({
         ...
         '",
			...
			success : function(res,textStatus,jqXHR)
			{
				...
				{
					$("#checkPrestaShopFilesVersion").prepend("<img src=\"../img/admin/warning.gif\" /> ");
					$("#checkPrestaShopFilesVersion").append("<a id=\"toggleChangedList\" class=\"button\" href=\"\">' .
         $this->l('See or hide the list') . '</a><br/>");
    ...
    private function _cleanUp ($path)
    {
        ...
        $skipDirs = array('backups', 'pclzip', 'autoupgrade', 
        '.', '..', '.svn');
        $skipFiles = array('autoload.php', 'init.php', 'settings.inc.php', 
        'config.inc.php', 'Tools.php', 'AdminUpgrade.php');
        $skipFiles[] = $this->standalone ? 'ajax-upgradetab.php' : 'ajax-tab.php';
        if (is_dir($path)) {
            $fp = opendir($path);
            while ($file = readdir($fp)) {
                if (! in_array($file, $skipDirs) and
                 ! $this->_skipFile('', $path . $file, 'backup')) {
                    $fullpath = $path . $file;
                    if (is_dir($fullpath))
    ...
    private static function ZipExtract ($fromFile, $toDir)
    {
        if (! file_exists($toDir))
            if (! @mkdir($toDir, 0777)) {
                $this->next = 'error';
                $this->nextDesc = sprintf(
                $this->l('unable to create directory %s'), $toDir);
                return false;
            }
        if (class_exists('ZipArchive', false)) {
            $zip = new ZipArchive();
            if ($zip->open($fromFile) === true) {
                if (@$zip->extractTo($toDir . '/') && $zip->close()) {
                    return true;
                } else {
                    $this->next = 'error';
    ...
    private function _listArchivedFiles ()
    {
        if (! empty($this->currentParams['backupFilesFilename'])) {
            if (class_exists('ZipArchive', false)) {
                ...
                require_once ($this->prodRootDir . '/tools/pclzip/pclzip.lib.php');
                if ($zip = new PclZip(
                $this->currentParams['backupFilesFilename']))
