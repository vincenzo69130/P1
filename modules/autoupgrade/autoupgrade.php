...
class Autoupgrade extends Module
{
    function __construct ()
    {
        ...
        if (! defined('_PS_ADMIN_DIR_')) {
            if (defined('PS_ADMIN_DIR'))
                ...
                $this->_errors[] = $this->l(
                'This version of PrestaShop cannot be upgraded : PS_ADMIN_DIR constant is missing');
        ...
        'Provides an automated method to upgrade your shop to the last PrestaShop version. Caution : custom theme are not updated.');
    }
    function install ()
    {
        $res = true;
        ...
        if (! $idTab) {
            ...
            $tab->id_parent = 9;
            $tab->name = array_fill(1, sizeof(Language::getLanguages(false)), 
            'Upgrade');
            $res &= $tab->save();
        } else
            $tab = new Tab($idTab);
        ...
        if (file_exists(
        $autoupgradeDir . DIRECTORY_SEPARATOR . 'ajax-upgradetab.php'))
            $res &= unlink(
            $autoupgradeDir . DIRECTORY_SEPARATOR . 'ajax-upgradetab.php');
        if (! defined('_PS_MODULE_DIR_')) {
            define('_PS_MODULE_DIR_', _PS_ROOT_DIR_ . '/modules/');
        }
        $res &= copy(_PS_MODULE_DIR_ . 'autoupgrade/ajax-upgradetab.php', 
        $autoupgradeDir . DIRECTORY_SEPARATOR . 'ajax-upgradetab.php');
        $res &= copy(_PS_MODULE_DIR_ . 'autoupgrade/logo.gif', 
        _PS_ROOT_DIR_ . DIRECTORY_SEPARATOR . 'img/t/AdminSelfUpgrade.gif');
        if (! $res or ! Tab::getIdFromClassName('AdminSelfUpgrade') or
         ! parent::install())
            return false;
        return true;
    }
    public function uninstall ()
    {
        ...
        $res = $tab->delete();
        if (file_exists(
        _PS_ADMIN_DIR_ . DIRECTORY_SEPARATOR . 'tabs' . 'AdminUpgrade.php')) {
        ...
        if (file_exists(
        _PS_ADMIN_DIR_ . DIRECTORY_SEPARATOR . 'autoupgrade' .
         DIRECTORY_SEPARATOR . 'ajax-upgradetab.php'))
            $res &= @unlink(
            _PS_ADMIN_DIR_ . DIRECTORY_SEPARATOR . 'autoupgrade' .
             DIRECTORY_SEPARATOR . 'ajax-upgradetab.php');
