...
class awLinePlot extends awPlot implements awLegendable
{
    ...
    public function setFilledArea ($start, $stop, $background)
    {
        if ($stop <= $start) {
            awImage::drawError(
            "Class LinePlot: End position can not be greater than begin position in setFilledArea().");
        }
    ...
    $aliasing)
    {
        ...
        for ($key = $start; $key <= $stop; $key ++) {
            ...
            if ($value !== NULL) {
                $p = awAxis::toPosition($this->xAxis, $this->yAxis, 
        ...
        // Draw backgrounds
        if ($this->lineBackground instanceof awColor or
         $this->lineBackground instanceof awGradient) {
            ...
            $driver->filledPolygon($this->lineBackground, 
            $backgroundPolygon);
        }
        ...
        if ($this->lineHide === FALSE) {
            ...
            foreach ($polygon->all() as $point) {
                if ($prev !== NULL) {
                    $driver->line($this->lineColor, 
                    new awLine($prev, $point, $this->lineStyle, 
                    $this->lineThickness));
                }
                $prev = $point;
            }
    ...
    protected function xAxisPoint ($position)
    {
        ...
        return awAxis::toPosition($this->xAxis, $this->yAxis, 
        new awPoint($position, $y));
    }
...
class awSimpleLinePlot extends awPlot implements awLegendable
{
    ...
    $aliasing)
    {
        ...
        $p1 = awAxis::toPosition($this->xAxis, $this->yAxis, 
        new awPoint($this->lineStart, $this->lineValue));
        $p2 = awAxis::toPosition($this->xAxis, $this->yAxis, 
        new awPoint($this->lineStop, $this->lineValue));
        $driver->line($this->lineColor, 
        new awLine($p1->move($inc, 0), $p2->move($inc, 0), $this->lineStyle, 
        $this->lineThickness));
    }
    public function getXAxisNumber ()
    {
