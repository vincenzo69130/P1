...
abstract class awPlot extends awComponent
{
    ...
    {
        return floor(
        is_null($this->xMin) ? array_min($this->datax) : $this->xMin);
    }
    /**
    ...
    {
        return (ceil(
        is_null($this->xMax) ? array_max($this->datax) : (float) $this->xMax)) +
         ($this->getXCenter() ? 1 : 0);
    }
    /**
    ...
    public function init (awDriver $driver)
    {
        ...
        list ($leftSpace, $rightSpace, $topSpace, $bottomSpace) = $this->getSpace(
        $x2 - $x1, $y2 - $y1);
    ...
    public function drawEnvelope (awDriver $driver)
    {
        ...
        // Draw top axis
        if ($this->xAxisPosition === awPlot::TOP or
         $this->xAxisPosition === awPlot::BOTH) {
        ...
        // Draw bottom axis
        if ($this->xAxisPosition === awPlot::BOTTOM or
         $this->xAxisPosition === awPlot::BOTH) {
        ...
        // Draw left axis
        if ($this->yAxisPosition === awPlot::LEFT or
         $this->yAxisPosition === awPlot::BOTH) {
        ...
        // Draw right axis
        if ($this->yAxisPosition === awPlot::RIGHT or
         $this->yAxisPosition === awPlot::BOTH) {
    ...
    public function setValues ($datay, $datax = NULL)
    {
        ...
        if (count($datay) === count($datax)) {
            ...
            awImage::drawError(
            "Class Plot: Plots must have the same number of X and Y points.");
        }
    }
    /**
    ...
    private function checkArray (&$array)
    {
        ...
        if (count($array) < 1) {
            awImage::drawError(
            "Class Plot: Your plot must have at least 1 value.");
        }
    }
}
...
class awPlotGroup extends awComponentGroup
{
    ...
    public function init (awDriver $driver)
    {
        ...
        for ($i = 0; $i < count($this->components); $i ++) {
            ...
            $component->setSpace($this->space->left, 
            $this->space->right, $this->space->top, $this->space->bottom);
        ...
        $this->axis->left->tick('major')->setNumber(
        $this->axis->left->getLabelNumber());
        $this->axis->right->tick('major')->setNumber(
        $this->axis->right->getLabelNumber());
        // Set X axis on zero
        if ($this->xAxisZero) {
    ...
    public function drawEnvelope (awDriver $driver)
    {
        list ($x1, $y1, $x2, $y2) = $this->getPosition();
        // Hide unused axis
        foreach (array(awPlot::LEFT, awPlot::RIGHT, awPlot::TOP, 
        awPlot::BOTTOM) as $axis) {
