...
function registerClass ($class, $abstract = FALSE)
{
    ...
    eval(
    $abstract . " class " . ARTICHOW_PREFIX . $class . " extends aw" . $class .
     " { }");
}
/*
...
function registerInterface ($interface)
{
    ...
    eval(
    "interface " . ARTICHOW_PREFIX . $interface . " extends aw" . $interface .
     " { }");
}
// Some useful files
...
class awImage
{
    ...
    {
        if ($format === awImage::JPEG or $format === awImage::PNG or
         $format === awImage::GIF) {
            $this->format = $format;
        }
    }
    ...
    protected function selectDriver ($driver)
    {
        ...
        switch ($string) {
            ...
            default:
                ...
                awImage::drawError(
                'Class Image: Unknown driver type (\'' . $string . '\')');
                break;
        }
    }
    /**
    ...
    public function drawComponent (awComponent $component)
    {
        ...
        if ($component->w === NULL and $component->h === NULL) {
            list ($width, $height) = $driver->setSize($component->width, 
            $component->height);
        ...
        if ($component->top !== NULL and $component->left !== NULL) {
            $driver->setAbsPosition($border + $shadow->left + $component->left, 
        ...
        $component->drawComponent($driver, $x1, $y1, $x2, $y2, 
        $this->antiAliasing);
    ...
    protected function drawShadow ()
    {
        ...
        $this->shadow->draw($driver, new awPoint(0, 0), 
        new awPoint($this->width, $this->height), awShadow::IN);
    }
    /**
    ...
    public static function drawError ($message)
    {
        ...
        $driver->init($image);
        // Display title
        $driver->filledRectangle(new awWhite(), 
        new awLine(new awPoint(0, 0), new awPoint($width, $height)));
        $driver->filledRectangle(new awRed(), 
        new awLine(new awPoint(0, 0), new awPoint(110, 25)));
        $text = new awText("Artichow error", new awFont3(), new awWhite(), 0);
        $driver->string($text, new awPoint(5, 6));
        // Display red box
        $driver->rectangle(new awRed(), 
        new awLine(new awPoint(0, 25), new awPoint($width - 90, $height - 1)));
        // Display error image
        $file = ARTICHOW_IMAGE . DIRECTORY_SEPARATOR . 'error.png';
        $imageError = new awFileImage($file);
        $driver->copyImage($imageError, new awPoint($width - 81, $height - 81), 
        new awPoint($width - 1, $height - 1));
        // Draw message
        $text = new awText(strip_tags($message), new awFont2(), 
        new awBlack(), 0);
    ...
    {
        $file = ARTICHOW_IMAGE . DIRECTORY_SEPARATOR . 'errors' .
         DIRECTORY_SEPARATOR . $error . '.png';
