...
class awPoint extends awShape
{
    ...
    {
        return new awPoint($this->x + $x, $this->y + $y);
    }
}
registerClass('Point');
/**
...
class awLine extends awShape
{
    ...
    {
        return new awLine(new awPoint($x1, $y1), new awPoint($x2, $y2));
    }
    /**
    ...
    {
        $square = pow($this->p2->x - $this->p1->x, 2) +
    ...
    public function isTopToBottom (awPolygon $polygon)
    {
        ...
        if ($this->isHorizontal()) {
            ...
            return ($this->isOnBoxTopSide($top, $xMin, $xMax, $yMin) and
    ...
    {
        if ($point->y === $yMax and $point->x >= $xMin and $point->x <= $xMax) {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    {
        if ($point->x === $xMax and $point->y >= $yMin and $point->y <= $yMax) {
            return TRUE;
        } else {
            return FALSE;
        }
