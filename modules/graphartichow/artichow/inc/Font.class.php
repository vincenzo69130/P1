...
abstract class awFont
{
     ...
     */
    public function draw (awDriver $driver, awPoint $point, awText $text, 
    $width = NULL)
    {
...
class awFileFont extends awFont
{
    ...
    public function setName ($name)
    {
        ...
        if (strpos($fontInfo['dirname'], '/') !== 0) {
            ...
            $name = ARTICHOW_FONT . DIRECTORY_SEPARATOR .
             $fontInfo['basename'];
            $fontInfo = pathinfo($name);
        }
        $this->name = $fontInfo['dirname'] . DIRECTORY_SEPARATOR .
         $fontInfo['basename'];
        if (array_key_exists('extension', $fontInfo) and
         $fontInfo['extension'] !== '') {
