...
class awMingDriver extends awDriver
{
    ...
    public function setAntiAliasing ($bool)
    {
        if ($this->movie !== NULL) {
            ...
            $this->movie->add(
            new SWFAction(str_replace("\r", "", $actionscript)));
        }
    }
    /**
    ...
    public function string (awText $text, awPoint $point, $width = NULL)
    {
        ...
        if ($this->isCompatibleWithFont($font) === FALSE) {
            awImage::drawError(
            'Class MingDriver: Incompatible font type (\'' . get_class($font) .
             '\')');
        }
    ...
    public function line (awColor $color, awLine $line)
    {
        if ($line->getThickness() > 0 and $line->isHidden() === FALSE) {
            ...
            $mingLine->setLine($line->getThickness(), $red, $green, $blue, 
            $alpha);
            list ($p1, $p2) = $line->getLocation();
            $mingLine->movePenTo($this->x + round($p1->x), 
            $this->y + round($p1->y));
            switch ($line->getStyle()) {
                case awLine::SOLID:
                    $mingLine->drawLineTo($this->x + round($p2->x), 
                    $this->y + round($p2->y));
    ...
    public function filledRectangle ($background, awLine $line)
    {
        ...
        if ($background instanceof awColor) {
            ...
            list ($flashGradient, $style) = $this->getGradient(
            $background);
        ...
        $shape->movePenTo($this->x + round($p1->x), 
        $this->y + round($p1->y));
        // Depending on the points' relative positions,
        // we have two drawing possibilities
        if (($p1->x <= $p2->x and $p1->y <= $p2->y) or
         ($p1->x >= $p2->x and $p1->y >= $p2->y)) {
            $shape->drawLineTo($this->x + round($p2->x), 
            $this->y + round($p1->y));
            $shape->drawLineTo($this->x + round($p2->x), 
            $this->y + round($p2->y));
            $shape->drawLineTo($this->x + round($p1->x), 
            $this->y + round($p2->y));
            $shape->drawLineTo($this->x + round($p1->x), 
            $this->y + round($p1->y));
        } else {
