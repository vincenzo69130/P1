...
class awMark
{
    ...
    protected function changeType ($image)
    {
        ...
        $this->setImage(
        new awFileImage(ARTICHOW_IMAGE . DIRECTORY_SEPARATOR . $image . '.png'));
    }
    protected function drawCircle (awPoint $point)
    {
        $this->driver->filledEllipse($this->fill, $point, $this->size, 
        $this->size);
    ...
    protected function drawSquare (awPoint $point)
    {
        ...
        $y2 = $y1 + $this->size;
        $this->border->rectangle($this->driver, new awPoint($x1, $y1), 
        new awPoint($x2, $y2));
        $size = $this->border->visible() ? 1 : 0;
        $this->driver->filledRectangle($this->fill, 
        new awLine(new awPoint($x1 + $size, $y1 + $size), 
    ...
    protected function drawTriangle (awPoint $point, $inverted = FALSE)
    {
        ...
        if ($inverted === TRUE) {
            ...
            // Upper left corner
            $triangle->append(
            new awPoint($x - $size / 2, $y - $size / (2 * sqrt(3))));
            // Upper right corner
            $triangle->append(
            new awPoint($x + $size / 2, $y - $size / (2 * sqrt(3))));
        } else {
    ...
    protected function drawCross (awPoint $point, $upright = FALSE)
    {
        ...
        $this->driver->line($this->fill, 
        new awLine(new awPoint($x11, $y11), new awPoint($x12, $y12)));
        $this->driver->line($this->fill, 
        new awLine(new awPoint($x21, $y21), new awPoint($x22, $y22)));
    }
    protected function drawImage (awPoint $point)
    {
        if ($this->image instanceof awImage) {
            ...
            $y1 = (int) ($y - $width / 2);
            $y2 = $y1 + $height;
            $this->border->rectangle($this->driver, 
            new awPoint($x1 - 1, $y1 - 1), new awPoint($x2 + 1, $y2 + 1));
            $this->driver->copyImage($this->image, new awPoint($x1, $y1), 
            new awPoint($x2, $y2));
        }
    }
}
