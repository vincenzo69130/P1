...
class awLegend implements awPositionable
{
     ...
     */
    public function add (awLegendable $legendable, $title, 
    $type = awLegend::LINE)
    {
    ...
    {
        $this->padding = array((int) $left, (int) $right, (int) $top, 
        (int) $bottom);
    }
    /**
    ...
    public function draw (awDriver $driver)
    {
        ...
        for ($i = 0; $i < $count; $i ++) {
            ...
            //			$font = $text->getFont();
            $widths[$i] = $driver->getTextWidth($text) +
             $this->textMargin->left + $this->textMargin->right;
        ...
        $height = ($heightMax + $this->space) * $rows - $this->space +
         $this->padding[2] + $this->padding[3];
        ...
        // Draw legend shadow
        $this->shadow->draw($driver, $p, $p->move($width, $height), 
        awShadow::OUT);
        // Draw legends base
        ...
        for ($i = 0; $i < $count; $i ++) {
            ...
            // Draw legend text
            $driver->string($texts[$i], 
            $p->move(
            $this->padding[0] + $previousColumns + 10 + 5 +
             $this->textMargin->left, 
            $this->padding[2] + $row * ($heightMax + $this->space) +
             $heightMax / 2 - $heights[$i] / 2));
            // Draw legend icon
            switch ($type) {
                case awLegend::LINE:
                case awLegend::MARK:
                case awLegend::MARKONLY:
                    ...
                    $y = $this->padding[2] + $row * ($heightMax + $this->space) +
                     $heightMax / 2 - $component->getLegendLineThickness();
                    // Draw two lines
                    if ($component->getLegendLineColor() !==
                     NULL) {
                        $color = $component->getLegendLineColor();
                        if ($color instanceof awColor and
                         $type !== awLegend::MARKONLY) {
                            $driver->line($color, 
                            new awLine(
                            $p->move($x,  // YaPB ??
                            $y +
                             $component->getLegendLineThickness() / 2), 
                            $p->move($x + 10, 
    ...
    private function drawBase (awDriver $driver, awPoint $p, $width, $height)
    {
        ...
        $driver->filledRectangle($this->background, 
        new awLine($p->move($size, $size), 
