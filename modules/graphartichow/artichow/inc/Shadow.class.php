...
class awShadow
{
    ...
    {
        return new awSide(
        ($this->position === awShadow::LEFT_TOP or
         $this->position === awShadow::LEFT_BOTTOM) ? $this->size : 0, 
        ($this->position === awShadow::RIGHT_TOP or
         $this->position === awShadow::RIGHT_BOTTOM) ? $this->size : 0, 
        ($this->position === awShadow::LEFT_TOP or
         $this->position === awShadow::RIGHT_TOP) ? $this->size : 0, 
        ($this->position === awShadow::LEFT_BOTTOM or
         $this->position === awShadow::RIGHT_BOTTOM) ? $this->size : 0);
    }
    /**
     * Draw shadow
    ...
    public function draw (awDriver $driver, awPoint $p1, awPoint $p2, $mode)
    {
        ...
        $driver = clone $driver;
        $color = ($this->color instanceof awColor) ? $this->color : new awColor(
        125, 125, 125);
        switch ($this->position) {
            case awShadow::RIGHT_BOTTOM:
                ...
                $width = $t2->x - $t1->x;
                $height = $t2->y - $t1->y;
                $driver->setAbsPosition($t1->x + $driver->x, 
                $t1->y + $driver->y);
                $driver->filledRectangle($color, 
                new awLine(new awPoint($width - $this->size, $this->size), 
                new awPoint($width - 1, $height - 1)));
                $driver->filledRectangle($color, 
                new awLine(new awPoint($this->size, $height - $this->size), 
            ...
            case awShadow::LEFT_TOP:
                ...
                $driver->filledRectangle($color, 
                new awLine(new awPoint($this->size, 0), 
            ...
            case awShadow::RIGHT_TOP:
                ...
                $height = max($height + 1, $this->size);
                $driver->filledRectangle($color, 
                new awLine(new awPoint($width - $this->size, 0), 
            ...
            case awShadow::LEFT_BOTTOM:
                ...
                $t1->y + $driver->y);
                $driver->filledRectangle($color, 
                new awLine(new awPoint(0, $this->size), 
    ...
    private function smoothPast (awDriver $driver, awColor $color, $width, 
    $height)
    {
        if ($this->smooth) {
    ...
    private function smoothFuture (awDriver $driver, awColor $color, $width, 
    $height)
    {
        if ($this->smooth) {
            for ($i = 0; $i < $this->size; $i ++) {
