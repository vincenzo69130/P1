...
class awAxis
{
     ...
     * @var array
     */
    protected $rangeCallback = array('toValue' => 'toProportionalValue', 
    ...
    {
        $this->line = new awVector(new awPoint(0, 0), new awPoint(0, 0));
        $this->label = new awLabel();
        $this->padding = new awSide();
        $this->title = new awLabel(NULL, NULL, NULL, 0);
    ...
    {
        $this->rangeCallback = array('toValue' => (string) $toValue, 
    ...
    public function setXCenter (awAxis $axis, $value)
    {
        ...
        if ($this->line->isVertical() === FALSE) {
            awImage::drawError(
            "Class Axis: setXCenter() can only be used on vertical axes.");
        }
    ...
    public function setYCenter (awAxis $axis, $value)
    {
        ...
        if ($this->line->isHorizontal() === FALSE) {
            awImage::drawError(
            "Class Axis: setYCenter() can only be used on horizontal axes.");
        }
        $p = $axis->getPointFromValue($value);
    ...
    protected function getPointFromPosition ($position)
    {
        ...
        $size = $vector->getSize();
        return $vector->p1->move(cos($angle) * $size * $position, 
    ...
    public function autoScale ()
    {
        ...
        if ($min == 0) {
            $this->setRange($min, $intervalReal * $pow);
        } elseif ($max == 0) {
            $this->setRange($intervalReal * $pow * - 1, 0);
        }
    ...
    protected function getVector ()
    {
        ...
        // Compute paddings
        $vector = new awVector(
        $this->line->p1->move(cos($angle) * $this->padding->left, 
        - 1 * sin($angle) * $this->padding->left), 
        $this->line->p2->move(- 1 * cos($angle) * $this->padding->right, 
