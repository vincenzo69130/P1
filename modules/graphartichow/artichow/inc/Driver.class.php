...
abstract class awDriver
{
     ...
     */
    abstract public function copyResizeImage (awImage $image, awPoint $d1, 
    awPoint $d2, awPoint $s1, awPoint $s2, $resample = TRUE);
    /**
     * Draw a string
     *
     ...
     */
    abstract public function arc (awColor $color, awPoint $center, $width, 
    $height, $from, $to);
    /**
     ...
     * @param int $to End angle
     */
    abstract public function filledArc (awColor $color, awPoint $center, $width, 
    $height, $from, $to);
    /**
     ...
     * @param int $height Ellipse height
     */
    abstract public function ellipse (awColor $color, awPoint $center, $width, 
    $height);
    /**
     ...
     * @param int $height Ellipse height
     */
    abstract public function filledEllipse ($background, awPoint $center, $width, 
    $height);
    /**
...
abstract class awFontDriver
{
     ...
     */
    abstract public function string (awDriver $driver, awText $text, 
    awPoint $point, $width = NULL);
    /**
...
class awPHPFontDriver extends awFontDriver
{
    ...
    public function string (awDriver $driver, awText $text, awPoint $point, 
    $width = NULL)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class PHPFontDriver: Incompatibility between driver and font - You should never see this error message: have you called awDriver::isCompatibleWithFont() properly?');
                break;
        }
    }
    /**
     ...
     */
    private function gdString (awGDDriver $driver, awText $text, awPoint $point, 
    $width = NULL)
    {
        $angle = $text->getAngle();
        if ($angle !== 90 and $angle !== 0) {
            awImage::drawError(
            "Class PHPFontDriver: You can only use 0° and 90° angles.");
        }
        if ($angle === 90) {
        ...
        if ($width !== NULL) {
            $characters = floor(
            $width / ($this->getGDTextWidth($text) / strlen($textString)));
            if ($characters > 0) {
        ...
        foreach ($lines as $i => $line) {
            ...
            $function($driver->resource, $font->font, 
            $driver->x + $point->x + $addX, 
            $driver->y + $point->y + $addY + $addAngle, $line, $rgb);
        }
    }
    public function getTextWidth (awText $text, awDriver $driver)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class PHPFontDriver: Cannot get text width - incompatibility between driver and font');
                break;
        }
    }
    public function getTextHeight (awText $text, awDriver $driver)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class PHPFontDriver: Cannot get text height - incompatibility between driver and font');
                break;
        }
    }
    /**
...
class awFileFontDriver extends awFontDriver
{
    ...
    $width = NULL)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class fileFontDriver: Incompatibility between driver and font - You should never see this error message: have you called awDriver::isCompatibleWithFont() properly?');
                break;
        }
    }
    /**
    ...
    $width = NULL)
    {
        ...
        $font = $text->getFont();
        if ($font instanceof awTTFFont === FALSE and
         $font->getExtension() === NULL) {
            $font->setExtension('ttf');
        }
        $filePath = $font->getName() . '.' . $font->getExtension();
        $box = imagettfbbox($font->getSize(), $text->getAngle(), $filePath, 
        $text->getText());
        $textHeight = - $box[5];
        ...
        // Restore old text
        $text->setText(
        substr($text->getText(), 0, strlen($text->getText()) - 1));
        $textString = $text->getText();
        // Split text if needed
        if ($width !== NULL) {
        ...
        $color = $text->getColor();
        $rgb = $driver->getColor($color);
        imagettftext($driver->resource, $font->getSize(), $text->getAngle(), 
        $driver->x + $point->x +
         $textWidth *
         sin($text->getAngle() / 180 * M_PI), 
        $driver->y + $point->y + $textHeight, $rgb, $filePath, $textString);
    }
    public function getTextWidth (awText $text, awDriver $driver)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class FileFontDriver: Cannot get text width - incompatibility between driver and font');
                break;
        }
    }
    public function getTextHeight (awText $text, awDriver $driver)
    {
        switch ($driver->getDriverString()) {
            ...
            default:
                awImage::drawError(
                'Class FileFontDriver: Cannot get text height - incompatibility between driver and font');
                break;
        }
    }
    private function getGDTextWidth (awText $text)
    {
        ...
        if ($box === FALSE) {
            awImage::drawError(
            "Class FileFontDriver: Unable to get font width (GD).");
        }
        list (, , $x2, , , , $x1, ) = $box;
        return abs($x2 - $x1);
    }
    private function getGDTextHeight (awText $text)
    {
        ...
        if ($box === FALSE) {
            awImage::drawError(
            "Class FileFontDriver: Unable to get font height (GD).");
        }
        list (, , , $y2, , , , $y1) = $box;
        return abs($y2 - $y1);
    }
    private function getGDAverageWidth (awFileFont $font)
    {
        ...
        $box = imagettfbbox($font->getSize(), 0, 
        $font->getName() . '.' . $font->getExtension(), $text);
        if ($box === FALSE) {
            awImage::drawError(
            "Class FileFontDriver: Unable to get font average width.");
        }
