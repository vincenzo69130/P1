...
class awPie extends awComponent
{
    ...
    public function __construct ($values, $colors = awPie::COLORED)
    {
        ...
        if (is_array($colors)) {
            ...
            switch ($colors) {
                case awPie::AQUA:
                    $this->colors = array(new awColor(131, 220, 215), 
                    new awColor(131, 190, 215), new awColor(131, 160, 215), 
                    new awColor(160, 140, 215), new awColor(190, 131, 215), 
                    new awColor(220, 131, 215));
                    break;
                case awPie::EARTH:
                    $this->colors = array(new awColor(97, 179, 110), 
                    new awColor(130, 179, 97), new awColor(168, 179, 97), 
                    new awColor(179, 147, 97), new awColor(179, 108, 97), 
                    new awColor(99, 107, 189), new awColor(99, 165, 189));
                    break;
                case awPie::DARK:
                    $this->colors = array(new awColor(140, 100, 170), 
                    new awColor(130, 170, 100), new awColor(160, 160, 120), 
                    new awColor(150, 110, 140), new awColor(130, 150, 160), 
                    new awColor(90, 170, 140));
                    break;
                default:
                    $this->colors = array(new awColor(187, 213, 151), 
                    new awColor(223, 177, 151), new awColor(111, 186, 132), 
                    new awColor(197, 160, 230), new awColor(165, 169, 63), 
                    new awColor(218, 177, 89), new awColor(116, 205, 121), 
                    new awColor(200, 201, 78), new awColor(127, 205, 177), 
                    new awColor(205, 160, 160), new awColor(190, 190, 190));
                    break;
            }
        }
        parent::__construct();
    ...
    public function setBorder (awColor $color)
    {
        if (ARTICHOW_DEPRECATED === TRUE) {
            awImage::drawError(
            'Class Pie: Method setBorder() has been deprecated since Artichow 1.0.9. Please use setBorderColor() instead.');
        } else {
    ...
    {}
    public function drawComponent (awDriver $driver, $x1, $y1, $x2, $y2, 
    $aliasing)
    {
        ...
        foreach ($this->values as $key => $value) {
            ...
            if (array_key_exists($key, $this->explode)) {
                ...
                if ($aliasing) {
                    ...
                    $side->set(max($side->left, $posX * - 2), 
                    max($side->right, $posX * 2), max($side->top, $posY * - 2), 
        ...
        if ($aliasing) {
            ...
            $driver = $image->getDriver($width / $image->width, 
            $height / $image->height, ($width / 2 + $side->left) / $image->width, 
        ...
        for ($i = $this->size; $i > 0; $i --) {
            foreach ($values as $key => $value) {
                ...
                $driver->filledArc($color, $explode->move($x, $y + $i), $width, 
                $height, $from, $to);
                unset($color);
                if ($this->border instanceof awColor) {
                    $point = $explode->move($x, $y);
                    if ($i === $this->size) {
                        $driver->arc($this->border, 
                        $point->move(0, $this->size), $width, $height, $from, 
                        $to);
                    }
                }
            }
        ...
        foreach ($values as $key => $value) {
            $color = $this->colors[$key % count($this->colors)];
            list ($from, $to, $explode) = $value;
            $driver->filledArc($color, $explode->move($x, $y), $width, $height, 
            $from, $to);
            if ($this->border instanceof awColor) {
                $point = $explode->move($x, $y);
        ...
        if ($aliasing) {
            ...
            $mainDriver->copyResizeImage($image, 
            new awPoint($x1 - $side->left / 2, $y1 - $side->top / 2), 
            new awPoint($x1 - $side->left / 2 + $image->width / 2, 
            $y1 - $side->top / 2 + $image->height / 2), new awPoint(0, 0), 
            new awPoint($image->width, $image->height), TRUE);
            $driver = $mainDriver;
        }
        // Get labels values
        $pc = array();
    ...
    private function checkArray (&$array)
    {
        if (is_array($array) === FALSE) {
            awImage::drawError(
            "Class Pie: You tried to set values that are not an array.");
        }
        ...
        if (count($array) < 1) {
            awImage::drawError(
            "Class Pie: Your graph must have at least 1 value.");
        }
    }
}
registerClass('Pie');
