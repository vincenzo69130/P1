...
class awBarPlot extends awPlot implements awLegendable
{
    ...
    $aliasing)
    {
        ...
        // Get base position
        $zero = awAxis::toPosition($this->xAxis, $this->yAxis, 
        new awPoint(0, $zero));
        ...
        for ($key = 0; $key < $count; $key ++) {
            ...
            if ($value !== NULL) {
                $position = awAxis::toPosition($this->xAxis, $this->yAxis, 
                new awPoint($key, $value));
                $barStart = $barPosition +
                 ($this->identifier - 1) * $this->barSpace + $position->x;
        ...
        foreach ($this->datay as $key => $value) {
            if ($value !== NULL) {
                ...
                new awPoint($key, $value));
                $point = new awPoint(
                $barPosition + ($this->identifier - 1) * $this->barSpace +
                 $position->x + $barSize / 2 + 1 + $this->depth, 
    ...
    {
        // Draw shadow
        $this->barShadow->draw($driver, $p1, $p2, awShadow::OUT);
        if (abs($p2->y - $p1->y) > 1) {
            $this->barBorder->rectangle($driver, $p1, $p2);
            if ($this->barBackground !== NULL) {
                $size = $this->barBorder->visible() ? 1 : 0;
                $b1 = $p1->move($size, $size);
                $b2 = $p2->move(- 1 * $size, - 1 * $size);
                // Draw background
                $driver->filledRectangle($this->barBackground, 
                new awLine($b1, $b2));
            }
        }
    }
