...
class GraphArtichow extends ModuleGraphEngine
{
    ...
    function __construct ($type = null)
    {
        if ($type != null) {
            ...
            'Artichow is a library which enables the display of simple picture-based graphs using PHP and GD.');
        }
    }
    function install ()
    {
    ...
    {
        return '<img src="' . $drawer . '&width=' . $params['width'] . '&height=' .
         $params['height'] . '" />';
    }
    private function drawLine ($values)
    {
        ...
        if (! isset($values[0]) or ! is_array($values[0])) {
            ...
            $this->_plot->setFillGradient(
            new LinearGradient(new Color(255, 246, 211, 20), 
            new Color(240, 225, 160, 50), 90));
            $this->_plot->setColor(new Color(80, 50, 0, 0));
        } // Multiple layers
else {
            $tabColors = array(new Color(120, 120, 30, 10), 
            new Color(66, 127, 195, 10), new Color(195, 65, 60, 10), 
            new Color(85, 163, 57, 10), new Color(255, 35, 152, 10));
            $tabFilledColors = array(new Color(120, 120, 30, 90), 
            new Color(66, 127, 195, 90), new Color(195, 65, 60, 90), 
            new Color(85, 163, 57, 90), new Color(255, 35, 152, 90));
        ...
        if (! is_array($values[0])) //simpleline
{
            ...
            $this->_plot->setBarGradient(
            new LinearGradient(new Color(254, 255, 216), 
            new Color(233, 231, 218), 90));
    ...
    public function setLegend ($legend)
    {
        ...
        foreach ($legend as $k => $val)
            $legend[$k] = html_entity_decode(
            htmlentities(html_entity_decode($val), ENT_NOQUOTES, 'utf-8'));
        switch ($this->_type) {
            case 'pie':
            ...
            default:
                if ($this->_group == NULL) {
                    if (isset($this->_plot->xAxis, $this->_plot->yAxis, 
                    $this->_plot->grid)) {
                    ...
                    if (isset($this->_group->axis->bottom, 
                    $this->_group->axis->left, $this->_group->grid)) {
                        $this->_group->axis->bottom->setLabelText($legend);
                        $this->_group->axis->bottom->label->setFont(
                        new Tuffy(7));
        ...
        if (isset($titles['main'])) {
            if (is_array($titles['main']) && $this->_group != NULL) {
                ...
                foreach ($titles['main'] as $key => $val)
                    if ($key != 0)
                        $this->_group->legend->add($this->_plot[$i ++], $val, 
                        ($this->_type == 'line') ? Legend::LINE : Legend::BACKGROUND);
                $titles['main'] = $titles['main'][0];
            }
            $this->_graph->title->set($titles['main']);
            $this->_graph->title->setFont(new Tuffy(10));
            $this->_graph->title->setAlign(Positionable::CENTER, 
            Positionable::MIDDLE);
        }
        if (isset($titles['x'])) {
