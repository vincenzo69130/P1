...
class GraphGoogleChart extends ModuleGraphEngine
{
    ...
    private function drawColumn ($max_y)
    {
        ...
        $url = 'bvs&chxt=x,y&chxr=1,0,' . $max_y . '&chbh=' .
         $this->getChbh($sizeof_values) . '&chg=0,12.5&chxl=0:|';
            ...
            if (! isset($this->_values[0]) || ! is_array($this->_values[0])) {
                ...
                foreach ($this->_values as $k => $value) {
                    if (! isset($this->_values[$k][$i]))
                        $this->_values[$k][$i] = 0;
                    $this->_values[$k][$i] = ($this->_values[$k][$i] * 100) /
                     $max_y;
                }
        return ($url);
    }
    ...
    {
        return ('lc'./*&chxt=x,y*/'&chbh=' . $this->getChbh(sizeof($this->_values)) .
         '&chg=0,12.5&chxl=0:|');
    }
    ...
    public function draw ()
    {
        ...
        if (! isset($this->_values[0]) || ! is_array($this->_values[0]))
            $url .= (isset($this->_titles['main'])) ? '&chtt=' .
             urlencode($this->_titles['main']) : '';
        else {
