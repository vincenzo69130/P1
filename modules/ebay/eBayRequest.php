...
class eBayRequest
{
    ...
    public function makeRequest ($request)
    {
        ...
        if ($this->debug == true) {
            if (! file_exists(dirname(__FILE__) . '/log/request.php'))
                file_put_contents(dirname(__FILE__) . '/log/request.php', 
                "<?php\n\n", FILE_APPEND | LOCK_EX);
            file_put_contents(dirname(__FILE__) . '/log/request.php', 
            date('d/m/Y H:i:s') . "\n\n" . $request . "\n\n" . $response .
             "\n\n-------------------\n\n", FILE_APPEND | LOCK_EX);
    ...
    function fetchToken ()
    {
        ...
        $requestXml .= '<RequesterCredentials><Username>' . $this->username .
         '</Username></RequesterCredentials>';
    ...
    function saveCategories ()
    {
        ...
        $requestXml .= '<eBayAuthToken>' . Configuration::get('EBAY_API_TOKEN') .
         '</eBayAuthToken>';
        $requestXml .= '</RequesterCredentials>';
        ...
        $categoriesMultiSkuCompliant = $this->GetCategoryFeatures(
        'VariationsEnabled');
        // Save categories
        foreach ($this->response->CategoryArray->Category as $cat) {
            ...
            Db::getInstance()->autoExecute(_DB_PREFIX_ . 'ebay_category', 
            array('id_category_ref' => pSQL($category['CategoryID']), 
            'id_category_ref_parent' => pSQL($category['CategoryParentID']), 
            'id_country' => '8', 'level' => pSQL($category['CategoryLevel']), 
            'is_multi_sku' => pSQL($category['IsMultiSku']), 
            'name' => pSQL($category['CategoryName'])), 'INSERT');
        }
        // Return
        return true;
    }
    function GetCategoryFeatures ($featureID)
    {
        ...
        $requestXml .= '<GetCategoryFeatures xmlns="urn:ebay:apis:eBLBaseComponents">' .
         "\n";
        $requestXml .= '  <RequesterCredentials>' . "\n";
        $requestXml .= '    <eBayAuthToken>' .
         Configuration::get('EBAY_API_TOKEN') . '</eBayAuthToken>' . "\n";
        $requestXml .= '  </RequesterCredentials>' . "\n";
        $requestXml .= '  <DetailLevel>ReturnAll</DetailLevel>' . "\n";
    ...
    function getSuggestedCategories ($query)
    {
        ...
        $requestXml .= '  <Version>' . $this->compatibilityLevel . '</Version>' .
         "\n";
        $requestXml .= '  <WarningLevel>High</WarningLevel>' . "\n";
        $requestXml .= '  <Query>' . substr(strtolower($query), 0, 350) .
         '</Query>' . "\n";
        ...
        if (isset(
        $response->SuggestedCategoryArray->SuggestedCategory[0]->Category->CategoryID))
    ...
    function addFixedPriceItem ($datas = array())
    {
        ...
        $requestXml .= '    <Description><![CDATA[' . $datas['description'] .
         ']]></Description>' . "\n";
        $requestXml .= '    <PrimaryCategory>' . "\n";
        $requestXml .= '      <CategoryID>' . $datas['categoryId'] .
         '</CategoryID>' . "\n";
        $requestXml .= '    </PrimaryCategory>' . "\n";
        ...
        $requestXml .= '    <PayPalEmailAddress>' .
         Configuration::get('EBAY_PAYPAL_EMAIL') . '</PayPalEmailAddress>' . "\n";
        $requestXml .= '    <PostalCode>' .
         Configuration::get('EBAY_SHOP_POSTALCODE') . '</PostalCode>' . "\n";
        ...
        $requestXml .= '        <ShippingServicePriority>1</ShippingServicePriority>' .
         "\n";
        $requestXml .= '        <ShippingService>' . $datas['shippingService'] .
         '</ShippingService>' . "\n";
        $requestXml .= '        <FreeShipping>false</FreeShipping>' . "\n";
        $requestXml .= '        <ShippingServiceCost currencyID="EUR">' .
         $datas['shippingCost'] . '</ShippingServiceCost>' . "\n";
        $requestXml .= '      </ShippingServiceOptions>' . "\n";
        $requestXml .= '    </ShippingDetails>' . "\n";
        ...
        $this->error = '';
        $this->errorCode = '';
        if (isset($this->response->Errors) && isset($this->response->Ack) &&
         (string) $this->response->Ack != 'Success' &&
         (string) $this->response->Ack != 'Warning')
            foreach ($this->response->Errors as $e) {
             ...
             // Checking Success
        $this->itemID = 0;
        if (isset($this->response->Ack) && ((string) $this->response->Ack ==
         'Success' || (string) $this->response->Ack == 'Warning')) {
            $this->fees = 0;
    ...
    function reviseFixedPriceItem ($datas = array())
    {
        ...
        if (Configuration::get('EBAY_SYNC_OPTION_RESYNC') != 1) {
            $requestXml .= '    <Title>' . substr($datas['name'], 0, 55) .
             '</Title>' . "\n";
            $requestXml .= '    <Description><![CDATA[' . $datas['description'] .
             ']]></Description>' . "\n";
            $requestXml .= '    <ShippingDetails>' . "\n";
            ...
            $requestXml .= '        <ShippingServicePriority>1</ShippingServicePriority>' .
             "\n";
            $requestXml .= '        <ShippingService>' .
             $datas['shippingService'] . '</ShippingService>' . "\n";
            $requestXml .= '        <FreeShipping>false</FreeShipping>' . "\n";
            $requestXml .= '        <ShippingServiceCost currencyID="EUR">' .
             $datas['shippingCost'] . '</ShippingServiceCost>' . "\n";
            $requestXml .= '      </ShippingServiceOptions>' . "\n";
        ...
        if (isset($datas['variations'])) {
            ...
            foreach ($datas['variations'] as $key => $variation) {
                ...
                $requestXml .= '        <SKU>prestashop-' . $key . '</SKU>' .
                 "\n";
                if (! isset($datas['noPriceUpdate']))
                    $requestXml .= '        <StartPrice>' . $variation['price'] .
                     '</StartPrice>' . "\n";
                $requestXml .= '        <Quantity>' . $variation['quantity'] .
                 '</Quantity>' . "\n";
            ...
            foreach ($datas['variations'] as $key => $variation)
                foreach ($variation['variations'] as $kv => $v)
                    if (! isset($attributeUsed[md5($v['name'] . $v['value'])]) &&
                     isset($variation['pictures'][$kv])) {
                        if ($lastSpecificName != $v['name'])
                            $requestXml .= '        <VariationSpecificName>' .
                             $v['name'] . '</VariationSpecificName>' . "\n";
                        $requestXml .= '        <VariationSpecificPictureSet>' .
                         "\n";
                        $requestXml .= '          <VariationSpecificValue>' .
                         $v['value'] . '</VariationSpecificValue>' . "\n";
                        $requestXml .= '          <PictureURL>' .
                         $variation['pictures'][$kv] . '</PictureURL>' . "\n";
                        $requestXml .= '        </VariationSpecificPictureSet>' .
                         "\n";
    ...
    function getOrders ($CreateTimeFrom, $CreateTimeTo, $page)
    {
        ...
        $requestXml .= '  <ErrorLanguage>fr_FR</ErrorLanguage>' . "\n";
        $requestXml .= '  <WarningLevel>High</WarningLevel>' . "\n";
        $requestXml .= '  <CreateTimeFrom>' . $CreateTimeFrom .
         '</CreateTimeFrom>' . "\n";
            ...
            foreach ($this->response->OrderArray->Order as $order) {
                ...
                for ($i = 0; isset($order->TransactionArray->Transaction[$i]); $i ++) {
                    ...
                    if ($id_product > 0)
                        ...
                        if ((int) $id_product > 0)
                            $itemList[] = array('id_product' => $id_product, 
                            'id_product_attribute' => 0, 'quantity' => $quantity, 
                            'price' => (string) $transaction->TransactionPrice);
                        else {
                ...
                $orderList[] = array('id_order_ref' => (string) $order->OrderID, 
                'amount' => (string) $order->AmountPaid, 
                'status' => (string) $order->CheckoutStatus->Status, 
                'date' => substr((string) $order->CreatedTime, 0, 10) . ' ' .
                 substr((string) $order->CreatedTime, 11, 8), 
                'name' => (string) $order->ShippingAddress->Name, 
                'firstname' => substr(trim($name[0]), 0, 32), 
                'familyname' => (isset($name[1]) ? substr(trim($name[1]), 0, 32) : substr(
                trim($name[0]), 0, 32)), 
                ...
                'date_add' => substr((string) $order->CreatedTime, 0, 10) . ' ' .
                 substr((string) $order->CreatedTime, 11, 8));//'object' => $order
