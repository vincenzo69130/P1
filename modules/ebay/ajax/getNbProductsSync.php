...
if (file_exists($configPath)) {
    ...
    array('sync' => (int) ($_GET['action'])), 'UPDATE', 
	...
	AND `id_category_default` IN (SELECT `id_category` FROM `' . _DB_PREFIX_ .
     'ebay_category_configuration` WHERE `id_ebay_category` > 0 AND `sync` = 1)');
    echo $nbProducts;
} else
    echo 'ERROR';
