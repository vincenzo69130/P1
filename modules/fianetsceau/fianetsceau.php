...
class FianetSceau extends Module
{
    ...
    {
        return (parent::install() and $this->registerHook('rightColumn') and
         $this->registerHook('updateOrderStatus') and Db::getInstance()->Execute(
        '
    ...
    private function getProcess ()
    {
        ...
        if (! preg_match('#^[0-9]+$#', Tools::getValue('FIANET_SCEAU_SITEID')))
            ...
            Configuration::updateValue('FIANET_SCEAU_SITEID', 
            Tools::getValue('FIANET_SCEAU_SITEID'));
        Configuration::updateValue('FIANET_SCEAU_PRIVATEKEY', 
        Tools::getValue('FIANET_SCEAU_PRIVATEKEY'));
        if ((int) Tools::getValue('fia_net_mode'))
            $dataSync = (($site_id = Configuration::get('FIANET_SCEAU_SITEID')) ? '<img src="http://www.prestashop.com/modules/fianetsceau.png?site_id=' .
             urlencode($site_id) . '" style="float:right" />' : 'toto');
        else
            $dataSync = '';
    ...
    public function getContent ()
    {
        ...
        if (! is_callable('curl_init'))
            $output .= parent::displayError(
            'You need to enable Curl library to use this module');
		...
		<form action="' . Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']) . '" method="post">
			<fieldset class="width2">
				<legend><img src="' . $this->_path . 'logo.gif" alt="" class="middle" />' .
         $this->l('Settings') . '</legend>
				<label>' . $this->l('Your site ID') . '</label>
				<div class="margin-form">
					<input type="text" name="FIANET_SCEAU_SITEID" value="' .
         Configuration::get('FIANET_SCEAU_SITEID') . '" />
					<p class="clear">' . $this->l('Sample:') .
         ' site_id = \'<b>XXXXX</b>\' ' . $this->l('(numbers only)') . '</p>
				</div>
				<label>' . $this->l('Private Key') . '</label>
				<div class="margin-form">
					<input type="text" name="FIANET_SCEAU_PRIVATEKEY" value="' .
         Configuration::get('FIANET_SCEAU_PRIVATEKEY') . '" />
         ...
         $this->l('Production') . '</label></span>
				</div>
				<p class="clear">&nbsp;</p>
				<input type="submit" name="submitFianet" value="' .
    ...
    private function sendXML ()
    {
        ...
        '<?xml version="1.0" encoding="ISO-8859-1"?><control></control>');
        ...
        $cryptKey = md5(
        Configuration::get('FIANET_SCEAU_PRIVATEKEY') . '_' . (int) $order->id .
         '+' . $order_date . '=' . $customer->email);
        $control->addChild('crypt', $cryptKey);
        $XMLInfo = $control->asXML();
        $CheckSum = md5($XMLInfo);
        $curl = curl_init(
        'https://www.fia-net.com/engine/' .
         (! Configuration::get('FIA_NET_SEAL_MODE') ? 'preprod/' : '') .
         'sendrating.cgi');
        ...
        'SiteID=' . $SiteID . '&XMLInfo=' . $XMLInfo . '&CheckSum=' . $CheckSum);
    ...
    public function hookUpdateOrderStatus ($params)
    {
        ...
        'SELECT * FROM `' . _DB_PREFIX_ . 'fianet_seal` a WHERE a.id_order = ' .
         (int) $params['id_order']);
        ...
        if (! $res) {
            ...
            '
				...
				VALUES (' . (int) $order->id . ', 0,' .
             (int) $params['newOrderStatus']->logable . ', ' .
             (int) $params['newOrderStatus']->id . ', NOW(), NOW())');
        } else {
            ...
            '
				...
				SET valid = ' . (int) $params['newOrderStatus']->logable .
             ', upload_success = ' . (int) $upload_success . ', status = ' .
             (int) $params['newOrderStatus']->id . ', date_upd = NOW()
    ...
    {
        global $cookie;
        return '<a href="javascript:;" onclick="varwin=window.open(\'https://www.fia-net.com/certif/certificat.php?key=' .
         Configuration::get('FIANET_SCEAU_SITEID') . '&amp;lang=' .
         Language::getIsoById((int) ($cookie->id_lang)) .
         '\', \'certificat\', \'width=650, height=510\', \'toolbar=no, location=no,directories=no, status=no, menubar=no, scrollbars=no, resizable=yes,dependent=yes\');"><img src="https://www.fia-net.com/img/logos/' .
         (($cookie->id_lang != 2) ? 'en/' : '') .
         'rouge3bc.gif" title="Voir la fiche marchand sur Fia-net.com" alt="Voir la fiche marchand sur Fia-net.com" /></a>';
    }
}
