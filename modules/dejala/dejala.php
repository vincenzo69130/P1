    ...
    public function getContent ()
    {
        ...
        if (! empty($_POST)) {
            ...
            if (count($errors))
				...
				' . $this->l('Settings updated') . (($method == 'signin' or
                 $method == 'register' or $method == 'golive') ? '<img src="http://www.prestashop.com/modules/dejala.png?pspid=' .
             ...
             Validate::isUnsignedId($djlCart->id_delivery)) {
                ...
                if ($response['status'] == 200) {
                    if ($l_delivery && $l_delivery['status'] &&
                     $l_delivery['status']['labels'] && $l_delivery['status']['labels'][Language::getIsoById(
                    (int) $this->id_lang)])
                        echo $this->l('Order') . ' ' . $l_delivery['status']['labels'][Language::getIsoById(
                        (int) $this->id_lang)] . '<br/>';
                    else
    ...
    public function hookProcessCarrier ($params)
    {
        ...
        if (! empty($dejalaCarrierID) && ! empty($carrierID) &&
         ((int) ($dejalaCarrierID) == (int) ($carrierID))) {
            ...
            $date_shipping = 'NULL';
            if (isset($_POST['shipping_day']) and
             ! empty($_POST['shipping_day']) and
             (10 <= strlen($_POST['shipping_day']))) {
