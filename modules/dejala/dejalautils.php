...
class DejalaUtils
{
    ...
    {
        if ($mode !== 'PROD')
            $serviceURL = $dejalaConfig->getRootServiceURI('TEST') .
             '/mystore/delivery';
        else
            $serviceURL = $dejalaConfig->getRootServiceURI('PROD') .
             '/mystore/delivery';
        $responseArray = $this->makeRequest($dejalaConfig, $serviceURL, 
        $delivery, 'POST', TRUE);
    ...
    public function getStoreCalendar ($dejalaConfig, &$calendar)
    {
        ...
        $responseArray = $this->makeRequest($dejalaConfig, $serviceURL, array(), 
        'GET', TRUE);
        if (! ($xml = strstr($responseArray['response'], '<?xml')))
            $xml = null;
        else {
    ...
    public function getStoreQuotation ($dejalaConfig, $quotationElements, 
    &$products)
    {
        ...
        if (! ($xml = strstr($responseArray['response'], '<?xml')))
            ...
            foreach ($productsNodeList as $productNode) {
                ...
                foreach ($exceptionNodeList as $exceptionNode)
                    $currentProduct['calendar']['exceptions'][] = $this->getNodeValue(
                    $exceptionNode);
                    /*$currentProduct = array();
				$nodeList = $productNode->childNodes;
				foreach ($nodeList as $element) {
        ...
        if ($node instanceof DOMElement) {
            ...
            foreach ($childNodes as $childNode) {
                if ($childNode instanceof DOMElement) {
                    $onlyText = FALSE;
                    $value[$childNode->nodeName] = $this->getNodeValue(
                    $childNode);
                }
            }
            if ($onlyText)
    ...
    public function getStoreDeliveries ($dejalaConfig, &$deliveries, 
    $args = array())
    {
        $serviceURL = $dejalaConfig->getRootServiceURI() . '/mystore/deliveries';
        $responseArray = $this->makeRequest($dejalaConfig, $serviceURL, $args, 
        'GET', TRUE);
        if (! ($xml = strstr($responseArray['response'], '<?xml')))
            $xml = null;
        else {
    ...
    public function makeRequest ($dejalaConfig, $serviceURL, $args, 
    $method = 'POST', $needAuth = TRUE)
    {
        ...
        if ($needAuth) {
            ...
            $dejalaConfig->login . ':' . $dejalaConfig->password);
