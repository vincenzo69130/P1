...
class CalendarUtils
{
    ...
    public function adjustHour ($dateUtc, $calendar)
    {
        ...
        if ($currentMin > 0) {
            ...
            $dateUtc = mktime($currentHour, 0, 0, date('m', $dateUtc), 
            date('d', $dateUtc), date('Y', $dateUtc));
        }
        // si on est avant l'heure de départ, on se met à l'heure de départ
        if ($currentHour < $startHour) {
            $dateUtc = mktime($startHour, 0, 0, date('m', $dateUtc), 
            date('d', $dateUtc), date('Y', $dateUtc));
        }
        return ($dateUtc);
    }
    ...
    public function addDelay ($dateUtc, $delay, $calendar, $exceptions)
    {
        ...
        $dateUtc = $this->getNextDateAvailable($dateUtc, $calendar, 
        $exceptions);
        if (! $dateUtc)
            return (null);
        if ($delay == '0.5') {
            ...
            if ($hour < 12) {
                $dateUtc = mktime('14', 0, 0, date('m', $dateUtc), 
                date('d', $dateUtc), date('Y', $dateUtc));
            } else {
        ...
        while ($deliveryDelay --) {
            ...
            $dateUtc = mktime(0, 0, 0, date('m', $dateUtc), date('d', $dateUtc), 
            date('Y', $dateUtc));
            $dateUtc = $this->getNextDateAvailable($dateUtc, $calendar, 
            $exceptions);
        }
    ...
    public function skipOneDay ($dateUtc)
    {
        ...
        $dateUtc = mktime(0, 0, 0, date('m', $dateUtc), 
        date('d', $dateUtc), date('Y', $dateUtc));
        return $dateUtc;
    }
    public function skipCurDay ($dateUtc)
    {
        $currentDayZero = mktime(0, 0, 0, date('m', time()), date('d', time()), 
        date('Y', time()));
        $dateUtcZero = mktime(0, 0, 0, date('m', $dateUtc), date('d', $dateUtc), 
        date('Y', $dateUtc));
    ...
    public function getNextDateAvailable ($dateUtc, $calendar, $exceptions)
    {
        ...
        do {
            $dateUtc = strtotime(date("Y-m-d", $dateUtc) . " +1 day");
            $isDateFree = $this->isDateAvailable($dateUtc, $calendar, 
            $exceptions);
            $loopcount ++;
