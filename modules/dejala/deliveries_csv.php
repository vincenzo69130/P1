...
if (! is_null($from) && ! is_null($to) && (strlen($from) == 10) &&
 (strlen($to) == 10)) {
    $dateFrom = mktime(0, 0, 1, (int) (substr($from, 3, 2)), 
    (int) (substr($from, 0, 2)), (int) (substr($from, 6, 4)));
    $dateTo = mktime(23, 59, 59, (int) (substr($to, 3, 2)), 
    (int) (substr($to, 0, 2)), (int) (substr($to, 6, 4)));
    if ($dateFrom > $dateTo) {
        $tmp = $dateTo;
        $dateTo = $dateFrom;
        $dateFrom = $tmp;
    ...
    $deliveries = array();
    $responseArray = $djlUtil->getStoreDeliveries($dejalaConfig, $deliveries, 
    array('from_utc' => $dateFrom, 'to_utc' => $dateTo));
    if ($responseArray['status'] = '200') {
        ...
        foreach ($deliveries as $key => $delivery) {
            ...
            $l_delivery['creation_date'] = date('d/m/Y', 
            $delivery['creation_utc']);
            $l_delivery['creation_time'] = date('H\hi', 
            $delivery['creation_utc']);
            if (isset($delivery['shipping_start_utc'])) {
                $l_delivery['shipping_date'] = date('d/m/Y', 
                $delivery['shipping_start_utc']);
                $l_delivery['shipping_start'] = date('H\hi', 
                $delivery['shipping_start_utc']);
                $l_delivery['shipping_stop'] = date('H\hi', 
            ...
            if (isset($l_delivery['delivery_utc'])) {
                $l_delivery['delivery_date'] = date('d/m/Y', 
                $delivery['delivery_utc']);
                $l_delivery['delivery_time'] = date('H\hi', 
                $delivery['delivery_utc']);
            }
