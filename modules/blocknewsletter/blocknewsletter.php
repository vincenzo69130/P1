...
class Blocknewsletter extends Module
{
    ...
    public function uninstall ()
    {
        if (! parent::uninstall())
            return false;
        return Db::getInstance()->Execute(
        'DROP TABLE ' . _DB_PREFIX_ . 'newsletter');
    }
    public function getContent ()
    {
        $this->_html = '<h2>' . $this->displayName . '</h2>';
        if (Tools::isSubmit('submitUpdate')) {
            if (isset($_POST['new_page']) and
             Validate::isBool((int) ($_POST['new_page'])))
                Configuration::updateValue('NW_CONFIRMATION_NEW_PAGE', 
                $_POST['new_page']);
            if (isset($_POST['conf_email']) and
             VAlidate::isBool((int) ($_POST['conf_email'])))
                Configuration::updateValue('NW_CONFIRMATION_EMAIL', 
                pSQL($_POST['conf_email']));
            if (! empty($_POST['voucher']) and
             ! Validate::isDiscountName($_POST['voucher']))
                $this->_html .= '<div class="alert">' .
                 $this->l('Voucher code is invalid') . '</div>';
            else {
                Configuration::updateValue('NW_VOUCHER_CODE', 
                pSQL($_POST['voucher']));
                $this->_html .= '<div class="conf ok">' . $this->l('Updated') .
                 '</div>';
            }
        }
    ...
    private function _displayForm ()
    {
		...
		<form method="post" action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '">
         ...
         (Configuration::get('NW_CONFIRMATION_NEW_PAGE') ? 'checked="checked" ' : '') .
         '/>' . $this->l('yes') . '
         ...
         (Configuration::get('NW_CONFIRMATION_EMAIL') ? 'checked="checked" ' : '') .
         '/>' . $this->l('yes') . '
         ...
         Configuration::get('NW_VOUCHER_CODE') . '" />
					<p>' . $this->l('Leave blank for disabling') .
         '</p>
				</div>
    ...
    private function isNewsletterRegistered ($customerEmail)
    {
        ...
        'SELECT `email` FROM ' . _DB_PREFIX_ . 'newsletter WHERE `email` = \'' .
         pSQL($customerEmail) . '\''))
            return 1;
        if (! $registered = Db::getInstance()->getRow(
        'SELECT `newsletter` FROM ' . _DB_PREFIX_ . 'customer WHERE `email` = \'' .
         pSQL($customerEmail) . '\''))
            return - 1;
    ...
    private function newsletterRegistration ()
    {
        ...
        elseif ($_POST['action'] == '1') {
            $registerStatus = $this->isNewsletterRegistered(
            pSQL($_POST['email']));
            if ($registerStatus < 1)
            ...
            elseif ($registerStatus == 1) {
                ...
                'DELETE FROM ' . _DB_PREFIX_ . 'newsletter WHERE `email` = \'' .
                 pSQL($_POST['email']) . '\''))
                    return $this->error = $this->l(
...
elseif ($registerStatus == 2) {
                if (! Db::getInstance()->Execute(
                'UPDATE ' . _DB_PREFIX_ .
                 'customer SET `newsletter` = 0 WHERE `email` = \'' .
                 pSQL($_POST['email']) . '\''))
                    return $this->error = $this->l(
                    'Error during unsubscription');
                return $this->valid = $this->l('Unsubscription successful');
...
elseif ($_POST['action'] == '0') {
            ...
            elseif ($registerStatus == - 1) {
                ...
                'INSERT INTO ' . _DB_PREFIX_ .
                 'newsletter (email, newsletter_date_add, ip_registration_newsletter, http_referer) VALUES (\'' .
                 pSQL($_POST['email']) . '\', NOW(), \'' .
                 pSQL(Tools::getRemoteAddr()) . '\', 
					(SELECT c.http_referer FROM ' . _DB_PREFIX_ .
                 'connections c WHERE c.id_guest = ' . (int) ($cookie->id_guest) .
...
elseif ($registerStatus == 0) {
                if (! Db::getInstance()->Execute(
                'UPDATE ' . _DB_PREFIX_ .
                 'customer SET `newsletter` = 1, newsletter_date_add = NOW(), `ip_registration_newsletter` = \'' .
                 pSQL(Tools::getRemoteAddr()) . '\' WHERE `email` = \'' .
                 pSQL($_POST['email']) . '\''))
                    return $this->error = $this->l('Error during subscription');
                $this->sendVoucher(pSQL($_POST['email']));
                return $this->valid = $this->l('Subscription successful');
    ...
    private function sendVoucher ($email)
    {
        ...
        if ($discount = Configuration::get('NW_VOUCHER_CODE'))
            return Mail::Send((int) ($cookie->id_lang), 'newsletter_voucher', 
            Mail::l('Newsletter voucher'), array('{discount}' => $discount), 
    ...
    function hookLeftColumn ($params)
    {
        ...
        if (Tools::isSubmit('submitNewsletter')) {
            ...
            if ($this->error) {
                $smarty->assign(
                array('color' => 'red', 'msg' => $this->error, 
                'nw_value' => isset($_POST['email']) ? pSQL($_POST['email']) : false, 
                'nw_error' => true, 'action' => $_POST['action']));
            } elseif ($this->valid) {
                if (Configuration::get('NW_CONFIRMATION_EMAIL') and
                 isset($_POST['action']) and (int) ($_POST['action']) == 0)
                    Mail::Send((int) ($params['cookie']->id_lang), 
                    'newsletter_conf', Mail::l('Newsletter confirmation'), 
                    array(), pSQL($_POST['email']), NULL, NULL, NULL, NULL, NULL, 
                    dirname(__FILE__) . '/mails/');
                $smarty->assign(
                array('color' => 'green', 'msg' => $this->valid, 
                'nw_error' => false));
            }
        }
