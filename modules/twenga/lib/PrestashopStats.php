...
class PrestashopStats
{
    ...
    private static function buildUrlToQuery ($url, $params = array())
    {
        ...
        if (Configuration::get('PS_TWENGA_KEY') !== false &&
         Configuration::get('PS_TWENGA_KEY') !== '')
            $params['key'] = Configuration::get('PS_TWENGA_KEY');
    ...
    public function actSubscription ()
    {
        ...
        if (trim($return['response']) !== '' &&
         Validate::isMd5($return['response']))
    ...
    {
        $params = array('order_amount_ttc' => (float) $amount_ttc, 
        'order_amount_ht' => (float) $amount_ht);
        $str = self::buildUrlToQuery(self::$arr_ps_stats_url[__FUNCTION__], 
        $params);
        $this->executeQuery($str);
    }
