...
class TwengaObj
{
    ...
    {
        if ($password !== '' && Validate::isCleanHtml($password) &&
    ...
    {
        if ($hashkey !== '' && Validate::isCleanHtml($hashkey) &&
         is_string($hashkey) && strlen($hashkey) <= 32) {
    ...
    {
        if (self::$user_name !== NULL && self::$password !== NULL &&
         self::$hashkey !== NULL) {
            $site_exist = false;
            try {
    ...
    public function __construct ()
    {
        ...
        if (self::$arr_api_url === NULL) {
            ...
            if (self::PARTNER_AUTH_KEY === NULL)
                throw new TwengaException(
                self::$translation_object->l(
                'To activate the Twenga plugin, "PARTNER_AUTH_KEY" contant must be set. Default installation of Prestashop contains this value.', 
                basename(__FILE__, '.php')));
            if (Configuration::get('TWENGA_HASHKEY') !== false &&
             self::$hashkey === NULL)
                self::$hashkey = Configuration::get('TWENGA_HASHKEY');
            if (Configuration::get('TWENGA_USER_NAME') !== false &&
             self::$user_name === NULL)
                self::$user_name = Configuration::get('TWENGA_USER_NAME');
            if (Configuration::get('TWENGA_PASSWORD') !== false &&
             self::$password === NULL)
     ...
     */
    private static function executeQuery ($query, $params = array(), 
    $authentication = true)
    {
        $defaultParams = array(CURLOPT_HEADER => TRUE, 
        CURLOPT_RETURNTRANSFER => TRUE, CURLINFO_HEADER_OUT => TRUE, 
        CURLOPT_HTTPAUTH => CURLAUTH_BASIC);
        if ($authentication)
            $defaultParams[CURLOPT_USERPWD] = md5(self::$user_name) . ':' .
             md5(self::$password);
    ...
    public static function checkParams ($method_name, $params)
    {
        ...
        if (class_exists($classname)) {
            ...
            if (! $fields instanceof TwengaFields)
                throw new TwengaException(
                self::$translation_object->l(
                'Object for validate fields must be an instance of TwengaFields class', 
                basename(__FILE__, '.php')));
            try {
    ...
    public function getSubscriptionLink ($params = array())
    {
        ...
        dirname(__FILE__) . '/TwengaFieldsGetSubscriptionLink.php');
        ...
        $str_params = self::buildUrlToQuery(self::$arr_api_url[__FUNCTION__], 
        $params);
        try {
            ...
            throw new TwengaException(
            self::$translation_object->l($e->getMessage(), 
            basename(__FILE__, '.php')), $e->getCode());
        }
    ...
    {
        require_once realpath(
        dirname(__FILE__) . '/TwengaFieldsGetTrackingScript.php');
        $params['key'] = self::$hashkey;
        $params['PARTNER_AUTH_KEY'] = self::PARTNER_AUTH_KEY;
        try {
            self::checkParams(__FUNCTION__, $params);
    ...
    {
        require_once realpath(
        dirname(__FILE__) . '/TwengaFieldsOrderValidate.php');
    ...
    public static function parseXml ($resource)
    {
        if ($resource != '') {
            ...
            if ($xml->getName() === 'error')
                throw new TwengaException(
                self::$translation_object->l((string) $xml->message, 
                basename(__FILE__, '.php')), (int) $xml->code);
            return $xml;
        } else
            throw new TwengaException(
            self::$translation_object->l('HTTP response is empty'));
    }
    /**
    ...
    protected static function checkStatusCode ($status_code)
    {
        switch ($status_code) {
            ...
            case 201:
                break;
            default:
                throw new TwengaException('', (int) $status_code);
        }
    }
...
class TwengaException extends Exception
{
    ...
    {
        if ($code !== 0) {
            $error_label = self::$translation_object->l(
            'This call to Twenga Web Services failed and returned an HTTP status of %d. That means:', 
            basename(__FILE__, '.php')) . "\n";
            $error_label = sprintf($error_label, $code);
            if ($message === '' || empty($message) || $message === NULL) {
                switch ($code) {
                    case 11:
                        $message .= self::$translation_object->l(
                        'The function you tried to access does not exist', 
                        basename(__FILE__, '.php'));
                        break;
                    case 12:
                        $message .= self::$translation_object->l(
                        'A required field is empty', basename(__FILE__, '.php'));
                        break;
                    case 13:
                        $message .= self::$translation_object->l(
                        'The type of the parameter that has been sent is different from parameter expected type', 
                        basename(__FILE__, '.php'));
                        break;
                    case 21:
                        $message .= self::$translation_object->l(
                        'Hash key is not valid', basename(__FILE__, '.php'));
                        break;
                    case 24:
                        $message .= self::$translation_object->l(
                        'Hash key is required', basename(__FILE__, '.php'));
                        break;
                    case 31:
                        $message .= self::$translation_object->l(
                        'No order found. Please check parameters you sent', 
                        basename(__FILE__, '.php'));
                        break;
                    case 401:
                        $message .= self::$translation_object->l(
                    ...
                    case 403:
                        $message .= self::$translation_object->l(
                        'Authentication failed.', basename(__FILE__, '.php'));
                        break;
                    case 404:
                        $message .= self::$translation_object->l('Invalid url.', 
                        basename(__FILE__, '.php'));
                        break;
                    case 500:
                        $message .= self::$translation_object->l(
                    ...
                    case 503:
                        $message .= self::$translation_object->l(
                        'The server is unavailable for the moment', 
                        basename(__FILE__, '.php'));
                        break;
                    default:
                        $message .= self::$translation_object->l(
                        'This call to PrestaShop Web Services returned an unexpected HTTP status of:', 
                        basename(__FILE__, '.php')) . $code;
                }
            }
            $message = self::$translation_object->l('Error #', 
            basename(__FILE__, '.php')) . $code . " : \n" . $error_label .
             $message . '';
        }
