...
abstract class TwengaFields
{
    ...
    private function checkFields ()
    {
        ...
        if (! is_array($this->requiredFields) and
         $this->noRequiredFields === false) {
    ...
    private function checkFieldAttributs ($name, $field_attributs)
    {
        ...
        if (! Validate::isInt($field_attributs[0]))
            throw new TwengaFieldsException(
            Tools::displayError(
            'To add a field, you have to set a maximum length for checking the value. Error occurred for the value') .
             ' : ' . $name);
             // attribut at key 1 is an array list of the validator
        if (! is_array($field_attributs[1]))
            throw new TwengaFieldsException(
            Tools::displayError(
            'To add a field, you have to set validators for checking value. Error occurred for the value') .
             ' : ' . $name);
             // Check if validators setted are valid. 
        foreach ($field_attributs[1] as $validator) {
            if (! function_exists($validator) &&
             ! method_exists('Validate', $validator) &&
             ! method_exists($this->className, $validator))
                throw new TwengaFieldsException(
                Tools::displayError('The Validator') . ' ' . $validator . ' ' .
                 Tools::displayError('does\'nt exist'));
        }
        // attribut at key 2 means that this fields is required or not
        if (isset($field_attributs[2]) and
         Validate::isBool($field_attributs[2]))
    ...
    public function getField ($name)
    {
        if (! is_array($this->fields))
            throw new TwengaFieldsException(
            Tools::displayError(
            'To get a field you have to saved some fields !'));
        if (! key_exists($name, $this->fields))
            throw new TwengaFieldsException(
            Tools::displayError('The field') . ' <b>' . $name . '</b> ' .
             Tools::displayError('doesn\'t exist.'));
    ...
    private function isValidate ($key, $value)
    {
        ...
        $str_return = '';
        // check the length
        if (strlen((string) $value) > $fieldValidate[0] and
         $fieldValidate[0] !== 0)
            return Tools::displayError(
            'Wrong length of the value. Must be set between 1 and ') .
             $fieldValidate[0] . '<br />' . "\n";
             // check each validators.
        foreach ($fieldValidate[1] as $validator) {
            $user_function = function_exists($validator) ? $validator : '';
            $user_function = method_exists('Validate', $validator) ? array(
            'Validate', $validator) : $user_function;
            $user_function = method_exists($this->className, $validator) ? array(
            $this->className, $validator) : $user_function;
            try {
                $bool = call_user_func($user_function, $value);
                if (! $bool)
                    $str_return .= Tools::displayError(
                    'Value don\'t respect the validator : ') . '<b>' . $validator .
                     '</b><br />' . "\n";
                ;
            } catch (TwengaFieldsException $e) {
    ...
    private function compareFields ()
    {
        if (is_array($this->requiredFields) and ! empty($this->requiredFields)) {
            $arr_compare = array_intersect_key($this->requiredFields, 
            $this->params);
    ...
    private function requiredFieldsAreSet ()
    {
        if (is_array($this->requiredFields) and ! empty($this->requiredFields)) {
            ...
            if (! empty($fields_not_set))
                throw new TwengaFieldsException(
                Tools::displayError('Some fields must be set') . ' : <b>' .
                 implode(', ', $fields_not_set) . '</b>');
        }
        return true;
    }
    /**
    ...
    public function setParams ($params)
    {
        if (empty($params))
            throw new TwengaFieldsException(
            Tools::displayError('Params must be filled'));
        $this->params = $params;
        return $this;
    }
    /**
    ...
    public function checkParams ()
    {
        if (! is_array($this->params) || empty($this->params))
            throw new TwengaFieldsException(
            Tools::displayError('Params must be setted before check it'));
        try {
            $this->compareFields();
        ...
        foreach ($this->params as $key => $value) {
            ...
            if ($validate !== '')
                $str_message .= Tools::displayError('The field') . ' <b>' . $key .
                 '</b> ' . Tools::displayError(
                'is a wrong value, see details :') . '<br />' . "\n<em>" .
                 $validate . '</em>';
        }
        if ($str_message !== '')
    ...
    public function l ($string, $specific = false)
    {
        ...
        $currentKey = '<{' . $this->name . '}' . _THEME_NAME_ . '>' . $source .
         '_' . md5($string2);
        $defaultKey = '<{' . $this->name . '}prestashop>' . $source . '_' .
         md5($string2);
