...
class BlockSupplier extends Module
{
    ...
    function hookLeftColumn ($params)
    {
        global $smarty, $link;
        $smarty->assign(
        array('suppliers' => Supplier::getSuppliers(false), 'link' => $link, 
    ...
    public function displayForm ()
    {
        ...
        Configuration::get('SUPPLIER_DISPLAY_TEXT')) ? 'checked="checked" ' : '') .
         '/>
        ...
        Configuration::get('SUPPLIER_DISPLAY_TEXT')) ? 'checked="checked" ' : '') .
         ...
         $this->l('Disabled') . '" title="' . $this->l('Disabled') . '" /></label>
					&nbsp;&nbsp;&nbsp;' . $this->l('Display') .
         ' <input type="text" size="2" name="text_nb" value="' . Tools::getValue(
        'text_nb', Configuration::get('SUPPLIER_DISPLAY_TEXT_NB')) . '" /> ' .
         $this->l('elements') . '
					<p class="clear">' .
         $this->l('To display suppliers as a plain-text list') . '</p>
				</div>
				<label>' . $this->l('Use a drop-down list') . '</label>
				<div class="margin-form">
					<input type="radio" name="form_list" id="form_list_on" value="1" ' .
         (Tools::getValue('form_list', 
        Configuration::get('SUPPLIER_DISPLAY_FORM')) ? 'checked="checked" ' : '') .
         '/>
        ...
        Configuration::get('SUPPLIER_DISPLAY_FORM')) ? 'checked="checked" ' : '') .
         ...
         $this->l('Disabled') . '" title="' . $this->l('Disabled') . '" /></label>
					<p class="clear">' .
         $this->l('To display suppliers as a drop-down list') . '</p>
				</div>
				<center><input type="submit" name="submitBlockSuppliers" value="' .
