...
* @copyright 2007-2011 PrestaShop SA * @version Release: $Revision: 9095
...
<p><label for="id_country">{l s='Country' mod='carriercompare'}</label>
<select name="id_country" id="id_country">
	{foreach from=$countries item=country}
	<option value="{$country.id_country}" {if $id_country==
		$country.id_country}selected="selected"{/if}>{$country.name|escape:'htmlall':'UTF-8'}</option>
	{/foreach}
</select></p>

<p id="states" style="display: none;"><label for="id_state">{l s='State'
mod='carriercompare'}</label> <select name="id_state" id="id_state">

</select></p>

<p><label for="zipcode">{l s='Zipcode' mod='carriercompare'}</label> <input
	type="text" name="zipcode" id="zipcode"
	value="{$zipcode|escape:'htmlall':'UTF-8'}" /> ({l s='Needed for
certain carriers' mod='carriercompare'})</p>
...
<table cellspacing="0" cellpadding="0" id="availableCarriers_table"
	...
	<thead>
		<tr>
			...
			<th class="carrier_infos item">{l s='Information'
			mod='carriercompare'}</th>
			<th class="carrier_price last_item">{l s='Price'
			mod='carriercompare'}</th>
		</tr>
	</thead>
...
<p class="warning center" id="noCarrier" style="display: none;">{l s='No
carrier is available for this selection' mod='carriercompare'}</p>

<p class="center"><input class="exclusive_large"
	id="carriercompare_submit" type="submit" name="carriercompare_submit"
	value="{l s='Refresh' mod='carriercompare'}" /></p>
</fieldset>
</form>
{/if}
