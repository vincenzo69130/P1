...
class BlockSpecials extends Module
{
    ...
    function __construct ()
    {
        ...
        'Adds a block with current product specials.');
    }
    ...
    public function getContent ()
    {
        ...
        if (Tools::isSubmit('submitSpecials')) {
            Configuration::updateValue('PS_BLOCK_SPECIALS_DISPLAY', 
    ...
    public function displayForm ()
    {
        ...
        Configuration::get('PS_BLOCK_SPECIALS_DISPLAY')) ? 'checked="checked" ' : '') .
         '/>
        ...
        Configuration::get('PS_BLOCK_SPECIALS_DISPLAY')) ? 'checked="checked" ' : '') .
         ...
         $this->l('Disabled') . '" title="' . $this->l('Disabled') . '" /></label>
					<p class="clear">' .
         $this->l('Show the block even if no product is available.') . '</p>
				</div>
				<center><input type="submit" name="submitSpecials" value="' .
    ...
    public function hookRightColumn ($params)
    {
        if (Configuration::get('PS_CATALOG_MODE'))
            return;
        global $smarty;
        if (! $special = Product::getRandomSpecial(
        (int) ($params['cookie']->id_lang)) and
         ! Configuration::get('PS_BLOCK_SPECIALS_DISPLAY'))
            return;
        $smarty->assign(
        array('special' => $special, 
        'priceWithoutReduction_tax_excl' => Tools::ps_round(
        $special['price_without_reduction'], 2), 
