...
class HIPAY_MAPI_Affiliate extends HIPAY_MAPI_lockable
{
    ...
    protected function setAmount ()
    {
        if ($this->percentageTarget > 0) {
            $this->_amount = sprintf('%.02f', 
            ($this->_baseAmount / 100) * $this->val);
        } else {
    ...
    {
        if ($this->customerId <= 0 || $this->accountId <= 0 || $this->val <= 0 ||
         $this->percentageTarget < 0)
            throw new Exception(
            'Customer identifier, account number, value or invalid target');
        return true;
    }
