...
class HIPAY_MAPI_PaymentParams extends HIPAY_MAPI_lockable
{
     ...
     * @return boolean
     */
    public function setAccounts ($itemAccount, $taxAccount = 0, 
    $insuranceAccount = 0, $fixedCostAccount = 0, $shippingCostAccount = 0)
    {
        if ($this->_locked)
            return false;
    ...
    public function setPaymentMethod ($paymentMethod)
    {
        ...
        if ($paymentMethod != HIPAY_MAPI_METHOD_SIMPLE &&
         $paymentMethod != HIPAY_MAPI_METHOD_MULTI)
            return false;
    ...
    public function setCaptureDay ($captureDay)
    {
        ...
        if (($captureDay != HIPAY_MAPI_CAPTURE_MANUAL &&
         $captureDay != HIPAY_MAPI_CAPTURE_IMMEDIATE && $captureDay <= 0) ||
         $captureDay > HIPAY_MAPI_CAPTURE_MAX_DAYS)
            return false;
    ...
    public function setEmailAck ($email_ack)
    {
        ...
        if (strlen($email_ack) > HIPAY_MAPI_MAX_ACKMAIL_LENGTH ||
         (! HIPAY_MAPI_UTILS::checkemail($email_ack) && $email_ack != ''))
            return false;
    ...
    public function check ()
    {
        ...
        if ($this->itemAccount <= 0 || $this->taxAccount <= 0 ||
         $this->insuranceAccount <= 0 || $this->fixedCostAccount <= 0 ||
