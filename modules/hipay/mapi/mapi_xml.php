    ...
    public function getXML ($t = 0, $noshow = true)
    {
        ...
        foreach ($this as $name => $value) {
            ...
            if (! is_array($this->$name) && ! is_object($this->$name) &&
             ...
             method_exists($this->$name, 'getXML')) {
    ...
    protected function getXMLArray ($array, $t = 0, $noshow = true)
    {
        ...
        foreach ($array as $name => $value) {
            if (substr($name, 0, 1) == '_')
                continue;
            if (! is_array($array[$name]) && ! is_object($array[$name]) &&
             ! is_bool($array[$name])) {
                $xml .= str_repeat(chr(9), $t + 1) .
                 "<_aKey_$name>$value</_aKey_$name>\n";
             ...
             method_exists($array[$name], 'getXML')) {
