...
class HIPAY_MAPI_COMM_XML
{
    ...
    &$err_keyword, &$err_value, &$err_code)
    {
        $url = '';
        $err_msg = '';
        $err_keyword = '';
     ...
     * @return boolean
     */
    public static function analyzeNotificationXML ($xml, &$operation, &$status, 
    &$date, &$time, &$transid, &$origAmount, &$origCurrency, &$idformerchant, 
    &$merchantdatas)
    {
        $operation = '';
        $status = '';
