...
class HIPAY_MAPI_Payment extends HIPAY_MAPI_XML
{
    ...
    protected function compute ()
    {
        ...
        foreach ($this->order as $key => $order) {
            $this->_taxTotalAmount[$key] = $this->_taxItemsAmount[$key] +
             $this->_taxShippingAmount[$key] + $this->_taxInsuranceAmount[$key] +
             $this->_taxFixedCostAmount[$key];
            $this->_orderTotalAmountHT[$key] = $order->getShippingAmount() +
             $order->getInsuranceAmount() + $order->getFixedCostAmount() +
             $this->_itemsTotalAmount[$key];
            $this->_orderTotalAmount[$key] = $this->_orderTotalAmountHT[$key] +
             $this->_taxTotalAmount[$key];
    ...
    protected function computeTaxes ()
    {
        ...
        foreach ($this->items as $item) {
            ...
            $this->_taxItemsAmount[$cur] += ($tItemsAmount *
             $item->getQuantity());
            $item->lock();
            $cur ++;
        }
        ...
        foreach ($this->order as $order) {
            ...
            foreach ($taxArr as $key => $tax) {
                $amount = HIPAY_MAPI_UTILS::computeTax(
     ...
     */
    public function getTotalTaxes (&$tItemsAmount, &$tShippingAmount, 
    ...
    protected function computeAffiliates ()
    {
        foreach ($this->order as $k => $order) {
            ...
            foreach ($tAffiliate as $key => $affiliate) {
                ...
                if ($percentageTarget > 0) {
                    ...
                    if ($percentageTarget & HIPAY_MAPI_TTARGET_TAX)
                        $baseAmount += $this->_taxItemsAmount[$k] +
                         $this->_taxFixedCostAmount[$k] +
                         $this->_taxInsuranceAmount[$k] +
                         $this->_taxShippingAmount[$k];
            ...
            if ($totalAmount > $this->_orderTotalAmount[$k]) {
                throw new Exception(
                'The total amount to distribute is greather than the transaction amount (' .
                 $totalAmount . '/' . $this->_orderTotalAmount . ')');
            }
        }
    }
}