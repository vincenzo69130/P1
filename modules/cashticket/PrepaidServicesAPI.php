...
class PrepaidServicesAPI
{
    ...
    private static $base_url = array(
    'T' => array(
    'create_disposition_url' => 'https://shops.test.at.paysafecard.com/pscmerchant/CreateDispositionServlet', 
    ...
    'P' => array(
    'create_disposition_url' => 'https://shops.cc.at.paysafecard.com/pscmerchant/CreateDispositionServlet', 
    ...
    public static function createDisposition ($configuration, $mid, $mtid, 
    $amount, $currency, $okurl, $nokurl, $businesstype, $reportingcriteria)
    {
        $language = 'en';
        $params = 'currency=' . $currency . '&mid=' . $mid . '&mtid=' . $mtid .
         '&amount=' . $amount . '&businesstype=' . $businesstype .
         '&reportingcriteria=' . $reportingcriteria . '&okurl=' . $okurl .
         '&nokurl=' . $nokurl . '&language=' . $language;
        list ($rc, $msg, $data) = self::_doHttpRequest(
        self::getBaseUrl('create_disposition_url', $configuration['env']), 
        $params, $configuration['keyring_file'], $configuration['keyring_pw'], 
        $configuration['keyring_prepaid']);
        if ($rc == 0) {
            $data_array = explode("\n", $data, 7);
    ...
    public static function getDispositionState ($configuration, $mid, $mtid, 
    $currency_iso_code)
    {
        $language = 'en';
        $params = 'mid=' . $mid . '&mtid=' . $mtid . '&language=' . $language;
        list ($rc, $msg, $data) = self::_doHttpRequest(
        self::getBaseUrl('get_disposition_state_url', $configuration['env']), 
        $params, $configuration['keyring_file'], $configuration['keyring_pw'], 
        $configuration['keyring_prepaid']);
        if ($rc == 0) {
            $dataarray = explode("\n", $data, 7);
            ...
            return array($resultcode, $errorcode, $errormessage, $amount, 
            $currency, $state);
        } else {
            $resultcode = '9001';
            $errorcode = $rc;
    ...
    public static function getSerialNumbers ($configuration, $mid, $mtid, 
    $currency_iso_code)
    {
        $language = 'en';
        $params = 'mid=' . $mid . '&mtid=' . $mtid . '&language=' . $language;
        list ($rc, $msg, $data) = self::_doHttpRequest(
        self::getBaseUrl('get_serial_number_url', $configuration['env']), 
        $params, $configuration['keyring_file'], $configuration['keyring_pw'], 
        $configuration['keyring_prepaid']);
        if ($rc == 0) {
    ...
    public static function executeDebit ($configuration, $mid, $mtid, $amount, 
    $currency, $close_flag = 1)
    {
        $language = 'en';
        $params = 'currency=' . $currency . '&mid=' . $mid . '&mtid=' . $mtid .
         '&amount=' . $amount . '&close=' . $close_flag . '&language=' .
         $language;
        list ($rc, $msg, $data) = self::_doHttpRequest(
        self::getBaseUrl('execute_debit_url', $configuration['env']), $params, 
        $configuration['keyring_file'], $configuration['keyring_pw'], 
        $configuration['keyring_prepaid']);
        if ($rc == 0) {
            $dataarray = explode("\n", $data, 7);
            $resultcode = trim($dataarray[0]);
    ...
    private static function _doHttpRequest ($url, $urlparam, $keyringfile, 
    $keyringpw, $cakeyringfile)
    {
