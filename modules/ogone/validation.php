...
$neededVars = array('orderID', 'amount', 'currency', 'PM', 'ACCEPTANCE', 
'STATUS', 'CARDNO', 'PAYID', 'NCERROR', 'BRAND', 'SHASIGN');
...
if (Validate::isLoadedObject($cart)) {
    ...
    foreach ($_GET as $key => $value)
        if (strtoupper($key) != 'SHASIGN' and $value != '' and
    ...
    if ($sha1 == $_GET['SHASIGN']) {
        switch ($_GET['STATUS']) {
            case 1:
				...
				$ogone->validate((int) $_GET['orderID'], 
                Configuration::get('PS_OS_ERROR'), 0, $_GET['NCERROR'] . $params, 
                $_GET['secure_key']);
                break;
            case 2:
                ...
                $ogone->l('Error (auth. refused)') . '<br />' . $_GET['NCERROR'] .
            ...
            case 9:
				/* Payment OK */
				$ogone->validate((int) $_GET['orderID'], 
                Configuration::get('PS_OS_PAYMENT'), (float) ($_GET['amount']), 
                $ogone->l('Payment authorized / OK') . $params, 
                $_GET['secure_key']);
                break;
            case 6:
            case 7:
            case 8:
                ...
                if ($id_order = (int) (Order::getOrderByCartId(
                (int) ($_GET['orderID'])))) {
                    ...
                    Configuration::get('PS_OS_ERROR'), $id_order);
                    $history->addWithemail(true, array());
                }
                break;
            default:
                $ogone->validate((int) $_GET['orderID'], 
                Configuration::get('PS_OS_ERROR'), (float) ($_GET['amount']), 
                $ogone->l('Unknown status:') . ' ' . $_GET['STATUS'] . $params, 
                $_GET['secure_key']);
        }
        exit();
    } else {
        $message = $ogone->l('Invalid SHA-1 signature') . '<br />' .
         $ogone->l('SHA-1 given:') . ' ' . $_GET['SHASIGN'] . '<br />' .
         $ogone->l('SHA-1 calculated:') . ' ' . $sha1 . '<br />' .
         $ogone->l('Plain key:') . ' ' . $shasign;
        $ogone->validate((int) $_GET['orderID'], 
        Configuration::get('PS_OS_ERROR'), 0, $message . '<br />' . $params, 
        $_GET['secure_key']);
    }
}