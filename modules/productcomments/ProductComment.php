...
class ProductComment extends ObjectModel
{
    ...
    'id_customer' => 'isUnsignedId', 'content' => 'isMessage', 
    ...
    public static function getByProduct ($id_product, $p = 1, $n = null)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'customer` c ON c.`id_customer` = pc.`id_customer`
		WHERE pc.`id_product` = ' .
         (int) ($id_product) . ($validate == '1' ? ' AND pc.`validate` = 1' : '') . '
		ORDER BY pc.`date_add` DESC
		' .
         ($n ? 'LIMIT ' . (int) (($p - 1) * $n) . ', ' . (int) ($n) : ''));
    }
    public static function getByCustomer ($id_product, $id_customer, 
    $last = false, $id_guest = false)
    {
		...
		WHERE pc.`id_product` = ' .
         (int) $id_product . ' AND ' .
         (! $id_guest ? 'pc.`id_customer` = ' . (int) $id_customer : 'pc.`id_guest` = ' .
         (int) $id_guest) . '
		ORDER BY pc.`date_add` DESC ' .
         ($last ? ' LIMIT 1' : ''));
        if (! $results)
    ...
    public static function getGradeByProduct ($id_product, $id_lang)
    {
		...
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_comment_grade` pcg ON (pcg.`id_product_comment` = pc.`id_product_comment`)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'product_comment_criterion` pcc ON (pcc.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
		LEFT JOIN `' .
         _DB_PREFIX_ . 'product_comment_criterion_lang` pccl ON (pccl.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
    ...
    {
        /* Get all grades */
        $grades = ProductComment::getGradeByProduct((int) ($id_product), 
        (int) ($id_lang));
        ...
        for ($i = 0; $i < count($grades); ++ $i)
            if (array_key_exists($grades[$i]['id_product_comment_criterion'], 
            $criterionsGradeTotal) === false)
        ...
        foreach ($criterionsGradeTotal as $key => $criterionGradeTotal)
            $averages[(int) ($key)] = (int) ($total) ? ((int) ($criterionGradeTotal) /
             (int) ($total)) : 0;
        return $averages;
    }
    /**
    ...
    public static function getCommentNumber ($id_product)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'product_comment` pc
		WHERE `id_product` = ' .
         (int) ($id_product) . ($validate == '1' ? ' AND `validate` = 1' : ''))) ===
         false)
            return false;
    ...
    public static function getByValidate ($validate = '0', $deleted = false)
    {
		...
		LEFT JOIN `' .
         ...
         'customer` c ON (c.`id_customer` = pc.`id_customer`) 
		LEFT JOIN `' .
         _DB_PREFIX_ . 'product_lang` pl ON (pl.`id_product` = pc.`id_product`) 
