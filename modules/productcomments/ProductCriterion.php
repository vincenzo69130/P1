...
class ProductCommentCriterion
{
    ...
    {
        if (! Validate::isUnsignedId($id_lang) || ! Validate::isMessage($name))
     ...
     * @return boolean succeed
     */
    public static function addToProduct ($id_product_comment_criterion, 
    $id_product)
    {
     ...
     * @return boolean succeed
     */
    public static function addGrade ($id_product_comment, 
    $id_product_comment_criterion, $grade)
    {
     ...
     * @return boolean succeed
     */
    public static function update ($id_product_comment_criterion, $id_lang, 
    $name)
    {
        if (! Validate::isUnsignedId($id_product_comment_criterion) ||
         ! Validate::isUnsignedId($id_lang) || ! Validate::isMessage($name))
            die(Tools::displayError());
        return (Db::getInstance()->Execute(
        '
		...
		WHERE `id_product_comment_criterion` = ' .
         (int) ($id_product_comment_criterion) . ' AND
		`id_lang` = ' . (int) ($id_lang)));
    }
    /**
    ...
    public static function getByProduct ($id_product, $id_lang)
    {
		...
		INNER JOIN `' .
         _DB_PREFIX_ . 'product_comment_criterion_product` pccp ON pcc.`id_product_comment_criterion` = pccp.`id_product_comment_criterion`
