...
class BlockWishList extends Module
{
        ...
        foreach ($sql as $query)
            if ($query)
                if (! Db::getInstance()->Execute(trim($query)))
                    return false;
        if (! parent::install() or ! $this->registerHook('rightColumn') or
         ! $this->registerHook('productActions') or ! $this->registerHook(
        'cart') or ! $this->registerHook('customerAccount') or
    ...
    public function uninstall ()
    {
        ...
        'DROP TABLE ' . _DB_PREFIX_ . 'wishlist') and Db::getInstance()->Execute(
        'DROP TABLE ' . _DB_PREFIX_ . 'wishlist_email') and Db::getInstance()->Execute(
        'DROP TABLE ' . _DB_PREFIX_ . 'wishlist_product') and Db::getInstance()->Execute(
        'DROP TABLE ' . _DB_PREFIX_ . 'wishlist_product_cart') and
         parent::uninstall());
    }
        ...
        if (Tools::isSubmit('submitSettings')) {
            ...
            if ($activated != 0 and $activated != 1)
                $this->_html .= '<div class="alert error">' .
                 $this->l('Activate module : Invalid choice.') . '</div>';
    ...
    private function _displayFormView ()
    {
		...
		<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post" id="listing">
			<fieldset>
				<legend><img src="' . $this->_path .
         'img/icon/package_go.png" alt="" title="" />' . $this->l('Listing') . '</legend>
        ...
        foreach ($customers as $customer) {
            ...
            $this->_html .= '>' .
             htmlentities($customer['firstname'], ENT_COMPAT, 'UTF-8') . ' ' .
             htmlentities($customer['lastname'], ENT_COMPAT, 'UTF-8') .
             '</option>';
        }
        $this->_html .= '
					</select>
        ...
        foreach ($wishlists as $wishlist) {
            ...
            $this->_html .= '>' .
             htmlentities($wishlist['name'], ENT_COMPAT, 'UTF-8') . '</option>';
        }
        $this->_html .= '
					</select>
				</div>';
    ...
    public function hookRightColumn ($params)
    {
        ...
        if ($params['cookie']->isLogged()) {
            $wishlists = Wishlist::getByIdCustomer(
            $params['cookie']->id_customer);
            if (empty($params['cookie']->id_wishlist) === true || WishList::exists(
            $params['cookie']->id_wishlist, $params['cookie']->id_customer) ===
             false) {
                if (! sizeof($wishlists))
                    $id_wishlist = false;
                else {
            ...
            $smarty->assign(
            array('id_wishlist' => $id_wishlist, 'isLogged' => true, 
            'wishlist_products' => ($id_wishlist == false ? false : WishList::getProductByIdCustomer(
            $id_wishlist, $params['cookie']->id_customer, 
            $params['cookie']->id_lang, null, true)), 'wishlists' => $wishlists, 
            'ptoken' => Tools::getToken(false)));
        } else
            $smarty->assign(
            array('wishlist_products' => false, 'wishlists' => false));
    ...
    private function _displayProducts ($id_wishlist)
    {
        ...
        $products = WishList::getProductByIdCustomer((int) ($id_wishlist), 
        (int) ($wishlist->id_customer), (int) ($cookie->id_lang));
        for ($i = 0; $i < sizeof($products); ++ $i) {
            ...
            if (! Validate::isLoadedObject($obj))
                ...
                if (! isset($products[$i]['cover']))
                    $products[$i]['cover'] = Language::getIsoById(
                    (int) ($cookie->id_lang)) . '-default';
            }
        }
        $this->_html .= '
		<table class="table">
			<thead>
				<tr>
					...
					<th class="item" style="text-align:center;width:150px;">' .
         $this->l('Quantity') . '</th>
					<th class="item" style="text-align:center;width:150px;">' .
         $this->l('Priority') . '</th>
				</tr>
        ...
        foreach ($products as $product) {
            $this->_html .= '
				<tr>
					<td class="first_item">
						<img src="' . $link->getImageLink($product['link_rewrite'], 
            $product['cover'], 'small') . '" alt="' .
             htmlentities($product['name'], ENT_COMPAT, 'UTF-8') . '" style="float:left;" />
						' . $product['name'];
            if (isset($product['attributes_small']))
                $this->_html .= '<br /><i>' .
                 htmlentities($product['attributes_small'], ENT_COMPAT, 'UTF-8') .
                 '</i>';
            $this->_html .= '
					</td>
					<td class="item" style="text-align:center;">' .
             (int) ($product['quantity']) . '</td>
					<td class="item" style="text-align:center;">' .
             $priority[(int) ($product['priority']) % 3] . '</td>
				</tr>';
        }
    ...
    public function hookAdminCustomers ($params)
    {
        ...
        if (! sizeof($wishlists))
            $this->_html .= $customer->lastname . ' ' . $customer->firstname .
             ' ' . $this->l('No wishlist.');
        else {
            $this->_html .= '<form action="' .
             Tools::safeOutput($_SERVER['REQUEST_URI']) .
             '" method="post" id="listing">';
            $id_wishlist = (int) (Tools::getValue('id_wishlist'));
            if (! $id_wishlist)
                $id_wishlist = $wishlists[0]['id_wishlist'];
            $this->_html .= '<span>' . $this->l('Wishlist') .
             ': </span> <select name="id_wishlist" onchange="$(\'#listing\').submit();">';
            foreach ($wishlists as $wishlist) {
                ...
                $this->_html .= '>' .
                 htmlentities($wishlist['name'], ENT_COMPAT, 'UTF-8') .
                 '</option>';
            }
            $this->_html .= '</select>';
            $this->_displayProducts((int) ($id_wishlist));
