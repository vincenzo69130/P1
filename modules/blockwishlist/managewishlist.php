    ...
    if (empty($id_wishlist) === false) {
        if (! strcmp($action, 'update')) {
            ...
            if (! strcmp($action, 'delete'))
                ...
                (int) ($cookie->id_customer), $id_product, $id_product_attribute);
            $products = WishList::getProductByIdCustomer($id_wishlist, 
            $cookie->id_customer, $cookie->id_lang);
            ...
            for ($i = 0; $i < sizeof($products); ++ $i) {
                ...
                if (! Validate::isLoadedObject($obj))
                    ...
                    if ($products[$i]['id_product_attribute'] != 0) {
                        $combination_imgs = $obj->getCombinationImages(
                        (int) ($cookie->id_lang));
                    ...
                    if (! isset($products[$i]['cover']))
                        ...
                        $cookie->id_lang) . '-default';
                }
                $products[$i]['bought'] = false;
                for ($j = 0, $k = 0; $j < sizeof($bought); ++ $j) {
                    if ($bought[$j]['id_product'] == $products[$i]['id_product'] and $bought[$j]['id_product_attribute'] ==
            ...
            $smarty->assign(
            array('products' => $products, 'productsBoughts' => $productBoughts, 
            'id_wishlist' => $id_wishlist, 'refresh' => $refresh, 
            'token_wish' => $wishlist->token));
            if (Tools::file_exists_cache(
            _PS_THEME_DIR_ . 'modules/blockwishlist/managewishlist.tpl'))
                $smarty->display(
                _PS_THEME_DIR_ . 'modules/blockwishlist/managewishlist.tpl');
            elseif (Tools::file_exists_cache(
            dirname(__FILE__) . '/managewishlist.tpl'))
