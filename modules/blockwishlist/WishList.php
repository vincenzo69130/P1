...
class WishList extends ObjectModel
{
    ...
    'name' => 'isMessage', 'token' => 'isMessage');
    ...
    public function delete ()
    {
        ...
        'DELETE FROM `' . _DB_PREFIX_ . 'wishlist_email` WHERE `id_wishlist` = ' .
         (int) ($this->id));
        Db::getInstance()->Execute(
        'DELETE FROM `' . _DB_PREFIX_ .
         'wishlist_product` WHERE `id_wishlist` = ' . (int) ($this->id));
        ...
        if (isset($old_carts) and $old_carts != false)
            foreach ($old_carts as $old_cart)
                ...
                '
					...
					WHERE id_cart=' . (int) ($old_cart['id_cart']) . ' AND id_product=' .
                 (int) ($old_cart['id_product']) . ' AND id_product_attribute=' .
                 (int) ($old_cart['id_product_attribute']));
            ...
            foreach ($freshwish as $prodcustomer) {
                ...
                '
					...
					(
						...
						WHERE `id_wishlist_product`=' .
                 (int) ($prodcustomer['id_wishlist_product']) . ' AND `id_cart`=' .
                 (int) ($prodcustomer['id_cart']) . '
					)
					WHERE `id_wishlist_product`=' .
                 (int) ($prodcustomer['id_wishlist_product']) .
    ...
    $id_lang, $id_product = null, $quantity = false)
    {
	  ...
	  FROM `' . _DB_PREFIX_ . 'wishlist_product` wp
		...
		AND wp.`id_wishlist` = ' . (int) ($id_wishlist) . (empty(
        $id_product) === false ? ' AND wp.`id_product` = ' . (int) ($id_product) : '') .
    ...
    $id_product, $id_product_attribute)
    {
        ...
        if (empty($result) === true or $result === false or ! sizeof($result) or
    ...
    $id_product_attribute, $id_cart, $quantity)
    {
        if (! Validate::isUnsignedId($id_wishlist) or
         ! Validate::isUnsignedId($id_product) or
         ! Validate::isUnsignedId($quantity))
    ...
    {
        if (! Validate::isUnsignedId($id_wishlist) or empty($email) or
