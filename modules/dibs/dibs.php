...
class dibs extends PaymentModule
{
     ...
     * @var array
     */
    private static $accepted_lang = array('da', 'en', 'es', 'fi', 'fo', 'fr', 
    'it', 'nl', 'no', 'pl', 'sv');
    /**
    ...
    public function __construct ()
    {
        ...
        if (self::$site_url === NULL) {
            if (method_exists('Tools', 'getProtocol'))
                self::$site_url = Tools::htmlentitiesutf8(
                Tools::getProtocol() . $_SERVER['HTTP_HOST'] . __PS_BASE_URI__);
            else
                self::$site_url = Tools::htmlentitiesutf8(
                (! is_null($use_ssl) && $use_ssl ? 'https://' : 'http://') .
        ...
        self::$MORE_SETTINGS = Configuration::get('DIBS_MORE_SETTINGS') != '' ? unserialize(
        Tools::htmlentitiesDecodeUTF8(Configuration::get('DIBS_MORE_SETTINGS'))) : array();
        if (! isset(self::$MORE_SETTINGS['k1']) or
         (isset(self::$MORE_SETTINGS['k1']) and
         (self::$MORE_SETTINGS['k1'] === '' or self::$MORE_SETTINGS['k2'] === '')))
            $this->warning = $this->l(
            'For security reasons, you must set key #1 and key #2 used by MD5 control of DIBS API.');
        if (! self::$ID_MERCHANT or self::$ID_MERCHANT === '')
            $this->warning = $this->l(
            'You have to set your merchant ID to use DIBS API.');
    ...
    public function install ()
    {
        return (parent::install() and $this->registerHook('orderConfirmation') and
         $this->registerHook('payment') and Configuration::updateValue(
        'DIBS_ACCEPTED_URL', 
        self::$site_url .
         (substr(trim(self::$site_url), - 1, 1) === '/' ? '' : '/') .
         'order-confirmation.php') and
         Configuration::updateValue('DIBS_CANCELLED_URL', self::$site_url) and
         Configuration::updateValue('DIBS_TESTING', 1) and Configuration::updateValue(
        'DIBS_MORE_SETTINGS', 
        Tools::htmlentitiesUTF8(
        serialize(
        array('flexwin_color' => 'blue', 'logo_color' => 'black', 'k1' => '', 
        'k2' => ''))), true));
    }
    public function uninstall ()
    {
        return (parent::uninstall() and
         Configuration::deleteByName('DIBS_ACCEPTED_URL') and
         Configuration::deleteByName('DIBS_ID_MERCHANT') and
         Configuration::deleteByName('DIBS_CANCELLED_URL') and
         Configuration::deleteByName('DIBS_TESTING') and
         Configuration::deleteByName('DIBS_MORE_SETTINGS'));
    }
    public function hookOrderConfirmation ($params)
    {
        ...
        if ($params['objOrder']->valid)
            $this->smarty->assign(
            array('status' => 'ok', 'id_order' => $params['objOrder']->id));
        else
    ...
    private function preProcess ()
    {
        if (Tools::isSubmit('submitModule')) {
            self::$ID_MERCHANT = (Tools::getValue('idMerchant') !== '' ? Tools::getValue(
            'idMerchant') : self::$ID_MERCHANT);
            self::$ACCEPTED_URL = ((Validate::isUrl(
            Tools::getValue('acceptedUrl'))) ? Tools::getValue('acceptedUrl') : self::$ACCEPTED_URL);
            self::$CANCELLED_URL = ((Validate::isUrl(
            Tools::getValue('cancelledUrl'))) ? Tools::getValue('cancelledUrl') : self::$CANCELLED_URL);
            self::$TESTING = (int) isset($_POST['testing']);
            self::$MORE_SETTINGS['flexwin_color'] = Tools::getValue(
            'flexwin_color');
            ...
            Configuration::updateValue('DIBS_CANCELLED_URL', 
            self::$CANCELLED_URL);
            Configuration::updateValue('DIBS_TESTING', self::$TESTING);
            Configuration::updateValue('DIBS_MORE_SETTINGS', 
            Tools::htmlentitiesUTF8(serialize(self::$MORE_SETTINGS)));
            $data_sync = '';
            if (self::$ID_MERCHANT !== '' and self::$TESTING !== 1 and
             self::$MORE_SETTINGS['k1'] !== '' and
             self::$MORE_SETTINGS['k2'] !== '')
                $data_sync = '<img src="http://www.prestashop.com/modules/dibs.png?site_id=' .
                 urlencode(self::$ID_MERCHANT) . '" style="float:right" />';
            echo '<div class="conf confirm"><img src="../img/admin/ok.gif"/>' .
             $this->l('Configuration updated') . $data_sync . '</div>';
        }
    }
    private function _displayPresentation ()
    {
        $href = '';
        $langIsoCode = Language::getIsoById(
		...
		<fieldset class="width2">
         ...
         $this->l('Get a DIBS account') . '</legend>
			<p>
				' .
         $this->l(
        'Please click on the following link to access the DIBS subscription form:') .
         ' <a href="' . $href . '" class="link" target="_blank" >&raquo; ' .
         $this->l('Link') . ' &laquo;</a>
			</p>
			<p>' .
         $this->l(
        'Depending on language and country rules, subscription parameters may be different.') .
         '<br />' . $this->l('Please click on the appropriate flags:') . '&nbsp;';
        foreach (dibs::$dibs_subscription_link as $lang => $url) {
            $out .= '<a href="' . $url . '" title="' . $lang .
             '" class="link" target="_blank" ><img src="' . dibs::$site_url .
             'modules/dibs/img/' . $lang . '.jpg" /></a>';
        }
        $out .= '
			</p>
		</fieldset>';
    ...
    public function getContent ()
    {
        ...
        $logo_colors = array('yellow', 'grey', 'blue', 'black', 'purple', 
        'green');
        $str = '<h2>' . $this->displayName . '</h2>' .
         $this->_displayPresentation() . '<br />
		<form action="' . Tools::htmlentitiesutf8($_SERVER['REQUEST_URI']) . '" method="post">
			<fieldset class="width2">
				<legend><img src="../img/admin/contact.gif" />' . $this->l('Settings') . '</legend>
				<label>' . $this->l('Logo color') . '<br /><br />
				<img src="' . self::$site_url . 'modules/dibs/logos/dibs_' .
         self::$MORE_SETTINGS['logo_color'] . '.jpg" />
				</label>
				<div class="margin-form">';
        foreach ($logo_colors as $logo_color)
            $str .= '<input type="radio" name="logo_color" ' .
             (self::$MORE_SETTINGS['logo_color'] === $logo_color ? 'checked ' : '') .
             'value="' . $logo_color . '" /> ' . $logo_color . '<br />';
        $str .= '	<p>' .
         $this->l(
        'The basic color of the logo which appears on the payment page.') . '</p>
				</div>
				<label>' . $this->l('Merchant ID') . ' <sup>*</sup></label>
				<div class="margin-form">
					<input type="text" size="20" name="idMerchant" value="' .
         self::$ID_MERCHANT . '" />
					<p>' . $this->l('See the e-mail sent by DIBS.') . '</p>
				</div>
				<label>' . $this->l('Secure Key #1') . ' <sup>*</sup></label>
				<div class="margin-form">
					<input type="text" size="20" name="k1" value="' .
         Tools::htmlentitiesUTF8(self::$MORE_SETTINGS['k1']) . '" />
				</div>
				<label>' . $this->l('Secure Key #2') . ' <sup>*</sup></label>
				<div class="margin-form">
					<input type="text" size="20" name="k2" value="' .
         Tools::htmlentitiesUTF8(self::$MORE_SETTINGS['k2']) . '" />
					<p>' . $this->l('These keys are used for security improvement.') .
         '<br />' .
         $this->l(
        'To obtain these keys, go to the DIBS administration interface and under \'Integration\', select the MD5 Keys menu. Please ensure the MD5 control is activated, otherwise the module will not work.') . '</p>
				</div>
				<label>' . $this->l('Use DIBS test module') . '</label>
				<div class="margin-form">
					<input type="checkbox" name="testing" ' .
         (self::$TESTING === 1 ? 'checked ' : '') . 'value="testing" />
					<p>' .
         $this->l(
        'When this field is declared, the transaction is not dispatched to the card issuer, but is instead handled by the DIBS test module.') .
         $this->l('See also Step 5 of the 10 Step Guide for more information.') .
         ' <a href="http://tech.dibs.dk/10_step_guide/your_own_test/" title="' .
         $this->l('See also Step 5 of the 10 Step Guide for more information.') .
         '" target="_blank">> ' . $this->l('Link') . '</a><br />' .
         $this->l(
        'During the initial integration with DIBS, there is no need to insert this parameter, since all default transactions will reach the DIBS test system until DIBS has approved integration. Should the test system be used at a later date, this will be activated at DIBS (contact DIBS support for reactivating the test mode of your shop).') . '</p>
				</div>
				<label>' . $this->l(
        'Flexwin color') . '</label>
				<div class="margin-form">';
        foreach ($flexwin_colors as $flexwin_color)
            $str .= '<input type="radio" name="flexwin_color" ' .
             (self::$MORE_SETTINGS['flexwin_color'] === $flexwin_color ? 'checked ' : '') .
             'value="' . $flexwin_color . '" /> ' . $flexwin_color . '<br />';
        $str .= '	<p>' . $this->l('The basic color theme of FlexWin.') . '</p>
				</div>
				<label>' . $this->l('Accepted url') . ' <sup>*</sup></label>
				<div class="margin-form">
					<input type="text" size="20" name="acceptedUrl" value="' .
         self::$ACCEPTED_URL . '" />
					<p>' . $this->l(
        'URL of the page to be displayed if the purchase is approved.') . '</p>
				</div>
				<label>' . $this->l('Cancelled url') . ' <sup>*</sup></label>
				<div class="margin-form">
					<input type="text" size="20" name="cancelledUrl" value="' .
         self::$CANCELLED_URL . '" />
					<p>' .
         $this->l(
        'URL of the page to be displayed if the customer cancels the payment.') . '</p>
				</div>
				<br /><center><input type="submit" name="submitModule" value="' .
         $this->l('Update settings') . '" class="button" /></center>
			</fieldset>
		</form>';
        return $str;
    }
    ...
    {
        if ((self::$ID_MERCHANT === false || self::$ID_MERCHANT === '' ||
         self::$ID_MERCHANT === NULL) || (self::$ACCEPTED_URL === false ||
         self::$ACCEPTED_URL === '' || self::$ACCEPTED_URL === NULL))
            return '';
        ...
        $dibsParams['amount'] = $params['cart']->getOrderTotal(
        true, Cart::BOTH) * 100; // The smallest unit of an amount, cent for EUR
        $dibsParams['accepturl'] = self::$ACCEPTED_URL . '?id_cart=' .
         (int) ($params['cart']->id) . '&id_module=' . (int) ($this->id) .
         '&key=' . $customer->secure_key; // The URL of the page to be displayed if the purchase is approved.
        $dibsParams['orderid'] = $params['cart']->id . '_' .
         date('YmdHis'); // The shop's order number for this particular puchase. It can be seen later when payment is captured, and will in some instances appear on the customer's bank statement (max. 50 characters, both numerals and letters may be used).
        $currency_num = 0;
        // for 1.3 compatibility
        if (! isset($currency->iso_code_num) or
         $currency->iso_code_num == '') {
            $array_currency_iso_num = array('DKK' => 208, 'EUR' => 978, 
            'USD' => 840, 'GBP' => 826, 'SEK' => 752, 'AUD' => 036, 'CAD' => 124, 
            'ISK' => 352, 'JPY' => 392, 'NZD' => 554, 'NOK' => 578, 'CHF' => 756, 
            'TRY' => 949);
        ...
        $dibsParams['lang'] = in_array(strtolower($lang->iso_code), 
        self::$accepted_lang) ? $lang->iso_code : ''; // optional - This parameter determines the language in which the page will be opened. The following values are accepted: da=Danish en=English es=Spanish fi=Finnish fo=Faroese fr=French it=Italian nl=Dutch no=Norwegian pl=Polish (simplified) sv=Swedish Default language is Danish.
        ...
        $dibsParams['uniqueoid'] = (int) ($params['cart']->id) . '_' .
         date('YmdHis') . '_' . $params['cart']->secure_key; // optional - If this field exists, the orderid-field must be unique, i.e. there is no existing transaction with DIBS with the same order number. If such a transaction already exists, payment will be rejected with reason=7. Unless you are unable to generate unique order numbers, we strongly urge you to utilize this field.Note: Order numbers can be composed of a maximum of 50 characters (DIBS automatically removes surplus characters) and that uniqueoid is therefore unable to work as intended if order numbers consisting of more than 50 characters are used.
        $dibsParams['callbackurl'] = self::$site_url . 'modules/' .
        ...
        $md5_params = 'merchant=' . self::$ID_MERCHANT . '&orderid=' .
         $dibsParams['orderid'] . '&currency=' . $dibsParams['currency'] .
         '&amount=' . $dibsParams['amount'];
        $dibsParams['md5key'] = md5(
        self::$MORE_SETTINGS['k2'] .
         md5(self::$MORE_SETTINGS['k1'] . $md5_params)); // optional - This variable enables a MD5 key control of the values received by DIBS. This control  confirms that the values sent to DIBS has not been tampered with during the transfer. The MD5 key is calculated as:  MD5(key2 + MD5(key1 + "merchant=&orderid=&transact="))  Where key1 and key2 are shop specific keys available through the DIBS administration interface, and + is the concatenation operator. NB! MD5 key check must also be enabled through the DIBS administration interface in order to work. Further details on MD5-key control.  
		 ...
		 * -------------------------------------- */
        // delivery params
        $dibsParams['delivery1.Name'] = $address->firstname . ' ' .
         $address->lastname;
        ...
        foreach ($products as $key => $product) {
            $dibsParams['ordline' . $count_products . '-1'] = 'ref.' .
             ($product['reference'] != '' ? $product['reference'] : 'NC') .
             '/id.' . $product['id_product'];
            $dibsParams['ordline' . $count_products . '-2'] = $product['name'];
            $dibsParams['ordline' . $count_products . '-3'] = strip_tags(
        ...
        // Price info
        $dibsParams['priceinfo1.Deliverycosts'] = $params['cart']->getOrderTotal(
        true, Cart::ONLY_SHIPPING);
        $dibsParams['priceinfo2.ProductsAmount'] = $params['cart']->getOrderTotal(
        true, Cart::BOTH_WITHOUT_SHIPPING);
        $dibsParams['priceinfo3.AmountWithoutTax'] = $params['cart']->getOrderTotal(
        false, Cart::BOTH);
        $dibsParams['priceinfo4.AmountTotalTax'] = (float) ($params['cart']->getOrderTotal(
        true, Cart::BOTH) - $params['cart']->getOrderTotal(false, Cart::BOTH));
