...
class StatsNewsletter extends ModuleGraph
{
    ...
    public function hookAdminStatsModules ($params)
    {
        if (Module::isInstalled('blocknewsletter')) {
			...
			<fieldset class="width3"><legend><img src="../modules/' .
             ...
             '</legend>
				<p>' .
             $this->l('Registrations from customers:') . ' ' .
             (int) ($totals['customers']) .
             '</p>
				<p>' .
             $this->l('Registrations from visitors:') . ' ' .
             (int) ($totals['visitors']) . '</p>
				<p>' . $this->l('Both:') .
             ' ' . (int) ($totals['both']) .
             '</p>
				<center>' .
            ...
            $this->_html = '<p>' .
             $this->l('Module Newsletter Block must be installed') . '</p>';
        return $this->_html;
    }
    private function getTotals ()
    {
		...
		WHERE n.`newsletter_date_add` BETWEEN ' .
         ModuleGraph::getDateBetween());
        return array('customers' => $result1['customers'], 
    ...
    protected function setAllTimeValues ($layers)
    {
        ...
        $this->_query2 . $this->getDate());
        foreach ($result1 as $row)
            $this->_values[0][(int) (substr($row['newsletter_date_add'], 0, 4))] += 1;
        if ($result2)
            foreach ($result2 as $row)
                $this->_values[1][(int) (substr($row['newsletter_date_add'], 0, 
                4))] += 1;
        foreach ($this->_values[2] as $key => $zerofill)
            $this->_values[2][$key] = $this->_values[0][$key] +
             $this->_values[1][$key];
    }
    protected function setYearValues ($layers)
    {
        ...
        if ($result2)
            foreach ($result2 as $row)
                $this->_values[1][(int) (substr($row['newsletter_date_add'], 5, 
                2))] += 1;
        foreach ($this->_values[2] as $key => $zerofill)
            $this->_values[2][$key] = $this->_values[0][$key] +
             $this->_values[1][$key];
    ...
    protected function setMonthValues ($layers)
    {
        ...
        if ($result2)
            foreach ($result2 as $row)
                $this->_values[1][(int) (substr($row['newsletter_date_add'], 8, 
                2))] += 1;
        foreach ($this->_values[2] as $key => $zerofill)
            $this->_values[2][$key] = $this->_values[0][$key] +
             $this->_values[1][$key];
    ...
    protected function setDayValues ($layers)
    {
        ...
        if ($result2)
            foreach ($result2 as $row)
                $this->_values[1][(int) (substr($row['newsletter_date_add'], 11, 
                2))] += 1;
        foreach ($this->_values[2] as $key => $zerofill)
            $this->_values[2][$key] = $this->_values[0][$key] +
             $this->_values[1][$key];
