...
if (! $cookie->isLogged()) {
    ...
    'SELECT id_customer FROM ' . _DB_PREFIX_ . 'customer WHERE email=\'' .
     pSQL($customer_email) . '\' AND is_guest=0');
...
'
	REPLACE INTO `' .
 _DB_PREFIX_ .
 'mailalert_customer_oos` (`id_customer`, `customer_email`, `id_product` , `id_product_attribute`)
	VALUES (' .
 (int) ($id_customer) . ', \'' . pSQL($customer_email) . '\', ' .
