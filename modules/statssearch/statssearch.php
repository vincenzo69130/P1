...
class StatsSearch extends ModuleGraph
{
    ...
    function __construct ()
    {
        ...
        'Display which keywords have been searched by your visitors.');
    }
    function install ()
    {
        if (! parent::install() or ! $this->registerHook('search') or
    ...
    function uninstall ()
    {
        if (! parent::uninstall())
            return false;
        return (Db::getInstance()->Execute(
        'DROP TABLE `' . _DB_PREFIX_ . 'statssearch`'));
    }
    function hookSearch ($params)
    {
        Db::getInstance()->Execute(
        'INSERT INTO `' . _DB_PREFIX_ .
         'statssearch` (`keywords`,`results`,`date_add`) VALUES (\'' .
         pSQL($params['expr']) . '\', ' . (int) ($params['total']) . ', \'' .
         date('Y-m-d H:i:s') . '\')');
    }
    function hookAdminStatsModules ()
    {
		...
		<thead>
         ...
         $this->l('Occurrences') . '</th>
				<th style="width:50px; text-align: right">' .
         $this->l('Results') . '</th>
			</tr>
		</thead><tbody>';
        foreach ($result as $row) {
            if (Tools::strlen($row['keywords']) >=
             Configuration::get('PS_SEARCH_MINWORDLEN'))
                $table .= '<tr>
					<td>' . $row['keywords'] . '</td>
