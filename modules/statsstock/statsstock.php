...
class StatsStock extends Module
{
    ...
    function hookAdminStatsModules ()
    {
        ...
        if (Tools::isSubmit('submitCategory'))
            $cookie->statsstock_id_category = Tools::getValue(
            'statsstock_id_category');
        $ru = $currentIndex . '&module=' . $this->name . '&token=' .
         Tools::getValue('token');
        $currency = new Currency(Configuration::get('PS_CURRENCY_DEFAULT'));
        $filter = ((int) $cookie->statsstock_id_category ? 'WHERE p.id_product IN (SELECT cp.id_product FROM ' .
         _DB_PREFIX_ . 'category_product cp WHERE cp.id_category = ' .
         (int) $cookie->statsstock_id_category . ')' : '');
		...
		SELECT p.id_product, p.reference, pl.name,
			IFNULL((
				...
				FROM ' .
         _DB_PREFIX_ . 'product_attribute pa WHERE p.id_product = pa.id_product
        ...
        foreach (Category::getSimpleCategories($cookie->id_lang) as $category)
            echo '<option value="' . (int) $category['id_category'] . '" ' .
             ($cookie->statsstock_id_category == $category['id_category'] ? 'selected="selected"' : '') .
        ...
        if (! count($products))
            ...
            $rollup = array('quantity' => 0, 'wholesale_price' => 0, 
            'stockvalue' => 0);
            ...
            foreach ($products as $product) {
                ...
                echo '<tr>
					...
					<td>' .
                 Tools::displayPrice($product['wholesale_price'], $currency) . '</td>
					<td>' .
                 Tools::displayPrice($product['stockvalue'], $currency) . '</td>
				</tr>';
            }
            echo '
				<tr>
             ...
             '</td>
					<td>' .
             Tools::displayPrice($rollup['wholesale_price'] / count($products), 
            $currency) . '</td>
					<td>' .
             Tools::displayPrice($rollup['stockvalue'], $currency) . '</td>
				</tr>
			</table>
			<p>* ' .
             $this->l('Average price when the product has attributes.') . '</p>';
        }
    }
}
