...
class BlockViewed extends Module
{
    ...
    function __construct ()
    {
        ...
        'Adds a block displaying last-viewed products.');
    }
    function install ()
    {
    ...
    public function getContent ()
    {
        ...
        if (Tools::isSubmit('submitBlockViewed')) {
            ...
            elseif ((int) ($productNbr) == 0)
                ...
                Configuration::updateValue('PRODUCTS_VIEWED_NBR', 
                (int) ($productNbr));
    ...
    public function displayForm ()
    {
		...
		<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">
         ...
         Configuration::get('PRODUCTS_VIEWED_NBR') . '" />
					<p class="clear">' .
         $this->l('Define the number of products displayed in this block') . '</p>
				</div>
				<center><input type="submit" name="submitBlockViewed" value="' .
    ...
    function hookRightColumn ($params)
    {
        ...
        $productsViewed = (isset($params['cookie']->viewed) and
         ! empty($params['cookie']->viewed)) ? array_slice(
        explode(',', $params['cookie']->viewed), 0, 
        Configuration::get('PRODUCTS_VIEWED_NBR')) : array();
        if (sizeof($productsViewed)) {
            ...
            foreach ($productsViewed as $productViewed) {
                ...
                if (! isset($productsImagesArray[$productViewed]) ||
                 (! $obj->active = $productsImagesArray[$productViewed]['active']))
                    ...
                    $obj->cover = (int) ($productsImagesArray[$productViewed]['id_product']) .
                     '-' .
            ...
            if ($id_product and ! in_array($id_product, $productsViewed)) {
				...
				' . ($cookie->id_customer ? 'AND cug.`id_customer` = ' .
                 (int) ($cookie->id_customer) : 'AND cg.`id_group` = 1'));
                if ($result['total'])
            ...
            if (! sizeof($productsViewedObj))
                return;
            $smarty->assign(
            array('productsViewedObj' => $productsViewedObj, 
