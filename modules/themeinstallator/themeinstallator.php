...
class ThemeInstallator extends Module
{
	...
	*/
    ...
    private function getTheNativeModules ()
    {
        ...
        'http://www.prestashop.com/xml/modules_list.xml'));
        $natives = array();
        if ($xml)
        ...
        $natives = array('bankwire', 'birthdaypresent', 
        'blockadvertising', 'blockbestsellers', 'blockcart', 'blockcategories', 
        'blockcms', 'blockcurrencies', 'blockinfos', 'blocklanguages', 
        'blocklink', 'blockmanufacturer', 'blockmyaccount', 'blocknewproducts', 
        'blocknewsletter', 'blockpaymentlogo', 'blockpermanentlinks', 'blockrss', 
        'blocksearch', 'blockspecials', 'blocksupplier', 'blocktags', 
        'blockuserinfo', 'blockvariouslinks', 'blockviewed', 'blockwishlist', 
        'canonicalurl', 'cashondelivery', 'cheque', 'crossselling', 'dejala', 
        'editorial', 'feeder', 'followup', 'gadsense', 'ganalytics', 'gcheckout', 
        'graphartichow', 'graphgooglechart', 'graphvisifire', 
        'graphxmlswfcharts', 'gridhtml', 'gsitemap', 'hipay', 'homefeatured', 
        'loyalty', 'mailalerts', 'moneybookers', 'newsletter', 'pagesnotfound', 
        'paypal', 'paypalapi', 'productcomments', 'productscategory', 
        'producttooltip', 'referralprogram', 'reverso', 'sekeywords', 
        'sendtoafriend', 'statsbestcategories', 'statsbestcustomers', 
        'statsbestproducts', 'statsbestsuppliers', 'statsbestvouchers', 
        'statscarrier', 'statscatalog', 'statscheckup', 'statsdata', 
        'statsequipment', 'statsgeolocation', 'statshome', 'statslive', 
        'statsnewsletter', 'statsorigin', 'statspersonalinfos', 'statsproduct', 
        'statsregistrations', 'statssales', 'statssearch', 'themeinstallator', 
        'statsvisits', 'tm4b', 'trackingfront', 'watermark');
        return $natives;
    }
    ...
    private function recurseCopy ($src, $dst)
    {
        ...
        while (($file = readdir($dir)) !== false)
            if (strncmp($file, '.', 1) != 0) {
                ...
                elseif (is_readable($src . '/' . $file) and $file != 'Thumbs.db' and
                 $file != '.DS_Store' and substr($file, - 1) != '~')
	...
	*/
    ...
    private function init_defines ()
    {
        ...
        if (! Tools::isSubmit('cancelExport') and
         (Tools::isSubmit('exportTheme') or Tools::isSubmit('submitExport')))
            $this->page = 'exportPage';
        $this->_html = '<form action="' . $currentIndex . '&configure=' .
         $this->name . '&token=' .
         Tools::htmlentitiesUTF8(Tools::getValue('token')) .
         '" method="POST" enctype=multipart/form-data>';
        if (Tools::isSubmit('modulesToExport') or
         Tools::isSubmit('submitModules'))
        ...
        $this->errors = false;
        if ($this->page == 'exportPage' and Tools::isSubmit('exportTheme') and
         ($theme = Tools::getValue('mainTheme')))
            if (! (is_dir(_PS_ALL_THEMES_DIR_ . $theme) and
             file_exists(_PS_ALL_THEMES_DIR_ . $theme . '/index.tpl') and
             $theme != 'prestashop')) {
                $this->page = 1;
                echo parent::displayError(
                '<b>' . $theme . '</b> is not a valid theme to export');
            }
    }
    private function handleInformations ()
    {
        if (Tools::isSubmit('submitImport1')) {
            if ($_FILES['themearchive']['error'] or
             ! file_exists($_FILES['themearchive']['tmp_name']))
                $this->errors .= parent::displayError(
                $this->l('An error has occurred during the file upload.'));
            elseif (substr($_FILES['themearchive']['name'], - 4) != '.zip')
                $this->errors .= parent::displayError(
                $this->l('Only zip files are allowed'));
            elseif (! rename($_FILES['themearchive']['tmp_name'], ARCHIVE_NAME))
                $this->errors .= parent::displayError(
                $this->l('An error has occurred during the file copy.'));
            elseif (Tools::ZipTest(ARCHIVE_NAME))
                $this->page = 2;
            else
                $this->errors .= parent::displayError(
                $this->l('Zip file seems to be broken'));
        } elseif (Tools::isSubmit('submitImport2')) {
            if (! Validate::isModuleUrl($url = Tools::getValue('linkurl'), 
            $this->errors))
                $this->errors .= parent::displayError(
                $this->l('Only zip files are allowed'));
            elseif (! copy($url, ARCHIVE_NAME))
                $this->errors .= parent::displayError(
                $this->l('Error during the file download'));
            elseif (Tools::ZipTest(ARCHIVE_NAME))
                $this->errors .= parent::displayError(
                $this->l('Zip file seems to be broken'));
            else
        ...
        if ($this->page == 2 and file_exists(ARCHIVE_NAME)) {
            if (! Tools::ZipExtract(ARCHIVE_NAME, _IMPORT_FOLDER_)) {
                $this->errors .= parent::displayError(
                $this->l('Error during zip extraction'));
                $this->page = 1;
            }
        }
    ...
    public function getContentExport ()
    {
        ...
        if (Tools::isSubmit('submitExport') and $this->error === false and
         $this->checkPostedDatas() == true) {
	...
	*/
    private function _loadDocForm ()
    {
		...
		<fieldset>
			...
			<label>' .
         $this->l('You may want to check the documentation') . '</label>
			<div class="margin-form">
				<ul>';
        $i = 0;
        foreach ($docname as $row)
            $doc .= '<li><i><a target="_blank" href="' . _MODULE_DIR_ .
             $this->name . '/import/' . $docpath[$i ++] . '">' . $row .
             '</a></i>';
        $doc .= '
				</ul>
			<p class="clear">' .
    ...
    private function getModules ()
    {
        ...
        foreach ($this->xml->modules->module as $row) {
            if (strval($row['action']) == 'install' and
             ! in_array(strval($row['name']), $this->native_modules))
    ...
    private function updateImages ()
    {
        foreach ($this->xml->images->image as $row) {
            ...
            if ($foo)
                ...
                '
					INSERT INTO `' .
                 _DB_PREFIX_ . 'image_type` (`name`, `width`, `height`, `products`, `categories`, `manufacturers`, `suppliers`, `scenes`)
					VALUES (
						\'' . pSQL($row['name']) . '\',
						' . (int) ($row['width']) . ',
						' . (int) ($row['height']) . ',
    ...
    private function _displayForm4 ()
    {
        ...
        if (isset($this->to_export) and $this->to_export)
            foreach ($this->to_export as $row) {
                ...
                if ($flag ++ == 0)
                    $msg .= '<b>' .
                     $this->l('The following modules have been installed') .
                     ' :</b><br />';
                self::recurseCopy(_IMPORT_FOLDER_ . 'modules/' . $row, 
                _PS_ROOT_DIR_ . '/modules/' . $row);
                ...
                while (isset($hookedModule[++ $count]))
                    if ($hookedModule[$count] == $row) {
                        ...
                        '
							INSERT INTO `' .
                         _DB_PREFIX_ .
                         'hook_module` (`id_module`, `id_hook`, `position`)
							VALUES (' .
                         (int) $obj->id . ', ' . (int) Hook::get($hook[$count]) .
                         ', ' . (int) $position[$count] . ')');
                        if ($exceptions[$count])
                            foreach ($exceptions[$count] as $file_name)
                                ...
                                '
									INSERT INTO `' .
                                 _DB_PREFIX_ .
                                 'hook_module_exceptions` (`id_module`, `id_hook`, `file_name`)
									VALUES (' .
                                 (int) $obj->id . ', ' .
                                 (int) Hook::get($hook[$count]) . ', "' .
                                 pSQL($file_name) . '")');
                    }
            }
        if (($val = (int) (Tools::getValue('nativeModules'))) != 1) {
            ...
            if ($val == 2 and $this->to_disable and
             count($this->to_disable))
            ...
            if ($this->to_enable and count($this->to_enable))
                foreach ($this->to_enable as $row) {
                    ...
                    if ($flag ++ == 0)
                        $msg .= '<b>' .
                         $this->l('The following modules have been enabled') .
                         ' :</b><br />';
                    $msg .= '<i>- ' . pSQL($row) . '</i><br />';
                    Db::getInstance()->Execute(
                    '
        ...
        $this->_html .= '
			<input type="submit" class="button" name="submitThemes" value="' .
         $this->l('Previous') .
         '" />
			<input type="submit" class="button" name="Finish" value="' .
         $this->l('Finish') . '" />
		</form>';
    }
    private function _displayForm3 ()
    {
        ...
        if ($this->selectedVariations and count($this->selectedVariations) > 0) {
            ...
            foreach ($this->selectedVariations as $count => $theme) {
                if ($theme == 'prestashop')
                    continue;
                self::recurseCopy(_IMPORT_FOLDER_ . 'themes/' . $theme, 
                _PS_ALL_THEMES_DIR_ . $theme);
                if (file_exists(_PS_ALL_THEMES_DIR_ . $theme))
                    $ok[] = $theme;
            }
            if (sizeof($ok) > 0) {
                $msg = $this->l(
                'The following themes were successfully imported') . ':<ul><i>';
                foreach ($ok as $row)
                    $msg .= '<li> ' . $row;
                $msg .= '</i></ul>';
                $this->_msg = parent::displayConfirmation($msg);
        ...
        if ($this->to_install and sizeof($this->to_install) > 0) {
            ...
            foreach ($this->to_install as $row)
                if (file_exists(_IMPORT_FOLDER_ . 'modules/' . $row))
                    $var .= '<input type="checkbox" name="modulesToExport[]" id="' .
                     $row . '" value="' . $row .
                     '" checked /> <label style="display:bock;float:none" for="' .
                     $row . '">' . $row .
                     (file_exists(_PS_MODULE_DIR_ . $row) ? ' <span style="font-size:0.8em">-> ' .
                     $this->l(
                    'Warning: a module with the same name already exists') .
                     '</span>' : '') . '</label><br />';
            if ($var != '')
                $this->_html .= '
					<fieldset>
						<legend>' .
                 $this->l('Select the theme\'s modules you wish to install') . '</legend>
        ...
        $this->_html .= '
         ...
         '</legend>
				<ul class="margin-form" style="list-style:none">
					<li><input type="radio" name="nativeModules" value="1" id="nativemoduleconfig1"/> <label style="display:bock;float:none" for="nativemoduleconfig1">' .
         $this->l('Current configuration') .
         '</label>
					<li><input type="radio" name="nativeModules" value="2" id="nativemoduleconfig2" checked="checked" /> <label style="display:bock;float:none" for="nativemoduleconfig2">' .
         $this->l('Theme\'s configuration') .
         '</label>
					<li><input type="radio" name="nativeModules" value="3" id="nativemoduleconfig3" /> <label style="display:bock;float:none" for="nativemoduleconfig3">' .
         $this->l('Both') . '</label>
				</ul>
			</fieldset>
			<p>&nbsp;</p>';
        $this->_html .= '
         ...
         '</legend>
				<ul class="margin-form" style="list-style:none">
					<li><input type="radio" name="imagesConfig" value="1" id="imageconfig1" checked /> <label style="display:bock;float:none" for="imageconfig1">' .
         $this->l('Add new configuration') .
         '</label>
					<li><input type="radio" name="imagesConfig" value="2" id="imageconfig2" /> <label style="display:bock;float:none" for="imageconfig2">' .
         $this->l('Add all (you may lose your current config)') .
         '</label>
					<li><input type="radio" name="imagesConfig" value="3" id="imageconfig3" /> <label style="display:bock;float:none" for="imageconfig3">' .
         $this->l('Don\'t change anything') . '</label>
				</ul>
			</fieldset>';
        $this->_html .= '
			<p class="clear">&nbsp;</p>
			<input type="submit" class="button" name="prevThemes" value="' .
         $this->l('Previous') .
         '" />
			<input type="submit" class="button" name="submitModules" value="' .
         $this->l('Next') . '" />
		</fieldset>
		</form>';
    }
    private function _displayForm2 ()
    {
        ...
        $this->l('You are going to install the following theme') . ' :<br /> <b>' .
         ...
         $xml->variations->variation[0]['from'] . ' -> v' .
         $xml->variations->variation[0]['to'] . '<br />' .
         (file_exists(
        _PS_ALL_THEMES_DIR_ . strval(
        $xml->variations->variation[0]['directory'])) ? $this->l(
        'Warning : You already have a theme with the same folder\'s name') : '') . '
			</i>');
        if (file_exists(_IMPORT_FOLDER_ . 'doc') and sizeof($xml->docs->doc) != 0)
            self::_loadDocForm();
        if (sizeof($xml->variations->variation) > 1) {
            ...
            while ($xml->variations->variation[++ $count]) {
                $foo = (file_exists(
                _PS_ALL_THEMES_DIR_ .
                 strval($xml->variations->variation[$count]['directory'])) ? 1 : 0);
                $var .= '<input type="checkbox" name="variation[]" id="' .
                 strval($xml->variations->variation[$count]['directory']) .
                 '" value="' .
                 strval($xml->variations->variation[$count]['directory']) . '" ' .
                 ($foo == 1 ? '' : 'checked') .
                 '/> <label style="display:bock;float:none" for="' .
                 strval($xml->variations->variation[$count]['directory']) . '">' .
                 strval($xml->variations->variation[$count]['name']) .
                 ' <span style="font-size:0.8em">' . $this->l('for Prestashop') .
                 ' v' . strval($xml->variations->variation[$count]['from']) .
                 ' -> v' . strval($xml->variations->variation[$count]['to']) .
                 '  ' .
                 ($foo == 1 ? $this->l(
                '(Warning: a folder with the same name already exists)') : '') .
                 '</span></label><br />';
            }
            $this->_html .= '
			<fieldset>
				<legend>' .
             $this->l('Choose the variations that you wish to import') . '</legend>
				<label for="nomain">' .
             $this->l('Main theme') .
             '</label>
				<div class="margin-form">
					<input type="checkbox" name="variation[]" id="nomain" value="' .
             $xml->variations->variation[0]['directory'] .
             '" checked />
					<p class="clear">' .
             $this->l(
            'Uncheck this field if you do not want to install the main theme.') . '</p>
				</div>
				<h3>' .
             $this->l('Select the variations you wish to import') . '</h3>
				<p class="margin-form">' . $var .
             '</p>
...
';
    ...
    private function _displayForm1 ()
    {
        ...
        $themeList = array();
        foreach ($tmp as $row)
            if (is_dir(_PS_ALL_THEMES_DIR_ . $row) and
             file_exists(_PS_ALL_THEMES_DIR_ . $row . '/index.tpl') and
             $row != 'prestashop')
                $themeList[] = $row;
        if (count($themeList) > 0) {
            ...
            foreach ($themeList as $row)
                $tmp .= '<option value="' . $row . '" ' .
                 ($row == _THEME_NAME_ ? 'selected="selected"' : '') . '>' . $row .
                 '</option>';
            $this->_html .= '
				<fieldset>
					<legend>' .
             $this->l('Export a theme') . '</legend>
					<label>' . $this->l('Select a theme') . '</label>
					<div class="margin-form">
						<select style="width:350px" name="mainTheme">' . $tmp .
             '</select>
					</div>
					<input type="submit" class="button" name="exportTheme" value="' .
             $this->l('Export this theme') . '" />
				</fieldset>
				<div class="clear">&nbsp;</div>';
        }
        $this->_html .= '
			<fieldset>
				<legend>' .
         $this->l('Import from your computer') . '</legend>
				<input type="hidden" name="MAX_FILE_SIZE" value="100000000" />
				<label for="themearchive">' .
         $this->l('Archive File') . '</label>
				<div class="margin-form">
         ...
         '</p>
				...
				<input type="submit" class="button" name="submitImport1" value="' .
         $this->l('Next') . '">
			</fieldset>
			<div class="clear">&nbsp;</div>
		';
        $this->_html .= '
			<fieldset>
				<legend>' .
         $this->l('Import from the web') . '</legend>
				<label for="linkurl">' .
         $this->l('Archive URL') .
         '</label>
				<div class="margin-form">
					<input type="text"  id="linkurl" name="linkurl" value="' .
         (Tools::getValue('linkurl') ? Tools::safeOutput(
        Tools::getValue('linkurl')) : 'http://') .
         '"/>
				</div>
				<input type="submit" class="button" name="submitImport2" value="' .
         $this->l('Next') . '">
			</fieldset>
			<div class="clear">&nbsp;</div>';
        ...
        $this->_html .= '
			<fieldset>
				<legend>' .
         $this->l('Import from FTP') . '</legend>
				<label for="linkurl">' .
         $this->l('Select archive') . '</label>
				<div class="margin-form">
         ...
         '
				...
				<input type="submit" class="button" name="submitImport3" value="' .
         $this->l('Next') . '">
			</fieldset>
			<div class="clear">&nbsp;</div>
		</form>';
...
*/
    ...
    {
        $this->_html .= '<input type="hidden" name="mainTheme" value="' .
         Tools::safeOutput(Tools::getValue('mainTheme')) . '" />';
        if ($this->error === false and class_exists('ZipArchive', false) and
         ($zip = new ZipArchive())) {
            if (! ($zip->open(_EXPORT_FOLDER_ . 'archive.zip', 
            ZipArchive::OVERWRITE) === true) or
             ! $zip->addEmptyDir('test') === true)
                $this->_html .= parent::displayError(
                'Permission denied. Please set permisssion to 666 on this folder: ' .
                 _EXPORT_FOLDER_);
            $zip->close();
            if ($this->error === false)
                $this->_html .= parent::displayConfirmation(
                'Fill this formular to export this theme: <b>' .
                 Tools::safeOutput(Tools::getValue('mainTheme')) .
                 '</b> in a ZIP file');
        }
    }
    private function archiveThisFile ($obj, $file, $serverPath, $archivePath)
    {
        if (is_dir($serverPath . $file)) {
            ...
            foreach ($dir as $row)
                if ($row != '.' and $row != '..')
                    $this->archiveThisFile($obj, $row, $serverPath . $file . '/', 
                    $archivePath . $file . '/');
	...
	*/
    private function generateArchive ()
    {
        ...
        if ($zip->open(_EXPORT_FOLDER_ . $zip_file_name, ZipArchive::OVERWRITE) ===
         true) {
            ...
            while (isset($_FILES['mydoc_' . ++ $count])) {
                if (! $_FILES['mydoc_' . $count]['name'])
                    continue;
                if (! $zip->addFile($_FILES['mydoc_' . $count]['tmp_name'], 
                'doc/' . $_FILES['mydoc_' . $count]['name']))
                    $this->error = true;
            }
            foreach ($this->variations as $row) {
                $array = explode('¤', $row);
                $this->archiveThisFile($zip, $array[1], _PS_ALL_THEMES_DIR_, 
                'themes/');
            }
            foreach ($this->to_export as $row)
                if (! in_array($row, $this->native_modules))
                    $this->archiveThisFile($zip, $row, 
                    dirname(__FILE__) . '/../../modules/', 'modules/');
            $zip->close();
            if ($this->error === false) {
                ob_end_clean();
        ...
        $this->_html .= parent::displayError(
        $this->l('An error occurred during the archive generation'));
    }
    /*
	...
	*/
    private function generateXML ()
    {
        ...
        '<?xml version="1.0" encoding="UTF-8"?><!-- Copyright Prestashop --><theme></theme>');
        $theme->addAttribute('version', Tools::getValue('version'));
        $theme->addAttribute('name', 
        Tools::htmlentitiesUTF8(Tools::getValue('theme_name')));
        $author = $theme->addChild('author');
        $author->addAttribute('name', 
        Tools::htmlentitiesUTF8(Tools::getValue('author_name')));
        $author->addAttribute('email', 
        Tools::htmlentitiesUTF8(Tools::getValue('email')));
        $author->addAttribute('url', 
        Tools::htmlentitiesUTF8(Tools::getValue('website')));
        $descriptions = $theme->addChild('descriptions');
        $languages = Language::getLanguages();
        foreach ($languages as $language) {
            $val = Tools::htmlentitiesUTF8(
            Tools::getValue('body_title_' . $language['id_lang']));
            $description = $descriptions->addChild('description', 
            Tools::htmlentitiesUTF8($val));
	...
	*/
    private function _initList ()
    {
        ...
        '
			...
			LEFT JOIN `' .
         _DB_PREFIX_ . 'hook_module` c ON c.id_hook = a.id_hook
			LEFT JOIN `' .
         _DB_PREFIX_ .
         'module` d ON c.id_module = d.id_module
			LEFT OUTER JOIN `' .
         _DB_PREFIX_ . 'hook_module_exceptions` hme ON (hme.id_module = c.id_module AND hme.id_hook = a.id_hook)
			GROUP BY id_module, id_hook
			ORDER BY name_module');
        foreach ($this->hook_list as &$row)
            $row['exceptions'] = trim(
            preg_replace('/(,,+)/', ',', $row['exceptions']), ',');
    }
    /*
	** Fill module's vars
	*/
        ...
        foreach ($this->to_install as $string)
            foreach ($this->hook_list as $tmp)
                if ($tmp['name_module'] == $string)
                    $this->to_hook[] = $string . ';' . $tmp['name_hook'] . ';' .
                     $tmp['position'] . ';' . $tmp['exceptions'];
	...
	*/
    private function getImageState ()
    {
        ...
        'SELECT name, width, height, products, categories, manufacturers, suppliers, scenes FROM `' .
         _DB_PREFIX_ . 'image_type`');
        foreach ($table as $row)
            $this->image_list[] = $row['name'] . ';' . $row['width'] . ';' .
             $row['height'] . ';' . ($row['products'] == 1 ? 'true' : 'false') .
	...
	*/
    private function getThemeVariations ()
    {
        ...
        $this->variations[] = Tools::getValue('theme_name') . '¤' .
         Tools::getValue('mainTheme') . '¤' . $from . '¤' . $to;
        $count = 0;
    ...
    private function getDocumentation ()
    {
        ...
        while (Tools::isSubmit('documentationName_' . ++ $count)) {
            if (! ($filename = Tools::htmlentitiesUTF8(
            $_FILES['mydoc_' . $count]['name'])))
                continue;
            $name = Tools::htmlentitiesUTF8(
            Tools::getValue('documentationName_' . $count));
    ...
    private function checkPostedDatas ()
    {
        ...
        if ($mail and ! preg_match('#^[\w.-]+@[\w.-]+\.[a-zA-Z]{2,6}$#', $mail))
            $this->_html .= parent::displayError(
            $this->l('There is an error in your e-mail syntax!'));
        elseif ($website and
         (! Validate::isURL($website) or ! Validate::isAbsoluteUrl($website)))
            $this->_html .= parent::displayError(
            $this->l('There is an error in your URL syntax!'));
        elseif (! $this->checkVersionsAndCompatibility() or ! $this->checkNames() or
	...
	*/
    private function checkDocumentation ()
    {
        ...
        while ($this->error == false and isset($_FILES['mydoc_' . ++ $count])) {
            ...
            elseif ($_FILES['mydoc_' . $count]['error'] > 0 or
             $_FILES['mydoc_' . $count]['size'] > 1048576)
                $this->_html .= parent::displayError(
                $this->l('An error occurred during documentation upload'));
            elseif (! $name or ! Validate::isGenericName($name) or
             strlen($name) > MAX_NAME_LENGTH)
                $this->_html .= parent::displayError(
                $this->l('Please enter a valid documentation name'));
        }
        if ($this->error == true)
            return false;
        return true;
	...
	*/
    private function checkNames ()
    {
        ...
        $count = 0;
        if (! $author or ! Validate::isGenericName($author) or
         strlen($author) > MAX_NAME_LENGTH)
            $this->_html .= parent::displayError(
            $this->l('Please enter a valid author name'));
        elseif (! $name or ! Validate::isGenericName($name) or
         strlen($name) > MAX_NAME_LENGTH)
            $this->_html .= parent::displayError(
            $this->l('Please enter a valid theme name'));
        while ($this->error === false and Tools::isSubmit('myvar_' . ++ $count)) {
            if ((int) (Tools::getValue('myvar_' . $count)) == - 1)
                continue;
            $name = Tools::getValue('themevariationname_' . $count);
            if (! $name or ! Validate::isGenericName($name) or
             strlen($name) > MAX_NAME_LENGTH)
                $this->_html .= parent::displayError(
                $this->l('Please enter a valid theme variation name'));
        }
        if ($this->error == true)
            return false;
        return true;
    ...
    private function checkVersionsAndCompatibility ()
    {
        ...
        if (! preg_match("#^[0-9][.][0-9]$#", Tools::getValue('version')) or
         ! preg_match($exp, Tools::getValue('compa_from')) or
         ! preg_match($exp, Tools::getValue('compa_to')) or
         version_compare(Tools::getValue('compa_from'), 
        Tools::getValue('compa_to')) == 1)
            $this->_html .= parent::displayError(
            $this->l(
            'Syntax error on version field. Only digits and points are allowed and the compatibility should be increasing or equal.'));
        while ($this->error === false and Tools::isSubmit('myvar_' . ++ $count)) {
            if ((int) (Tools::getValue('myvar_' . $count)) == - 1)
                continue;
            $from = Tools::getValue('compafrom_' . $count);
            $to = Tools::getValue('compato_' . $count);
            if (! preg_match($exp, $from) or ! preg_match($exp, $to) or
             version_compare($from, $to) == 1)
                $this->_html .= parent::displayError(
                $this->l(
                'Syntax error on version. Only digits and points are allowed and compatibility should be increasing or equal.'));
        }
        if ($this->error == true)
            return false;
        return true;
    ...
    private function ModulesInformationForm ()
    {
        if ($this->to_install and count($this->to_install)) {
            ...
            foreach ($this->to_install as $key => $val)
                $tmp .= '<input type="checkbox" name="modulesToExport[]" value="' .
                 $val . '" id="' . $val . '" ' .
                 (in_array($val, $this->to_export) ? 'checked="checked"' : "") .
                 '/> <label style="display:bock;float:none" for="' . $val . '">' .
                 $val . '</label><br />';
            $this->_html .= '
				<fieldset>
					<legend>' . $this->l('Modules') . '</legend>
					<label>' .
             $this->l('Select the modules that you wish to export') . '</label>
    ...
    private function AuthorInformationForm ()
    {
        ...
        $mail = Tools::getValue('email') ? Tools::htmlentitiesUTF8(
        Tools::getValue('email')) : Tools::htmlentitiesUTF8($cookie->email);
        $author = Tools::getValue('author_name') ? Tools::htmlentitiesUTF8(
        Tools::getValue('author_name')) : Tools::htmlentitiesUTF8(
        ($employee->firstname) . ' ' . $employee->lastname);
        $website = Tools::getValue('website') ? Tools::htmlentitiesUTF8(
        Tools::getValue('website')) : Tools::getHttpHost(true);
        $this->_html .= '
			<fieldset>
         ...
         $author . '" name="author_name" maxlength="' . MAX_NAME_LENGTH . '" />
				</div>
					<label>' . $this->l('Email') .
         '</label>
				<div class="margin-form">
					<input type="text" value="' .
         $mail . '" name="email" maxlength="' . MAX_EMAIL_LENGTH . '"/>
				</div>
					<label>' . $this->l('Website') .
         '</label>
				<div class="margin-form">
					<input type="text" value="' .
         $website . '" name="website" maxlength="' . MAX_WEBSITE_LENGTH . '"/>
				</div>
			</fieldset>
			<div class="clear">&nbsp;</div>';
    }
    private function ThemeInformationForm ()
    {
        ...
        $iso = Language::getIsoById((int) ($cookie->id_lang));
        $divLangName = 'title';
        $val = Tools::getValue('theme_name') ? Tools::getValue('theme_name') : Tools::getValue(
        'mainTheme');
        $this->_html .= '
		<fieldset>
         ...
         '</label>
			<div class="margin-form">
				<input type="text" value="' .
         $val . '" name="theme_name" maxlength="' . MAX_NAME_LENGTH . '" />
				<p class="clear">' .
         $this->l('Your theme\'s name') . '</p>
			</div>
        ...
        foreach ($languages as $language) {
            ...
            $this->_html .= '
             ...
             ';float: left;">
					<input type="text" name="body_title_' .
             $language['id_lang'] . '" id="body_title_' . $language['id_lang'] .
             '" maxlength="' . MAX_DESCRIPTION_LENGTH . '" size="64" value="' .
             $val . '" />
				</div>';
        }
        $this->_html .= $this->displayFlags($languages, $defaultLanguage, 
        $divLangName, 'title', true);
        $this->_html .= '
				<p class="clear">' .
         $this->l('Enter a short description of your theme') . '</p>
			</div>';
        ...
        $this->_html .= '
         ...
         '</label>
			<div class="margin-form">
				<input type="text" value="' .
         $val . '" name="version" maxlength="' . MAX_T_VER_LENGTH . '" />
				<p class="clear">' .
         $this->l('Your theme\'s version') . '</p>
			</div>';
        ...
        'Compatible From') .
         '</label>
				<div class="margin-form">
					<input type="text" value="' .
         $val . '" name="compa_from" maxlength="' . MAX_COMPATIBILITY_VER . '" />
				</div>
			</div>
         ...
         '</label>
				<div class="margin-form">
					<input type="text" value="' .
         $val2 . '" name="compa_to" maxlength="' . MAX_COMPATIBILITY_VER . '"/>
				</div>
			</div>
			<p class="clear">&nbsp;</p>';
    }
    private function DocInformationForm ()
    {
        ...
        $this->_html .= '
			<label>' .
         $this->l('Add documentation') . '</label>
			<p class="margin-form">' .
         $this->l('Give the user some help. Add a field by clicking here') .
         '
				<a href="javascript:addDocumentation(0);"><img alt="add" title="add" src="' .
         _MODULE_DIR_ . $this->name . '/add.png' . '" /></a>.<br />' .
         $this->l('File extension must be .txt or .pdf') . '
			</p>
    ...
    private function VariationInformationForm ()
    {
        ...
        $this->_html .= '
         ...
         '
				<a href="javascript:addVariation(-1);"><img alt="add" title="add" src="' .
         _MODULE_DIR_ . $this->name . '/add.png' .
         '" /></a>
			</label>
			<p class="margin-form">' .
         $this->l('Select theme to include and its compatibility.') . '</p>
			<script type="text/javascript">
				var path = "' . $this->l('Path') . '";
				var delete_img = "' . _MODULE_DIR_ .
         $this->name . '/delete.png";
         ...
         $this->l('Documentation') . '";
				var compatibility_length = "' .
         MAX_COMPATIBILITY_VER . '";
				var compatibility_from = "' .
         DEFAULT_COMPATIBILITY_FROM . '";
				var compatibility_to = "' .
         DEFAULT_COMPATIBILITY_TO . '";
				var select_default = "' .
         $this->l('Choose a theme') . '";
				var themes = Array();
				var themes_id = Array();
		';
        $id = 0;
        foreach ($this->theme_list as $row) {
            if (! is_dir(_PS_ALL_THEMES_DIR_ . $row) or
             ! file_exists(_PS_ALL_THEMES_DIR_ . $row . '/index.tpl') or
             $row == 'prestashop' or $row == Tools::getValue('mainTheme'))
                continue;
		...
		<div class="clear">&nbsp;</div>
			<input type="submit" class="button" name="cancelExport" value="' .
         $this->l('Cancel') .
         '" />&nbsp;
			<input type="submit" class="button" name="submitExport" value="' .
         $this->l('Generate the archive now!') . '" />
		</form>';
    }
}
