...
class GraphVisifire extends ModuleGraphEngine
{
    ...
    function __construct ($type = null)
    {
        if ($type != null) {
            ...
            'Visifire is a set of open source data visualization components - powered by Microsoft Silverlight 2 beta 2.');
        }
    }
    function install ()
    {
		...
		<div id="VisifireChart' . $divid .
         '">
			<script language="javascript" type="text/javascript">
				var vChart = new Visifire("../modules/graphvisifire/visifire/Visifire.xap", ' .
            ...
            if ($size == sizeof($this->_legend)) {
                if (! is_array($this->_values[array_rand($this->_values)])) {
                    ...
                    for ($i = 0; $i < $size; $i ++) {
                        ...
                        str_replace('>', '&gt;', 
                        str_replace('&', '&amp;', 
                        str_replace('&quot', "'", $this->_legend[$i])))) . '" ';
                    ...
                    foreach ($this->_values as $layer => $values) {
                        $this->_xml .= '<vc:DataSeries Name="' . (isset(
                        $this->_titles['main'][$layer + 1]) ? $this->_titles['main'][$layer +
                         1] : '') . '" RenderAs="' . $this->_type . '">';
