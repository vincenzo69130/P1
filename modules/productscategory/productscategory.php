...
class productsCategory extends Module
{
    ...
    public function getContent ()
    {
        ...
        elseif (Tools::isSubmit('submitCross')) {
            Configuration::updateValue('PRODUCTSCATEGORY_DISPLAY_PRICE', 
            Tools::getValue('displayPrice'));
    ...
    public function hookProductFooter ($params)
    {
        ...
        if (isset($params['category']->id_category))
            ...
            if (isset($product->id_category_default) and
        ...
        (int) ($cookie->id_lang), 1, 100); /* 100 products max. */
        ...
        if (is_array($categoryProducts) and sizeof($categoryProducts)) {
                ...
                foreach ($categoryProducts as $key => $categoryProduct)
                    if ($categoryProduct['id_product'] != $idProduct) {
                        if ($taxes == 0 or $taxes == 2)
                            $categoryProducts[$key]['displayed_price'] = Product::getPriceStatic(
                            (int) $categoryProduct['id_product'], true, NULL, 2);
                        elseif ($taxes == 1)
                            $categoryProducts[$key]['displayed_price'] = Product::getPriceStatic(
                            (int) $categoryProduct['id_product'], false, NULL, 2);
                    }
                 // Get positions
            $middlePosition = round(
            $sizeOfCategoryProducts / 2, 0);
            $productPosition = $this->getCurrentProduct($categoryProducts, 
            (int) $idProduct);
            ...
            if ($sizeOfCategoryProducts > 30) {
                $categoryProducts = array_slice($categoryProducts, 
                $middlePosition - 15, 30, true);
                $middlePosition = 15;
            }
        }
        // Display tpl
        ...
        'ProdDisplayPrice' => Configuration::get(
    ...
    public function hookHeader ($params)
    {
        ...
        Tools::addJS(
        array($this->_path . 'productscategory.js', 
