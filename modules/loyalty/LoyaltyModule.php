...
class LoyaltyModule extends ObjectModel
{
    ...
    protected $fieldsValidate = array('id_loyalty_state' => 'isInt', 
    'id_customer' => 'isInt', 'id_discount' => 'isInt', 'id_order' => 'isInt', 
    'points' => 'isInt');
        ...
        if (Validate::isLoadedObject($cart)) {
            ...
            if (isset($newProduct) and ! empty($newProduct)) {
                ...
                if ($taxesEnabled == PS_TAX_EXC)
                    $cartProductsNew['price'] = number_format(
                    $newProduct->getPrice(false, 
                    (int) ($newProduct->getIdProductAttributeMostExpensive())), 2, 
                    '.', '');
                else
                    $cartProductsNew['price_wt'] = number_format(
                    $newProduct->getPrice(true, 
                    (int) ($newProduct->getIdProductAttributeMostExpensive())), 2, 
                    '.', '');
            ...
            foreach ($cartProducts as $product) {
                if (! (int) (Configuration::get('PS_LOYALTY_NONE_AWARD')) and
                 Product::isDiscounted((int) $product['id_product'])) {
                    global $smarty;
                    if (isset($smarty) and is_object($newProduct) and
                     $product['id_product'] == $newProduct->id)
                        $smarty->assign('no_pts_discounted', 1);
                    continue;
                }
                $total += ($taxesEnabled == PS_TAX_EXC ? $product['price'] : $product['price_wt']) *
    ...
    public static function getVoucherValue ($nbPoints, $id_currency = NULL)
    {
        ...
        Configuration::get('PS_LOYALTY_POINT_VALUE'), 
    ...
    public static function getPointsByCustomer ($id_customer)
    {
		...
		AND f.id_loyalty_state IN (' .
         (int) (LoyaltyStateModule::getValidationId()) . ', ' .
         (int) (LoyaltyStateModule::getNoneAwardId()) . ')') +
		...
		WHERE f.id_customer = ' . (int) ($id_customer) .
         '
		AND f.id_loyalty_state = ' .
         (int) LoyaltyStateModule::getCancelId() . ' AND points < 0');
    }
    public static function getAllByIdCustomer ($id_customer, $id_lang, 
    $onlyValidate = false, $pagination = false, $nb = 10, $page = 1)
    {
        $query = '
		...
		LEFT JOIN `' . _DB_PREFIX_ .
         'orders` o ON (f.id_order = o.id_order)
		LEFT JOIN `' .
         _DB_PREFIX_ .
         'loyalty_state_lang` fsl ON (f.id_loyalty_state = fsl.id_loyalty_state AND fsl.id_lang = ' .
         (int) ($id_lang) . ')
		WHERE f.id_customer = ' . (int) ($id_customer);
        if ($onlyValidate === true)
            $query .= ' AND f.id_loyalty_state = ' .
             (int) LoyaltyStateModule::getValidationId();
        $query .= ' GROUP BY f.id_loyalty ' .
         ($pagination ? 'LIMIT ' . (((int) ($page) - 1) * (int) ($nb)) . ', ' .
         (int) ($nb) : '');
    ...
    public static function getDiscountByIdCustomer ($id_customer, $last = false)
    {
		...
		FROM `' . _DB_PREFIX_ . 'loyalty` f
		LEFT JOIN `' .
         _DB_PREFIX_ . 'orders` o ON (f.`id_order` = o.`id_order`)
		WHERE f.`id_customer` = ' . (int) ($id_customer) . ' 
		AND f.`id_discount` > 0
		AND o.`valid` = 1';
        if ($last === true)
    ...
    public static function registerDiscount ($discount)
    {
        ...
        $items = self::getAllByIdCustomer((int) $discount->id_customer, NULL, 
        true);
        foreach ($items as $item) {
            $f = new LoyaltyModule((int) $item['id_loyalty']);
            /* Check for negative points for this order */
            $negativePoints = (int) Db::getInstance()->getValue(
            'SELECT SUM(points) points FROM ' . _DB_PREFIX_ .
             'loyalty WHERE id_order = ' . (int) $f->id_order .
    ...
    public static function getOrdersByIdDiscount ($id_discount)
    {
		...
		FROM `' . _DB_PREFIX_ .
         'loyalty` f
		WHERE f.id_discount = ' .
         (int) ($id_discount) . ' AND f.id_loyalty_state = ' .
         (int) (LoyaltyStateModule::getConvertId()));
    ...
    private function historize ()
    {
		...
		INSERT INTO `' .
         _DB_PREFIX_ .
         'loyalty_history` (`id_loyalty`, `id_loyalty_state`, `points`, `date_add`)
		VALUES (' .
         (int) ($this->id) . ', ' . (int) ($this->id_loyalty_state) . ', ' .
         (int) ($this->points) . ', NOW())');
    }
}
