...
class Watermark extends Module
{
    ...
    public function __construct ()
    {
        ...
        array('WATERMARK_TYPES', 'WATERMARK_Y_ALIGN', 'WATERMARK_X_ALIGN', 
        ...
        if (! isset($this->transparency) or ! isset($this->xAlign) or
         ! isset($this->yAlign))
            $this->warning = $this->l(
            'Watermark image must be uploaded in order for this module to work correctly.');
    }
    ...
    public function uninstall ()
    {
        return (parent::uninstall() and
         Configuration::deleteByName('WATERMARK_TYPES') and
         Configuration::deleteByName('WATERMARK_TRANSPARENCY') and
         Configuration::deleteByName('WATERMARK_Y_ALIGN') and
    ...
    private function _postValidation ()
    {
        ...
        elseif ($transparency < 0 || $transparency > 100)
            ...
            'Transparency is not in allowed range.');
        if (empty($yalign))
        ...
        if (isset($_FILES['PS_WATERMARK']['tmp_name']) and
         ! empty($_FILES['PS_WATERMARK']['tmp_name'])) {
    ...
    {
        Configuration::updateValue('WATERMARK_TYPES', 
        implode(',', Tools::getValue('image_types')));
        Configuration::updateValue('WATERMARK_Y_ALIGN', 
        Tools::getValue('yalign'));
        Configuration::updateValue('WATERMARK_X_ALIGN', 
        Tools::getValue('xalign'));
        Configuration::updateValue('WATERMARK_TRANSPARENCY', 
        Tools::getValue('transparency'));
        //submited watermark
        if (isset($_FILES['PS_WATERMARK']) and
         ! empty($_FILES['PS_WATERMARK']['tmp_name'])) {
            ...
            $this->maxImageSize))
                $this->_errors[] = $error;
                /* Copy new watermark */
            elseif (! copy($_FILES['PS_WATERMARK']['tmp_name'], 
            dirname(__FILE__) . '/watermark.gif'))
                $this->_errors[] = Tools::displayError(
                'an error occurred while uploading watermark: ' .
                 $_FILES['PS_WATERMARK']['tmp_name'] . ' to ' . $dest);
        }
        if ($this->_errors)
            foreach ($this->_errors as $error)
                $this->_html .= '<div class="module_error alert error"><img src="../img/admin/warning.gif" alt="' .
                 $this->l('ok') . '" /> ' . $this->l($error) . '</div>';
        else
    ...
    private function _displayForm ()
    {
        ...
        $this->_html .= '<form action="' .
         ...
         $this->name . '/logo.gif" />' . $this->l('Watermark details') .
         '</legend>
				<p>' .
         $this->l(
        'Once you have set up the module, regenerate the images using the "Images" tool in Preferences. However, the watermark will be added automatically to new images.') .
         '</p>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr>
						<td />
						<td>' .
         (file_exists(dirname(__FILE__) . '/watermark.gif') ? '<img src="../modules/' .
         $this->name . '/watermark.gif?t=' . time() . '" />' : $this->l(
        'No watermark uploaded.')) . '</td>
					</tr>
					<tr>
						<td>' . $this->l('Watermark file') .
         '</td>
						<td>
							<input type="file" name="PS_WATERMARK" />
							<p style="color:#7F7F7F; font-size:0.85em; margin:0; padding:0;">' .
         $this->l('Must be in GIF format') .
         '</p>
						</td>
					</tr>
					<tr>
						<td width="270" style="height: 35px;">' .
         $this->l('Watermark transparency (0-100)') .
         '</td>
					    <td><input type="text" name="transparency" value="' .
         Tools::getValue('transparency', 
        Configuration::get('WATERMARK_TRANSPARENCY')) .
         '" style="width: 30px;" /></td>
					</tr>
					<tr><td width="270" style="height: 35px;">' .
         $this->l('Watermark X align') . '</td>
					    <td>
						<select id="xalign" name = "xalign">';
        foreach ($this->xaligns as $align)
            $this->_html .= '<option value="' . $align . '"' .
             (Tools::getValue('xalign', Configuration::get('WATERMARK_X_ALIGN')) ==
             $align ? ' selected="selected"' : '') . '>' . $this->l($align) .
             '</option>';
        $this->_html .= '</select>
					    </td>
					</tr>
					<tr><td width="270" style="height: 35px;">' .
         $this->l('Watermark Y align') . '</td>
					    <td>
						<select id="yalign" name = "yalign">';
        foreach ($this->yaligns as $align)
            $this->_html .= '<option value="' . $align . '"' .
             (Tools::getValue('yalign', Configuration::get('WATERMARK_Y_ALIGN')) ==
             $align ? ' selected="selected"' : '') . '>' . $this->l($align) .
             '</option>';
        $this->_html .= '</select>
					    </td>
					...
					<tr><td width="270" style="height: 35px;">' .
         $this->l('Choose image types for watermark protection.') . '</td><td>';
        $selected_types = explode(',', Configuration::get('WATERMARK_TYPES'));
        foreach (ImageType::getImagesTypes('products') as $type) {
            $this->_html .= '<label style="float:none; ">
						<input type="checkbox" value="' .
             $type['id_image_type'] . '" name="image_types[]"' .
             (in_array($type['id_image_type'], $selected_types) ? ' checked="checked"' : '') .
        ...
        $this->_html .= '</td></tr>
					...
					<tr><td colspan="2" align="center"><input class="button" name="btnSubmit" value="' .
    ...
    public function hookwatermark ($params)
    {
        ...
        foreach ($this->imageTypes as $imageType) {
            ...
            if (! imageResize($file, $newFile, (int) ($imageType['width']), 
    ...
    private function watermarkByImage ($imagepath, $watermarkpath, $outputpath)
    {
        ...
        if (! $imagew = imagecreatefromgif($watermarkpath))
            die(
            $this->l(
            'The watermark image is not a real gif, please CONVERT the image.'));
        ...
        if (! imagecopymerge($image, $imagew, $xpos, $ypos, 0, 0, 
        $watermarkWidth, $watermarkHeight, $this->transparency))
            return false;
