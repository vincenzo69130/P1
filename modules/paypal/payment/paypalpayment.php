    ...
    public function getAuthorisation ()
    {
        ...
        if (sizeof($this->_logs))
            return false;
             // Making request
        $vars = '?fromPayPal=1';
        $returnURL = Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ .
         'modules/paypal/payment/submit.php' . $vars;
        $cancelURL = Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ .
         'order.php';
        ...
        $request = '&Amt=' . urlencode($paymentAmount) . '&PAYMENTACTION=' .
         urlencode($paymentType) . '&ReturnUrl=' . urlencode($returnURL) .
         '&CANCELURL=' . urlencode($cancelURL) . '&CURRENCYCODE=' .
         urlencode($currencyCodeType) . '&NOSHIPPING=1';
        ...
        $request .= '&LOCALECODE=' .
         strtoupper(Language::getIsoById($cart->id_lang));
        if (Configuration::get('PAYPAL_HEADER'))
            $request .= '&HDRIMG=' .
             urlencode(Configuration::get('PAYPAL_HEADER'));
        ...
        if ($discounts == 0) {
            if ($params['cart']->id_customer) {
                ...
                $taxCalculationMethod = Group::getPriceDisplayMethod(
            ...
            for ($i = 0; $i < sizeof($products); $i ++) {
                ...
                $request .= '&L_AMT' . $i . '=' .
                 urlencode($this->PayPalRound($products[$i][$priceField]));
                $request .= '&L_QTY' . $i . '=' .
                 urlencode($products[$i]['cart_quantity']);
                $amt += $this->PayPalRound(
                $products[$i][$priceField] * $products[$i]['cart_quantity']);
            }
            ...
            $request .= '&TAXAMT=' .
             urlencode(
            (float) max($this->PayPalRound($paymentAmount - $amt - $shipping), 
            0));
        } else {
            $products = $cart->getProducts();
            $description = 0;
        ...
        $request .= '&SHIPTONAME=' .
         urlencode($address->firstname . ' ' . $address->lastname);
