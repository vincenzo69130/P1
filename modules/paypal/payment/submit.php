    ...
    if (is_array($result) and sizeof($result)) {
        if (strtoupper($result['ACK']) == 'SUCCESS') {
            if (isset($result['TOKEN'])) {
                ...
                header(
                'Location: https://' . $ppPayment->getPayPalURL() .
                 '/webscr&cmd=_express-checkout&token=' .
                 urldecode(strval($cookie->paypal_token)) . '&useraction=commit');
                exit();
            } else
                $logs[] = '<b>' .
                 $ppPayment->l('No token given by PayPal', 'submit') . '</b>';
        } else
            $logs[] = '<b>' . $ppPayment->l('PayPal returned error', 'submit') .
             '</b>';
    }
    $ppPayment->displayPayPalAPIError(
    $ppPayment->l('Authorisation to PayPal failed', 'submit'), $logs);
}
function displayConfirm ()
{
    ...
    if ($cart->id_currency !=
     $ppPayment->getCurrency((int) $cart->id_currency)->id) {
        $cart->id_currency = (int) ($ppPayment->getCurrency(
        (int) $cart->id_currency)->id);
    ...
    'this_path_ssl' => Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ .
     'modules/' . $ppPayment->name . '/', 'mode' => 'payment/'));
...
function validOrder ()
{
    ...
    if (! $payerID = Tools::htmlentitiesUTF8(strval(Tools::getValue('PayerID'))))
        die('Invalid payerID');
    $ppPayment->makePayPalAPIValidation($cookie, $cart, $cart->id_currency, 
    $payerID, 'payment');
}
// #####
// Process !!
