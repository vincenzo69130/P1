...
class PaypalLib extends Paypal
{
    ...
    public function makeCall ($host, $script, $methodName, $string)
    {
        ...
        $request = 'METHOD=' . urlencode($methodName) . '&VERSION=' .
         urlencode(PAYPAL_API_VERSION);
        $request .= '&PWD=' .
         urlencode(Configuration::get('PAYPAL_API_PASSWORD')) . '&USER=' .
         urlencode(Configuration::get('PAYPAL_API_USER'));
        $request .= '&SIGNATURE=' .
         urlencode(Configuration::get('PAYPAL_API_SIGNATURE')) . $string;
        // Making connection
        ...
        if (! Configuration::get('PAYPAL_DEBUG_MODE'))
            $this->_logs = array();
        $toExclude = array('TOKEN', 'SUCCESSPAGEREDIRECTREQUESTED', 'VERSION', 
        'BUILD', 'ACK', 'CORRELATIONID');
        $this->_logs[] = '<b>' . $this->l('PayPal response:') . '</b>';
        foreach ($response as $k => $res) {
            if (! Configuration::get('PAYPAL_DEBUG_MODE') and
             in_array($k, $toExclude))
                continue;
            $this->_logs[] = $k . ' -> ' . $res;
        }
        return $response;
