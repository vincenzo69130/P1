...
class PayPalConnect extends Paypal
{
    ...
    {
        $this->_logs[] = $this->l('Making new connection to') . ' \'' . $host .
         $script . '\'';
    ...
    private function _connectByCURL ($url, $body)
    {
        ...
        if (! $ch)
            ...
            if (! $result)
                $this->_logs[] = $this->l(
                'Send with CURL method failed ! Error:') . ' ' . curl_error($ch);
            else
    ...
    private function _connectByFSOCK ($host, $script, $body)
    {
        ...
        if (! $fp)
            ...
            $this->_logs[] = $this->l(
            'Connect with fsockopen method successful');
            $this->_logs[] = $this->l('Sending this params:') . ' ' . $header .
             $body;
            ...
            if (! $result)
                ...
                $this->_logs[] = $this->l(
                'Send with fsockopen method successful');
        }
        return (isset($result) ? $result : false);
    }
