...
if (function_exists('curl_exec')) {
    ...
    if (! $ch)
        ...
        if (strtoupper($result) != 'VERIFIED')
            $errors .= $paypal->getL('curlmethod') . $result . ' cURL error:' .
             curl_error($ch);
        curl_close($ch);
    }
} elseif (($fp = @fsockopen('ssl://' . $paypalServer, 443, $errno, $errstr, 30)) ||
 ($fp = @fsockopen($paypalServer, 80, $errno, $errstr, 30))) {
    // fsockopen ready
    ...
    while (! feof($fp)) {
        ...
        $read .= $reading;
        if (strtoupper($reading) == 'VERIFIED' or
         strtoupper($reading) == 'INVALID') {
            $result = $reading;
            break;
        }
    }
...
if (strtoupper($result) == 'VERIFIED') {
    ...
    if (empty($errors)) {
        ...
        elseif (Order::getOrderByCartId((int) ($cart_secure[0])))
            ...
            $paypal->validateOrder((int) $cart_secure[0], 
            Configuration::get('PS_OS_PAYMENT'), (float) ($_POST['mc_gross']), 
            $paypal->displayName, $paypal->getL('transaction') . $_POST['txn_id'], 
            array('transaction_id' => $_POST['txn_id'], 
            'payment_status' => $_POST['payment_status']), NULL, false, 
            $cart_secure[1]);
    }
} else
...
if (! empty($errors) and isset($_POST['custom'])) {
    if (strtoupper($_POST['payment_status']) == 'PENDING')
        $paypal->validateOrder((int) $cart_secure[0], 
        Configuration::get('PS_OS_PAYPAL'), (float) $_POST['mc_gross'], 
        $paypal->displayName, 
        $paypal->getL('transaction') . $_POST['txn_id'] . '<br />' . $errors, 
        array('transaction_id' => $_POST['txn_id'], 
        'payment_status' => $_POST['payment_status']), NULL, false, 
        $cart_secure[1]);
    else
        $paypal->validateOrder((int) $cart_secure[0], 
        Configuration::get('PS_OS_ERROR'), 0, $paypal->displayName, 
