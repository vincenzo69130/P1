    ...
    if (is_array($result) and sizeof($result)) {
        if (strtoupper($result['ACK']) == 'SUCCESS') {
            if (isset($result['TOKEN'])) {
                ...
                header(
                'Location: https://' . $ppExpress->getPayPalURL() .
                 '/webscr&cmd=_express-checkout&token=' .
                 urldecode(strval($cookie->paypal_token)));
                exit();
            } else
                $logs[] = '<b>' .
                 $ppExpress->l('No token given by PayPal', 'submit') . '</b>';
        } else
            $logs[] = '<b>' . $ppExpress->l('PayPal returned error', 'submit') .
             '</b>';
    }
    $ppExpress->displayPayPalAPIError(
    $ppExpress->l('Authorisation to PayPal failed', 'submit'), $logs);
}
function getInfos ()
{
    global $ppExpress, $cookie;
    ...
    $logs = $ppExpress->getLogs();
    if (! is_array($result) or ! isset($result['ACK']) or
     strtoupper($result['ACK']) != 'SUCCESS') {
        $logs[] = '<b>' .
         $ppExpress->l('Cannot retrieve PayPal account information', 'submit') .
         '</b>';
        $ppExpress->displayPayPalAPIError(
        $ppExpress->l('PayPal returned error', 'submit'), $logs);
    } elseif (! isset($result['TOKEN']) or
     $result['TOKEN'] != $cookie->paypal_token) {
        $logs[] = '<b>' .
         $ppExpress->l(
        'Token given by PayPal is not the same as the cookie token', 'submit') .
         '</b>';
        $ppExpress->displayPayPalAPIError(
        $ppExpress->l('PayPal returned error', 'submit'), $logs);
    }
...
function displayConfirm ()
{
    ...
    $smarty->assign(
    array('back' => 'paypal', 'logo' => $ppExpress->getLogo(), 
    ...
    'this_path_ssl' => Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ .
     'modules/' . $ppExpress->name . '/', 'payerID' => $payerID, 
    'mode' => 'express/'));
...
function submitConfirm ()
{
    ...
    elseif (! $currency = (int) (Tools::getValue('currency_payement')))
        die('No currency');
    elseif (! $payerID = Tools::htmlentitiesUTF8(
    strval(Tools::getValue('payerID'))))
        die('No payer ID');
    elseif (! $cart->getOrderTotal(true, Cart::BOTH))
        die('Empty cart');
    $ppExpress->makePayPalAPIValidation($cookie, $cart, $currency, $payerID, 
    'express');
}
function submitAccount ()
{
    ...
    elseif (! @checkdate(Tools::getValue('months'), Tools::getValue('days'), 
    Tools::getValue('years')) and
     ! (Tools::getValue('months') == '' and Tools::getValue('days') == '' and
     Tools::getValue('years') == ''))
        ...
        $customer->birthday = (empty($_POST['years']) ? '' : (int) ($_POST['years']) .
         '-' . (int) ($_POST['months']) . '-' . (int) ($_POST['days']));
        /* Customer and address, same fields, caching data */
        $errors = $customer->validateControler();
        $address = new Address();
        $address->id_customer = 1;
        $errors = array_unique(
        array_merge($errors, $address->validateControler()));
        if (! sizeof($errors)) {
            $customer->active = 1;
            if (! $customer->add())
                $errors[] = Tools::displayError(
                ...
                if (! $address->add())
                    $errors[] = Tools::displayError(
                    'an error occurred while creating your address');
                else {
                    ...
                    Module::hookExec('createAccount', 
                    array('_POST' => $_POST, 'newCustomer' => $customer));
                    // Next !
                    $payerID = strval(
                    Tools::getValue('payerID'));
...
function submitLogin ()
{
    ...
    elseif (! Validate::isPasswd($passwd))
        ...
        if (! $authentication or ! $customer->id)
            ...
            $cookie->passwd = $customer->passwd;
            $cookie->email = $customer->email;
            if (Configuration::get('PS_CART_FOLLOWING') and
             (empty($cookie->id_cart) or
...
function displayLogin ()
{
    ...
    if ($cookie->isLogged(true) and isset($result['EMAIL']) and
    ...
    elseif ($cookie->isLogged(true))
        $cookie->makeNewLog();
         // Smarty assigns
    $smarty->assign(
    array('email' => $email, 'ppToken' => strval($cookie->paypal_token), 
    'errors' => $errors, 'payerID' => $payerID));
    // Display all and exit
    include (_PS_ROOT_DIR_ . '/header.php');
...
function displayAccount ()
{
    ...
    'days' => $days, 'sl_day' => (isset($selectedDays) ? $selectedDays : 0), 
    'countries' => $countries, 
    'sl_country' => (isset($selectedCountry) ? $selectedCountry : 0), 
    'email' => $email, 
    'firstname' => (Tools::getValue('customer_firstname') ? Tools::htmlentitiesUTF8(
    strval(Tools::getValue('customer_firstname'))) : $result['FIRSTNAME']), 
    'lastname' => (Tools::getValue('customer_lastname') ? Tools::htmlentitiesUTF8(
    strval(Tools::getValue('customer_lastname'))) : $result['LASTNAME']), 
    'street' => (Tools::getValue('address1') ? Tools::htmlentitiesUTF8(
    strval(Tools::getValue('address1'))) : (isset($result['SHIPTOSTREET']) ? $result['SHIPTOSTREET'] : '')), 
    'city' => (Tools::getValue('city') ? Tools::htmlentitiesUTF8(
    strval(Tools::getValue('city'))) : (isset($result['SHIPTOCITY']) ? $result['SHIPTOCITY'] : '')), 
    'zip' => (Tools::getValue('postcode') ? Tools::htmlentitiesUTF8(
    strval(Tools::getValue('postcode'))) : (isset($result['SHIPTOZIP']) ? $result['SHIPTOZIP'] : '')), 
    'payerID' => $payerID, 'ppToken' => strval($cookie->paypal_token), 
    'errors' => $errors));
    // Display all and exit
    include (_PS_ROOT_DIR_ . '/header.php');
...
if (! isset($cookie->paypal_token) or ! $cookie->paypal_token)
    ...
    if ((! Tools::isSubmit('submitAccount') and
     ! Tools::isSubmit('submitLogin')) or sizeof($errors)) {
        if (isset($cookie->paypal_token) and isset($cookie->paypal_token_date) and
         (time() - 10800 > $cookie->paypal_token_date)) {
        ...
        if (Tools::getValue('email') and Tools::getValue('payerID')) {
            ...
            $email = Tools::htmlentitiesUTF8(
            strval(Tools::getValue('email')));
            $payerID = Tools::htmlentitiesUTF8(
            strval(Tools::getValue('payerID')));
