...
if (! Tools::isSubmit('secure_key') or
 Tools::getValue('secure_key') != $blockcms->secure_key or
 ! Tools::isSubmit('action'))
    die(1);
if (Tools::getValue('action') == 'getCms') {
    if (! Validate::isInt(Tools::getValue('id_cms_category')) or
    ...
    '
	SELECT cl.`meta_title`, c.`id_cms` FROM `' .
     _DB_PREFIX_ . 'cms` c
	JOIN `' .
    ...
    if (sizeof($cms_categories) or sizeof($cms_pages)) {
        ...
        foreach ($cms_categories as $cms_category) {
            $html .= '
					<tr>
						<td width="5%" class="center"><input type="checkbox" class="noborder cmsBox" name="cmsBox[]" value="1_' .
             $cms_category['id_cms_category'] . '" ' .
             (in_array(
            array('id_cms' => $cms_category['id_cms_category'], 
            'is_category' => 1), $cms_selected) ? 'checked="checked"' : '') . '></td>
        ...
        foreach ($cms_pages as $cms_page) {
            $html .= '
					<tr>
						<td width="5%" class="center"><input type="checkbox" class="noborder cmsBox" name="cmsBox[]" value="0_' .
             $cms_page['id_cms'] . '" ' . (in_array(
            array('id_cms' => $cms_page['id_cms'], 'is_category' => 0), 
            $cms_selected) ? 'checked="checked"' : '') . '></td>
						<td width="10%">' . $cms_page['id_cms'] . '</td>
						<td width="85%">' .
             $cms_page['meta_title'] . '</td>
					</tr>';
        }
    } else
        $html .= $blockcms->getL(
    ...
    if ($table = Tools::getValue('table_right') or
     $table = Tools::getValue('table_left'));
    {
        $pos = 0;
