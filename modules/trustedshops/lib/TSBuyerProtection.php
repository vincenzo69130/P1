...
class TSBuyerProtection extends AbsTrustedShops
{
    ...
    public function __construct ()
    {
        ...
        'PREPAYMENT' => $this->l('Prepayment'), 'CHEQUE' => $this->l('Cheque'), 
        'PAYBOX' => $this->l('Paybox'), 'PAYPAL' => $this->l('PayPal'), 
        'CASH_ON_PICKUP' => $this->l('Cash on pickup'), 
        'FINANCING' => $this->l('Financing'), 'LEASING' => $this->l('Leasing'), 
        'T_PAY' => $this->l('T-Pay'), 'CLICKANDBUY' => $this->l('Click&Buy'), 
        ...
        $this->tab_name = $this->l(
        'Trusted Shops quality seal and buyer protection');
        ...
        if (! method_exists('Tools', 'jsonDecode') ||
         ! method_exists('Tools', 'jsonEncode')) {
            $this->warnings[] = $this->l(
            'Json functions must be implemented in your php version');
        } else {
            foreach ($this->available_languages as $iso => $lang) {
                if ($lang === '')
                    $this->available_languages[$iso] = Language::getLanguage(
                    Language::getIdByIso($iso));
                TSBuyerProtection::$CERTIFICATE[strtoupper($iso)] = (array) Tools::jsonDecode(
                Tools::htmlentitiesDecodeUTF8(
                Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'CERTIFICATE_' .
                 strtoupper($iso))));
            }
            if (TSBuyerProtection::$SHOPSW === NULL) {
                TSBuyerProtection::$SHOPSW = Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'SHOPSW');
                TSBuyerProtection::$ET_CID = Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'ET_CID');
                TSBuyerProtection::$ET_LID = Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'ET_LID');
                TSBuyerProtection::$DEFAULT_LANG = (int) Configuration::get(
                'PS_LANG_DEFAULT');
                TSBuyerProtection::$CAT_ID = (int) Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'CAT_ID');
                TSBuyerProtection::$ENV_API = Configuration::get(
                TSBuyerProtection::PREFIX_TABLE . 'ENV_API');
            }
        }
    }
    public function install ()
    {
        ...
        foreach ($this->available_languages as $iso => $lang)
            ...
            TSBuyerProtection::PREFIX_TABLE . 'CERTIFICATE_' . strtoupper($iso), 
            Tools::htmlentitiesUTF8(
            Tools::jsonEncode(
            array('stateEnum' => '', 'typeEnum' => '', 'url' => '', 'tsID' => '', 
            'user' => '', 'password' => ''))));
        ...
        '');
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE . 'ENV_API', 
        TSBuyerProtection::ENV_PROD);
        $req = 'CREATE TABLE IF NOT EXISTS `' . _DB_PREFIX_ .
         TSBuyerProtection::DB_ITEMS . '` (
        ...
        if (! array_key_exists(
        Language::getIsoById((int) Configuration::get('PS_LANG_DEFAULT')), 
        ...
        $category->active = 0;
        $category->add();
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE . 'CAT_ID', 
        intval($category->id));
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE .
         'SECURE_KEY', strtoupper(Tools::passwdGen(16)));
        return true;
    }
    public function uninstall ()
    {
        ...
        Configuration::deleteByName(
        TSBuyerProtection::PREFIX_TABLE . 'SECURE_KEY');
        return true;
    }
    /**
    ...
    {
        return $this->site_url . 'modules/' . self::$module_name .
         '/cron_garantee.php?secure_key=' .
         Configuration::get(TSBuyerProtection::PREFIX_TABLE . 'SECURE_KEY');
    }
    /**
    ...
    private function _checkCertificate ($certificate)
    {
        ...
        'INVALID_TS_ID' => $this->l(
        'No certificate has been allocated to the Trusted Shops ID'), 
    ...
    private function _requestForProtectionV2ParamsValidator ($params)
    {
        ...
        array('name' => 'currency', 'length' => 3, 
        'validator' => array('isString')), 
        ...
        'ereg' => '#[0-9]{4}\-[0-9]{2}\-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}#'), 
        array('name' => 'shopSystemVersion', 'validator' => array(
        'isCleanHtml')), 
        ...
        foreach ($mandatory_keys as $key) {
            ...
            if ($bool_flag) {
                if (isset($key['length']))
                    $bool_flag = strlen((string) $params[$key['name']]) ===
                     $key['length'];
                ...
                if (isset($key['validator']))
                    foreach ($key['validator'] as $validator)
                        if (method_exists('Validate', $validator))
                            $bool_flag = ! Validate::$validator(
                            (string) $params[$key['name']]) ? false : $bool_flag;
                if (isset($key['ereg']))
                    $bool_flag = ! preg_match($key['ereg'], 
                    $params[$key['name']]) ? false : $bool_flag;
            }
            if (! $bool_flag) {
                $this->errors[] = sprintf(
                $this->l(
                'The field %s is wrong, please ensure it was correctly filled.'), 
                $key['name']);
                break;
            }
        }
    ...
    private function _requestForProtectionV2 ($params)
    {
        ...
        if ($testing_params) {
            try {
                $code = $client->requestForProtectionV2($params['tsID'], 
                $params['tsProductID'], $params['amount'], $params['currency'], 
                $params['paymentType'], $params['buyerEmail'], 
                $params['shopCustomerID'], $params['shopOrderID'], 
                $params['orderDate'], $params['shopSystemVersion'], 
                $params['wsUser'], $params['wsPassword']);
                if ($code < 0)
                    throw new TSBPException($code, TSBPException::FRONT_END);
            } catch (SoapFault $fault) {
                $this->errors[] = $this->l('Code #') . $fault->faultcode .
                 ',<br />' . $this->l('message:') . $fault->faultstring;
            ...
            $this->errors[] = $this->l(
            'Some parameters sending to "requestForProtectionV2" method are wrong or missing.');
    }
    /**
    ...
    private function _getRequestState ($params)
    {
        ...
        $code = 0;
        try {
            $code = $client->getRequestState($params['tsID'], 
    ...
    public function cronTask ()
    {
        ...
        $mktime = mktime(date('H'), date('i') - 20, date('s'), 
        ...
        foreach ($to_check as $application) {
            $code = $this->_getRequestState(
            array('tsID' => $application['ts_id'], 
            'applicationID' => $application['id_application']));
            if (! empty($this->errors)) {
                $return_message = '<p style="color:red;">' .
                 $this->l(
                'Trusted Shops API returns an error concerning the application #') .
                 $application['id_application'] . ': <br />' .
                 implode(', <br />', $this->errors) . '</p>';
                $this->errors = array();
            } elseif ($code > 0) {
                $return_message = sprintf(
                $this->l(
                'Trusted Shops application number %1$d was successfully processed. The guarantee number is: %2$d'), 
                $application['id_application'], $code);
            }
            $sql = '
			UPDATE `' . $db_name . '`
    ...
    private function _makeRegistrationLink ($shopsw, $et_cid, $et_lid, $lang)
    {
        if (array_key_exists($lang, $this->registration_link))
            return $this->registration_link[$lang] .
             sprintf('?shopsw=%s&et_cid=%s&et_lid=%s', urlencode($shopsw), 
            urlencode($et_cid), urlencode($et_lid));
        return false;
    }
    /**
    ...
    {
        return '<script type="text/javascript" >$().ready(function(){window.open("' .
         $link .
         '");});</script>
		<noscript><p><a href="' .
         $link . '" target="_blank" title="' . $this->l('Registration Link') .
         '" class="link">' . $this->l('Click to get the Registration Link') .
         '</a><p></noscript>';
    }
    /**
    ...
    private function _submitRegistrationLink ()
    {
        ...
        TSBuyerProtection::$SHOPSW = (Validate::isCleanHtml(
        Tools::getValue('shopsw'))) ? Tools::getValue('shopsw') : '';
        TSBuyerProtection::$ET_CID = (Validate::isCleanHtml(
        Tools::getValue('et_cid'))) ? Tools::getValue('et_cid') : '';
        TSBuyerProtection::$ET_LID = (Validate::isCleanHtml(
        Tools::getValue('et_lid'))) ? Tools::getValue('et_lid') : '';
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE . 'SHOPSW', 
        TSBuyerProtection::$SHOPSW);
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE . 'ET_CID', 
        TSBuyerProtection::$ET_CID);
        Configuration::updateValue(TSBuyerProtection::PREFIX_TABLE . 'ET_LID', 
        TSBuyerProtection::$ET_LID);
        $link_registration = $this->_makeRegistrationLink(
        TSBuyerProtection::$SHOPSW, TSBuyerProtection::$ET_CID, 
        TSBuyerProtection::$ET_LID, Tools::getValue('lang'));
        $this->confirmations[] = $this->l(
        'Registration link has been created. Follow this link if you were not redirected earlier:') .
         '&nbsp;<a href="' . $link_registration . '" class="link">&gt;' .
         $this->l('Link') . '&lt;</a>';
    ...
    private function _saveProtectionItems ($protection_items, $ts_id)
    {
		...
		DELETE ts, p, pl
		FROM `' . _DB_PREFIX_ .
         TSBuyerProtection::DB_ITEMS . '` AS ts
        ...
        foreach ($protection_items as $key => $item) {
            ...
            $this->available_languages)) {
                ...
                $product->link_rewrite[(int) Configuration::get(
                'PS_LANG_DEFAULT')] = 'trustedshops';
            }
            ...
            $product->quantity = 1000;
            $product->price = ToolsCore::convertPrice($item->grossFee, 
            Currency::getIdByIsoCode($item->currency));
    ...
    private function _submitAddCertificate ()
    {
        ...
        try {
            $checked_certificate = $this->_checkCertificate(
            ToolsCore::getValue('new_certificate'));
        } catch (TSBPException $e) {
            $this->errors[] = $e->getMessage();
        }
        if ($checked_certificate) {
            TSBuyerProtection::$CERTIFICATE[strtoupper(
            $checked_certificate->certificationLanguage)] = array(
            'stateEnum' => $checked_certificate->stateEnum, 
            'typeEnum' => $checked_certificate->typeEnum, 
            'url' => $checked_certificate->url, 
            'tsID' => $checked_certificate->tsID, 'user' => '', 'password' => '');
            // update the configuration var
            Configuration::updateValue(
            TSBuyerProtection::PREFIX_TABLE . 'CERTIFICATE_' .
             strtoupper($checked_certificate->certificationLanguage), 
            Tools::htmlentitiesUTF8(
            Tools::jsonEncode(
            TSBuyerProtection::$CERTIFICATE[strtoupper(
            $checked_certificate->certificationLanguage)])));
            $this->confirmations[] = $this->l(
            'Certificate has been added successfully.');
            if ($checked_certificate->typeEnum === 'EXCELLENCE') {
                try {
                    $protection_items = $this->_getProtectionItems(
                    $checked_certificate->tsID);
                    if ($protection_items)
                        $this->_saveProtectionItems($protection_items, 
                        $checked_certificate->tsID);
                } catch (TSBPException $e) {
                    $this->errors[] = $e->getMessage();
                }
    ...
    private function _submitEditCertificate ()
    {
        ...
        if ((is_array($edit) and count($edit) > 1) or
         (is_array($delete) and count($delete) > 1)) {
            $this->errors[] = $this->l(
            'You must edit or delete a Certificate one at a time');
            return false;
        }
        // delete action :
        if (is_array($delete) and
         isset(TSBuyerProtection::$CERTIFICATE[$delete[0]]['tsID'])) {
            ...
            $this->confirmations[] = $this->l('The certificate') . ' "' .
             $certificate_to_delete . '" (' . $this->l('language') . ' : ' .
             $delete[0] . ') ' . $this->l('has been deleted successfully');
        }
        // edit action :
        if (is_array($edit)) {
    ...
    private function _submitChangeCertificate ()
    {
        ...
        if ($user != '' and $password != '') {
            ...
            $check_login = false;
            try {
                $check_login = $this->_checkLogin(
                TSBuyerProtection::$CERTIFICATE[$iso_lang]['tsID'], $user, 
                $password);
            } catch (TSBPException $e) {
                $this->errors[] = $e->getMessage();
            }
            if ($check_login) {
                ...
                Configuration::updateValue(
                TSBuyerProtection::PREFIX_TABLE . 'CERTIFICATE_' . $iso_lang, 
                Tools::htmlentitiesUTF8(
                Tools::jsonEncode(TSBuyerProtection::$CERTIFICATE[$iso_lang])));
                $this->confirmations[] = $this->l(
                'Certificate login has been successful.');
            }
        } else {
            $this->errors[] = $this->l(
            'You have to set a username and a password before any change.');
        }
        return true;
    }
    /**
    ...
    {
        if (Configuration::get(TSBuyerProtection::PREFIX_TABLE . 'ENV_API') !=
         $env_api)
            Configuration::updateValue(
            TSBuyerProtection::PREFIX_TABLE . 'ENV_API', $env_api);
    ...
    public function getContent ()
    {
        ...
        if (is_array(self::$CERTIFICATE))
            foreach (self::$CERTIFICATE as $certif)
                $bool_display_certificats = (isset($certif['tsID']) &&
                 $certif['tsID'] != '') ? true : $bool_display_certificats;
        if ($bool_display_certificats)
            $out .= $this->_displayFormCertificatesList();
        if (isset($posts_return['edit_certificate']) &&
         $posts_return['edit_certificate'] &&
         is_array($posts_return['edit_certificate'])) {
            $out .= '<br />';
            $out .= $this->_displayFormEditCertificate(
            $posts_return['edit_certificate']);
        }
        $out .= '<br />';
    ...
    private function _displayPresentation ()
    {
        ...
        return '
			<div style="text-align:right; margin:10px 20px 10px 0">
				<img src="' .
         __PS_BASE_URI__ . 'modules/' . self::$module_name . '/img/siegel.gif" alt="logo"/>
			</diV>
		<h3>' .
         $this->l('Seal of Approval and Buyer Protection') .
         '</h3>
		<p>' .
         $this->l(
		...
		<h3>' .
         $this->l('More trust leads to more sales!') .
         '</h3>
		<p>' .
         $this->l(
        'The Trusted Shops seal of approval is the optimal way to increase the trust of your online customers. Trust increases customers\' willingness to buy from you.') . '</p>
		<h3>' .
         $this->l('Less abandoned purchases') .
         '</h3>
		<p>' .
         $this->l(
        'Give your online customers a strong reason to buy with the Trusted Shops Buyer Protection. This additional security leads to less shopping basket abandonment') . '</p>
		<h3>' .
         $this->l('Profitable and long-term customer relationship') .
         '</h3>
		<p>' .
         $this->l(
        'For many online shoppers, the Trusted Shops Seal of Approval with Buyer Protection is an effective sign of quality for safe shopping on the internet. One-time buyers become regular customers.') . '</p><br />
		<h3>' . $this->l('Environment type') .
         '</h3>
		<p>' .
         $this->l('You are currently using the mode :') . ' <b>' .
         TSBuyerProtection::$ENV_API . '</b></p><br />' . $link;
    }
    private function _displayFormRegistrationLink ($link = false)
    {
        $out = '
		<form action="' .
         $this->_makeFormAction(strip_tags($_SERVER['REQUEST_URI']), 
        $this->id_tab) .
         '" method="post" >
			<fieldset>
				<legend><img src="../img/admin/cog.gif" alt="" />' .
         $this->l('Get the Registration Link') .
         '</legend>
				<p>' .
         $this->l('This variable was sent to you via e-mail by TrustedShops') .
         '</p>
				<label>' .
         $this->l('Internal identification of shop software at Trusted Shops') . '</label>
				<div class="margin-form">
					<input type="text" name="shopsw" value="' .
         TSBuyerProtection::$SHOPSW . '"/>
				</div>
				<br />
				<br class="clear" />
				<label>' .
         $this->l('Etracker channel') . '</label>
				<div class="margin-form">
					<input type="text" name="et_cid" value="' .
         TSBuyerProtection::$ET_CID . '"/>
				</div>
				<br class="clear" />
				<label>' .
         $this->l('Etracker campaign') . '</label>
				<div class="margin-form">
					<input type="text" name="et_lid" value="' .
         TSBuyerProtection::$ET_LID . '"/>
				</div>
        ...
        foreach ($this->available_languages as $iso => $lang)
            if (is_array($lang))
                $out .= '<option value="' . $iso . '" ' .
                 ((int) $lang['id_lang'] === TSBuyerProtection::$DEFAULT_LANG ? 'selected' : '') .
                 '>' . $lang['name'] . '</option>';
        $out .= '</select>
				</div>
        ...
        $out .= '<input type="submit" name="submit_registration_link" class="button" value="' .
         $this->l('send') . '"/>
				</div>
			</fieldset>
		</form>';
    ...
    private function _displayFormAddCertificate ()
    {
        ...
        $this->id_tab) .
         '" method="post" >
			<fieldset>
				<legend><img src="../img/admin/cog.gif" alt="" />' .
         $this->l('Add Trusted Shops certificate') . '</legend>
				<label>' . $this->l(
        'New certificate') .
         '</label>
				<div class="margin-form">
					<input type="text" name="new_certificate" value="" style="width:300px;"/>&nbsp;
					<input type="submit" name="submit_add_certificate" class="button" value="' .
         $this->l('Add it') . '"/>
				</div>
			</fieldset>
		</form>';
    ...
    private function _displayFormCertificatesList ()
    {
		...
		<fieldset>
			<legend><img src="../img/admin/cog.gif" alt="" />' .
         $this->l('Manage Trusted Shops certificates') . '</legend>
				<table width="100%">
					<thead>
        ...
        foreach (TSBuyerProtection::$CERTIFICATE as $lang => $certificate) {
            ...
            if (isset($certificate['tsID']) and $certificate['tsID'] !== '') {
                ...
                if ($certificate['typeEnum'] === 'EXCELLENCE') {
                    $out .= '<input type="checkbox" name="certificate_edit[]" value="' .
                     $lang . '" />';
                    $out .= $certificate['user'] == '' ? '<br /><b style="color:red;font-size:0.7em;">' .
                     $this->l('Login or password missing') . '</b>' : '';
                } else {
                    $out .= $this->l('No need');
                }
                $out .= '
							...
							<td>';
                if ($certificate['typeEnum'] === 'EXCELLENCE') {
                    $out .= '<input type="checkbox" name="certificate_delete[]" value="' .
                     $lang . '" />';
                } else {
                    $out .= $this->l('No need');
                }
        ...
        $out .= '
				...
				<div style="text-align:center;"><input type="submit" name="submit_edit_certificate" class="button" value="' .
         $this->l('Edit certificate') . '"/></div>
			</fieldset>
		</form>
		';
        return $out;
    ...
    private function _displayFormEditCertificate ($certificate)
    {
		...
		<script type="text/javascript" src="' .
         $this->site_url .
         'modules/trustedshops/lib/js/payment.js" ></script>
		<script type="text/javascript">
			$().ready(function()
			{
				TSPayment.payment_type = $.parseJSON(\'' .
         Tools::jsonEncode(TSBuyerProtection::$payments_type) .
         '\');
				TSPayment.payment_module = $.parseJSON(\'' .
         Tools::jsonEncode($payment_module_collection) .
         '\');
        ...
        $this->id_tab) .
         '" method="post" >
			<fieldset>
				<legend><img src="../img/admin/tab-tools.gif" alt="" />' .
         $this->l('Edit certificate') .
         '</legend>
				<input type="hidden" name="iso_lang" value="' .
         $certificate['language'] . '" />
				<label>' . $this->l('Language') . '</label>
				<div class="margin-form">' .
         $certificate['language'] . '</div>
				<label>' . $this->l('Shop url') . '</label>
				<div class="margin-form">' . $certificate['url'] . '</div>
				<label>' . $this->l('Certificate id') . '</label>
				<div class="margin-form">' .
         $certificate['tsID'] . '</div>
				<label>' .
         $this->l('User Name') .
         ' <sup>*</sup></label>
				<div class="margin-form"><input type="text" name="user" value="' .
         $certificate['user'] . '" style="width:300px;"/></div>
				<label>' . $this->l('Password') .
         ' <sup>*</sup></label>
				<div class="margin-form"><input type="text" name="password" value="' .
         $certificate['password'] . '" style="width:300px;"/></div>
				<div id="payment-type">
					<label>' .
         $this->l('Payment type to edit') . ' <sup>*</sup></label>
					<div class="margin-form">
						<select name="payment_type">';
        foreach (TSBuyerProtection::$payments_type as $type => $translation)
            $out .= '	<option value="' . $type . '" >' . $translation .
             '</option>';
        $out .= '		</select>&nbsp;' . $this->l('with') . '&nbsp;
						<select name="payment_module">';
        foreach ($payment_module_collection as $module_info)
            $out .= '		<option value="' . $module_info['id_module'] . '" >' .
             $module_info['name'] . '</option>';
        $out .= '		</select>&nbsp;' . $this->l('payment module') .
         '&nbsp;<input type="button" value="' . $this->l('Add it') . '" class="button" name="add_payment_module" />
					</div><!-- .margin-form -->
					<div id="payment_type_list">';
        $input_output = '';
        if (isset($certificate['payment_type']) and
         ! empty($certificate['payment_type'])) {
            foreach ($certificate['payment_type'] as $payment_type => $modules) {
                $out .= '	<label style="clear:both;" class="payment-type-label" >' .
                 TSBuyerProtection::$payments_type[$payment_type] . '</label>';
                $out .= '	<div class="margin-form" id="block-payment-' .
                 $payment_type . '">';
                foreach ($modules as $module_id) {
                    $out .= '<b class="payment-module-label" id="label-module-' .
                     $module_id . '"></b>';
                    $input_output .= '<input type="hidden" value="' . $module_id .
                     '" class="choosen_payment_type" name="choosen_payment_type[' .
                     $payment_type . '][]">';
                }
                $out .= '	</div><!-- .margin-form -->';
            }
        ...
        $out .= '</div><!-- #payment_type_list -->
         ...
         '</p>
			<p style="text-align:center;">
				<input type="submit" name="submit_change_certificate" class="button" value="' .
         $this->l('Update it') . '"/>
			</p>
			</fieldset>
		</form>';
    ...
    {
        $out = '<fieldset>
				<legend><img src="../img/admin/warning.gif" alt="" />' .
         $this->l('Cronjob configuration') . '</legend>';
        $out .= '<p>' .
         $this->l(
        'If you are using a Trusted Shops EXCELLENCE cetificate in your shop, set up a cron job on your web server.') .
         '<br />' . $this->l('Run the script file ') . ' <b style="color:red;">' .
         $this->getCronFilePath() . '</b> ' .
         $this->l('with an interval of 10 minutes.') . '<br /><br />' .
         $this->l(
        'The corresponding line in your cron file may look like this:') .
         ' <br /><b style="color:red;">*/10 * * * * ' . $this->getCronFilePath() .
         '>/dev/null 2>&1</b><br />' . '</p>';
        $out .= '</fieldset>';
        return $out;
    }
    public function hookRightColumn ($params)
    {
        ...
        if (array_key_exists($lang, $this->available_languages) and
         isset(TSBuyerProtection::$CERTIFICATE[$lang]['tsID'])) {
            TSBuyerProtection::$smarty->assign('trusted_shops_id', 
            TSBuyerProtection::$CERTIFICATE[$lang]['tsID']);
            TSBuyerProtection::$smarty->assign('onlineshop_name', 
            ConfigurationCore::get('PS_SHOP_NAME'));
            $url = str_replace(array('#shop_id#', '#shop_name#'), 
            array(TSBuyerProtection::$CERTIFICATE[$lang]['tsID'], 
            urlencode(
            str_replace('_', '-', ConfigurationCore::get('PS_SHOP_NAME')))), 
            TSBuyerProtection::$certificate_link[$lang]);
            ...
            if ($displayWidget == false)
                return '';
            return $this->display(TSBuyerProtection::$module_name, 
            'seal_of_approval.tpl');
        }
    }
    /**
    ...
    public function hookPaymentTop ($params)
    {
        ...
        if (TSBuyerProtection::$CERTIFICATE[$lang]['typeEnum'] ==
         'CLASSIC' or
         (TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'INTEGRATION' and
         TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'PRODUCTION' and
         TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'TEST'))
            return '';
             // If login parameters missing for the certificate an error occurred
        if ((TSBuyerProtection::$CERTIFICATE[$lang]['user'] == '' or
         TSBuyerProtection::$CERTIFICATE[$lang]['password'] == '') and
         TSBuyerProtection::$CERTIFICATE[$lang]['typeEnum'] == 'EXCELLENCE') {
            return '';
            // Display here has been deprecated by Trusted Shop
            return '
			<p style="color:red;text-align:center;font-size:14px;font-weight:bold;">' .
             $this->l(
            'The Trusted Shop Buyer Protection needs a login to succeed. Please contact the shop administrator.') .
             '<br />' . $this->l('Problem occurred with your language:') . ' "' .
             $lang . '"</p>';
        }
    ...
    private function _orderConfirmationExcellence ($params, $lang)
    {
        ...
        if (count($item) > 1) {
            $this->errors[] = $this->l(
            'A buyer can\'t buy multiple Buyer Protection Products.');
            die($this->errors);
        }
        $item = $item[0];
        ...
        $arr_params['orderDate'] = date('Y-m-d\TH:i:s', 
        strtotime($params['objOrder']->date_add));
        ...
        $return = '';
        if (! empty($this->errors))
            $return = '<p style="color:red">' . implode('<br />', $this->errors) .
             '</p>';
             //else
    ...
    private function _orderConfirmationClassic ($params, $lang)
    {
        ...
        TSBuyerProtection::$smarty->assign(
        array('tax_label' => 'TTC', 'buyer_protection' => $arr_params));
        return $this->display(TSBuyerProtection::$module_name, 
        'order-confirmation-tsbp-classic.tpl');
    }
    /**
    ...
    public function hookOrderConfirmation ($params)
    {
        ...
        if (((TSBuyerProtection::$CERTIFICATE[$lang]['user'] == '' or
         TSBuyerProtection::$CERTIFICATE[$lang]['password'] == '') and
         TSBuyerProtection::$CERTIFICATE[$lang]['typeEnum'] == 'EXCELLENCE') or
         (TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'INTEGRATION' and
         TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'PRODUCTION' and
         TSBuyerProtection::$CERTIFICATE[$lang]['stateEnum'] !== 'TEST'))
            return '';
