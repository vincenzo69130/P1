...
class TrustedShops extends Module
{
    ...
    public function __construct ()
    {
        ...
        foreach (self::$objects_list as $object) {
            $this->limited_countries = array_merge($this->limited_countries, 
            $object->limited_countries);
        ...
        'Build confidence in your online shop with the Trusted Shops quality seal, buyer protection and customer rating.');
    ...
    public function install ()
    {
        ...
        $return = ($return) ? (parent::install() and
         $this->registerHook('orderConfirmation') and
         $this->registerHook('newOrder') and $this->registerHook('rightColumn') and
         $this->registerHook('paymentTop') and
         $this->registerHook('orderConfirmation')) : $return;
    ...
    public function getContent ()
    {
        ...
        foreach ($tabs['title'] as $key => $title)
            $out .= '<li id="menuTab' . $key . '" class="menuTabButton' .
             ((int) $key === (int) Tools::getValue('id_tab') ? ' selected' : '') .
             '">' . ($key + 1) . '. ' . $title . '</li>';
        $out .= '</ul>';
        // Display content Tabs
        $out .= '<div id="tabList">';
        foreach ($tabs['content'] as $key => $content)
            $out .= '<div id="menuTab' . $key . 'Sheet" class="tabItem' .
             ((int) $key === (int) Tools::getValue('id_tab') ? ' selected' : '') .
             '">' . $content . '</div>';
        ...
        return (! empty($this->errors) ? $this->displayErrors() : $this->displayConfirmations()) .
         $out;
    }
    ...
    private function checkObjectsErrorsOrConfirmations ()
    {
        foreach (self::$objects_list as $object) {
            ...
            if (! empty($object->confirmations))
                $this->confirmations = array_merge($this->confirmations, 
