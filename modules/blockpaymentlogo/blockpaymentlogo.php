...
class BlockPaymentLogo extends Module
{
    ...
    public function getContent ()
    {
        ...
        if (Tools::isSubmit('submitConfiguration'))
            if (Validate::isUnsignedInt(Tools::getValue('id_cms'))) {
                Configuration::updateValue('PS_PAYMENT_LOGO_CMS_ID', 
        ...
        if (! sizeof($cmss))
			...
			<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">
             ...
             $this->l('Configure') . '</legend>
					<label>' . $this->l('Page CMS for link') . ':</label>
					<div class="margin-form">
						<select name="id_cms"><option value="0">(' . $this->l('Select a page') .
             ')</option>';
            foreach ($cmss as $cms)
                $html .= '<option value="' . $cms['id_cms'] . '"' .
                 (Configuration::get('PS_PAYMENT_LOGO_CMS_ID') == $cms['id_cms'] ? ' selected="selected"' : '') .
                 '>' . $cms['meta_title'] . '</option>';
            $html .= '</select>
					</div>
					<p class="center"><input class="button" type="submit" name="submitConfiguration" value="' .
    ...
    public function hookLeftColumn ($params)
    {
        ...
        if (! Configuration::get('PS_PAYMENT_LOGO_CMS_ID'))
            return;
        $cms = new CMS((int) (Configuration::get('PS_PAYMENT_LOGO_CMS_ID')), 
        (int) ($cookie->id_lang));
