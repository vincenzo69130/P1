...
class StatsOrigin extends ModuleGraph
{
    ...
    function __construct ()
    {
        ...
        'Display the websites your visitors come from.');
    }
    function install ()
    {
    ...
    private function getOrigins ($dateBetween)
    {
        ...
        while ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->nextRow($result)) {
            if (! isset($row['http_referer']) or empty($row['http_referer']))
                ...
                $website = preg_replace('/^www./', '', 
                parse_url($row['http_referer'], PHP_URL_HOST));
    ...
    function hookAdminStatsModules ()
    {
        ...
        $this->_html = '<fieldset class="width3"><legend><img src="../modules/' .
         $this->name . '/logo.gif" /> ' . $this->l('Origin') . '</legend>';
        if (sizeof($websites)) {
            $this->_html .= '
			<center><p><img src="../img/admin/down.gif" />' .
             $this->l(
            'Here is the percentage of the 10 most popular referrer websites by which visitors went through to get to your shop.') . '</p>
			' .
             ModuleGraph::engine(array('type' => 'pie')) .
             '</center>
			<p><a href="' .
             Tools::safeOutput($_SERVER['REQUEST_URI']) .
             '&export=1&exportType=top"><img src="../img/admin/asterisk.gif" />' .
             $this->l('CSV Export') . '</a></p><br /><br />
            ...
            foreach ($websites as $website => $total)
                $this->_html .= '<tr><td>' .
                 (! strstr($website, ' ') ? '<a href="' . Tools::getProtocol() .
                 $website . '">' : '') . $website .
                 (! strstr($website, ' ') ? '</a>' : '') .
                 '</td><td style="text-align: right">' . $total . '</td></tr>';
            $this->_html .= '</table></center></div>';
        } else
            $this->_html .= '<p><strong>' . $this->l('Direct links only') .
             '</strong></p>';
        ...
        'When visiting a webpage, the referrer is the URL of the previous webpage from which a link was followed.') .
         '<br />
				' .
         $this->l(
        'A referrer enables you to know which keywords are entered by visitors in search engines when getting to your shop and allows you to optimize web promotion.') . '<br /><br />
         ...
         '</li>
					<li class="bullet">' .
         $this->l(
        'A partner with whom you made a link exchange in order to bring in sales or attract new customers') . '</li>
				</ul>
			</p>
		</fieldset>';
        return $this->_html;
