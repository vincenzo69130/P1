...
class GoogleRequest
{
    ...
    function GoogleRequest ($id, $key, $server_type = "sandbox", $currency = "USD")
    {
        ...
        $this->request_url = $this->base_url . "request/Merchant/" .
         $this->merchant_id;
        $this->merchant_checkout = $this->base_url . "merchantCheckout/Merchant/" .
         $this->merchant_id;
    ...
    public function SendServer2ServerCart ($xml_cart, $die = true)
    {
        ...
        if ($status != 200) {
            ...
            $this->log->logRequest(
            "Redirecting to: " . $data[$root]['redirect-url']['VALUE']);
    ...
    function SendCancelOrder ($google_order, $reason, $comment)
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
         ...
         $this->schema_url . "\" google-order-number=\"" . $google_order . "\">
                  <reason>" .
         (substr(htmlentities(strip_tags($reason)), 0, GOOGLE_REASON_LENGTH)) . "</reason>
                  <comment>" .
         (substr(htmlentities(strip_tags($comment)), 0, GOOGLE_REASON_LENGTH)) . "</comment>
                  </cancel-order>";
        return $this->SendReq($this->request_url, 
        $this->GetAuthenticationHeaders(), $postargs);
    }
     ...
     * @return array the status code and body of the response
     */
    function SendMerchantOrderNumber ($google_order, $merchant_order, 
    $timeout = false)
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
     ...
     * @return array the status code and body of the response
     */
    function SendBuyerMessage ($google_order, $message, $send_mail = "true", 
    $timeout = false)
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
         ...
         "\" google-order-number=\"" . $google_order . "\">
                  <message>" .
         (substr(htmlentities(strip_tags($message)), 0, GOOGLE_MESSAGE_LENGTH)) . "</message>
     ...
     * @return array the status code and body of the response
     */
    function SendDeliverOrder ($google_order, $carrier = "", $tracking_no = "", 
    $send_mail = "true")
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
    ...
    function SendShipItems ($google_order, $items_list = array(), $send_mail = "true")
    {
        ...
        foreach ($items_list as $item) {
            ...
            if (count($item->tracking_data_list)) {
                ...
                foreach ($item->tracking_data_list as $tracking_data) {
                    $postargs .= "<tracking-data>
                            <carrier>" .
                     htmlentities($tracking_data['carrier']) . "</carrier>
                            <tracking-number>" .
                     $tracking_data['tracking-number'] . "</tracking-number>
                          </tracking-data>\n";
                }
                $postargs .= "</tracking-data-list>\n";
            }
        ...
        $postargs .= "</item-shipping-information-list>\n" . "<send-email>" .
         strtolower($send_mail) . "</send-email>
                  </ship-items>";
        return $this->SendReq($this->request_url, 
        $this->GetAuthenticationHeaders(), $postargs);
    }
     ...
     * @return array the status code and body of the response
     */
    function SendBackorderItems ($google_order, $items_list = array(), 
    $send_mail = "true")
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
     ...
     * @return array the status code and body of the response
     */
    function SendResetItemsShippingInformation ($google_order, 
    $items_list = array(), $send_mail = "true")
    {
        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
    ...
    function GetAuthenticationHeaders ()
    {
        ...
        $headers[] = "User-Agent: GC-PHP-Sample_code (" . PHP_SAMPLE_CODE_VERSION .
         "/ropu)";
        return $headers;
    }
    /**
    ...
    function parse_headers ($message)
    {
        ...
        $headers = $this->get_headers_x(
        substr($message, 0, $head_end + strlen(DOUBLE_ENTER)));
        ...
        switch ($status_code[1]) {
            case '200':
                $parsed = $this->parse_headers(
                substr($message, $head_end + strlen(DOUBLE_ENTER)));
