...
class GoogleCart
{
     ...
     * ("-#" suffix)
     */
    var $multiple_tags = array('flat-rate-shipping' => array(), 
    'merchant-calculated-shipping' => array(), 'pickup' => array(), 
    'parameterized-url' => array(), 'url-parameter' => array(), 'item' => array(), 
    'us-state-area' => array('tax-area'), 'us-zip-area' => array('tax-area'), 
    'us-country-area' => array('tax-area'), 'postal-area' => array('tax-area'), 
    'alternate-tax-table' => array(), 'world-area' => array('tax-area'), 
    'default-tax-rule' => array(), 'alternate-tax-rule' => array(), 
    'gift-certificate-adjustment' => array(), 'coupon-adjustment' => array(), 
    'coupon-result' => array(), 'gift-certificate-result' => array(), 'method' => array(), 
    'anonymous-address' => array(), 'result' => array(), 'string' => array());
    var $ignore_tags = array('xmlns' => true, 'checkout-shopping-cart' => true, 
    // Dont know how to translate these tag yet
    'merchant-private-data' => true, 'merchant-private-item-data' => true);
    /**
    ...
    function GoogleCart ($id, $key, $server_type = "sandbox", $currency = "USD")
    {
        ...
        $this->checkout_url = $this->base_url . "checkout/Merchant/" .
         $this->merchant_id;
        $this->checkoutForm_url = $this->base_url . "checkoutForm/Merchant/" .
         $this->merchant_id;
...
die();
     ...
     * @return void
     */
    function SetMerchantCalculations ($url, $tax_option = "false", $coupons = "false", 
    $gift_cert = "false")
    {
        $this->merchant_calculations_url = $url;
        $this->merchant_calculated_tax = $this->_GetBooleanValue($tax_option, 
        "false");
        $this->accept_merchant_coupons = $this->_GetBooleanValue($coupons, 
        "false");
        $this->accept_gift_certificates = $this->_GetBooleanValue($gift_cert, 
        "false");
    }
    /**
    ...
    function GetXML ()
    {
        ...
        foreach ($this->item_arr as $item) {
            ...
            if ($item->merchant_private_item_data != '') {
                ...
                'merchantprivate')) {
                    $item->merchant_private_item_data->AddMerchantPrivateToXML(
                    $xml_data);
                } else {
            ...
            if ($item->tax_table_selector != '')
                $xml_data->Element('tax-table-selector', 
                $item->tax_table_selector);
                 //      Carrier calculation
            if ($item->item_weight != '' &&
             $item->numeric_weight !== '') {
                $xml_data->EmptyElement('item-weight', 
                array('unit' => $item->item_weight, 
        ...
        if ($this->merchant_private_data != '') {
            if (is_object($this->merchant_private_data) &&
             get_class($this->merchant_private_data) == 'merchantprivate') {
        ...
        foreach ($this->shipping_arr as $ship) {
            ...
            if ($ship->type == "flat-rate-shipping" ||
             $ship->type == "merchant-calculated-shipping") //  If shipping-company calc support addr-filtering and shipping restrictions as a subatag of shipping-company-calculated-shipping
//           ||$ship->type == "shipping-company-calculated-shipping" 
            {
                ...
                if (isset($shipping_restrictions)) {
                    ...
                    if ($shipping_restrictions->allowed_restrictions) {
                        ...
                        for ($i = 0; $i <
                         count(
                        $shipping_restrictions->allowed_country_codes_arr); $i ++) {
                            ...
                            if ($postal_pattern != "") {
                                $xml_data->Element('postal-code-pattern', 
                                $postal_pattern);
                            }
                    ...
                    if ($shipping_restrictions->excluded_restrictions) {
                        ...
                        for ($i = 0; $i <
                         count(
                        $shipping_restrictions->excluded_country_codes_arr); $i ++) {
                            $xml_data->Push('postal-area');
                ...
                if ($ship->type == "merchant-calculated-shipping") {
                    ...
                    if (isset($address_filters)) {
                        ...
                        if ($address_filters->excluded_restrictions) {
                            ...
                            for ($i = 0; $i <
                             count($address_filters->excluded_country_codes_arr); $i ++) {
                                $xml_data->Push('postal-area');
                ...
                foreach ($CCSoptions as $CCSoption) {
                    ...
                    array('currency' => $this->currency));
                    $xml_data->Element('shipping-company', 
                    $CCSoption->shipping_company);
                    $xml_data->Element('shipping-type', 
                    $CCSoption->shipping_type);
                    $xml_data->Element('carrier-pickup', 
                    $CCSoption->carrier_pickup);
                ...
                $xml_data->Push('ship-from', 
                array('id' => $ship->ShippingPackage->ship_from->id));
                $xml_data->Element('city', 
                $ship->ShippingPackage->ship_from->city);
                $xml_data->Element('region', 
                $ship->ShippingPackage->ship_from->region);
                $xml_data->Element('postal-code', 
                $ship->ShippingPackage->ship_from->postal_code);
                $xml_data->Element('country-code', 
                $ship->ShippingPackage->ship_from->country_code);
                $xml_data->Pop('ship-from');
                $xml_data->EmptyElement('width', 
                array('unit' => $ship->ShippingPackage->unit, 
            ...
            if (count($this->default_tax_rules_arr) != 0) {
                ...
                foreach ($this->default_tax_rules_arr as $curr_rule) {
                    if ($curr_rule->country_area != "") {
                        ...
                        $xml_data->Element('shipping-taxed', 
                        $curr_rule->shipping_taxed);
    ...
    function CheckoutServer2Server ($proxy = array(), $certPath = '')
    {
        ...
        $this->server_url == "https://checkout.google.com/" ? "Production" : "sandbox", 
        $this->currency);
    ...
    $loc = "en_US", $showtext = true, $style = "trans")
    {
        ...
        if ($this->variant == "text") {
            $data .= "<div align=center><form method=\"POST\" action=\"" . $url .
             "\"" .
             ($this->googleAnalytics_id ? " onsubmit=\"setUrchinInputCode();\"" : "") . ">
            ...
            $data .= "<div><img alt=\"Checkout\" src=\"" . "" . $this->server_url .
             ...
             "&w=" . $width . "&h=" . $height . "&style=" . $style . "&variant=" .
             $this->variant . "&loc=" . $loc . "\" height=\"" . $height . "\"" .
             " width=\"" . $width . "\" /></div>";
        }
        $data .= "</div>";
        return $data;
    ...
    function CheckoutButtonNowCode ($size = "large", $variant = true, 
    $loc = "en_US", $showtext = true, $style = "trans")
    {
        switch (strtolower($size)) {
            case "small":
        ...
        if ($this->variant == "text") {
            ...
            $data .= "<div><img alt=\"Checkout\" src=\"" . "" . $this->server_url .
             "buttons/buy.gif?merchant_id=" . "" . $this->merchant_id . "&w=" .
             $width . "&h=" . $height . "&style=" . $style . "&variant=" .
             $this->variant . "&loc=" . $loc . "\" height=\"" . $height . "\"" .
             " width=\"" . $width . "\" /></div>";
        }
        if ($showtext) {
    ...
    function xml2html ($data, $path = '', &$rta)
    {
        ...
        foreach ($data as $tag_name => $tag) {
            ...
            if (is_array($tag)) {
                ...
                $rta .= '<input type="hidden" name="' . $new_path . '" value="' .
                 $tag . '"/>' . "\n";
            }
        }
    }
    ...
    function CalcHmacSha1 ($data)
    {
        ...
        $hmac = pack('H*', 
        $hashfunc(
        ($key ^ $opad) . pack('H*', $hashfunc(($key ^ $ipad) . $data))));
        return $hmac;
    }
