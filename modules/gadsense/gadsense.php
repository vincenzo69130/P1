...
class GAdsense extends Module
{
    function __construct ()
    {
        ...
        if ($this->id and ! Configuration::get('GADSENSE_ID'))
            $this->warning = $this->l(
            'You have not yet set your Google Adsense code');
        $this->description = $this->l(
        'Integrate Google Adsense script into your shop');
    ...
    public function getContent ()
    {
        ...
        if (Tools::isSubmit('submitGAdsense') and
         ($gai = Tools::getValue('gadsense_id'))) {
		...
		<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">
         ...
         $this->l('Settings') . '</legend>
				...
				<div class="margin-form">
					<textarea name="gadsense_id" cols="90" rows="10" />' . Tools::safeOutput(
        Tools::getValue('gadsense_id', Configuration::get('GADSENSE_ID'))) . '</textarea>
					<p class="clear">' . $this->l('Example:') . ' <br /><br /><img src="../modules/gadsense/adsense_script.gif"></p>
				</div>
				<center><input type="submit" name="submitGAdsense" value="' .
