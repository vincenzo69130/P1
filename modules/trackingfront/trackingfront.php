...
class TrackingFront extends Module
{
    ...
    public function postProcess ()
    {
        ...
        if (Tools::isSubmit('ajaxProductFilter')) {
			...
			FROM `' . _DB_PREFIX_ .
             'referrer`
			WHERE `id_referrer` = ' .
             (int) (Tools::getValue('id_referrer')) . ' AND `passwd` = \'' .
             pSQL(Tools::getValue('token')) . '\'');
            ...
            elseif (! Validate::isPasswd($passwd, 1))
				...
				WHERE `name` = \'' .
                 pSQL($login) . '\' AND `passwd` = \'' . pSQL($passwd) . '\'');
                if (! isset($result['id_referrer']) or
                 ! ($tracking_id = (int) ($result['id_referrer'])))
    ...
    public function isLogged ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'referrer`
		WHERE `id_referrer` = ' .
         (int) ($cookie->tracking_id) . ' AND `passwd` = \'' .
         pSQL($cookie->tracking_passwd) . '\'');
    ...
    public function displayAccount ()
    {
        ...
        array(array('id_referrer' => (int) ($cookie->tracking_id))), 
        $fakeEmployee);
        ...
        'orders' => $this->l('Orders'), 'base_fee' => $this->l('Base fee'), 
        'percent_fee' => $this->l('Percent fee'), 
        'click_fee' => $this->l('Click fee'), 'sales' => $this->l('Sales'), 
        ...
        $echo = '
		<script type="text/javascript" src="../../js/jquery/datepicker/ui/i18n/ui.datepicker-' .
		...
		<script type="text/javascript">
			...
			{
				$.getJSON("stats.php",{ajaxProductFilter:1,id_referrer:' .
         $referrer->id . ',token:"' . $cookie->tracking_passwd . '",id_product:0},
					function(j) {';
         ...
         '\');
			...
			function showProductLines()
			{
				...
				for (var i = 0; i < productIds.length; ++i)
					$.getJSON("stats.php",{ajaxProductFilter:1,token:"' .
         $cookie->tracking_passwd . '",id_referrer:' . $referrer->id .
         ',id_product:productIds[i]},
						function(result) {
							var newLine = newProductLine(' .
         $referrer->id . ', result[0], (irow++%2 ? 204 : 238));
        ...
        return $this->display(__FILE__, 'header.tpl') . $echo .
         $this->display(__FILE__, 'account.tpl') . $echo2;
    }
}
