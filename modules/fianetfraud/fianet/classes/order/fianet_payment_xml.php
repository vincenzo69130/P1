...
class fianet_payment_xml
{
    ...
    function set_cb_number ($cb, $dateval)
    {
        if (! eregi("^[0-9]{16}$", $cb)) {
            fianet_insert_log(
            "fianet_payment_xml.php - set_cb_number() <br />Cb number format is invalid, must be 00001111222233334444\n");
        } elseif (! eregi("^[0-9]{2}/[0-9]{4}$", $dateval)) {
            fianet_insert_log(
            "fianet_payment_xml.php - set_cb_number() <br />Validity date format is invalid, must be MM/YYYY\n");
        } else {
            $crypt = new HashMD5();
    ...
    function get_xml ()
    {
        ...
        if ($this->type != null) {
            ...
            fianet_insert_log(
            "fianet_payment_xml.php - get_xml() <br />Type is undefined\n");
            return;
        }
        return ($xml);
    }
