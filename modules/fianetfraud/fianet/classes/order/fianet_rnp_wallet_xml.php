...
class fianet_rnp_wallet_xml
{
    ...
    function generate_wallet_crypt_data ($order_id, $billing_lastname, 
    $customer_email, $total)
    {
        $total = number_format($total, 2, '.', '');
        $this->generate_crypt_key($order_id, $billing_lastname, $customer_email, 
        $total);
    }
    function get_xml ()
    {
        ...
        $xml .= "\t\t" . '<crypt version="' . $this->crypt_version . '">' .
         $this->crypt . '</crypt>' . "\n";
        $xml .= "\t" . '</wallet>' . "\n";
        return ($xml);
    }
    function generate_crypt_key ($order_id, $billing_lastname, $customer_email, 
    $total)
    {
        ...
        $this->order_date_time_seconds, $customer_email, $order_id, $total, 
        $billing_lastname);
    }
}
