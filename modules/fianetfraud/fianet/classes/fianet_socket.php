...
class fianet_socket
{
    ...
    var $data = array();
    var $path;
    function fianet_socket ($fianet_url, $fianet_file, $method = 'GET', 
    $data = array())
    {
        if (is_array($data)) {
            $this->data = $data;
    ...
    function parse_fianet_url ($fianet_url, $fianet_file)
    {
        if (ereg("^http://", $fianet_url)) {
            ...
            $this->port = 80;
            $this->path = eregi_replace("http://[^/]+/", "/", $fianet_url) .
             $fianet_file;
        }
        if (ereg("^https://", $fianet_url)) {
            $this->is_ssl = true;
            $this->host = eregi_replace("https://([^/]+)/.+", "\\1", 
            $fianet_url);
            $this->port = 443;
            $this->path = eregi_replace("https://[^/]+/", "/", $fianet_url) .
             $fianet_file;
        }
         //debug($this->path);
    }
        ...
        if ($this->method == 'POST') {
            ...
            if ($data != '') {
                if (strlen($this->path . "?" . $data) > 2048) {
                    fianet_insert_log(
                    "fianet_socket.php - build_header() <br />\nMaximum length in get method reached(" .
                     strlen($this->path . "?" . $data) . ") : <br />\n" .
                     $this->path . "?" . $data);
                }
