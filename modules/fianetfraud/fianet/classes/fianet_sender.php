...
class fianet_sender
{
    ...
    function send_orders_singet ()
    {
        if (! count($this->orders) > 0) {
            fianet_insert_log(
            "fianet_sender.php - send_orders_singet() <br />\nNo order to send.");
            return;
        }
    ...
    function send_orders_stacking ()
    {
        if (! count($this->orders) > 0) {
            fianet_insert_log(
            "fianet_sender.php - send_orders_stackfast() <br />\nNo order to send.");
            return;
        }
        if (! $this->check_order_data()) {
            fianet_insert_log(
    ...
    function get_evaluation ($order_list, $Separ = ',')
    {
        ...
        if (count($order_list) > 0) {
            ...
            foreach ($order_list_by_stack as $stack) {
                ...
                $this->get_evaluation_by_stack($stack, $Separ), $evaluations);
            }
        }
        return ($evaluations);
    }
    function get_evaluation_by_stack ($order_list, $Separ = ',')
    {
        if (count($order_list) > 0) {
            ...
            if ($result === false) {
                fianet_insert_log(
                "fianet_sender.php - get_evaluation() <br />\nError when opening file : <br />\n" .
                 $url);
            } else {
                $xmldata = $result['data'];
	...
	*/
    ...
    function process_result_stacking ($xml_data)
    {
        ...
        if (isset($xml_array['validstack']['unluck'])) {
            fianet_insert_log(
            "fianet_sender.php - process_result_stacking() <br />\nError : <br />\n" .
    ...
    function send_fsock_singet ($data)
    {
        ...
        if ($res === false) {
            fianet_insert_log(
            "fianet_sender.php - send_fsock_post() <br />\nError when connecting : <br />\n" .
             $url_action . $path);
            return (false);
        }
        return (true);
    ...
    function get_reevaluated_order ()
    {
        ...
        if ($res === false) {
            fianet_insert_log(
            "fianet_sender.php - get_reevaluated_order() <br />\nError when opening file : <br />\n" .
             $url);
        } else {
            $xmldata = $res['data'];
