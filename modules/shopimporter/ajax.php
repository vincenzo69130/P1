...
if (! Tools::getValue('ajax') ||
 Tools::getValue('token') != sha1(_COOKIE_KEY_ . 'ajaxShopImporter') ||
 (! empty($moduleName) && ! ctype_alnum($moduleName)))
    die();
...
if (Tools::isSubmit('checkAndSaveConfig')) {
    ...
    foreach ($shopImporter->supportedImports as $key => $import)
        if (array_key_exists('alterTable', $import))
            ...
            'SHOW COLUMNS FROM `' . _DB_PREFIX_ . bqSQL($import['table']) . '`');
    foreach ($columns as $column)
        if ($column['Field'] == $import['identifier'] . '_' . $moduleName)
            Db::getInstance()->Execute(
            'ALTER IGNORE TABLE `' . _DB_PREFIX_ . bqSQL($import['table']) .
             '` DROP `' . bqSQL($import['identifier'] . '_' . $moduleName) . '`');
    if ($link = @mysql_connect(Tools::getValue('server'), 
    Tools::getValue('user'), Tools::getValue('password'))) {
        if (! @mysql_select_db(Tools::getValue('database'), $link))
            die(
            '{"hasError" : true, "error" : ["' .
             Tools::displayError('The database selection cannot be made.') .
             '"]}');
        else {
            @mysql_close($link);
        ...
        die(
        '{"hasError" : true, "error" : ["' .
         Tools::displayError('Link to database cannot be established.') . '"]}');
}
if (Tools::isSubmit('getData') || Tools::isSubmit('syncLang') ||
 Tools::isSubmit('syncCurrency')) {
    ...
    if (file_exists('../../modules/' . $moduleName . '/' . $moduleName . '.php')) {
        ...
        if (! method_exists($importModule, $getMethod))
            ...
            $return = call_user_func_array(array($importModule, $getMethod), 
            array($limit, $nbr_import));
            $shopImporter = new shopImporter();
            $shopImporter->genericImport($className, $return, (bool) $save);
        }
    }
}
if (Tools::isSubmit('getDataWS') || Tools::isSubmit('syncLangWS') ||
 Tools::isSubmit('syncCurrencyWS')) {
    ...
    if (file_exists('../../modules/' . $moduleName . '/' . $moduleName . '.php')) {
        ...
        try {
            ...
            if (! method_exists($importModule, $getMethod))
                die(
                '{"hasError" : true, "error" : ["not_exist"], "datas" : []}');
            else {
                $return = call_user_func_array(array($importModule, $getMethod), 
                array($limit, $nbr_import));
                $shopImporter = new shopImporter();
            ...
            die(
            '{"hasError" : true, "error" : [' . json_encode($e->getMessage()) .
             '], "datas" : []}');
        }
    }
}
