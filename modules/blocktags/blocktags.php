...
class BlockTags extends Module
{
		...
		<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post">
         ...
         'logo.gif" alt="" title="" />' . $this->l('Settings') . '</legend>
				...
				<div class="margin-form">
					<input type="text" name="tagsNbr" value="' .
         (int) (Configuration::get('BLOCKTAGS_NBR')) . '" />
					<p class="clear">' .
         $this->l('Set the number of tags to be displayed in this block') . '</p>
				</div>
				<center><input type="submit" name="submitBlockTags" value="' .
    ...
    {
        global $smarty;
        $tags = Tag::getMainTags((int) ($params['cookie']->id_lang), 
        (int) (Configuration::get('BLOCKTAGS_NBR')));
        if (! sizeof($tags))
            return false;
        foreach ($tags as &$tag)
            $tag['class'] = 'tag_level' .
             ($tag['times'] > BLOCKTAGS_MAX_LEVEL ? BLOCKTAGS_MAX_LEVEL : $tag['times']);
