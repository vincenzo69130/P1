...
class ReferralProgramModule extends ObjectModel
{
    ...
    public $date_add;
    public $date_upd;
    protected $fieldsRequired = array('id_sponsor', 'email', 'lastname', 
    'firstname');
    protected $fieldsSize = array('id_sponsor' => 8, 'email' => 255, 
    'lastname' => 128, 'firstname' => 128, 'id_customer' => 8, 
    'id_discount' => 8, 'id_discount_sponsor' => 8);
    protected $fieldsValidate = array('id_sponsor' => 'isUnsignedId', 
    'email' => 'isEmail', 'lastname' => 'isName', 'firstname' => 'isName', 
    'id_customer' => 'isUnsignedId', 'id_discount' => 'isUnsignedId', 
    ...
    public function registerDiscountForSponsor ($id_currency)
    {
        if ((int) $this->id_discount_sponsor > 0)
            return false;
        return $this->registerDiscount((int) $this->id_sponsor, 'sponsor', 
        (int) $id_currency);
    }
    public function registerDiscountForSponsored ($id_currency)
    {
        if (! (int) $this->id_customer or (int) $this->id_discount > 0)
            return false;
        return $this->registerDiscount((int) $this->id_customer, 'sponsored', 
        (int) $id_currency);
    }
    ...
    $id_currency = 0)
    {
        ...
        array('REFERRAL_DISCOUNT_TYPE', 'REFERRAL_PERCENTAGE', 
        ...
        if ($configurations['REFERRAL_DISCOUNT_TYPE'] == 1)
            ...
            /* Fixed amount */
        elseif ($configurations['REFERRAL_DISCOUNT_TYPE'] == 2 and
         isset($configurations['REFERRAL_DISCOUNT_VALUE_' . (int) ($id_currency)]))
            $discount->value = (float) $configurations['REFERRAL_DISCOUNT_VALUE_' .
             (int) ($id_currency)];
        ...
        $discount->name = $this->getDiscountPrefix() .
         Tools::passwdGen(6);
        $discount->description = Configuration::getInt(
     ...
     */
    public static function isEmailExists ($email, $getId = false, 
    $checkCustomer = true)
    {
