...
if (! preg_match("#.*\.html$#Ui", Tools::getValue('mail')) or
 ! preg_match("#.*\.html$#Ui", Tools::getValue('mail')))
    die(Tools::displayError());
$file = file_get_contents(
dirname(__FILE__) . '/mails/' .
 strval(preg_replace('#\.{2,}#', '.', Tools::getValue('mail'))));
...
$file = str_replace('{discount}', 
Discount::display(
(float) (Configuration::get('REFERRAL_DISCOUNT_VALUE_' . $cookie->id_currency)), 
(int) (Configuration::get('REFERRAL_DISCOUNT_TYPE')), 
new Currency($cookie->id_currency)), $file);
echo $file;
