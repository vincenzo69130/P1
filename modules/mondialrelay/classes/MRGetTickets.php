...
class MRGetTickets implements IMondialRelayWSMethod
{
    ...
    'Expeditions' => array('required' => true, 'value' => '', 
    'regexValidation' => '#^[0-9]{8}(;[0-9]{8})*$#'), 
    'Langue' => array('required' => true, 'value' => '', 
    ...
    public function init ()
    {
        ...
        $this->_fields['list']['Langue']['value'] = Configuration::get(
        'MR_LANGUAGE');
        ...
        foreach ($this->_fieldsList as &$rootCase) {
            ...
            foreach ($rootCase['list'] as $paramName => &$valueDetailed)
                if ($paramName != 'Texte' && $paramName != 'Security') {
                    $valueDetailed['value'] = strtoupper(
                    $valueDetailed['value']);
                    if (preg_match($valueDetailed['regexValidation'], 
                    ...
                    elseif ((! strlen($valueDetailed['value']) &&
                     ...
                     strlen($valueDetailed['value'])) {
                        ...
                        'hasn\'t a valide value format') . ' : ' .
                         $valueDetailed['value'];
                        $id_order = $this->_getOrderIdWithExpeditionNumber(
                        $rootCase['list']['Expeditions']['value']);
	 ...
	 */
    private function _updateTable ($id_order, $expeditionNumber, $URLA4, $URLA5, 
    &$success)
    {
        $query = '
    ...
    private function _parseResult ($client, $result, $params)
    {
        ...
        if (($errorNumber = $result['STAT']) != 0) {
            ...
            $this->_updateTable($id_order, $params['Expeditions'], $URLPDF_A4, 
            $URLPDF_A5, $success);
        }
    ...
    public function send ()
    {
        if ($client = new nusoap_client($this->_webserviceURL, true)) {
            ...
            foreach ($this->_fieldsList as $rootCase) {
                ...
                $result = $client->call('WSI2_GetEtiquettes', $params, 
