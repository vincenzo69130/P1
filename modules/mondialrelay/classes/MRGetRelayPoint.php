...
class MRGetRelayPoint implements IMondialRelayWSMethod
{
    ...
    public function init ()
    {
        ...
        if (! $address)
            throw new Exception(
            $this->_mondialrelay->l('Customer address can\'t be found'));
        $this->_fields['list']['Enseigne']['value'] = Configuration::get(
        'MR_ENSEIGNE_WEBSERVICE');
        $this->_fields['list']['Poids']['value'] = Configuration::get(
        'MR_WEIGHT_COEF') * $this->_weight;
        $this->_fields['list']['Pays']['value'] = Country::getIsoById(
        $address->id_country);
                    ...
                    $valueDetailed['value'], $matches))
                        ...
                        if ((! strlen($valueDetailed['value']) &&
                         ...
                         strlen($valueDetailed['value'])) {
                            ...
                            'hasn\'t a valide value format') . ' : ' .
                             $valueDetailed['value'];
                            $this->_resultList['error'][] = $error;
                        }
            }
        $concatenationValue .= $this->_webServiceKey;
        $this->_fields['list']['Security']['value'] = strtoupper(
	...
	*/
    private function _getRelayPointDetails ($relayPointList)
    {
        ...
        $MRRelayDetail = new MRRelayDetail(
        array('relayPointNumList' => $relayPointNumList, 
	...
	*/
    private function _addLinkHoursDetail (&$relayPointList)
    {
        ...
        foreach ($relayPointList as $num => $relayPoint)
            $relayPointNumList[] = $relayPoint['Num'];
        $permaList = MRRelayDetail::getPermaLink($relayPointNumList, 
        $this->_id_address_delivery);
        foreach ($relayPointList as $num => &$relayPoint) {
    ...
    private function _parseResult ($client, $result, $params)
    {
        ...
        $success = array();
        if ($client->fault)
            $errors[$errorTotal ++] = $this->_mondialRelay->l(
        ...
        if (($errorNumber = $result['STAT']) != 0) {
            ...
            if (! count($result))
                ...
                'MondialRelay can\'t find any relay point near your address. Maybe your address isn\'t properly filled ?');
            else {
	...
	*/
    public function send ()
    {
        if ($client = new nusoap_client($this->_webserviceURL, true)) {
            ...
            $result = $client->call('WSI2_RecherchePointRelais', $params, 
