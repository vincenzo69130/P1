...
MondialRelay::getToken('front') => array('MRGetRelayPoint', 
'addSelectedCarrierToDB'), 
MondialRelay::getToken('back') => array('MRGetTickets', 'MRCreateTickets', 
'MRDeleteHistory', 'uninstallDetail', 'DeleteHistory'));
...
if (! isset($accessPageList[$token]) ||
 ! in_array($method, $accessPageList[$token]))
    exit();
...
switch ($method) {
    case 'MRCreateTickets':
        ...
        break;
    case 'MRGetTickets':
        $params['detailedExpeditionList'] = Tools::getValue(
        'detailedExpeditionList');
        break;
    case 'DeleteHistory':
...
try {
    ...
    echo MondialRelay::jsonEncode(
    array('other' => array('error' => array($e->getMessage()))));
    exit(- 1);
}
