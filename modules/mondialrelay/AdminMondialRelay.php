...
class AdminMondialRelay extends AdminTab
{
    ...
    private function displayOrdersTable ()
    {
        ...
        if (empty($orders))
            $html .= '<h3 style="color:red;">' .
             $this->l('No orders with this state.') . '</h3>';
        else {
            ...
            foreach ($orders as $order) {
                if ($order['weight'] == 0) {
                    ...
                    $order['weight'] = round(
                    $mr_weight_coef * $result_weight['weight']);
                }
                $html .= '<tr id="PS_MRLineOrderInformation-' .
                 $order['id_order'] . '">';
                $html .= '<td><input type="checkbox" class="order_id_list" name="order_id_list[]" id="order_id_list" value="' .
                 $order['id_order'] . '" /></td>';
                $html .= '<td>' . $order['id_order'] . '</td>';
                $html .= '<td>' . $order['customer'] . '</td>';
                $html .= '<td>' .
                 Tools::displayPrice($order['total'], 
                new Currency($order['id_currency'])) . '</td>';
                $html .= '<td>' .
                 Tools::displayPrice($order['shipping'], 
                new Currency($order['id_currency'])) . '</td>';
                $html .= '<td>' .
                 Tools::displayDate($order['date'], $order['id_lang']) . '</td>';
                $html .= '<td><input type="text" name="weight_' .
                 $order['id_order'] . '" id="weight_' . $order['id_order'] .
                 '" size="7" value="' . $order['weight'] . '" /></td>';
                ...
                $html .= '
                 ...
                 $order['id_order'] . '&vieworder&token=' .
                 Tools::getAdminToken(
                'AdminOrders' . (int) (Tab::getIdFromClassName('AdminOrders')) .
                 (int) ($cookie->id_employee)) .
                 '">
						<img border="0" title="' .
                 $this->l('View') . '" alt="' . $this->l('View') . '" src="' .
                 _PS_IMG_ .
                 'admin/details.gif"/></a>
					</td>
					</tr>
					<tr class="PS_MRErrorList error" id="errorCreatingTicket_' .
                 $order['id_order'] .
                 '" style="display:none;">
						<td colspan="11" style="background:url(' .
                 _PS_IMG_ .
                 'admin/error2.png) 10px 10px no-repeat;">
							<span></span>
						</td>
					</tr>
					<tr class="PS_MRSuccessList" id="successCreatingTicket_' .
                 $order['id_order'] . '" style="display:none;">
						<td>' . $order['id_order'] .
                 '</td>
						<td colspan="10" style="background:url(' .
                 _PS_IMG_ . 'admin/ok2.png) 10px 5px no-repeat #DFFAD3;">
            ...
            $html .= '
				<div class="submit_button">
					<div class="PS_MRSubmitButton" id="PS_MRSubmitButtonGenerateTicket">
						<input type="button" name="generate" id="generate" value="' .
             $this->l('Generate') .
             '" class="button" />
					</div>
					<div class="PS_MRLoader" id="PS_MRSubmitGenerateLoader"><img src="' .
             MondialRelay::$moduleURL . 'images/getTickets.gif"</div>
				</div>';
            $html .= '</form>';
        }
        unset($mondialrelay);
    ...
    public function displayhistoriqueForm ()
    {
        ...
        $query = "SELECT * FROM `" . _DB_PREFIX_ .
         "mr_historique` ORDER BY `id` DESC ;";
							...
							<tbody>
								<tr>
									<th><input type="checkbox" id="toggleStatusHistoryList" /></th>
			 						...
			 						<th>' .
         $this->l('Print stick A4') . '</th>
			 						<th>' .
         $this->l('Print stick A5') . '</th>
			 					</tr>';
        foreach ($query as $k => $row) {
            $_html .= '
             ...
             '">
					<td>
						<input type="checkbox" id="PS_MRHistoryId_' .
             $row['id'] .
             '" class="history_id_list" name="history_id_list[]" value="' .
             $row['id'] . '" />
					</td>
					<td>' . $row['order'] . '</td>
					<td id="expeditionNumber_' .
             $row['order'] . '">' . $row['exp'] . '</td>
					<td id="URLA4_' . $row['order'] .
             '">
						<a href="' .
             $row['url_a4'] . '" target="a4"><img width="20" src="' .
             MondialRelay::$moduleURL . 'images/pdf_icon.jpg" /></a>
					</td>
					<td id="URLA5_' . $row['order'] .
             '">
						<a href="' .
             $row['url_a5'] . '" target="a5"><img width="20" src="' .
             MondialRelay::$moduleURL . 'images/pdf_icon.jpg" /></a>
					</td>
				</tr>';
        }
        $_html .= '
				...
				<div class="PS_MRSubmitButton">
					<input type="button" id="PS_MRSubmitButtonDeleteHistories" name="deleteSelectedHistories" value="' .
         $this->l('Delete selected history') . '" class="button" />
					<div class="PS_MRLoader" id="PS_MRSubmitDeleteHistoriesLoader">
						<img src="' .
         MondialRelay::$moduleURL . 'images/getTickets.gif"
					</div>
				</div>
			</form></div></fieldset>';
        return $_html;
    ...
    public function display ()
    {
        ...
        $html .= '
			<script type="text/javascript" language="javascript">
				var _PS_MR_MODULE_DIR_ = "' .
         MondialRelay::$moduleURL . '";
			</script>';
