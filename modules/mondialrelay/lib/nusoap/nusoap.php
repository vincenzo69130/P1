...
class nusoap_base
{
     ...
     */
    var $typemap = array(
    'http://www.w3.org/2001/XMLSchema' => array('string' => 'string', 
    'boolean' => 'boolean', 'float' => 'double', 'double' => 'double', 
    'decimal' => 'double', 'duration' => '', 'dateTime' => 'string', 
    'time' => 'string', 'date' => 'string', 'gYearMonth' => '', 'gYear' => '', 
    'gMonthDay' => '', 'gDay' => '', 'gMonth' => '', 'hexBinary' => 'string', 
    'base64Binary' => 'string', // abstract "any" types
    'anyType' => 'string', 
    'anySimpleType' => 'string', // derived datatypes
    'normalizedString' => 'string', 
    'token' => 'string', 'language' => '', 'NMTOKEN' => '', 'NMTOKENS' => '', 
    'Name' => '', 'NCName' => '', 'ID' => '', 'IDREF' => '', 'IDREFS' => '', 
    'ENTITY' => '', 'ENTITIES' => '', 'integer' => 'integer', 
    'nonPositiveInteger' => 'integer', 'negativeInteger' => 'integer', 
    'long' => 'integer', 'int' => 'integer', 'short' => 'integer', 
    'byte' => 'integer', 'nonNegativeInteger' => 'integer', 'unsignedLong' => '', 
    'unsignedInt' => '', 'unsignedShort' => '', 'unsignedByte' => '', 
    'positiveInteger' => ''), 
    'http://www.w3.org/2000/10/XMLSchema' => array('i4' => '', 'int' => 'integer', 
    'boolean' => 'boolean', 'string' => 'string', 'double' => 'double', 
    'float' => 'double', 'dateTime' => 'string', 'timeInstant' => 'string', 
    'base64Binary' => 'string', 'base64' => 'string', 'ur-type' => 'array'), 
     ...
     * @see	expandEntities
     */
    var $xmlEntities = array('quot' => '"', 'amp' => '&', 'lt' => '<', 
    'gt' => '>', 'apos' => "'");
    /**
     * constructor
     *
                ...
                if (get_class($val) == 'soapval') {
                    ...
                    foreach (get_object_vars($val) as $k => $v) {
                        $pXml = isset($pXml) ? $pXml .
                         $this->serialize_val($v, $k, false, false, false, 
                        false, $use) : $this->serialize_val($v, $k, false, false, 
                        false, false, $use);
                    }
                }
                if (isset($type) && isset($type_prefix)) {
                ...
                $type)) {
                    ...
                    if (is_array($val) && count($val) > 0) {
                        foreach ($val as $v) {
                            ...
                            $v, 'item', false, false, false, false, $use);
                            ++ $i;
                        }
    ...
    $use = 'encoded', $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/')
    {
        ...
        return '<?xml version="1.0" encoding="' . $this->soap_defencoding .
         '"?' . ">" . '<SOAP-ENV:Envelope' . $ns_string . ">" . $headers .
...
function timestamp_to_iso8601 ($timestamp, $utc = true)
{
    ...
    if ($pos !== FALSE) {
        if (strlen($datestr) == $pos + 5) {
            $datestr = substr($datestr, 0, $pos + 3) . ':' . substr($datestr, 
            - 2);
        }
    }
    if ($utc) {
...
class nusoap_fault extends nusoap_base
{
     ...
     */
    function nusoap_fault ($faultcode, $faultactor = '', $faultstring = '', 
    $faultdetail = '')
    {
        parent::nusoap_base();
    ...
    function serialize ()
    {
        ...
        $return_msg = '<?xml version="1.0" encoding="' . $this->soap_defencoding .
         ...
         $this->serialize_val($this->faultdetail, 'detail') . '</SOAP-ENV:Fault>' .
...
class nusoap_xmlschema extends nusoap_base
{
        ...
        switch ($name) {
            ...
            case 'complexType':
                ...
                if (isset($attrs['name'])) {
                    ...
                    //            </complexType>
                    if (isset(
                    $attrs['base']) && preg_match('/:Array$/', $attrs['base'])) {
            ...
            case 'element':
                ...
                if (isset($attrs['type'])) {
                    $this->xdebug(
                    "processing typed element " . $attrs['name'] . " of type " .
                     $attrs['type']);
                    if (! $this->getPrefix($attrs['type'])) {
                        if ($this->defaultNamespace[$pos]) {
                            $attrs['type'] = $this->defaultNamespace[$pos] . ':' .
                             $attrs['type'];
                            $this->xdebug(
                    ...
                    $this->xdebug(
                    "processing untyped element " . $attrs['name'] . ' type ' .
                     $type);
                    $this->currentElement = $attrs['name'];
            ...
            case 'import':
                if (isset($attrs['schemaLocation'])) {
                    $this->xdebug(
                    'import namespace ' . $attrs['namespace'] . ' from ' .
            ...
            case 'schema':
                ...
                $this->schemaInfo['schemaVersion'] = $this->getNamespaceFromPrefix(
                $prefix);
    ...
    function serializeSchema ()
    {
        ...
        if (sizeof($this->imports) > 0) {
            foreach ($this->imports as $ns => $list) {
                foreach ($list as $ii) {
                    if ($ii['location'] != '') {
                        $xml .= " <$schemaPrefix:import location=\"" .
        ...
        if ($typeDef = $this->getTypeDef($type)) {
            ...
            if ($typeDef['phpType'] == 'struct') {
                ...
                for ($i = 0; $i < 3; $i ++) {
					...
					<td><input type='text' name='parameters[" . $name .
                     "][]'></td></tr>";
                }
                $buffer .= '</table>';
                 // if scalar
    ...
    {
        $this->complexTypes[$name] = array('name' => $name, 
        'typeClass' => $typeClass, 'phpType' => $phpType, 
        'compositor' => $compositor, 'restrictionBase' => $restrictionBase, 
    ...
    {
        $this->simpleTypes[$name] = array('name' => $name, 
        'typeClass' => $typeClass, 'phpType' => $phpType, 
        'type' => $restrictionBase, 'enumeration' => $enumeration);
...
class soap_transport_http extends nusoap_base
{
    ...
    function connect ($connection_timeout = 0, $response_timeout = 30)
    {
        ...
        if ($this->io_method() == 'socket') {
            ...
            $this->debug(
            'calling fsockopen with host ' . $host . ' connection_timeout ' .
             $connection_timeout);
            // open socket
    ...
    $digestRequest = array(), $certRequest = array())
    {
        ...
        if ($authtype == 'basic') {
            ...
            if (isset($digestRequest['nonce'])) {
                ...
                $A1 = $username . ':' .
                 (isset($digestRequest['realm']) ? $digestRequest['realm'] : '') .
                 ':' . $password;
                // H(A1) = MD5(A1)
                $HA1 = md5($A1);
                ...
                if ($digestRequest['qop'] != '') {
                    $unhashedDigest = $HA1 . ':' . $nonce . ':' .
                     sprintf("%08d", $digestRequest['nc']) . ':' . $cnonce . ':' .
                ...
                'Digest username="' . $username . '", realm="' .
                 $digestRequest['realm'] . '", nonce="' . $nonce . '", uri="' .
                 $this->digest_uri . $opaque . '", cnonce="' . $cnonce . '", nc=' .
                 sprintf("%08x", $digestRequest['nc']) . ', qop="' .
    ...
    {
        if ($proxyhost) {
            $this->proxy = array('host' => $proxyhost, 'port' => $proxyport, 
    ...
    {
        $skipHeaders = array('HTTP/1.1 100', 'HTTP/1.0 301', 'HTTP/1.1 301', 
        'HTTP/1.0 302', 'HTTP/1.1 302', 'HTTP/1.0 401', 'HTTP/1.1 401', 
            ...
            if (strtolower($this->incoming_headers['content-encoding']) ==
             ...
             strtolower($this->incoming_headers['content-encoding']) == 'gzip') {
                ...
                if (function_exists('gzinflate')) {
                    ...
                    } elseif ($this->incoming_headers['content-encoding'] ==
                     'gzip') {
    ...
    function getCookiesForRequest ($cookies, $secure = false)
    {
        ...
        if ((! is_null($cookies)) && (is_array($cookies))) {
            foreach ($cookies as $cookie) {
                ...
                $this->debug(
                "check cookie for validity: " . $cookie['name'] . '=' .
                 $cookie['value']);
                ...
                $this->debug(
                'add cookie to Cookie-String: ' . $cookie['name'] . '=' .
                 $cookie['value']);
            }
        }
        return $cookie_str;
...
class nusoap_server extends nusoap_base
{
    ...
    {
        $this->debug(
        'in invoke_method, methodname=' . $this->methodname . ' methodURI=' .
        ...
        if ($this->wsdl) {
            ...
            } elseif ($this->opData = $this->wsdl->getOperationDataForSoapAction(
            $this->SOAPAction)) {
        ...
        if ($class == '') {
            ...
            $method_to_compare = (substr(phpversion(), 0, 2) == '4.') ? strtolower(
            $method) : $method;
        ...
        if (! $this->verify_method($this->methodname, 
        $this->methodparams)) {
            // debug
            $this->debug(
        ...
        if (! function_exists('call_user_func_array')) {
            if ($class == '') {
                ...
                if ($delim == '..') {
                    ...
                    $funcCall = "\$this->methodreturn = " . $class . "::" .
                     $method . "(";
                } else {
                    $this->debug(
                    ...
                    $funcCall .= "\$this->methodreturn = " . $instname . "->" .
                     $method . "(";
                }
            }
    ...
    function serialize_return ()
    {
        ...
        if (isset($this->methodreturn) && is_object($this->methodreturn) &&
         ...
         (get_class($this->methodreturn) == 'nusoap_fault'))) {
            ...
            if ($this->wsdl) {
                ...
                $this->methodname, 'output', $opParams);
                $this->appendDebug($this->wsdl->getDebug());
                $this->wsdl->clearDebug();
        ...
        if ($this->wsdl) {
            $this->debug(
            'have WSDL for serialization: style is ' . $this->opData['style']);
            ...
            $payload = '<ns1:' . $this->methodname . 'Response xmlns:ns1="' .
             $this->methodURI . '">' . $return_val . '</ns1:' . $this->methodname .
             "Response>";
        }
    ...
    function send_response ()
    {
        ...
        preg_match('/\$Revisio' . 'n: ([^ ]+)/', $this->revision, $rev);
        $this->outgoing_headers[] = "X-SOAP-Server: $this->title/$this->version (" .
         $rev[1] . ")";
        ...
        if (strlen($payload) > 1024 &&
         isset($this->headers) && isset($this->headers['accept-encoding'])) {
        ...
        //end code
        $this->outgoing_headers[] = "Content-Length: " . strlen(
        $payload);
    ...
    function parseRequest ($headers, $data)
    {
        ...
        $this->debug(
        'Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
        ...
        if ($err = $parser->getError()) {
            ...
            $this->debug(
            'methodname: ' . $this->methodname . ' methodURI: ' .
             $this->methodURI);
     ...
     * @access   public
     */
    function register ($name, $in = array(), $out = array(), $namespace = false, $soapaction = false, 
    $style = false, $use = false, $documentation = '', $encodingStyle = '')
    {
        ...
        if (false == $soapaction) {
            if (isset($_SERVER)) {
                ...
                $HTTPS = isset($_SERVER['HTTPS']) ? $_SERVER['HTTPS'] : (isset(
                $HTTP_SERVER_VARS['HTTPS']) ? $HTTP_SERVER_VARS['HTTPS'] : 'off');
            } elseif (isset($HTTP_SERVER_VARS)) {
        ...
        $this->operations[$name] = array('name' => $name, 'in' => $in, 
        'out' => $out, 'namespace' => $namespace, 'soapaction' => $soapaction, 
        'style' => $style);
        if ($this->wsdl) {
    ...
    $style = 'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
        global $HTTP_SERVER_VARS;
        if (isset($_SERVER)) {
            $SERVER_NAME = $_SERVER['SERVER_NAME'];
        ...
        'name' => $serviceName . 'Binding', 'style' => $style, 
        'transport' => $transport, 'portType' => $serviceName . 'PortType');
        $this->wsdl->ports[$serviceName . 'Port'] = array(
        'binding' => $serviceName . 'Binding', 'location' => $endpoint, 
...
class wsdl extends nusoap_base
{
     ...
     * @access public 
     */
    function wsdl ($wsdl = '', $proxyhost = false, $proxyport = false, $proxyusername = false, 
    $proxypassword = false, $timeout = 0, $response_timeout = 30, $curl_options = null, 
    $use_curl = false)
    {
        parent::nusoap_base();
        $this->debug(
            ...
            if (isset($bindingData['operations']) &&
             is_array($bindingData['operations'])) {
                foreach ($bindingData['operations'] as $operation => $data) {
                    ...
                    $this->bindings[$binding]['operations'][$operation]['input'] = isset(
                    $this->bindings[$binding]['operations'][$operation]['input']) ? array_merge(
                    $this->bindings[$binding]['operations'][$operation]['input'], 
                    $this->portTypes[$bindingData['portType']][$operation]['input']) : $this->portTypes[$bindingData['portType']][$operation]['input'];
                    $this->bindings[$binding]['operations'][$operation]['output'] = isset(
                    $this->bindings[$binding]['operations'][$operation]['output']) ? array_merge(
                    ...
                    $this->bindings[$binding]['operations'][$operation]['transport'] = isset(
                    $bindingData['transport']) ? $bindingData['transport'] : '';
                    $this->bindings[$binding]['operations'][$operation]['documentation'] = isset(
                    $this->portTypes[$bindingData['portType']][$operation]['documentation']) ? $this->portTypes[$bindingData['portType']][$operation]['documentation'] : '';
                    $this->bindings[$binding]['operations'][$operation]['endpoint'] = isset(
                    $bindingData['endpoint']) ? $bindingData['endpoint'] : '';
                }
            }
        }
    }
    ...
    function parseWSDL ($wsdl = '')
    {
        ...
        if (isset($wsdl_props['scheme']) &&
         ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
            ...
            if (isset($wsdl_props['scheme']) &&
             ($wsdl_props['scheme'] == 'file') && isset($wsdl_props['path'])) {
                    ...
                    switch ($name) {
                        ...
                        case 'operation':
                            ...
                            if (isset($attrs['name'])) {
                                ...
                                $this->bindings[$this->currentBinding]['operations'][$this->currentOperation]['endpoint'] = isset(
                                $this->bindings[$this->currentBinding]['endpoint']) ? $this->bindings[$this->currentBinding]['endpoint'] : '';
                            }
                            break;
                        case 'input':
                            $this->opStatus = 'input';
                ...
                case 'service':
                    switch ($name) {
                        case 'port':
                            ...
                            $this->ports[$this->currentPort]['binding'] = $this->getLocalPart(
                            $attrs['binding']);
                            break;
                        case 'address':
            ...
            switch ($name) {
                case 'import':
                    if (isset($attrs['location'])) {
                        ...
                        $this->debug(
                        'parsing import ' . $attrs['namespace'] . ' - ' .
                         $attrs['location'] . ' (' .
                ...
                case "binding":
                    if (isset($attrs['name'])) {
                        ...
                        $this->status = 'binding';
                        $this->bindings[$this->currentBinding]['portType'] = $this->getLocalPart(
                        $attrs['type']);
                        $this->debug(
    ...
    function getOperationData ($operation, $bindingType = 'soap')
    {
        ...
        foreach ($this->ports as $port => $portData) {
            ...
            if ($portData['bindingType'] == $bindingType) {
                ...
                foreach (array_keys(
                $this->bindings[$portData['binding']]['operations']) as $bOperation) {
    ...
    function webDescription ()
    {
		...
		<div class=content>
			...
			<div class=nav>
				<p>View the <a href="' .
         $PHP_SELF . '?wsdl">WSDL</a> for the service.
        ...
        foreach ($this->getOperations() as $op => $data) {
            ...
            foreach ($data as $donnie => $marie) { // loop through opdata
                if ($donnie == 'input' || $donnie == 'output') { // show input/output data
                    ...
                    $b .= "<font color='white'>" . ucfirst($donnie) .
                     ":</font> $marie<br>";
                }
            }
            $b .= '</div>';
    ...
    function serialize ($debug = 0)
    {
        ...
        if (sizeof($this->import) > 0) {
            foreach ($this->import as $ns => $list) {
                foreach ($list as $ii) {
                    if ($ii['location'] != '') {
                        $xml .= '<import location="' . $ii['location'] .
                         '" namespace="' . $ns . '" />';
                    } else {
            ...
            foreach ($this->messages as $msgName => $msgParts) {
                ...
                if (is_array($msgParts)) {
                    foreach ($msgParts as $partName => $partType) {
                        ...
                        $this->typemap[$this->namespaces['xsd']][$partType])) {
                        ...
                        $xml .= "\n" . '  <part name="' . $partName . '" ' .
                         $elementortype . '="' . $typePrefix . ':' . $localPart .
                         '" />';
                    }
                }
                $xml .= '</message>';
            ...
            foreach ($this->bindings as $bindingName => $attrs) {
                ...
                foreach ($attrs['operations'] as $opName => $opParts) {
                    ...
                    $binding_xml .= "\n" . '    <soap:operation soapAction="' .
                     $opParts['soapAction'] . '" style="' . $opParts['style'] .
                     '"/>';
                    ...
                    $binding_xml .= "\n" . '    <input><soap:body use="' .
                     $opParts['input']['use'] . '" namespace="' .
                     $opParts['input']['namespace'] . '"' . $enc_style .
                     '/></input>';
                    ...
                    $binding_xml .= "\n" . '    <output><soap:body use="' .
                     $opParts['output']['use'] . '" namespace="' .
                     $opParts['output']['namespace'] . '"' . $enc_style .
                     '/></output>';
                    $binding_xml .= "\n" . '  </operation>';
                    $portType_xml .= "\n" . '  <operation name="' .
                     $opParts['name'] . '"';
            ...
            if ($ns == $this->XMLSchemaVersion ||
             $ns == 'http://schemas.xmlsoap.org/soap/encoding/') {
                ...
                if ($uqType == 'Array') {
                    ...
                    return $this->serialize_val($value, 
                    $name, false, false, false, false, $use);
                }
                if ($uqType == 'boolean') {
            ...
            if (is_array($value)) {
                ...
                if (isset($typeDef['simpleContent']) &&
                 $typeDef['simpleContent'] == 'true') {
                    ...
                    $typeDef, $value, $ns, $uqType, $use, $encodingStyle);
                }
                $xml .= "</$elementName>";
            } else {
                $this->debug(
            ...
            if (is_array($value) && sizeof($value) >= 1) {
                ...
                foreach ($value as $k => $v) {
                    ...
                    $this->typemap['http://www.w3.org/2001/XMLSchema'])) {
                        ...
                        $typeDef['arrayType'], null, $this->XMLSchemaVersion, 
                        false, $use);
                    }
                }
            } else {
        ...
        if (isset($typeDef['attrs']) && is_array($typeDef['attrs'])) {
            ...
            foreach ($typeDef['attrs'] as $aName => $attrs) {
                if (isset($xvalue['!' . $aName])) {
                    ...
                    $this->debug(
                    'use default value of ' . $xvalue[$aName] . ' for attribute ' .
                     $aName);
                } else {
                    $xname = '';
                 ...
                 ((! isset($attrs['nillable'])) || $attrs['nillable'] != 'true')) {
                    ...
                    if (isset($attrs['maxOccurs']) &&
                     ...
                     $this->isArraySimpleOrStruct($v) == 'arraySimple') {
                        ...
                        if (is_null($v) && isset($attrs['minOccurs']) &&
                         $attrs['minOccurs'] == '0') {
                            // do nothing
                        } elseif (is_null($v) &&
    ...
    $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle = '')
    {
        if ($use == 'encoded' && $encodingStyle == '') {
            $encodingStyle = 'http://schemas.xmlsoap.org/soap/encoding/';
        }
        if ($style == 'document') {
            ...
            'struct', 'all', '', $elements);
            $this->addElement(
            array('name' => $name . 'Response', 
            'type' => $name . 'ResponseType', 'form' => 'qualified'));
            $out = array('parameters' => 'tns:' . $name . 'Response' . '^');
        }
        // get binding
        $this->bindings[$this->serviceName . 'Binding']['operations'][$name] = array(
        'name' => $name, 'binding' => $this->serviceName . 'Binding', 
        'endpoint' => $this->endpoint, 'soapAction' => $soapaction, 
        'style' => $style, 
        'input' => array('use' => $use, 'namespace' => $namespace, 
        'encodingStyle' => $encodingStyle, 'message' => $name . 'Request', 
        'parts' => $in), 
        'output' => array('use' => $use, 'namespace' => $namespace, 
        'encodingStyle' => $encodingStyle, 'message' => $name . 'Response', 
        'parts' => $out), 'namespace' => $namespace, 
...
class nusoap_parser extends nusoap_base
{
    ...
    function nusoap_parser ($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true)
    {
        ...
        if (! empty($xml)) {
            ...
            if ($pos_xml !== FALSE) {
                ...
                if (preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, 
                $res)) {
                    $xml_encoding = $res[1];
            ...
            $this->debug(
            'Entering nusoap_parser(), length=' . strlen($xml) . ', encoding=' .
             $encoding);
    ...
    function start_element ($parser, $name, $attrs)
    {
        ...
        $pos = $this->position ++;
        // and set mine
        $this->message[$pos] = array('pos' => $pos, 'children' => '', 
        'cdata' => '');
        ...
        foreach ($attrs as $key => $value) {
            ...
            if ($key_prefix == 'xmlns') {
                ...
                $value)) {
                    ...
                    $this->namespaces['xsi'] = $this->XMLSchemaVersion .
                     '-instance';
                }
                ...
                if (isset($this->message[$pos]['type']) &&
                 $this->message[$pos]['type'] == 'array') {
                    ...
                    if (isset($this->namespaces[$value_prefix])) {
                        ...
                        $this->message[$pos]['type_namespace'] = $attrs['xmlns:' .
                         $value_prefix];
                    }
				...
				*/
                ...
                if (preg_match($expr, $value, $regs)) {
                    ...
                    if (isset($this->namespaces[$regs[1]])) {
                        ...
                        $this->message[$pos]['arrayTypeNamespace'] = $attrs['xmlns:' .
                         $regs[1]];
                    }
    ...
    function buildVal ($pos)
    {
        ...
        $this->debug(
        'in buildVal() for ' . $this->message[$pos]['name'] .
                    ...
                    if ($notstruct) {
                        ...
                        if (isset($params[$this->message[$child_pos]['name']])) {
                            ...
                            $params[$this->message[$child_pos]['name']])) ||
                             (! isset(
                            $params[$this->message[$child_pos]['name']][0]))) {
...
class nusoap_client extends nusoap_base
{
     ...
     * @access   public
     */
    function nusoap_client ($endpoint, $wsdl = false, $proxyhost = false, 
    $proxyport = false, $proxyusername = false, $proxypassword = false, $timeout = 0, 
    ...
    $headers = false, $rpcParams = null, $style = 'rpc', $use = 'encoded')
    {
        ...
        // serialize envelope
        $soapmsg = $this->serializeEnvelope($payload, 
        $this->requestHeaders, $usedNamespaces, $style, $use, $encodingStyle);
        $this->debug(
        "endpoint=$this->endpoint, soapAction=$soapAction, namespace=$namespace, style=$style, use=$use, encodingStyle=$encodingStyle");
        $this->debug(
        'SOAP message length=' . strlen($soapmsg) . ' contents (max 1000 bytes)=' .
    ...
    function checkWSDL ()
    {
        ...
        $this->portName, 'soap')) {
            ...
            $this->debug(
            'got ' . count($this->operations) . ' operations from wsdl ' .
             $this->wsdlFile . ' for binding type ' . $this->bindingType);
        } elseif ($this->operations = $this->wsdl->getOperations(
        $this->portName, 'soap12')) {
            $this->appendDebug($this->wsdl->getDebug());
            $this->wsdl->clearDebug();
    ...
    function parseResponse ($headers, $data)
    {
        ...
        $this->debug(
        'Use encoding: ' . $this->xml_encoding . ' when creating nusoap_parser');
        $parser = new nusoap_parser($data, $this->xml_encoding, $this->operation, 
        $this->decode_utf8);
