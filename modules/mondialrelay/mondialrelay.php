...
class MondialRelay extends Module
{
    ...
    public static $MRFrontToken = '';
    public static $MRBackToken = '';
    ...
    public function install ()
    {
        ...
        if (! $result) {
            ...
            '
				...
				VALUES(3, "AdminMondialRelay",  "' .
             (int) ($pos) . '", "mondialrelay")');
            ...
            foreach ($languages as $language)
				...
				(id_lang, id_tab, name) 
				VALUES("' .
                 (int) ($language['id_lang']) . '", "' . (int) ($id_tab) .
                 '", "Mondial Relay")');
            ...
            foreach ($profiles as $profile)
				...
				VALUES(' .
                 $profile['id_profile'] . ', ' . (int) ($id_tab) .
                 ', 1, 1, 1, 1)');
            @copy(_PS_MODULE_DIR_ . 'mondialrelay/AdminMondialRelay.gif', 
        ...
        if (! Configuration::get('MONDIAL_RELAY')) {
            ...
            Configuration::updateValue('MONDIAL_RELAY_SECURE_KEY', 
            md5(time() . rand(0, 10)));
            ...
            '
				UPDATE `' .
             _DB_PREFIX_ . 'carrier` c, `' . _DB_PREFIX_ . 'mr_method` m
					SET `deleted` = 0
					WHERE c.id_carrier = m.id_carrier');
            if (Configuration::get('MONDIAL_RELAY') < $this->version); // TODO : ADD upgrade process depending of the last and new version
        }
        return true;
    }
    /*
	...
	*/
    ...
    {
        return ($type == 'front') ? self::$MRFrontToken : (($type == 'back') ? self::$MRBackToken : NULL);
    }
    /*
	...
	*/
    ...
    {
        if (_PS_VERSION_ >= '1.3' && (! $this->registerHook('shipping') ||
         ! $this->registerHook('extraCarrier') ||
         ! $this->registerHook('updateCarrier') ||
         ! $this->registerHook('newOrder') ||
         ! $this->registerHook('BackOfficeHeader')))
            return false;
        if (_PS_VERSION_ >= '1.4' && (! $this->registerHook('processCarrier') ||
    ...
    public function uninstallCommonData ()
    {
        ...
        if ($result) {
            ...
            if (isset($id_tab) && ! empty($id_tab)) {
                Db::getInstance()->Execute(
                'DELETE FROM ' . _DB_PREFIX_ . 'tab WHERE id_tab = ' .
                 (int) ($id_tab));
                Db::getInstance()->Execute(
                'DELETE FROM ' . _DB_PREFIX_ . 'tab_lang WHERE id_tab = ' .
                 (int) ($id_tab));
                Db::getInstance()->Execute(
                'DELETE FROM ' . _DB_PREFIX_ . 'access WHERE id_tab = ' .
                 (int) ($id_tab));
            }
        }
        if (_PS_VERSION_ >= '1.4' && ! Db::getInstance()->Execute(
        '
    ...
    public function uninstall ()
    {
        ...
        'UPDATE ' . _DB_PREFIX_ .
         'carrier SET `active` = 0, `deleted` = 1 WHERE `name` = "mondialrelay"'))
            return false;
        return true;
    }
    /*
	...
	*/
    ...
    {
        if (Module::isInstalled('mondialrelay') && (($installedVersion = Configuration::get(
        'MONDIAL_RELAY')) ||
	...
	*/
    public static function initModuleAccess ()
    {
        ...
        $protocol = (Configuration::get('PS_SSL_ENABLED') || (! empty(
        $_SERVER['HTTPS']) && strtolower($_SERVER['HTTPS']) != 'off')) ? 'https://' : 'http://';
	...
	*/
    public static function getJqueryCompatibility ($overloadCurrent = false)
    {
        if ($overloadCurrent)
			...
			<script type="text/javascript" src="' .
             self::$moduleURL . '/jquery-1.4.4.min.js"></script>';
        return '
			<script type="text/javascript" src="' .
         self::$moduleURL . '/jquery-1.4.4.min.js"></script>
			<script type="text/javascript">
    ...
    public function hookBackOfficeHeader ()
    {
        ...
        return '
         ...
         $cssFilePath . '" />
			...
			<script type="text/javascript" src="' .
         $jsFilePath . '"></script>' . self::getJqueryCompatibility(true);
    }
    private function _postValidation ()
    {
        if (Tools::isSubmit('submitMR')) {
            if (Tools::getValue('mr_Enseigne_WebService') != '' and
             ! preg_match("#^[0-9A-Z]{2}[0-9A-Z ]{6}$#", 
            Tools::getValue('mr_Enseigne_WebService')))
                $this->_postErrors[] = $this->l('Invalid Shop');
            if (Tools::getValue('mr_code_marque') != '' and
             ! preg_match("#^[0-9]{2}$#", Tools::getValue('mr_code_marque')))
                $this->_postErrors[] = $this->l('Invalid Mark code');
            if (Tools::getValue('mr_Key_WebService') != '' and
             ! preg_match("#^[0-9A-Za-z_\'., /\-]{2,32}$#", 
            Tools::getValue('mr_Key_WebService')))
                $this->_postErrors[] = $this->l('Invalid Webservice Key');
            if (Tools::getValue('mr_Langage') != '' and
             ! preg_match("#^[A-Z]{2}$#", Tools::getValue('mr_Langage')))
                $this->_postErrors[] = $this->l('Invalid Language');
            if (! Tools::getValue('mr_weight_coef') or
             ! Validate::isInt(Tools::getValue('mr_weight_coef')))
            ...
            if (Configuration::get('MR_ENSEIGNE_WEBSERVICE') == '' or
             ...
             Configuration::get('MR_LANGUAGE') == '')
                ...
                'Please configure your Mondial Relay account settings before creating a carrier.');
            if (! preg_match("#^[0-9A-Za-z_\'., /\-]{2,32}$#", 
            Tools::getValue('mr_Name')))
                $this->_postErrors[] = $this->l('Invalid carrier name');
            if (Tools::getValue('mr_ModeCol') != 'CCC')
                $this->_postErrors[] = $this->l('Invalid Col mode');
            if (! preg_match("#^REL|24R|ESP|DRI|LDS|LDR|LD1$#", 
            Tools::getValue('mr_ModeLiv')))
                $this->_postErrors[] = $this->l('Invalid delivery mode');
            if (! Validate::isInt(Tools::getValue('mr_ModeAss')) or
             Tools::getValue('mr_ModeAss') > 5 or
             Tools::getValue('mr_ModeAss') < 0)
                $this->_postErrors[] = $this->l('Invalid Assurance mode');
            if (! Tools::getValue('mr_Pays_list'))
                $this->_postErrors[] = $this->l(
                'You must choose at least one delivery country.');
            ...
            if (! preg_match('#^[0-9A-Z_\-\'., /]{2,32}$#', strtoupper($addr1), 
            $match))
                $this->_postErrors[] = $this->l(
                'The Main address submited hasn\'t a good format');
        }
    }
    private function _postProcess ()
    {
        ...
        elseif (isset($_POST['submit_order_state']) and
         $_POST['submit_order_state']) {
            Configuration::updateValue('MONDIAL_RELAY_ORDER_STATE', 
            Tools::getValue('id_order_state'));
            Configuration::updateValue('MR_GOOGLE_MAP', 
            Tools::getValue('mr_google_key'));
            $this->_html .= '<div class="conf confirm"><img src="' .
             _PS_ADMIN_IMG_ . '/ok.gif" alt="" /> ' .
    ...
    public function hookOrderDetail ($params)
    {
        ...
        if ($carrier->is_module and $order->shipping_number) {
            ...
            $smarty->assign('followup', 
            $module_carrier->get_followup($order->shipping_number));
    ...
    public function hookOrderDetailDisplayed ($params)
    {
        ...
        if ((! $res) or ($res['MR_Selected_Num'] == 'LD1') or
         ($res['MR_Selected_Num'] == 'LDS'))
            return '';
        $smarty->assign('mr_addr', 
        $res['MR_Selected_LgAdr1'] . ($res['MR_Selected_LgAdr1'] ? ' - ' : '') .
	...
	*/
    ...
    public function hookextraCarrier ($params)
    {
            ...
            if ((Configuration::get('PS_SHIPPING_METHOD') and
             ...
             $carrier->getMaxDeliveryPriceByPrice($id_zone) === false))
                ...
                if ($row['range_behavior']) {
                    // Get id zone
                    $id_zone = (isset(
                    $cart->id_address_delivery) and $cart->id_address_delivery) ? Address::getZoneById(
                    (int) ($cart->id_address_delivery)) : (int) $defaultCountry->id_zone;
                    if ((Configuration::get('PS_SHIPPING_METHOD') and
                     (! Carrier::checkDeliveryPriceByWeight($row['id_carrier'], 
                    $cart->getTotalWeight(), $id_zone))) or
                     (! Configuration::get('PS_SHIPPING_METHOD') and
                     (! Carrier::checkDeliveryPriceByPrice($row['id_carrier'], 
                    $cart->getOrderTotal(true, self::BOTH_WITHOUT_SHIPPING), 
                    $id_zone, $cart->id_currency))))
                        unset($carriersList[$k]);
                }
        }
        ...
        'PS_ORDER_PROCESS_TYPE') : 0), 'new_base_dir' => self::$moduleURL, 
        'MRToken' => self::$MRFrontToken, 'carriersextra' => $carriersList, 
    ...
    public function getContent ()
    {
        ...
        if (! empty($_POST)) {
            ...
            if (! sizeof($this->_postErrors))
                ...
                $this->_html .= '<div class="alert error"><h3>' . $nbErrors . ' ' .
		...
		<div class="warn">' .
         $this->l(
        'Try to turn off the cache and put the force compilation to on if you have any problems with the module after an update') .
         '
		</div>
		<fieldset>
			<legend>
				<img src="../modules/mondialrelay/images/logo.gif" />' .
         $this->l('To create a Mondial Relay carrier') . '</legend>
				- ' .
         ...
         ' 
					<a href="index.php?tab=AdminCarriers&token=' .
         Tools::getAdminToken(
        'AdminCarriers' . (int) (Tab::getIdFromClassName('AdminCarriers')) .
         (int) ($cookie->id_employee)) . '" class="green">' .
         $this->l('The Carrier page') .
         '</a><br />
				- ' .
         $this->l(
        'To generate labels, you must have a valid and registered address of your store on your') .
         ' <a href="index.php?tab=AdminContact&token=' .
         Tools::getAdminToken(
        'AdminContact' . (int) (Tab::getIdFromClassName('AdminContact')) .
         (int) ($cookie->id_employee)) . '" class="green">' .
         $this->l('contact page') . '</a><br />
		</fieldset>
		<br class="clear" />
		<div class="PS_MRFormType">' . $this->settingsForm() . '</div>
		<div class="PS_MRFormType">' . $this->settingsstateorderForm() . '</div>
		<div class="PS_MRFormType">' . $this->advancedSettings() . '</div>
		<div class="PS_MRFormType">' . $this->addMethodForm() . '</div>
		<div class="PS_MRFormType">' . $this->shippingForm() .
         '</div><br class="clear" />';
        return $this->_html;
    }
    /*
	...
	*/
    ...
    {
        Configuration::updateValue('PS_MR_SHOP_NAME', 
        Tools::getValue('Expe_ad1'));
        $this->_html .= '<div class="conf confirm"><img src="' . _PS_ADMIN_IMG_ .
         '/ok.gif" alt="" /> ' . $this->l('Settings updated') . '</div>';
    }
    public function mrDelete ($id)
    {
        $id = Db::getInstance()->getValue(
        'SELECT `id_carrier` FROM `' . _DB_PREFIX_ .
         'mr_method` WHERE `id_mr_method` = "' . (int) ($id) . '"');
        Db::getInstance()->Execute(
        'UPDATE `' . _DB_PREFIX_ .
         'carrier` SET `active` = 0, `deleted` = 1 WHERE `id_carrier` = "' .
         (int) ($id) . '"');
        $this->_html .= '<div class="conf confirm"><img src="' . _PS_ADMIN_IMG_ .
         '/ok.gif" /> ' . $this->l('Delete successful') . '</div>';
    }
    public function mrUpdate ($type, $array, $keyArray)
    {
        ...
        if ($type == 'settings') {
            ...
            foreach ($array as $Key => $value) {
                ...
                $id = Db::getInstance()->getValue(
                'SELECT `id_carrier` FROM `' . _DB_PREFIX_ .
                 'mr_method` WHERE `id_mr_method` = "' . (int) ($key[0]) . '"');
                Db::getInstance()->Execute(
                'UPDATE ' . _DB_PREFIX_ . 'carrier SET active = "' .
                 (int) ($value) . '" WHERE `id_carrier` = "' . (int) ($id) . '"');
            }
            ...
            "SELECT * FROM " . _DB_PREFIX_ .
             "configuration WHERE name = 'PS_CARRIER_DEFAULT'");
            ...
            if (_PS_VERSION_ >= '1.4')
				...
				VALUES(NULL, "' .
                 pSQL('mondialrelay') . '", "1", "0", "1")');
            $get = Db::getInstance()->getRow(
            'SELECT * FROM `' . _DB_PREFIX_ . 'carrier` WHERE `id_carrier` = "' .
             mysql_insert_id() . '"');
            Db::getInstance()->Execute(
            'UPDATE `' . _DB_PREFIX_ . 'mr_method` SET `id_carrier` = "' .
             (int) ($get['id_carrier']) . '" WHERE `id_mr_method` = "' .
             pSQL($mainInsert) . '"');
            $weight_coef = Configuration::get('MR_WEIGHT_COEF');
            $range_weight = array('24R' => array(0, 20000 / $weight_coef), 
            ...
            Db::getInstance()->Execute(
            'INSERT INTO `' . _DB_PREFIX_ .
             'range_weight` (`id_carrier`, `delimiter1`, `delimiter2`)
										VALUES (' .
             (int) ($get['id_carrier']) . ', ' . $range_weight[$array[2]][0] .
             ', ' . $range_weight[$array[2]][1] . ')');
            Db::getInstance()->Execute(
            'INSERT INTO `' . _DB_PREFIX_ .
             'range_price` (`id_carrier`, `delimiter1`, `delimiter2`) VALUES (' .
             (int) ($get['id_carrier']) . ', 0.000000, 10000.000000)');
            $groups = Group::getGroups(Configuration::get('PS_LANG_DEFAULT'));
            foreach ($groups as $group)
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'carrier_group` (id_carrier, id_group) VALUES(' .
                 (int) ($get['id_carrier']) . ', ' . (int) ($group['id_group']) .
                 ')');
            $zones = Zone::getZones();
            foreach ($zones as $zone) {
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'carrier_zone` (id_carrier, id_zone) VALUES(' .
                 (int) ($get['id_carrier']) . ', ' . (int) ($zone['id_zone']) .
                 ')');
                $range_price_id = Db::getInstance()->getValue(
                'SELECT id_range_price FROM ' . _DB_PREFIX_ .
                 'range_price WHERE id_carrier = "' . (int) ($get['id_carrier']) .
                 '"');
                $range_weight_id = Db::getInstance()->getValue(
                'SELECT id_range_weight FROM ' . _DB_PREFIX_ .
                 'range_weight WHERE id_carrier = "' . (int) ($get['id_carrier']) .
                 '"');
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'delivery` (id_carrier, id_range_price, id_range_weight, id_zone, price) VALUES(' .
                 (int) ($get['id_carrier']) . ', ' . (int) ($range_price_id) .
                 ', NULL,' . (int) ($zone['id_zone']) . ', 0.00)');
                Db::getInstance()->Execute(
                'INSERT INTO `' . _DB_PREFIX_ .
                 'delivery` (id_carrier, id_range_price, id_range_weight, id_zone, price) VALUES(' .
                 (int) ($get['id_carrier']) . ', NULL, ' .
                 (int) ($range_weight_id) . ',' . (int) ($zone['id_zone']) .
                 ', 0.00)');
            }
            if (! in_array($default[0]['value'], $checkD))
                $default = Db::getInstance()->ExecuteS(
                "UPDATE " . _DB_PREFIX_ . "configuration SET value = '" .
                 (int) ($get['id_carrier']) .
                 "' WHERE name = 'PS_CARRIER_DEFAULT'");
        } else
            return false;
        $this->_html .= '<div class="conf confirm"><img src="' . _PS_ADMIN_IMG_ .
         '/ok.gif" /> ' . $this->l('Settings updated') .
         '<img src="http://www.prestashop.com/modules/mondialrelay.png?enseigne=' .
         urlencode(Tools::getValue('mr_Enseigne_WebService')) .
    ...
    public function addMethodForm ()
    {
        ...
        "SELECT * FROM " . _DB_PREFIX_ . "zone WHERE active = 1");
        $output = '
		<form action="' .
         ...
         '" method="post" >
			...
			<fieldset>
				<legend><img src="../modules/mondialrelay/images/logo.gif" alt="" />' .
         $this->l('Add a Shipping Method') . '</legend>
				<ul>
         ...
         '</sup>
					...
					<li>
						<label for="mr_Name" class="shipLabel">' .
         $this->l('Carrier\'s name') .
         '<sup>*</sup></label>
						<input type="text" id="mr_Name" name="mr_Name" ' .
         (Tools::getValue('mr_Name') ? 'value="' .
         Tools::safeOutput(Tools::getValue('mr_Name')) . '"' : '') . '/>
					</li>';
        /*<li>
        ...
        $output .= '<li>
						<label for="mr_ModeLiv" class="shipLabel">' .
         $this->l('Delivery mode') .
         '<sup>*</sup></label>
						<select name="mr_ModeLiv" id="mr_ModeLiv" style="width:200px">
						<option value="24R" selected >24R : ' .
         $this->l('Delivery to a relay point') . '</option>
						<option value="DRI" >DRI : ' .
         $this->l('Colis Drive delivery') .
         '</option>
						<option value="LD1" >LD1 : ' .
         $this->l('Home delivery RDC (1 person)') .
         '</option>
						<option value="LDS" >LDS : ' .
         $this->l('Special Home delivery (2 persons)') .
         '</option>
						</select>
					</li>
					
					<li>
						<label for="mr_ModeAss" class="shipLabel">' .
         $this->l('Insurance') . '<sup>*</sup></label>
						<select name="mr_ModeAss" id="mr_ModeAss" style="width:200px">
						<option value="0" selected>0 : ' .
         $this->l('No insurance') .
         '</option>
						<option value="1">1 : ' .
         $this->l('Complementary Insurance Lv1') .
         '</option>
						<option value="2">2 : ' .
         $this->l('Complementary Insurance Lv2') .
         '</option>
						<option value="3">3 : ' .
         $this->l('Complementary Insurance Lv3') .
         '</option>
						<option value="4">4 : ' .
         $this->l('Complementary Insurance Lv4') .
         '</option>
						<option value="5">5 : ' .
         $this->l('Complementary Insurance Lv5') .
         '</option>
						</select>
					</li>

					<li>	
					<label for="mr_Pays_list" class="shipLabel">' .
         $this->l('Delivery countries:') . '<sup>*</sup><br /><br />
					<span style="font-size:10px; width:200px;float:left; color:forestgreen">' .
         $this->l(
        'You can choose several countries by pressing Ctrl while selecting countries') . '</span>
					</label>	
						<select name="mr_Pays_list[]" id="mr_Pays_list" multiple size="5">
							<option value="FR">' . $this->l('France') . '</option>
							<option value="BE">' . $this->l(
        'Belgium') . '</option>
							<option value="LU">' .
         $this->l('Luxembourg') . '</option>
							<option value="ES">' . $this->l('Spain') .
         '</option>
						</select>
					</li>			
					<li class="PS_MRSubmit">
						<input type="submit" name="submitMethod" value="' .
         $this->l('Add a Shipping Method') . '" class="button" />
					</li>
				</ul>
			</fieldset>
		</form>';
    ...
    public function shippingForm ()
    {
		...
		<form action="' .
         ...
         '" method="post">
			<fieldset class="shippingList">
				<legend><img src="../modules/mondialrelay/images/logo.gif" />' .
         $this->l('Shipping Method\'s list') . '</legend>
				<ul>';
        if (! sizeof($query))
        ...
        foreach ($query as $Options) {
            $output .= '
					<li>
						<a href="' .
             'index.php?tab=AdminModules&configure=mondialrelay&token=' .
             Tools::getAdminToken(
            'AdminModules' . (int) (Tab::getIdFromClassName('AdminModules')) .
             (int) ($cookie->id_employee)) . '&delete_mr=' .
             $Options['id_mr_method'] .
             '"><img src="../img/admin/disabled.gif" alt="Delete" title="Delete" /></a>' .
             str_replace('_', ' ', $Options['mr_Name']) . ' (' .
             $Options['mr_ModeCol'] . '-' . $Options['mr_ModeLiv'] . ' - ' .
             $Options['mr_ModeAss'] . ' : ' . $Options['mr_Pays_list'] .
             ') 
						<div style="float:right;"><a href="index.php?tab=AdminCarriers&id_carrier=' .
             (int) ($Options['id_carrier']) . '&updatecarrier&token=' .
             Tools::getAdminToken(
            'AdminCarriers' . (int) (Tab::getIdFromClassName('AdminCarriers')) .
             (int) ($cookie->id_employee)) . '"><b><u>' .
             $this->l('Config Shipping.') . '</u></b></a></div>	
					</li>';
        }
        $output .= ' 
				</ul>
        ...
        $form .= '
         ...
         ' - 
					<a href="javascript:void(0);" id="PS_MRDisplayPersonalizedOptions">
						<font style="color:#00b511;">' .
         $this->l('Click to display / hide the options') . '</font>
					</a>' . '</legend>
			<div id="PS_MRAdvancedSettings">
				<p>' . $this->l('URL Cron Task:') . ' ' . Tools::getHttpHost(true, true) .
         _MODULE_DIR_ . $this->name . '/cron.php?secure_key=' .
         Configuration::get('MONDIAL_RELAY_SECURE_KEY') . '</p>
			</div>
			</fieldset>';
        return $form;
    }
	...
	*/
    public function personalizeFormFields ()
    {
        ...
        $addr1 = (Configuration::get('PS_MR_SHOP_NAME')) ? Configuration::get(
        'PS_MR_SHOP_NAME') : Configuration::get('PS_SHOP_NAME');
        // Check if a request exist and if errors occured, use the post variable
        if (Tools::isSubmit('PS_MRSubmitFieldPersonalization') &&
         count($this->_postErrors))
            $addr1 = Tools::safeOutput(Tools::getValue('Expe_ad1'));
        if (! Configuration::get('PS_MR_SHOP_NAME'))
            $warn .= '<div class="warn">' .
             $this->l(
            'Its seems you updated Mondialrelay without use the uninstall / install method, you have to set up this part to make working the generating ticket process') .
             '</div>';
             // Form
        $form = '<form action="' .
         Tools::safeOutput($_SERVER['REQUEST_URI']) .
        ...
        $form .= '
         ...
         $this->l('Advanced Settings') .
         ' - 
					<a href="javascript:void(0);" id="PS_MRDisplayPersonalizedOptions"><font style="color:#00b511;">' .
         $this->l('Click to display / hide the options') . '</font>	</a>' .
         '</legend>' . $warn .
         '
			<div id="PS_MRPersonalizedFields">
				...
				- ' .
         $this->l(
        'This part allow to override the data sent at MondialRelay when you want to generate Ticket. Some fields are restricted by the length, or forbidden char') . '.
				</div>
				<label for="PS_MR_SHOP_NAME">' .
         $this->l('Shop Name') . '</label>
				<div class="margin-form">
					<input type="text" name="Expe_ad1" value="' . $addr1 .
         '" /><br />
					<p>' .
         $this->l('The key used by Mondialrelay is') . ' <b>Expe_ad1</b> ' .
         $this->l('and has this default value') . '
				 	: <b>' .
         Configuration::get('PS_SHOP_NAME') .
         '</b></p>
				</div>
				...
				<div class="margin-form">
					<input type="submit" name="PS_MRSubmitFieldPersonalization"  value="' .
    ...
    public function settingsstateorderForm ()
    {
        ...
        $output = '';
        $output .= '<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) .
         '" method="post" class="form">';
        $output .= '<fieldset><legend><img src="../modules/mondialrelay/images/logo.gif" />' .
         $this->l('Settings') . '</legend>';
        $output .= '<label for="id_order_state">' . $this->l('Order state') .
         '</label>';
        ...
        foreach ($order_states as $order_state) {
            $output .= '<option value="' . $order_state['id_order_state'] .
             '" style="background-color:' . $order_state['color'] . ';"';
            if ($this->orderState == $order_state['id_order_state'])
        ...
        $output .= '<p>' .
         $this->l(
        'Choose the order state for labels. You can manage the labels on') . ' ';
        $output .= '<a href="index.php?tab=AdminMondialRelay&token=' .
         Tools::getAdminToken(
        'AdminMondialRelay' . (int) (Tab::getIdFromClassName(
        'AdminMondialRelay')) . (int) ($cookie->id_employee)) . '" class="green">' .
         $this->l('the Mondial Relay administration page') . '</a></p>';
        $output .= '</div>
		<div class="clear"></div>';
        $output .= '<div class="margin-form"><input type="submit" name="submit_order_state"  value="' .
         $this->l('Save') . '" class="button" /></div>';
    ...
    public function settingsForm ()
    {
        $output = '
         ...
         '</legend>
					<div>
						- ' .
         $this->l(
        'These parameters are provided by Mondial Relay once you subscribed to their service') . '
					</div>
					<ul>
						<li class="PS_MRRequireFields">
							<sup>* ' . $this->l('Required') .
         '</sup>
						...
						<li>
							<label for="mr_Enseigne_WebService" class="mrLabel">' .
         $this->l('Webservice Enseigne:') .
         '<sup>*</sup></label>
							<input id="mr_Enseigne_WebService" class="mrInput" type="text" name="mr_Enseigne_WebService" value="' .
         (Tools::getValue('mr_Enseigne_WebService') ? Tools::safeOutput(
        Tools::getValue('mr_Enseigne_WebService')) : Configuration::get(
        'MR_ENSEIGNE_WEBSERVICE')) .
         ...
         (Tools::getValue('mr_code_marque') ? Tools::safeOutput(
        Tools::getValue('mr_code_marque')) : Configuration::get('MR_CODE_MARQUE')) .
         '"/>
						</li>
						<li>
							<label for="mr_Key_WebService" class="mrLabel">' .
         $this->l('Webservice Key:') .
         '<sup>*</sup></label>
							<input id="mr_Key_WebService" class="mrInput" type="text" name="mr_Key_WebService" value="' .
         (Tools::getValue('mr_Key_WebService') ? Tools::safeOutput(
        Tools::getValue('mr_Key_WebService')) : Configuration::get(
        'MR_KEY_WEBSERVICE')) .
         '"/>
						</li>
						<li>
							<label for="mr_Langage" class="mrLabel">' .
         $this->l('Etiquette\'s Language:') . '<sup>*</sup></label>
							<select id="mr_Langage" name="mr_Langage" value="' .
         (Tools::getValue('mr_Langage') ? Tools::safeOutput(
        Tools::getValue('mr_Langage')) : Configuration::get('MR_LANGUAGE')) .
         '" >';
        $languages = Language::getLanguages();
        foreach ($languages as $language)
            $output .= '<option value="' . strtoupper($language['iso_code']) .
             '" ' .
             (strtoupper($language['iso_code']) ==
             Configuration::get('MR_LANGUAGE') ? 'selected="selected"' : '') .
             '>' . $language['name'] . '</option>';
        $output .= '</select>
						</li>
						<li>
							<label for="mr_weight_coef" class="mrLabel">' .
         $this->l('Weight Coefficient:') . '<sup>*</sup></label>
							<input class="mrInput" type="text" name="mr_weight_coef" value="' .
         (Tools::getValue('mr_weight_coef') ? Tools::safeOutput(
        Tools::getValue('mr_weight_coef')) : Configuration::get('MR_WEIGHT_COEF')) .
         '"  style="width:45px;"/>
         ...
         ')</span>
						...
						<li class="PS_MRSubmit">
							<input type="submit" name="submitMR" value="' .
         $this->l('Update Settings') . '" class="button" />
						</li>
					</ul>
				</fieldset>
			</form>';
    ...
    public function displayInfoByCart ($id_cart)
    {
        ...
        if (trim($simpleresul[0]['url_etiquette']) != '0')
            $html .= "<a href='" . $simpleresul[0]['url_etiquette'] .
             "' target='etiquette" . $simpleresul[0]['url_etiquette'] . "'>" .
             $this->l('Label URL') . "</a><br>";
        if (trim($simpleresul[0]['url_suivi']) != '0')
            $html .= "<a href='" . $simpleresul[0]['url_suivi'] .
             "' target='suivi" . $simpleresul[0]['exp_number'] . "'>" .
             $this->l('Follow-up URL') . "</a><br>";
        if (trim($simpleresul[0]['MR_Selected_Num']) != '')
            $html .= $this->l('Nb Point Relay :') .
             $simpleresul[0]['MR_Selected_Num'] . "<br>";
    ...
    {
        return (method_exists('Tools', 'jsonEncode')) ? Tools::jsonEncode(
    ...
    public static function ordersSQLQuery1_4 ($id_order_state)
    {
        return 'SELECT  o.`id_address_delivery` as id_address_delivery, 
			...
			WHERE (
				...
				ORDER BY moh.`date_add` DESC LIMIT 1) = ' .
         (int) ($id_order_state) . ' 
			AND ca.`external_module_name` = "mondialrelay"';
    }
