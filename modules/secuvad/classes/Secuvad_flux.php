...
class Secuvad_flux
{
    ...
    public function get_flux_xml_fraud ($id_order)
    {
        ...
        $this->flux_xml = '<?xml version="1.0" encoding="' . $this->encoding .
         '" ?>' . "\n";
        $this->flux_xml .= '<impaye><idsecuvad>' . $this->idsecuvad .
         '</idsecuvad><idtransaction>' . (int) ($this->id_order) .
         '</idtransaction><imptimestamp>' . $this->imp_time .
         '</imptimestamp></impaye>';
    ...
    private function get_flux_xml_order ()
    {
		...
		FROM `' . _DB_PREFIX_ .
         'secuvad_assoc_transport` at 
		JOIN `' .
         _DB_PREFIX_ . 'secuvad_transport_delay` td ON (at.`transport_delay_id` = td.`transport_delay_id`)  
		JOIN `' . _DB_PREFIX_ .
         'lang` l ON (l.`id_lang` = td.`id_lang`)
		WHERE l.`id_lang` = ' .
         ((isset($cookie->id_lang) and (int) ($cookie->id_lang)) ? (int) ($cookie->id_lang) : (int) (Configuration::get(
        'PS_LANG_DEFAULT'))) . '
		AND at.`id_carrier` = ' .
		...
		FROM `' . _DB_PREFIX_ .
         'module` m 
		JOIN  `' .
         _DB_PREFIX_ . 'secuvad_assoc_payment` sap ON (m.`id_module` = sap.`id_module`)
        ...
        if ($address_invoice->company == '')
            ...
            $flux_xml .= '<client mode="facturation" qualite="entreprise">' .
             "\n";
        $flux_xml .= '<nom titre="' . $gender . '">' . $address_invoice->lastname .
         '</nom>' . "\n";
        ...
        if ($address_invoice->company != '')
            $flux_xml .= '<societe>' . $address_invoice->company . '</societe>' .
             "\n";
        $flux_xml .= '<telephoneperso>' . $address_invoice->phone .
         '</telephoneperso>' . "\n";
        $flux_xml .= '<portable>' . $address_invoice->phone_mobile .
         '</portable>' . "\n";
        ...
        $flux_xml .= '<codepostal>' . $address_invoice->postcode .
         '</codepostal>' . "\n";
        ...
        $flux_xml .= '<codepostal>' . $address_delivery->postcode .
         '</codepostal>' . "\n";
        ...
        $flux_xml .= '<idtransaction>' . (int) ($this->id_order) .
         '</idtransaction>' . "\n";
        $flux_xml .= '<trstimestamp>' . $order->date_add . '</trstimestamp>' .
         "\n";
        $flux_xml .= '<montantttc devise="' . $currency->iso_code . '">' .
         $order->total_paid_real . '</montantttc>' . "\n";
        $flux_xml .= '<montantlivraison>' . $order->total_shipping .
         '</montantlivraison>' . "\n";
        $flux_xml .= '<ip>' . $ip . '</ip>' . "\n";
        $flux_xml .= '<transport transptype="' . $transptype . '" rapidite="' .
         $rapidite . '"></transport>' . "\n";
        ...
        elseif ($code_payment == "carte") {
            if (! empty($card_number)) {
                $cc_array = preg_split('/([X0-9]{4})/Ui', 
                strtoupper($card_number), - 1, 
                ...
                if (sizeof($cc_array)) {
                    ...
                    $bin42 = $bin_array[0] . '-' .
                     $bin_array[3]; // 6 char
                    if (strlen($bin42) != 7 and
                     strlen($bin4) != 4 and strlen($bin) != 7 and
                     strlen($card_number) != 16)
                        return false;
                    $flux_xml .= '<CB datevalidite="' . $card_expiration . '" ' .
                     (strlen($card_number) == 16 ? 'numcb="' . $card_number . '"' : '') .
                     ' ' . (strlen($bin) == 7 ? 'bin="' . $bin . '"' : '') . ' ' .
                     (strlen($bin4) == 4 ? 'bin4="' . $bin4 . '"' : '') . ' ' .
                     (strlen($bin42) == 7 ? 'bin42="' . $bin42 . '"' : '') .
                     '></CB>' . "\n";
                } else
                    return false;
            } else
    ...
    private function get_flux_xml_products ()
    {
        ...
        foreach ($products as $product) {
			...
			FROM `' . _DB_PREFIX_ .
             'secuvad_assoc_category` sac 
			JOIN `' .
             _DB_PREFIX_ . 'category_product` cp ON (cp.`id_category` = sac.`id_category`)
			JOIN `' .
             _DB_PREFIX_ .
             'category` c ON (c.`id_category` = cp.`id_category`)
			JOIN `' .
             _DB_PREFIX_ .
             'product_lang` pl ON (cp.`id_product` = pl.`id_product`) 
			JOIN `' .
             _DB_PREFIX_ .
             'lang` l ON (l.`id_lang` = pl.`id_lang` AND l.`id_lang` = ' .
             ((isset($cookie->id_lang) and (int) ($cookie->id_lang)) ? (int) ($cookie->id_lang) : (int) (Configuration::get(
            'PS_LANG_DEFAULT'))) . ')
			WHERE pl.`id_product` = ' .
			...
			', true);
            $flux_xml .= '<produit categorie="' . (int) ($data['category_id']) .
             '" reference="' . (int) ($product['product_id']) . '-' .
             (int) ($product['product_attribute_id']) . '" modele="' .
             addslashes($data['name']) . '" prix="' .
             (float) ($product['product_price_wt']) . '" quantite="' .
             (int) ($product['product_quantity']) . '"></produit>' . "\n";
        }
