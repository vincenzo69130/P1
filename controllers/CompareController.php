...
class CompareControllerCore extends FrontController
{
    ...
    public function preProcess ()
    {
        ...
        if (Tools::getValue('ajax') and Tools::getValue('id_product') and
         Tools::getValue('action')) {
            if (Tools::getValue('action') == 'add') {
                if (isset(self::$cookie->id_customer)) {
                    if (CompareProduct::getCustomerNumberProducts(
                    ...
                    if ((isset(self::$cookie->id_guest) and CompareProduct::getGuestNumberProducts(
                    self::$cookie->id_guest) <
                     Configuration::get('PS_COMPARATOR_MAX_ITEM')))
                        CompareProduct::addGuestCompareProduct(
                        (int) self::$cookie->id_guest, 
                        (int) Tools::getValue('id_product'));
                    else
                        die('0');
                }
                ...
                if (isset(self::$cookie->id_customer))
                    CompareProduct::removeCustomerCompareProduct(
                    (int) self::$cookie->id_customer, 
                    (int) Tools::getValue('id_product'));
                elseif (isset(self::$cookie->id_guest))
                    CompareProduct::removeGuestCompareProduct(
                    (int) self::$cookie->id_guest, 
                    (int) Tools::getValue('id_product'));
                else
                    die('0');
    ...
    public function process ()
    {
        ...
        if ($product_list = Tools::getValue('compare_product_list') and
        ...
        elseif (isset(self::$cookie->id_customer))
            $ids = CompareProduct::getCustomerCompareProducts(
            self::$cookie->id_customer);
        elseif (isset(self::$cookie->id_guest))
            $ids = CompareProduct::getGuestCompareProducts(
            self::$cookie->id_guest);
        else
            $ids = null;
        if ($ids) {
            if (sizeof($ids) > 0) {
                ...
                $listFeatures = array();
                foreach ($ids as $id) {
                    $curProduct = new Product((int) ($id), true, 
                    ...
                    foreach ($curProduct->getFrontFeatures(
                    self::$cookie->id_lang) as $feature)
                        $listFeatures[$curProduct->id][$feature['id_feature']] = $feature['value'];
                    $cover = Product::getCover((int) $id);
                    $curProduct->id_image = Tools::htmlentitiesUTF8(
                    Product::defineProductImage(
                    array('id_image' => $cover['id_image'], 'id_product' => $id), 
                    self::$cookie->id_lang));
                    $curProduct->allow_oosp = Product::isAvailableWhenOutOfStock(
                    $curProduct->out_of_stock);
                    $listProducts[] = $curProduct;
                }
                if (sizeof($listProducts) > 0) {
                    $width = 80 / sizeof($listProducts);
                    $hasProduct = true;
                    $ordered_features = Feature::getFeaturesForComparison($ids, 
                    self::$cookie->id_lang);
                    self::$smarty->assign(
                    array('ordered_features' => $ordered_features, 
                    'product_features' => $listFeatures, 
                    'products' => $listProducts, 'link' => new Link(), 
                    'width' => $width, 'homeSize' => Image::getSize('home')));
                    self::$smarty->assign('HOOK_EXTRA_PRODUCT_COMPARISON', 
                    Module::hookExec('extraProductComparison', 
                    array('list_ids_product' => $ids)));
                }
            }
        }
