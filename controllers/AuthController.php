...
class AuthControllerCore extends FrontController
{
    ...
    public function preProcess ()
    {
        ...
        if (Tools::isSubmit('SubmitCreate')) {
            if (! Validate::isEmail($email = Tools::getValue('email_create')) or
             empty($email))
        ...
        if (Tools::isSubmit('submitAccount') or
         Tools::isSubmit('submitGuestAccount')) {
            ...
            if (Tools::isSubmit('submitAccount'))
                self::$smarty->assign('email_create', 1);
                /* New Guest customer */
            if (! Tools::getValue('is_new_customer', 1) and
            ...
            if (! @checkdate(Tools::getValue('months'), Tools::getValue('days'), 
            Tools::getValue('years')) and ! (Tools::getValue('months') == '' and
            ...
            $this->errors = array_unique(
            array_merge($this->errors, $customer->validateControler()));
            /* Preparing address */
            $address = new Address();
            ...
            $this->errors = array_unique(
            array_merge($this->errors, $address->validateControler()));
            ...
            if (Country::isNeedDniByCountryId($address->id_country) and (! Tools::getValue(
            'dni') or ! Validate::isDniLite(Tools::getValue('dni'))))
                $this->errors[] = Tools::displayError(
            ...
            if (! sizeof($this->errors)) {
                ...
                if (Tools::isSubmit('newsletter')) {
                    $customer->ip_registration_newsletter = pSQL(
                    Tools::getRemoteAddr());
                ...
                if (! sizeof($this->errors)) {
                    ...
                    if ((int) ($country->contains_states) and
                     ! (int) ($address->id_state))
                        ...
                        if (Tools::isSubmit('is_new_customer'))
                            $customer->is_guest = ! Tools::getValue(
                            'is_new_customer', 1);
                        else
                            $customer->is_guest = 0;
                        if (! $customer->add())
                            ...
                            if (! $address->add())
                                ...
                                if (! $customer->is_guest) {
                                    if (! Mail::Send(
                                    (int) (self::$cookie->id_lang), 'account', 
                                    Mail::l('Welcome!'), 
                                ...
                                if (Tools::isSubmit('ajax')) {
                                    ...
                                    'errors' => $this->errors, 'isSaved' => true, 
        ...
        if (Tools::isSubmit('SubmitLogin')) {
            ...
            elseif (! Validate::isPasswd($passwd))
                ...
                if (! $authentication or ! $customer->id) {
                    ...
                    self::$cookie->passwd = $customer->passwd;
                    self::$cookie->email = $customer->email;
                    if (Configuration::get('PS_CART_FOLLOWING') and (empty(
                    self::$cookie->id_cart) or
                     Cart::getNbProducts(self::$cookie->id_cart) == 0))
                        self::$cookie->id_cart = (int) (Cart::lastNoneOrderedCart(
                        (int) ($customer->id)));
                        /* Update cart address */
        ...
        self::$smarty->assign('newsletter', 
        (int) Module::getInstanceByName('blocknewsletter')->active);
    }
    public function setMedia ()
    {
        ...
        Tools::addJS(
        array(_THEME_JS_DIR_ . 'tools/statesManagement.js', 
    ...
    {
        $addressItems = array();
        $addressFormat = AddressFormat::getOrderedAddressFields(
        Configuration::get('PS_COUNTRY_DEFAULT'), false, true);
        ...
        foreach (array('inv', 'dlv') as $addressType)
            self::$smarty->assign(
            array($addressType . '_adr_fields' => $addressFormat, 
