...
class OrderControllerCore extends ParentOrderController
{
    ...
    public function preProcess ()
    {
        ...
        if (! self::$cart->checkQuantities()) {
            ...
            'An item in your cart is no longer available for this quantity, you cannot proceed with your order.');
        }
        ...
        if (self::$cart->getOrderTotal(false, Cart::ONLY_PRODUCTS) <
         $minimalPurchase && $this->step != - 1) {
            $this->step = 0;
            $this->errors[] = Tools::displayError('A minimum purchase total of') .
             ' ' . Tools::displayPrice($minimalPurchase, $currency) . ' ' .
             Tools::displayError('is required in order to validate your order.');
        }
        if (! self::$cookie->isLogged(true) and
         in_array($this->step, array(1, 2, 3)))
            Tools::redirect(
            'authentication.php?back=' . urlencode(
            'order.php?step=' . $this->step));
        if ($this->nbProducts)
            ...
            case 3:
                ...
                if (($id_order = $this->_checkFreeOrder()) and $id_order) {
                    if (self::$cookie->is_guest) {
                        ...
                        Tools::redirect(
                        'guest-tracking.php?id_order=' . (int) $id_order .
    ...
    private function processAddressFormat ()
    {
        ...
        $invoiceAddressFields = AddressFormat::getOrderedAddressFields(
        $addressInvoice->id_country, false, true);
        $deliveryAddressFields = AddressFormat::getOrderedAddressFields(
        $addressDelivery->id_country, false, true);
        self::$smarty->assign(
        array('inv_adr_fields' => $invoiceAddressFields, 
    ...
    public function displayContent ()
    {
        ...
        self::$smarty->assign(
        array('currencySign' => $currency->sign, 
    ...
    public function autoStep ()
    {
        ...
        if ($this->step >= 2 and
         (! self::$cart->id_address_delivery or ! self::$cart->id_address_invoice))
        ...
        if ($delivery->deleted or $invoice->deleted) {
            ...
            Tools::redirect('order.php?step=1');
        } elseif ($this->step >= 3 and ! self::$cart->id_carrier and
         ! $isVirtualCart)
            Tools::redirect('order.php?step=2');
    }
    /*
    ...
    {
        if (! Tools::isSubmit('id_address_delivery') or ! Address::isCountryActiveById(
        (int) Tools::getValue('id_address_delivery')))
            ...
            'id_address_delivery'));
            self::$cart->id_address_invoice = Tools::isSubmit('same') ? self::$cart->id_address_delivery : (int) (Tools::getValue(
            'id_address_invoice'));
            if (! self::$cart->update())
                $this->errors[] = Tools::displayError(
    ...
    protected function _assignCarrier ()
    {
        ...
        self::$smarty->assign('is_guest', 
        (isset(self::$cookie->is_guest) ? self::$cookie->is_guest : 0));
    }
    /* Payment step */
    protected function _assignPayment ()
    {
        ...
        self::$smarty->assign(
        array('total_price' => (float) ($orderTotal), 
