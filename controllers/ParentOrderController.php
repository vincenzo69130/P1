...
class ParentOrderControllerCore extends FrontController
{
    ...
    public function preProcess ()
    {
        ...
        if (Configuration::get('PS_ORDER_PROCESS_TYPE') == 0 and
         strpos($_SERVER['PHP_SELF'], 'order.php') === false)
            Tools::redirect('order.php');
        if (Configuration::get('PS_ORDER_PROCESS_TYPE') == 1 and
         strpos($_SERVER['PHP_SELF'], 'order-opc.php') === false) {
        ...
        if (Tools::isSubmit('submitReorder') and
         $id_order = (int) Tools::getValue('id_order')) {
            $oldCart = new Cart(
            Order::getCartIdStatic((int) $id_order, 
            (int) self::$cookie->id_customer));
            $duplication = $oldCart->duplicate();
            if (! $duplication or ! Validate::isLoadedObject(
            $duplication['cart']))
                $this->errors[] = Tools::displayError(
                'Sorry, we cannot renew your order.');
            elseif (! $duplication['success'])
                $this->errors[] = Tools::displayError(
                'Missing items - we are unable to renew your order');
            else {
        ...
        if ($this->nbProducts) {
            if (Tools::isSubmit('submitAddDiscount') and
             Tools::getValue('discount_name')) {
                $discountName = Tools::getValue('discount_name');
                if (! Validate::isDiscountName($discountName))
                    $this->errors[] = Tools::displayError(
                    'Voucher name invalid.');
                else {
                    $discount = new Discount(
                    (int) (Discount::getIdByName($discountName)));
                    if (Validate::isLoadedObject($discount)) {
                        if ($tmpError = self::$cart->checkDiscountValidity(
                        $discount, self::$cart->getDiscounts(), 
                        self::$cart->getOrderTotal(), self::$cart->getProducts(), 
                        true))
            ...
            } elseif (isset($_GET['deleteDiscount']) and
             Validate::isUnsignedId($_GET['deleteDiscount'])) {
        ...
        self::$smarty->assign('back', 
        Tools::safeOutput(Tools::getValue('back')));
    }
    public function setMedia ()
    {
        ...
        if ((int) (Configuration::get('PS_BLOCK_CART_AJAX')) or
         Configuration::get('PS_ORDER_PROCESS_TYPE') == 1) {
    ...
    protected function _checkFreeOrder ()
    {
        if (self::$cart->getOrderTotal() <= 0) {
            ...
            $order->validateOrder((int) (self::$cart->id), 
            Configuration::get('PS_OS_PAYMENT'), 0, 
            Tools::displayError('Free order', false), null, array(), null, false, 
    ...
    protected function _updateMessage ($messageContent)
    {
        if ($messageContent) {
            ...
            elseif ($oldMessage = Message::getMessageByCartId(
            (int) (self::$cart->id))) {
                $message = new Message((int) ($oldMessage['id_message']));
                $message->message = htmlentities($messageContent, ENT_COMPAT, 
                'UTF-8');
                $message->update();
            } else {
                $message = new Message();
            ...
            if ($oldMessage = Message::getMessageByCartId(
            (int) (self::$cart->id))) {
                $message = new Message((int) ($oldMessage['id_message']));
                $message->delete();
            }
    ...
    protected function _processCarrier ()
    {
        ...
        if (isset(self::$cookie->id_customer) and self::$cookie->id_customer) {
            ...
            if (! ($id_zone = Address::getZoneById($address->id)))
                $this->errors[] = Tools::displayError(
                'No zone match with your address');
        } else
            $id_zone = Country::getIdZone(
            (int) Configuration::get('PS_COUNTRY_DEFAULT'));
        if (Validate::isInt(Tools::getValue('id_carrier')) and sizeof(
        Carrier::checkCarrierZone((int) (Tools::getValue('id_carrier')), 
        (int) ($id_zone))))
            self::$cart->id_carrier = (int) (Tools::getValue('id_carrier'));
        elseif (! self::$cart->isVirtualCart() and
         (int) (Tools::getValue('id_carrier')) == 0)
            $this->errors[] = Tools::displayError(
    ...
    protected function _assignSummaryInformations ()
    {
        global $currency;
        if (file_exists(
        _PS_SHIP_IMG_DIR_ . (int) (self::$cart->id_carrier) . '.jpg'))
        ...
        self::$smarty->assign(
        array('token_cart' => Tools::getToken(false), 
        ...
        'shippingCostTaxExc' => self::$cart->getOrderTotal(false, 
        Cart::ONLY_SHIPPING), 'customizedDatas' => $customizedDatas, 
        'CUSTOMIZE_FILE' => _CUSTOMIZE_FILE_, 
        'CUSTOMIZE_TEXTFIELD' => _CUSTOMIZE_TEXTFIELD_, 
        'lastProductAdded' => self::$cart->getLastProduct(), 
        'displayVouchers' => Discount::getVouchersToCartDisplay(
        (int) (self::$cookie->id_lang), 
        (isset(self::$cookie->id_customer) ? (int) (self::$cookie->id_customer) : 0)), 
        ...
        'HOOK_SHOPPING_CART_EXTRA' => Module::hookExec('shoppingCartExtra', 
        $summary)));
    }
    protected function _assignAddress ()
    {
        //if guest checkout disabled and flag is_guest  in cookies is actived
        if (Configuration::get('PS_GUEST_CHECKOUT_ENABLED') == 0 and ((int) self::$cookie->is_guest !=
         Configuration::get('PS_GUEST_CHECKOUT_ENABLED'))) {
            self::$cookie->logout();
            Tools::redirect('');
        } elseif (! Customer::getAddressesTotalById(
        (int) (self::$cookie->id_customer)))
        ...
        if (Validate::isLoadedObject($customer)) {
            ...
            foreach ($customerAddresses as $address) {
                ...
                $formatedAddressFieldsValuesList[$address['id_address']]['ordered_fields'] = AddressFormat::getOrderedAddressFields(
                $address['id_country']);
            ...
            self::$smarty->assign(
            array('addresses' => $customerAddresses, 
            'formatedAddressFieldsValuesList' => $formatedAddressFieldsValuesList));
            /* Setting default addresses for cart */
            if ((! isset(self::$cart->id_address_delivery) or
             empty(self::$cart->id_address_delivery)) and
             sizeof($customerAddresses)) {
                self::$cart->id_address_delivery = (int) ($customerAddresses[0]['id_address']);
                $update = 1;
            }
            if ((! isset(self::$cart->id_address_invoice) or
             empty(self::$cart->id_address_invoice)) and
             sizeof($customerAddresses)) {
            ...
            if (isset(self::$cart->id_address_delivery)) {
                ...
                if (Validate::isLoadedObject($deliveryAddress) and
            ...
            if (isset(self::$cart->id_address_invoice)) {
                ...
                (int) (self::$cart->id_address_invoice));
                if (Validate::isLoadedObject($invoiceAddress) and
    ...
    protected function _assignWrappingAndTOS ()
    {
        ...
        $wrapping_fees_tax_inc = $wrapping_fees *
         (1 + (((float) ($wrapping_fees_tax->rate) / 100)));
        // TOS
        $cms = new CMS(
        (int) (Configuration::get('PS_CONDITIONS_CMS_ID')), 
        (int) (self::$cookie->id_lang));
        $this->link_conditions = self::$link->getCMSLink($cms, 
        $cms->link_rewrite, true);
        if (! strpos($this->link_conditions, '?'))
            $this->link_conditions .= '?content_only=1';
        else
            $this->link_conditions .= '&content_only=1';
        self::$smarty->assign(
        array('checkedTOS' => (int) (self::$cookie->checkedTOS), 
        'recyclablePackAllowed' => (int) (Configuration::get(
        'PS_RECYCLABLE_PACK')), 
        ...
        'gift_wrapping_price' => (float) (Configuration::get(
        'PS_GIFT_WRAPPING_PRICE')), 
        'total_wrapping_cost' => Tools::convertPrice($wrapping_fees_tax_inc, 
        new Currency((int) (self::$cookie->id_currency))), 
        'total_wrapping_tax_exc_cost' => Tools::convertPrice($wrapping_fees, 
        new Currency((int) (self::$cookie->id_currency)))));
    }
    protected function _assignPayment ()
    {
        self::$smarty->assign(
        array('HOOK_TOP_PAYMENT' => Module::hookExec('paymentTop'), 
    ...
    {
        self::$cart->id_carrier = Carrier::getDefaultCarrierSelection($carriers, 
        (int) self::$cart->id_carrier);
        if (self::$cart->update())
